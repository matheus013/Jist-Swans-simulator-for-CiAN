 ...
 * The Controller also contains the implementation of static callbacks from the
 * simulation application to the JiST system.
 * 
...
public final class Controller implements ControllerRemote, Runnable {
	...
	/**
	 * Method stub for callback to register an new entity, creating an entity
	 * reference.
	 */
	...
	/**
	 * Method stub for callback to determine whether current event invocation is
	 * in restore mode.
	 */
	...
	static {
		try {
			method_getTHIS = Controller.class.getDeclaredMethod("getTHIS",
					new Class[] {});
			method_newEntityReference = Controller.class.getDeclaredMethod(
					"newEntityReference", new Class[] { Entity.class });
			method_getEntityReference = Controller.class.getDeclaredMethod(
					"getEntityReference", new Class[] { Object.class });
			method_entityInvocation = Controller.class.getDeclaredMethod(
					"entityInvocation", new Class[] { Method.class,
							Object.class, Object[].class });
			method_entityInvocationCont = Controller.class.getDeclaredMethod(
					"entityInvocationCont", new Class[] { Method.class,
							Object.class, Object[].class });
			method_popStateInFrame = Controller.class.getDeclaredMethod(
					"popStateInFrame", new Class[] {});
			method_pushStateOutFrame = Controller.class.getDeclaredMethod(
					"pushStateOutFrame",
					new Class[] { Event.ContinuationFrame.class });
			method_isModeRestore = Controller.class.getDeclaredMethod(
					"isModeRestore", new Class[] {});
			method_isModeSave = Controller.class.getDeclaredMethod(
					"isModeSave", new Class[] {});
	...
	static {
		try {
			...
			throw new RuntimeException(
					"should not happen: could not create static controller");
		}
	}
	 ...
	 * GUI logger.
	 */
	public static final GuiLog guilog = Main.GUILOG_SIZE > 0 ? new GuiLog(
			Main.GUILOG_SIZE) : null;
	 ...
	 */
	private final Pool.ContinuationPool continuationPool = new Pool.ContinuationPool(
			100);
	...
	// private final Scheduler events = new Scheduler.Calendar(1000000000,
	// 1000);
	...
	/**
	 * Provides a Thread with a slot to store the Controller instance. This
	 * thread runs the Controller, and enables the executing simulation to
	 * obtain its invoking Controller instance.
	 * 
	 * @author Rimon Barr &lt;barr+jist@cs.cornell.edu&gt;
	 * @since JIST1.0
	 */
	...
	/**
	 * Create and initialize a new Controller instance. May return a single
	 * controller, if JiST is SINGLE_CONTROLLER mode.
	 * 
	 * @param loader
	 ...
	 */
	public static Controller newController(ClassLoader loader)
			throws RemoteException {
	...
	/**
	 * Begin execution of simulation events. This method will exit when there
	 * are no more events to process, or when the simulation time limit is
	 * reached, whichever comes first.
	 */
	public void run() {
		startTime = System.currentTimeMillis();
		endAt(endSimulationTime);
		log.info((new Node(1)).getHostString()
				+ " starting controller event loop.");
		long numEvents = 0;
		...
		try {
			...
			if (numEvents > 0) {
				...
				log.info("TOTAL: " + Util.getHMS((long) seconds) + " real, "
						+ getSimulationTimeString() + " sim, " + numEvents
						+ "ev, " + ((long) (numEvents / seconds)) + "ev/s");
			}
			if (Main.COUNT_EVENTS) {
			...
			public void run() {
				...
				while (isRunning) {
					try {
						...
						if (simtime == 0)
							continue;
						if (simtime > endSimulationTime)
							continue;
						String msg = "sim-time=" + (simtime / simunitTicks)
								+ simunitString;
						if (endSimulationTime != JistAPI.END) {
							double percent = Util.round(simtime
									/ (float) endSimulationTime * 100, 2);
							msg += ", " + percent + "%";
						}
						...
						if (endSimulationTime != Long.MAX_VALUE
								&& completed > 0.001) {
	...
	/**
	 * Process an event. Set the simulation time and event information, and then
	 * invoke the appropriate entity method.
	 */
	private void processEvent() {
		...
		try {
			...
			try {
				result = currentEvent.method.invoke(
						entities[currentEvent.ref.getIndex()],
						currentEvent.args);
			} catch (InvocationTargetException e) {
				exception = e.getTargetException();
				if (exception instanceof JistException)
					throw (JistException) exception;
				if (exception instanceof VirtualMachineError)
					throw (VirtualMachineError) exception;
				if (currentCaller == null || call != null)
					throw e;
			} catch (IllegalArgumentException e) {
				throw new NoSuchMethodException("Unable to invoke METHOD: "
						+ currentEvent.method + " on target ENTITY: "
						+ entities[currentEvent.ref.getIndex()]);
			}
			...
			if (t instanceof JistException)
				throw (JistException) t;
			else if (t instanceof VirtualMachineError)
				throw (VirtualMachineError) t;
			else
				throw new JistException(
						"application exception propagated to event loop",
	 ...
	 * 
	 * @param method
	 *            invocation method for event
	 * @param ref
	 *            invocation entity reference for event
	 * @param args
	...
	/**
	 * Create an event using the event pool, using the given simulation time and
	 * the given invocation parameters.
	 * 
	 * @param method
	 *            invocation method for event
	 * @param ref
	...
	/**
	 * Create and register an outgoing call event. This event will be kept by
	 * the active Controller until the state saving is complete, and then
	 ...
	 * time of the current event, plus all the sleeps incurred during
	 * processing.
	 * 
	 ...
	 * 
	 * @param i
	 *            number of steps to advance simulation time
	 */
	public void advanceSimulationTime(long i) {
		currentSimulationTime += i;
		if (isDebugLogging) {
			if (log.isDebugEnabled()) {
				log.debug(" advancing simulation time to t="
			...
			if (JistAPI_Impl.isEntity(o)) {
				...
				if (o instanceof EntityRef) {
					ref = (EntityRef) o;
						...
						&& Proxy.getInvocationHandler(o) instanceof EntityRef) {
					ref = (EntityRef) Proxy.getInvocationHandler(o);
				} else {
					throw new RuntimeException("strange entity object");
				}
				return "entity:"
						+ ref.getController().toStringEntity(ref.getIndex());
			} else {
	 ...
	 * 
	 * @param loader
	 *            class loader / rewriter for controller
	 */
	...
	public static EntityRef getEntityReference(Object o) {
		return o instanceof Entity ? ((Entity) o)._jistMethod_Get__ref()
				: (EntityRef) o;
	}
	 ...
	 * @param ref
	 *            reference to entity being invoked
	 * @param params
	 *            invocation parameters
	 */
	public static void entityInvocation(Method meth, EntityRef ref,
			Object[] params) {
		if (Main.SINGLE_CONTROLLER) {
	 ...
	 *            method to be invoked
	 * @param entity
	 *            entity to be invoked
	 * @param params
	 *            invocation parameters
	 */
	public static void entityInvocation(Method meth, Entity entity,
			Object[] params) {
	 ...
	 *            method to be invoked
	 * @param callee
	 *            object (either entity or entityreference) to be invoked
	 * @param params
	 *            invocation parameters
	 */
	public static void entityInvocation(Method meth, Object callee,
			Object[] params) {
		entityInvocation(meth, callee instanceof EntityRef ? (EntityRef) callee
				: ((Entity) callee)._jistMethod_Get__ref(), params);
	}

	/**
	 * Application callback method to perform (intercept) a BLOCKING entity
	 * method invocation at the appropriate simulation time. This method will be
	 * called once when the blocking call is initiated, and once when the
	 * callback is restoring its state. This ping-pong is necessary to capture
	 ...
	 * @throws Throwable
	 *             whatever was thrown by blocking event
	 */
	public static Object entityInvocationCont(Method meth, EntityRef ref,
			Object[] params) throws JistAPI.Continuation, Throwable {
	 ...
	 *             whatever was thrown by blocking event
	 */
	public static Object entityInvocationCont(Method meth, Entity entity,
			Object[] params) throws JistAPI.Continuation, Throwable {
	 ...
	 *             whatever was thrown by blocking event
	 */
	public static Object entityInvocationCont(Method meth, Object callee,
			Object[] params) throws JistAPI.Continuation, Throwable {
		return entityInvocationCont(meth,
				callee instanceof EntityRef ? (EntityRef) callee
						: ((Entity) callee)._jistMethod_Get__ref(), params);
	}
	 ...
	 * @return callback result
	 * @throws Throwable
	 *             whatever was thrown by the blocking event
	 */
	 ...
	 * 
	 * @param f
	 *            next frame of outgoing execution stack.
	 */
	 ...
	 *             invalid logger class
	 */
	public void setLog(Class loggerClass) throws InstantiationException,
