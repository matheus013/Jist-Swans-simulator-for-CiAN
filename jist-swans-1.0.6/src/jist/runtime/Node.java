...
public final class Node implements Serializable {
	 ...
	 * 
	 * @param host
	 *            Internet address of node instance
	 * @param port
	 ...
	 * 
	 * @param host
	 *            Internet address or name of node instance
	 * @param port
	 *            port of node instance
	 * @throws UnknownHostException
	 *             thrown if named host can not be resolved
	 */
	public Node(String host, short port) throws UnknownHostException {
		...
		} catch (UnknownHostException e) // maybe is x.x.x.x format
		{
			try {
				...
				this.host = InetAddress.getByAddress(Pickle
						.intToByteArray(addr));
			} catch (Exception e1) {
	 ...
	 * 
	 * @param addr
	 *            Internet address of node instance as quad-byte array
	 * @param port
	 *            port of node instance
	 * @throws UnknownHostException
	 *             never
	 */
	public Node(byte[] addr, short port) throws UnknownHostException {
	...
	/**
	 * Instantiate node object with given host and port. Convenience method to
	 * accept integer ports and convert to short.
	 * 
	 * @param host
	 *            Internet address of node instance
	 * @param port
	...
	/**
	 * Instantiate node object with host string and port. Convenience method to
	 * accept integer ports and convert to short.
	 * 
	 * @param host
	 *            Internet address string of node instance
	 * @param port
	 *            port of node instance
	 * @throws UnknownHostException
	...
	/**
	 * Instantiate node object with given port on local host. Convenience method
	 * to accept integer ports and convert to short.
	 * 
	 * @param port
	 *            port of node instance
	 */
	...
	/**
	 * Instantiate node object from serialized information in byte array at
	 * given offset. The byte array must contain 4-bytes for the Internet
	 * address and 2-bytes for the port.
	 * 
	 * @param node
	 *            byte array with node information
	 * @param offset
	 *            location of information within byte array
	 */
	public Node(byte[] node, int offset) // 6 bytes
	{
		this(Pickle.arrayToInetAddress(node, offset), Pickle.arrayToShort(node,
				offset + 4));
	}
	...
	/**
	 * Serialize node instance into byte array. The byte array will be of length
	 * 6.
	 * 
	...
	/**
	 * Serialize node instance into byte array. The serialized byte
	 * representation of a node occupies 6 bytes.
	 * 
	 * @param b
	 *            byte array to place node information into
	 * @param offset
	 ...
	 * @throws UnknownHostException
	 *             thrown if host name within string can not be resolved
	 */
	public static Node parse(String s) throws UnknownHostException {
		try {
			s = " " + s.trim();
	...
	/**
	 * Parse string (host:port) into a Node, using the default port if the
	 * string does not contain a port.
	 * 
	 * @param s
	 *            serialized node string in format host[:port]
	 * @param defaultPort
	...
	/**
	 * Find local host Internet address. Attempts to discover local host IP
	 * address using a number of techniques.
	 * 
	 * @return Internet address of localhost, or null if it can not be
	 *         discovered.
	 */
	...
	private static InetAddress getLocalHostJava() {
		...
		try {
			local = InetAddress.getByName(loopback); // works more often than
														// next line!
			if (localhost != null
					&& !loopback.equals(localhost.getHostAddress())) {
	...
	/**
	 * Find local host Internet address using Unix mechanisms. Specifically,
	 * this method invokes 'ifconfig', and parses its output, searching for the
	...
	public static InetAddress getLocalHostLinux(String iface) {
		try {
			...
			BufferedReader stdInput = new BufferedReader(new InputStreamReader(
					p.getInputStream()));
			/*
			 * This is kinda hack... Get the ip address from second line of
			 * /sbin/ifconfig eth0, after the 'inet addr:xx.xxx.xxx.xxx'
			 */
			stdInput.readLine();
			String l = stdInput.readLine();
			if (l == null)
				return null;
			if (!l.trim().toLowerCase().startsWith("inet addr"))
				return null;
