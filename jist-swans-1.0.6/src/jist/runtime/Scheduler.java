...
public abstract class Scheduler {
	 ...
	 * functionality, there are methods for extracting elements other than the
	 * min.
	 * 
	 * @author Rimon Barr &lt;barr+jist@cs.cornell.edu&gt;
	 * @version $Id: Scheduler.java,v 1.14 2005-03-13 16:11:54 barr Exp $
	...
	static final class Heap extends Scheduler {
		...
		public void insert(Event ev) {
			try {
				...
				Event parentItem;
				while (i > 0
						&& ev.time < (parentItem = items[parent = (i - 1) / 2]).time) {
					items[i] = parentItem;
					i = parent;
				}
				// insert item
		...
		/**
		 * Return the element stored a given location in the internal heap array
		...
		public Event removeIndex(int i) {
			// check heap size
			if (size < i) {
				throw new ArrayIndexOutOfBoundsException("heap has less than "
						+ i + " items");
			}
			// get item
			Event item = items[i];
		 ...
		 * 
		 * @param i
		 *            index into internal heap array
		 */
	...
	/**
	 * Implementation of calendar queue event scheduler. See R.Brown. "Calendar
	 * queues: A fast O(1) priority queue implementation for the simulation
	 * event set problem." Comm. of ACM, 31(10):1220-1227, Oct 1988
	 */
	...
	static final class Calendar extends Scheduler {
		...
		/**
		 * Scan for and return next bin with an event with the next calendar
		 * year, or use minBin (above) to find bin with minimum event using
		 * direct search.
		 * 
	...
	/**
	 * Simple scheduler test... Schedule random time events and then dequeue and
	 * check that they are coming in order.
	 * 
	 * @param s
	 *            scheduler to test
	 */
