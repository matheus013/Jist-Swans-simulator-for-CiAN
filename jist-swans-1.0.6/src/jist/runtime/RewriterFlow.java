...
public final class RewriterFlow {
	...
	public static class FlowInfoMap {
		 ...
		 * 
		 * @param frames
		 *            hashmap of frame at beginning of each instruction
		 * @param pointsTo
		 *            hashmap of instructions that pointTo each instruction
		 */
		 ...
		 * 
		 * @param ih
		 *            BCEL instruction handle (program counter)
	...
	private static class WildcardReturnaddressType extends ReturnaddressType {
		...
		public boolean equals(Object o) {
			if (!(o instanceof ReturnaddressType))
				return false;
			return true;
		}
	...
	/**
	 * Perform data flow analysis to determine the frame at each execution point
	 * in the method.
	 * 
	 * @param cg
	 *            BCEL class object
	...
	public FlowInfoMap doFlow(ClassGen cg, MethodGen mg) {
		if (Rewriter.log.isDebugEnabled())
			Rewriter.log.debug("performing data-flow analysis of: "
					+ cg.getClassName() + "." + mg.getName());
		if (mg.isNative())
			throw new RuntimeException(
					"cannot perform flow analysis of native method");
		// initialize
		...
		if (!mg.isStatic()) {
			f.getLocals()
					.set(0,
							mg.getName().equals(Constants.CONSTRUCTOR_NAME) ? (Type) new UninitializedObjectType(
		...
		for (int j = 0; j < args.length; j++) {
			Type arg = args[j];
			if (arg.equals(Type.BOOLEAN))
				arg = Type.INT;
			if (arg.equals(Type.BYTE))
				arg = Type.INT;
	...
	/**
	 * Helper flow analysis method: "executes" a given instruction to produce
	 * the next frame, and returns all successors of this instruction.
	 * 
	 * @param ih
	 *            handle of instruction to execute
	 * @param f
	...
	private InstructionHandle[] execute(InstructionHandle ih, Frame f) {
		...
		if (inst instanceof ReturnInstruction) {
			...
			return new InstructionHandle[] { ((GotoInstruction) inst)
					.getTarget(), };
			...
			if (inst instanceof Select) {
				...
				return new InstructionHandle[] { ih.getNext(),
						((BranchInstruction) inst).getTarget(), };
			}
		} else if (inst instanceof JsrInstruction) {
			return new InstructionHandle[] { ((JsrInstruction) inst)
					.getTarget(), };
		} else if (inst instanceof RET) {
			// rimnode: are there other flow targets?
			return new InstructionHandle[] {};
		} else {
			return new InstructionHandle[] { ih.getNext(), // fall-through
			};
		}
	}

	/**
	 * Helper flow analysis method: merges two frames ala the VM spec, and
	 * stores result.
	 * 
	 * @param ih
	 *            location to merge information into
	 * @param f
	 *            frame to merge in
	 * @param cp
	 *            class constant pool
	...
	private boolean merge(InstructionHandle ih, Frame f, ConstantPool cp) {
		try {
			f = new Frame(f.getLocals().getClone(), removeRetTargets(f
					.getStack().getClone()));
			...
			if (old == null) {
				...
				LocalVariables newVars = old.getLocals(), oldVars = newVars
						.getClone();
				newVars.merge(f.getLocals());
				OperandStack newStack = old.getStack(), oldStack = newStack
						.getClone();
	...
	/**
	 * Replace any ReturnaddressTypes (placed on the stack by JSR) with
	 * WildcardReturnaddressTypes, so as to pass the verification performed
	 * during stack merge.
	 * 
	 * @param os
	 ...
	 * @param to
	 *            list of successor instructions
	 */
	...
	/**
	 * Small utility program to dump the methods of a given class file with flow
	 * information between every instruction.
	 * 
	 * @param args
			...
			for (int i = 0; i < args.length; i++) {
				...
				for (int j = 0; j < methods.length; j++) {
					try {
						...
						InstructionHandle ih = mg.getInstructionList()
								.getStart();
						while (ih != null) {
						...
						System.out.println("Error computing flow of: "
								+ methods[j]);
						e.printStackTrace();
					}
				}
