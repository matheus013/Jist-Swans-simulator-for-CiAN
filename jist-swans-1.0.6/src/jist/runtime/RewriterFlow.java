...
// Timestamp: <RewriterFlow.java Tue 2004/04/06 11:25:02 barr
...
public final class RewriterFlow
{
    ...
    public FlowInfoMap doFlow(ClassGen cg, MethodGen mg) {
        if (Rewriter.log.isDebugEnabled())
            Rewriter.log.debug("performing data-flow analysis of: " + cg.getClassName() + "." + mg.getName());
        if (mg.isNative())
        ...
        if (!mg.isStatic()) {
            f.getLocals().set(
                    ...
                    mg.getName().equals(Constants.CONSTRUCTOR_NAME) ? (Type) new UninitializedObjectType(
                            new ObjectType(mg.getClassName())) : (Type) new ObjectType(mg.getClassName()));
            localsOffset++;
        }
    ...
    private InstructionHandle[] execute(InstructionHandle ih, Frame f) {
        ...
        if (inst instanceof ReturnInstruction) {
            ...
            return new InstructionHandle[] { ((GotoInstruction) inst).getTarget(), };
            ...
            if (inst instanceof Select) {
                ...
                return new InstructionHandle[] { ih.getNext(), ((BranchInstruction) inst).getTarget(), };
            }
        } else if (inst instanceof JsrInstruction) {
            return new InstructionHandle[] { ((JsrInstruction) inst).getTarget(), };
