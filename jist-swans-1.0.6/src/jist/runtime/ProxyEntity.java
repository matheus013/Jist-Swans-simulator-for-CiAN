...
final class ProxyEntity extends Entity.Empty
{
    ...
    /**
     * Handles all invocations from the Controller on the proxy entity and
     * passes them on to the target object. This handler is bypassed if the
     * target object is Proxiable (i.e. it can receive calls from the Controller
     * and manage it's own EntityRef object)
     */
    public static class ProxyEntityHandler implements InvocationHandler
    {
         ...
         * 
         * @param proxy
         *            proxy entity object whose method was invoked
         * @param method
         *            method invoked on entity object
         * @param args
     ...
     * 
     * @param proxyTarget
     *            target object for the proxy entity
     * @param proxyInterface
     *            interface to be supported by the proxy entity
    ...
    protected static String[] isValidProxyEntity(Object proxyTarget, Class proxyInterface) {
        if (proxyTarget instanceof EntityRef)
            return null;
        Vector errors = null;
        ...
        if (!doesImplement(proxyTarget.getClass(), proxyInterface)) {
            if (errors == null)
                errors = new Vector();
            errors.add("target object of proxy entity does not implement interface: " + proxyInterface.getName());
        }
        // validate interface
        ...
        for (int j = 0; j < methods.length; j++) {
            ...
            if (numExceptions > 0 && !blocking) {
                if (errors == null)
                    errors = new Vector();
                errors.add("proxy entity implements interface '" + proxyInterface.getName()
                        + "' that has method that declares exceptions: " + methods[j].getName());
            }
            ...
            if (!Void.TYPE.equals(methods[j].getReturnType()) && !blocking) {
                if (errors == null)
                    errors = new Vector();
                errors.add("proxy entity implements interface '" + proxyInterface.getName()
                        + "' that has method with non-void return type: " + methods[j].getReturnType().getName());
            }
        }
        ...
        for (int j = 0; j < fields.length; j++) {
            ...
            if (!(Modifier.isFinal(mods) && Modifier.isStatic(mods))) {
                if (errors == null)
                    errors = new Vector();
                errors.add("proxy entity implements interface '" + proxyInterface.getName()
                        + "' that has non-final or non-static field: " + fields[j].getName());
            }
        }
    ...
    /**
     * Dynamically create a proxy entity for the given target object and
     * interface.
     * 
     * @param proxyTarget
     *            target object for proxy entity
     * @param proxyInterface
     *            interface supported by proxy entity
     * @return proxy entity
     */
    public static Object create(Object proxyTarget, Class[] proxyInterface) {
        ...
        // check whether target is already an Entity, or whether we need to
        // proxy it
        EntityRef ref = null;
        if (proxyTarget instanceof JistAPI.Proxiable) {
            ...
            Proxy pe = (Proxy) Proxy.newProxyInstance(proxyTarget.getClass().getClassLoader(), proxyInterface,
        ...
        Proxy per = (Proxy) Proxy.newProxyInstance(proxyInterface[0].getClassLoader(), proxyInterface, ref);
        return per;
    }
