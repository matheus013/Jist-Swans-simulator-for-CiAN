...
public abstract class Bootstrap extends Entity.Empty {
	...
	static {
		try {
			method_startSimulation = Bootstrap.class.getDeclaredMethod(
					"startSimulation", new Class[] { Rewriter.class,
	 ...
	 * @param controller
	 *            where to register the bootstrap entity and enqueue event
	 * @param name
	 ...
	 */
	public static void create(int type, Controller controller, String name,
			String[] args, Object properties) throws RemoteException {
		...
		case JistAPI.RUN_JPY:
			...
			break;
		default:
			throw new RuntimeException("invalid simulation driver type: "
					+ type);
		}
		...
		controller.addEvent(new Event(controller.getSimulationTime(),
				Bootstrap.method_startSimulation, simEntity
						._jistMethod_Get__ref(), new Object[] { cll, name,
						args, properties }));
	}
	 ...
	 * 
	 * @param rewriter
	 *            rewriting class loader instance
	 * @param name
	 *            script or application name
	 * @param args
	 *            command-line arguments
	 ...
	 *            object passed through to specific bootstrap implementation
	 */
	public abstract void startSimulation(Rewriter rewriter, String name,
			String[] args, Object properties);
	...
	/**
	 * A special-purpose Entity used to bootstrap the simulation. It merely
	 * calls the usual static main() method of the simulation application.
	 * 
	 * @author Rimon Barr &lt;barr+jist@cs.cornell.edu&gt;
	 * @since JIST1.0
	 */
	...
	static class JavaMain extends Bootstrap {
		/** {@inheritDoc} */
		public void startSimulation(Rewriter rewriter, String classname,
				String[] args, Object properties) {
			try {
				...
				System.err.println("JiST Java bootstrap - Class not found: "
						+ e.getMessage());
			} catch (InvocationTargetException e) {
				Throwable target = e.getTargetException();
				if (target instanceof VirtualMachineError)
					throw (VirtualMachineError) target;
				throw new JistException(
						"application exception in bootstrap event", target);
			} catch (Exception e) {
	...
	/**
	 * A special-purpose Entity used to bootstrap the simulation. It processes a
	 * BeanShell script.
	 * 
	 * @author Rimon Barr &lt;barr+jist@cs.cornell.edu&gt;
	 * @since JIST1.0
	...
	static class Bsh extends Bootstrap {
		/** {@inheritDoc} */
		public void startSimulation(Rewriter rewriter, String scriptname,
				String[] args, Object properties) {
			try {
				bsh.Interpreter interp = null;
				if (scriptname == null) {
					interp = new bsh.Interpreter(new InputStreamReader(
					...
					if (!script.exists()) {
						System.err
								.println("JiST BeanShell bootstrap - File not found: "
										+ scriptname);
						return;
					}
				...
				if (scriptname == null) {
					...
					Main.showVersion();
					System.out
							.println("*** EOF will start simulation loop. ***");
	...
	static class Jython extends Bootstrap {
		...
		public void startSimulation(Rewriter rewriter, String scriptname,
			...
			try {
				...
				pyargs.append(new org.python.core.PyString(
