...
/**
 * Facilitates traversal and and modification of a BCEL JavaClass structure. The
 * traversal does not visit all elements of the class, only the ones that are
 * important for JiST. Specifically, it visits (with upcalls) the class, fields,
 * method and each instruction.
 * 
...
public class ClassTraversal {
	 ...
	 * 
	 * @param jcti
	 *            object to receive upcalls during traversal
	 */
	 ...
	 */
	protected ClassGen processClassGen(ClassGen cg)
	 ...
	 */
	protected FieldGen processFieldGen(ClassGen cg, FieldGen fg)
			throws ClassNotFoundException {
	 ...
	 * 
	 * upcalls for each individual instruction, for instruction-level
	 * processing.
	 * 
	 * @param cg
	 *            BCEL class generator object
	 ...
	 */
	protected MethodGen processMethodGen(ClassGen cg, MethodGen mg)
			throws ClassNotFoundException {
	...
	/**
	 * Traverse and process an instruction. A single upcall is made for
	 ...
	 *            BCEL field generator object
	 * @param ihs
	 *            array of BCEL instruction handles
	 * @param i
	 *            instruction handle index
	 */
	protected void processInstruction(ClassGen cg, MethodGen mg,
			InstructionHandle[] ihs, int i) throws ClassNotFoundException {
	...
	/**
	 * ClassTraversal upcall interface. Specifies upcall methods for processing
	 * a class, its fields and methods, and the individual method instructions.
	 * 
	 * @author Rimon Barr &lt;barr+jist@cs.cornell.edu&gt;
	 * @since JIST1.0
	 */
	...
	public static interface Visitor {
		 ...
		 * @return processed BCEL field generator object
		 */
		FieldGen doField(ClassGen cg, FieldGen fg)
				throws ClassNotFoundException;
		 ...
		 * @return processed BCEL method generator object
		 */
		MethodGen doMethod(ClassGen cg, MethodGen mg)
				throws ClassNotFoundException;
		 ...
		 * @return processed BCEL method generator object
		 */
		MethodGen doMethodPost(ClassGen cg, MethodGen mg)
				throws ClassNotFoundException;

		/**
		 * Upcall to process instruction. Note that instructions can be inserted
		 * and deleted directly using the BCEL API, and therefore this method
		 ...
		 *            BCEL method generator object
		 * @param ih
		 *            BCEL instruction handle object to process
		 * @param i
		 *            BCEL instruction object to process
		 */
		void doInstruction(ClassGen cg, MethodGen mg, InstructionHandle ih,
				Instruction i) throws ClassNotFoundException;
	...
	/**
	 * Provides a default (empty) implementation of the <code>Visitor</code>.
	 * 
	 * @author Rimon Barr &lt;barr+jist@cs.cornell.edu&gt;
	 * @since JIST1.0
	 */
	...
	public static class Empty implements Visitor {
		...
		public void doInstruction(ClassGen cg, MethodGen mg,
				InstructionHandle ih, Instruction i)
				throws ClassNotFoundException {
		}
