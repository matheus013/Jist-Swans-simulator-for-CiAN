...
// Timestamp: <EventLocation.java Tue 2004/04/06 11:23:33 barr
...
class EventLocation extends Event
{
    ...
    /**
     * Computes the originating source location (if possible) for an event. This
     * is performed by throwing an exception, catching it, and inspecting the
     * stack. Consequently, it is slow (generating the exception, walking the
     * stack) and should be turned on only for debugging purposes. It may be
     * possible to use the rewriter to glean this information instead, but this
     * is so much simpler, and good enough.
     */
    private void computeLocation() {
        StackTraceElement[] stack = (new Throwable()).getStackTrace();
        for (int i = 0; i < stack.length; i++) {
            StackTraceElement el = stack[i];
            if (el.getClassName().startsWith(Main.class.getPackage().getName()))
                continue;
            if (el.getClassName().startsWith("$Proxy"))
                continue;
    ...
    /**
     * Computes the originating source runtime events (if possible) for an
     * event. This is performed by looking at current event in the active
     * controller.
     */
    private void computeSource() {
        ...
        while (ev != null && depth > 0) {
            depth--;
            ev = ev.getSource() instanceof EventLocation ? (EventLocation) ev.getSource() : null;
        }
        if (ev != null)
            ev.source = null;
    }
    ...
    public static void printEventTrace() {
        ...
        do {
            ...
            ev = ev instanceof EventLocation ? ((EventLocation) ev).getSource() : null;
        } while (ev != null);
    }
