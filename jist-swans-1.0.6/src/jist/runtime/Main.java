...
public final class Main {
	...
	/**
	 * debugging: event causality trace depth. (0=no cut-off, beware: will
	...
	public static void showVersion() {
		System.out.println("JiST v" + VERSION
				+ ", Java in Simulation Time Runtime.");
		System.out
				.println("Rimon Barr <barr+jist@cs.cornell.edu>, Cornell University.");
		System.out.println();
	}
	...
	public static void showUsage() {
		System.out
				.println("Usage: jist [-r host[:port]] [switches] <sim>   <-- engine mode");
		System.out
				.println("       jist -S [-p port] [-q] [-r host:[port]]  <-- server modes");
		...
		System.out
				.println("  -c, --conf        specify properties file [jist.properties]");
		System.out
				.println("  -l, --logger      specify simulation logger class");
		System.out
				.println("  --bsh             run input with BeanShell script engine");
		System.out
				.println("  --jpy             run input with Jython script engine");
		System.out.println("  --nocache         disable rewriter cache");
		System.out
				.println("  -r, --remote      specify remote job or processing server");
		System.out.println("  where: ");
		System.out
				.println("    <sim>  is:      simulation program with command-line arguments, or");
		System.out
				.println("                    simulation script with command-line arguments");
		System.out
				.println("                       ('--' implies interactive shell)");
		System.out.println("server:");
		System.out.println("  -S, --server      jist server mode");
		System.out
				.println("  -p, --port        listen for jobs on given port [3000]");
		System.out
				.println("  -q, --queue       act only as job queue server, do not process");
		System.out
				.println("  -r, --remote      process jobs from remote queue");
		System.out
				.println("  -x, --proxy       perform RMI connections via a proxy");
		System.out.println();
	}
	 ...
	 *             unable to parse remote host address:port
	 */
	private static CommandLineOptions parseCommandLineOptions(String[] args)
		...
		CmdLineParser.Option opt_version = parser.addBooleanOption('v',
				"version");
		CmdLineParser.Option opt_properties = parser.addStringOption('c',
				"conf");
		...
		CmdLineParser.Option opt_nocache = parser.addBooleanOption(']',
				"nocache");
		CmdLineParser.Option opt_remote = parser.addStringOption('r', "remote");
		CmdLineParser.Option opt_server = parser
				.addBooleanOption('S', "server");
		...
		if (parser.getOptionValue(opt_remote) != null) {
			options.remote = Node.parse(
					(String) parser.getOptionValue(opt_remote), JIST_PORT);
		}
		if (parser.getOptionValue(opt_server) != null) {
			options.server = true;
		}
		if (parser.getOptionValue(opt_port) != null) {
			options.port = Integer.parseInt((String) parser
					.getOptionValue(opt_port));
		}
		if (parser.getOptionValue(opt_queue) != null) {
			options.queue = true;
		}
		if (parser.getOptionValue(opt_proxy) != null) {
			options.proxy = Node.parse(
					(String) parser.getOptionValue(opt_proxy),
					ProxyPoint.PROXY_PORT);
		}
	 ...
	 *            jist client stub
	 * @param serverOut
	 *            local server output stream; null for local execution
	 * @param ping
	 *            remote client ping object, or null if local
	 */
	public static void runSimulation(CommandLineOptions options,
			Properties properties, RemoteJist.JistClientRemote remote,
			PrintStream serverOut, RemoteJist.PingRemote ping) {
		try {
			// set up jist logging
			if (properties != null) {
			...
			if (options.bsh || options.jpy || options.sim != null) {
				...
				String cachedir = options.nocache ? null : System
						.getProperty("java.io.tmpdir");
				Rewriter rewriter = new Rewriter(null, cachedir, remote,
						serverOut);
				Thread.currentThread().setContextClassLoader(rewriter);
				// instantiate controller
				Controller controller = Controller.newController(rewriter);
				if (options.bsh) {
					Bootstrap.create(JistAPI.RUN_BSH, controller, options.sim,
							options.args, null);
				} else if (options.jpy) {
					Bootstrap.create(JistAPI.RUN_JPY, controller, options.sim,
							options.args, null);
				} else if (options.sim != null) {
					Bootstrap.create(JistAPI.RUN_CLASS, controller,
							options.sim, options.args, null);
				}
				// set application logging
				if (options.logger != null) {
					controller.setLog(Class.forName(options.logger, true,
							rewriter));
				}
				// let the games begin!
				try {
	...
	public static void runSimulationRedirect(CommandLineOptions options,
		...
		try {
			...
			rout = new RemoteIO.PrintStreamWithExceptions(new PrintStream(
					new RemoteIO.RemoteOutputStream(remote.getStdOut())));
			rerr = new RemoteIO.PrintStreamWithExceptions(new PrintStream(
					new RemoteIO.RemoteOutputStream(remote.getStdErr())));
			System.setOut(rout);
			System.setErr(rerr);
			try {
				try {
			...
			// @author Elmar Schoch
			// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
			// Commented out, since rerr and rout are always != null and thus
			// System.out and System.err
			// are closed (normally setting rerr and rout to null is done by the
			// calling method (jobPump)
			// in currentJob.client.done();

			/*
			 * if(rerr!=null) rerr.close(); if(rout!=null) rout.close();
			 */
	 ...
	 *             rpc failure
	 */
	public static void runClient(CommandLineOptions options)
		...
		if (options.remote != null) {
			...
			RemoteJist.JobQueueServerRemote jqs = RemoteJist.JobQueueServer
					.getRemote(options.remote);
	 ...
	 *             local server simulation thread interuptted
	 */
	public static void recycleMem() throws InterruptedException {
		// parameters
		final long baseThreshold = 30 * 1024 * 1024;
		...
		if (Util.getUsedMemory() > baseThreshold) {
			...
			System.out.println("Memory used = " + Util.getUsedMemory()
					+ " bytes.");
		}
	}
		...
		while (true) {
			...
			try {
				...
				try {
					...
					runSimulationRedirect(currentJob.options,
							currentJob.properties, currentJob.client);
					...
					System.out
							.println("UNHANDLED SIMULATION PROCESSING EXCEPTION AT SERVER:");
	 ...
	 */
	public static Thread startDisplayThread(
			final RemoteJist.JobQueueServerRemote jqs) {
				...
				try {
					...
					while (isRunning()) {
						...
						if (c.isRunning()) {
							...
							long seconds = (long) ((System.currentTimeMillis() - c
									.getStartTime()) / 1000.0);
	...
	/**
	 * Initiate a server thread to ping client, and abort simulation if client
	 * dies.
	 * 
	 * @param ping
	 *            remote ping object, or null if local
	 * @param controller
	 *            controller running simulation
	 * @return ping thread, or null if ping object is null
	 */
	public static Thread startClientPingThread(
			final RemoteJist.PingRemote ping, final Controller controller) {
		if (ping == null)
			return null;
		Runnable runner = new Runnable() {
			public void run() {
	...
	public static void runServer(String host, CommandLineOptions options)
			...
			AlreadyBoundException, RemoteException, InterruptedException {
		...
		if (options.port != 0) {
			...
			RemoteJist.JobQueueServer jqs = new RemoteJist.JobQueueServer(
					options.port, System.out);
			r.bind(RemoteJist.JobQueueServer.JIST_JOBSERVER_RMI_NAME, jqs);
			System.out.println("Listening for simulations on " + host + ":"
					+ options.port + "...");
		...
		if (options.remote != null) {
			...
			while (true) {
				try {
					...
					System.out
							.println("Connection to queue server failed! Will try to reconnect in "
									+ (pause / 1000) + " seconds.");
					Thread.sleep(pause);
	...
	public static void main(String[] args) {
		...
		try {
			...
			// show usage
			if (options.help
					|| (options.sim == null && !script && !options.server)) {
				showVersion();
				showUsage();
				return;
			}
			...
			if (options.server) {
				if (options.properties != null) {
					System.out
							.println("invalid server mode option '-c'; type 'jist -h' for syntax");
					return;
				}
				if (options.logger != null) {
					System.out
							.println("invalid server mode option '-l'; type 'jist -h' for syntax");
					return;
				}
				if (script) {
					System.out
							.println("invalid server mode option '--bsh' or '--jpy'; type 'jist -h' for syntax");
					return;
				}
				if (options.nocache) {
					System.out
							.println("invalid server mode option '--nocache'; type 'jist -h' for syntax");
					return;
				}
				if (options.sim != null) {
					System.out
							.println("can not provide simulation program to server mode; type 'jist -h' for syntax");
				}
				if (options.remote != null && options.port != 0) {
					System.out
							.println("server should have either a local or remote job queue; type 'jist -h' for syntax");
					return;
				}
			} else {
				if (options.port != 0) {
					System.out
							.println("invalid client mode option '-p'; type 'jist -h' for syntax");
					return;
				}
				if (options.queue) {
					System.out
							.println("invalid client mode option '-q'; type 'jist -h' for syntax");
					return;
				}
				if (options.remote != null && options.sim == null) {
					System.out
							.println("client should have job to queue; type 'jist -h' for syntax");
					return;
				}
			}
			// set rmi proxy
			if (options.proxy != null) {
				ProxyPoint.setRmiProxy(options.proxy.getHost(),
