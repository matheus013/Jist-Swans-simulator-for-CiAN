...
public final class Main
{
     ...
     *            jist client stub
     */
    public static void runSimulationRedirect(CommandLineOptions options, Properties properties,
            RemoteJist.JistClientRemote remote) {
        PrintStream lout = System.out, lerr = System.err;
        try {
            PrintStream rout = null, rerr = null;
            rout = new RemoteIO.PrintStreamWithExceptions(new PrintStream(new RemoteIO.RemoteOutputStream(
                    remote.getStdOut())));
            rerr = new RemoteIO.PrintStreamWithExceptions(new PrintStream(new RemoteIO.RemoteOutputStream(
                    remote.getStdErr())));
            System.setOut(rout);
            System.setErr(rerr);
            try {
     ...
     *             rpc failure
     */
    public static void runClient(CommandLineOptions options) throws MalformedURLException, NotBoundException,
            RemoteException {
        // load properties
     ...
     *             local server simulation thread interuptted
     */
    public static void runServer(String host, CommandLineOptions options) throws MalformedURLException,
            NotBoundException, AlreadyBoundException, RemoteException, InterruptedException {
        ...
        if (options.remote != null) {
            ...
            while (true) {
                try {
                    ...
                    System.out.println("Connection to queue server failed! Will try to reconnect in " + (pause / 1000)
                            + " seconds.");
                    Thread.sleep(pause);
