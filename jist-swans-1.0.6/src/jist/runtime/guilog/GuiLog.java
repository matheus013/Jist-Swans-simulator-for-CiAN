...
public class GuiLog {
	...
	/**
	 * A list used to track the events as they are generated so that they can
	 * easily be deleted first-in, first-out.
	 */
	...
	/**
	 * GuiLog creates and shows the GUI when an outside synchronized thread
	 * instantiates an instance of GuiLog.
	 * 
	 * @param maxNumEvents
	 *            int Maximum number of Events allowed to be displayed in table.
	 */
	...
	private synchronized void createAndShowGUI() {
		...
		String[] columnNames = { "T", "Type", "Method", "Object", "Cont",
				"Parent", "Children" };
		...
		for (int i = 0; i < columnNames.length; i++) {
			myTableColumnModel.addColumn(createTableColumn(columnNames[i],
					widths[i], modelIndex[i], resizeable[i]));
		}
	 ...
	 * 
	 * @param name
	 *            String TableColumn's HeaderValue.
	 * @param width
	 *            int TableColumn's width.
	 * @param colIndex
	 *            int TableColumn's model index.
	 * @param resize
	 *            boolean If the TableColumn can be resized.
	 * @return TableColumn
	 */
	private TableColumn createTableColumn(String name, int width, int colIndex,
			boolean resize) {
	...
	/**
	 * helper method. Adds a component with layout constraints to the GuiLog's
	 * JPanel.
	 * 
	 * @param c
	 *            Component The component to be added.
	 * @param constraints
	 *            GridBagConstraints The constraints for the GridBagLayout
	 *            manager.
	 * @param x
	 *            int The GridBagConstraints' gridx field.
	 * @param y
	 *            int The GridBagConstraints' gridy field.
	 * @param w
	 *            int The GridBagConstraints' gridwidth field.
	 * @param h
	 *            int The GridBagConstraints' gridheight field.
	 */
	...
	public static class MouseAdapterHandler extends MouseAdapter {
		 ...
		 * 
		 * @param t
		 *            JTable The table that is receiving mouse clicks.
		 * @param m
		 *            EventTableModel The table's data model.
		 */
		 ...
		 * 
		 * @param e
		 *            MouseEvent The mouse event that occurs.
		 */
		public void mouseClicked(MouseEvent e) {
			JTable clickedTable = (JTable) e.getSource();
			int selectedRow = clickedTable.getSelectedRow(); // the row that was
																// clicked on
			int selectedColumn = clickedTable.getSelectedColumn(); // the column
																	// that was
																	// clicked
																	// on
			int rowHeight = clickedTable.getRowHeight(); // the row height of
															// each row in the
															// table
			...
			int y = rowHeight * jumpRow; // y-coordinate of top-left corner of
											// new rectangle view
			int width = (int) clickedTable.getPreferredScrollableViewportSize()
					.getHeight(); // width of new rectangle view
			int height = (int) clickedTable
					.getPreferredScrollableViewportSize().getHeight(); // height
																		// of
																		// new
																		// rectangle
																		// view

			// If the column selected is the parent column, and the parent is
			// still present, jump to the parent's row
	...
	public class EventCellRenderer extends DefaultTableCellRenderer {
		/**
		 * Returns the Component that renders the TableCell. Parameters are the
		 * same as those in the getTableCellRendererComponent() method in
	 ...
	 */
	public static class ChildrenCellEditor extends AbstractCellEditor implements
		 ...
		 * 
		 * @param t
		 *            JTable The table that the ChildrenCellEditor edits.
		 * @param m
		 *            EventTableModel The table's data model.
		 */
		...
		/**
		 * Returns the Component that edits the TableCell. Parameters are the
		 * same as those in the getTableCellEditorComponent() method in
		...
		public Component getTableCellEditorComponent(JTable table,
			...
			if (numChildren > 0) {
				for (int i = 0; i < numChildren; i++) {
					// store the child event's time and location in the table
					// into a hashtable
					String key = Long.toString(evn.getChild(i).getEvent().time);
					String hashvalue = Integer.toString(model.getChildIndex(
							row, i));
					ht.put(key, hashvalue);
					// add the child event to the comboBox
					comboBox.addItem(Long
							.toString(evn.getChild(i).getEvent().time));
				}
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				String selectedItem = (String) clickedComboBox
						.getSelectedItem();
			...
			int y = rowHeight * jumpRow; // y-coordinate of top-left corner of
											// new rectangle view
			int width = (int) table.getPreferredScrollableViewportSize()
					.getWidth(); // width of new rectangle view
			int height = (int) table.getPreferredScrollableViewportSize()
					.getHeight(); // height of new rectangle view
			...
			if (jumpRow != -1) {
				...
				table.setRowSelectionInterval(jumpRow, jumpRow);
				table.setSelectionBackground(Color.CYAN);
				cancelCellEditing(); // stop editing the cell. turns off the
										// JComboBox.
			}
		}
	}
	...
	/**
	 * UntilTask is a task that can be scheduled to either Pause or Resume the
	 * GuiLog.
	 */
	...
	/**
	 * An EventNode encapsulates the construct formed by an Event and the
	 * relationships it has with its parent and children Events. EventNodes
	 * strung together form a one-to-many tree structure.
	 */
	public static class EventNode {
		...
		/**
		 * The constructor for the EventNode creates the relationships of Parent
		 * and Children between the Event and its Parent.
		 * 
		 * @param ev
		 *            Event The node's Event.
		 * @param parent
		 *            EventNode The node's parent EventNode.
		 */
		 ...
		 * 
		 * @param i
		 *            int The desired index for the Child.
		 ...
		 * 
		 * @param child
		 *            EventNode The child EventNode to add.
		 */
		 ...
		 * 
		 * @param child
		 *            EventNode The child EventNode to remove.
		 */
	...
	public static class EventNodeComparator implements Comparator {
		/**
		 * Compares o1 to o2 and returns a negative number, 0, or a positive
		 * number if the o1 is less than, equal to, or greater than o2.
		 * 
		 * @param o1
		 *            Object
		 * @param o2
		...
		public int compare(Object o1, Object o2) {
			...
			if (o2 == null)
				return -1;
			return Util.sign(((EventNode) o1).getEvent().time
					- ((EventNode) o2).getEvent().time);
		}
	}
	...
	public static class EventTableModel extends AbstractTableModel {
		...
		/**
		 * Given an Event, returns the corresponding EventNode in the events[]
		 * array.
		 * 
		 * @param ev
		...
		/**
		 * Given an Event, returns the index of the events[] array where that
		 * Event is located.
		 * 
		 * @param ev
		 *            Event The desired Event.
		 * @return int The index of the events[] array where the desired Event
		 *         is located.
		 */
		public int findNodeIndex(Event ev) {
			for (int i = 0; i < numEvents; i++) {
				if (((EventNode) events[i]).getEvent().equals(ev))
					return i;
			}
			return -1;
		}
		...
		/**
		 * Given an EventNode's index, returns the Parent EventNode's index in
		 * the events[] array. This is also equivalent to the table row number.
		 * 
		 * @param index
		...
		/**
		 * Given an EventNode's index and a Child number, returns the specified
		 * Child EventNode's index in the events[] array. This is also
		 * equivalent to the table row number.
		 * 
		 * @param index
		 *            int The index of the EventNode.
		 * @param childNum
		 *            int The number of the desired Child EventNode.
		 * @return int The specified Child EventNode's index in the events[]
		 *         array.
		 */
		public int getChildIndex(int index, int childNum) {
			if (events[index].getNumChildren() > childNum) {
				return findNodeIndex(events[index].getChild(childNum)
						.getEvent());
			}
			return -1;
		}
		 ...
		 * 
		 * @param ev
		 *            Event The Event that is being added.
		 * @param parent
		 *            Event The Event's Parent Event.
		 */
		 ...
		 * 
		 * @param ev
		 *            Event The Event that is being deleted.
		 */
		...
		/**
		 * Returns the number of rows in the table. Overrided to comply with
		...
		/**
		 * Returns the number of columns in the table. Overrided to comply with
		 * AbstractTableModel Class.
		 * 
		 * @return int
		 */
		...
		/**
		 * Returns the EventNode corresponding to the requested row. This method
		 * is called internally by the TableCellRenderer Class. Overrided to
		 * comply with AbstractTableModel Class.
		 * 
		 * @param row
		 *            int
		 * @param col
		...
		/**
		 * Returns whether the cell at (row,col) is editable or not. Overrided
		 * to comply with AbstractTableModel Class.
		 * 
		 * @param row
		 *            int
		 * @param col
		...
		public boolean isCellEditable(int row, int col) {
			// Only column 6 can be edited, which is done by the
			// ChildrenCellEditor.
			if (col != 6) {
				return false;
			}
	...
	public void add(Event id, Event parent) {
		...
		// Create a copy of id and parent. This is done just in case methods
		// that call add() reuse the same address
		// for its Event id and Event parent. Also, must check if either is
		// null, else copy constructor will throw exception.
		if (id != null) {
	...
	/**
	 * If not stepping through one Event, checkLock attempts to acquire a lock
	 * on pauseLock. If it is successful, the method returns; otherwise, it
	 * waits. If stepping through one Event, then checkLock returns once so that
	 * one Event can be added.
	 */
	public void checkLock() {
		synchronized (pauselock) {
			// If not stepping one Event, proceed as normally and attempt to
			// acquire a lock
			if (!isStep) {
				while (paused) {
					try {
	...
	/**
	 * Prompts the user for the number of seconds to unpause the GuiLog, then
	 * unpauses it. After the user-entered time, pauses the GuiLog.
	 */
	public static void untilTimer() {
		// Prompt the user for the number of seconds to run the simulation.
		int numSeconds = Integer.parseInt((String) JOptionPane.showInputDialog(
				frame, "Enter number of seconds to run simulation:"));
	...
	/**
	 * Checks the number of Events in the list. If the thresh hold limit has
	 * been reached, one event is deleted.
	 */
	public void checkListSize() {
		// If numEventsThreshHold has been reached, delete one event. This
		// creates a smooth moving window of viewable Events.
	...
	/**
	 * Sets paused to false and notifies all methods that are synchronized on
	 * pauseLock.
	 */
	...
	/**
	 * Test program. Creates a GUI with a JTable. Creates Events, then adds and
	 * deletes them from the GuiLog.
	 * 
	 * @param args
	 *            String[]
	 */
	public static void main(String[] args) {
		...
		Event XX = new Event(60, m, null, null); // this event is created but
													// not added
		...
		if (gl == null)
			gl = new GuiLog(50);
		try {
			int sleepTime = 500; // miliseconds to wait between additions of
									// Events.
