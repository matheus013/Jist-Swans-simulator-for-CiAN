...
public final class Util
{
    ...
    public static String escapeChar(String s, char c) {
        ...
        while (index != -1) {
            s = s.substring(0, index) + IDENTIFIER_ESCAPE_CHAR + getHexCode(c) + s.substring(index + 1, s.length());
    ...
    public static void showThreads() {
        ...
        for (int i = 0; i < threads.length; i++) {
            if (threads[i] == null)
                continue;
            System.out.println("thread: alive=" + threads[i].isAlive() + " daemon=" + threads[i].isDaemon() + " info="
                    + threads[i]);
        }
    }
    ...
    static {
        try {
            method_printType = Util.class.getDeclaredMethod("printType", new Class[] { Object.class });
