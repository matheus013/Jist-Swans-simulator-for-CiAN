...
public final class Util {
	...
	/**
	 * Escape character in string. Given character is replaced with escape
	 * characters and hexadecimal representation.
	 * 
	 * @param s
	 *            string to escape
	 * @param c
	...
	public static String escapeChar(String s, char c) {
		...
		while (index != -1) {
			s = s.substring(0, index) + IDENTIFIER_ESCAPE_CHAR + getHexCode(c)
	...
	public static void showThreads() {
		...
		for (int i = 0; i < threads.length; i++) {
			...
			System.out.println("thread: alive=" + threads[i].isAlive()
					+ " daemon=" + threads[i].isDaemon() + " info="
					+ threads[i]);
		}
	}
	...
	/**
	 * Method that prints the type of the object passed to it. Useful for
	 * rewritter debugging as in: <code>
	 *   DUP
	...
	static {
		try {
			method_printType = Util.class.getDeclaredMethod("printType",
	 ...
	 * @throws IOException
	 *             on error from given inputstream
	 */
	...
	/**
	 * Return the Integer for a small integer, hopefully without a new
	 * allocation.
	 * 
	 * @param i
