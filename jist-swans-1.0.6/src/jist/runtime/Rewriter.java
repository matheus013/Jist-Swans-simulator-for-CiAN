 ...
 * applications at load-time to run under "Simulation Time". This implementation
 * uses the Apache Byte-Code Engineering Library (BCEL).
 * 
...
public final class Rewriter extends ClassLoader {
	 ...
	 */
	public static final String JIST_ENTITYREF = JIST_FIELD_PREFIX
			+ JIST_REF_SUFFIX;
	 ...
	 */
	public static final String JIST_STATIC_TRIGGER = JIST_FIELD_PREFIX
	 ...
	 */
	public static final String[] timeless = { Boolean.class.getName(),
			Character.class.getName(), Float.class.getName(),
			Double.class.getName(), Byte.class.getName(),
	 ...
	 */
	private static final String[] ignoredPackages = new String[] { "java.",
			"javax.", "sun.", "com.sun.", "jist.runtime.", "org.apache.bcel.",
			"org.apache.log4j.", "jargs.gnu.", "bsh.", "org.python.", "apple.",
			"org.xml.", "com.mysql.", };
	...
	/**
	 * Rewrite cache. This is not just for performance... Must have for
	 * type-safety.
	 */
	...
	static {
		try {
			method_initializeMethodStubs = Rewriter.class.getDeclaredMethod(
					"initializeMethodStubs", new Class[] { String.class });
	...
	/**
	 * Initialize an Rewriter instance, with a list of packages to be processed
	 * (others are ignored).
	 * 
	 * @param processedPackages
	 *            list of packages to process, or null to process all packages,
	 *            except those in the (default) ignoredPackages list.
	 * @param cacheDir
	 *            directory for rewritten classfile cache
	 * @param resources
	 *            remote resources finder
	 * @param serverOut
	 *            server machine output stream
	 */
	public Rewriter(String[] processedPackages, String cacheDir,
			RemoteJist.ResourceFinderRemote resources, PrintStream serverOut) {
		this.processedPackages = processedPackages;
		this.cacheDir = cacheDir;
		this.resources = resources;
		synchronized (repositoryLock) {
			this.jistRepository = new RemoteJist.RemoteRepository(resources,
					serverOut);
		}
		...
		// @author Elmar Schoch
		// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
		// Splitted up classfile lookup and call of "getTime()", since this
		// sometime led to
		// NullPointerExceptions

		// this.rewriterTime =
		// Repository.lookupClassFile(Rewriter.class.getName()).getTime();

		ClassPath.ClassFile cf = Repository.lookupClassFile(Rewriter.class
				.getName());
		if (cf != null) {
		...
		for (int i = 0; i < blockingClass.length; i++) {
			java.lang.reflect.Method[] methods = blockingClass[i]
					.getDeclaredMethods();
			for (int j = 0; j < methods.length; j++) {
				blocking.add(getSignature(methods[j]).intern());
				if (log.isDebugEnabled()) {
					log.debug("Found blocking method (blockingClass): "
							+ getSignature(methods[j]));
				}
			}
		}
		for (int i = 0; i < blockingMethod.length; i++) {
			...
			if (log.isDebugEnabled()) {
				log.debug("Found blocking method (blockingMethod): "
						+ getSignature(blockingMethod[i]));
			}
		}
	 ...
	 * @throws VerifyError
	 *             throw if given class does not satify various JIST programming
	 *             constraints.
	...
	public Class loadClass(String name, boolean resolve)
		...
		return isIgnored(name) ? super.loadClass(name, resolve) : this
				.findClass(name);
	}

	/**
	 * Called by loadClass and actually does all the rewriting work, but checks
	 * in caches first.
	 * 
	 * @param name
	 *            qualified class name (e.g. java.lang.String)
	...
	protected Class findClass(String name) throws ClassNotFoundException,
		...
		if (Main.REWRITE_CACHE && cacheDir != null)
			cl = getDiskRewrittenClass(name);
		if (cl == null) {
			synchronized (repositoryLock) {
				// initialize bcel repository, just in case it is accessed
				// internally during rewriting
				org.apache.bcel.util.Repository oldRepository = Repository
						.getRepository();
	 ...
	 * the ONLY point through which the Rewriter retrieves new JavaClass
	 * objects. Therefore, it is also a convenient point to perform caching and
	...
	public JavaClass lookupJavaClass(String name) throws ClassNotFoundException {
		...
		if (!(isIgnored(name) || isDoNotRewrite(jcl))) {
			for (int i = rewriters.size() - 1; i >= 0; i--) {
				JistAPI.CustomRewriter cr = (JistAPI.CustomRewriter) rewriters
						.elementAt(i);
				jcl = cr.process(jcl);
			}
		}
		...
		try {
			...
			System.out
					.println("Rewriter:507 ClassNotFoundException ignored (didn't find superclass of "
							+ jcl + ")");
		}
		// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

		if (sup != null)
			lookupJavaClass(sup.getClassName());
		// process callgraph
		if (!isIgnored(name)) {
			if (log.isDebugEnabled())
				log.debug("** updating call graph after loading: " + name);
	...
	public Class define(JavaClass jcl) {
		...
		if (log.isDebugEnabled())
			log.debug("** converting class structure back into bytecode for "
					+ name);
		byte[] b = jcl.getBytes();
		if (Main.REWRITE_CACHE && cacheDir != null)
			putDiskRewrittenClass(name, b);
	 ...
	 * @throws ClassNotFoundException
	 *             failure to find referenced class
	 */
	protected JavaClass rewriteClass(JavaClass jcl)
			throws ClassNotFoundException, VerifyError {
		String name = jcl.getClassName();
		...
		{
			if (log.isDebugEnabled())
				log.debug("** verifying class " + name);
			...
			if (errors.length > 0) {
				for (int i = 0; i < errors.length; i++) {
					if (log.isDebugEnabled())
						log.debug("verification error: " + errors[i]);
				}
		...
		if (isEntity(jcl)) {
			if (log.isDebugEnabled())
				log.debug("** verifying entity " + name);
			RewriterTraversalVerifyEntity ve = new RewriterTraversalVerifyEntity(
					this);
		...
		if (isEntity(jcl)) {
			if (log.isDebugEnabled())
				log.debug("** adding accessor methods for public entity fields "
						+ name);
			jcl = (new ClassTraversal(new RewriterTraversalAddAccessorMethods()))
					.processClass(jcl);
			if (log.isDebugEnabled())
				log.debug("** adding method stubs for entity " + name);
			jcl = (new ClassTraversal(
					new RewriterTraversalAddEntityMethodStubs(this)))
					.processClass(jcl);
			if (log.isDebugEnabled())
				log.debug("** adding entity self reference field and accessors for entity "
						+ name);
			jcl = (new ClassTraversal(new RewriterTraversalAddSelfEntityRef(
					true))).processClass(jcl);
			if (log.isDebugEnabled())
				log.debug("** adding entity interface to entity " + name);
			jcl = (new ClassTraversal(
					new RewriterTraversalImplementEntityInterface(true)))
					.processClass(jcl);
		}
		// timeless modifications
		if (isTimeless(jcl)) {
			if (log.isDebugEnabled())
				log.debug("** adding timeless interface to timeless object "
						+ name);
			jcl = (new ClassTraversal(
					new RewriterTraversalImplementTimelessInterface()))
					.processClass(jcl);
		}
		// common modifications
		if (jcl.isClass()) {
			if (log.isDebugEnabled())
				log.debug("** modifying all entity references in class " + name);
			jcl = (new ClassTraversal(
					new RewriterTraversalModifyEntityReferences(this)))
					.processClass(jcl);
			if (log.isDebugEnabled())
				log.debug("** removing field access from class " + name);
			jcl = (new ClassTraversal(new RewriterTraversalRemoveFieldAccess(
					this))).processClass(jcl);
			if (log.isDebugEnabled())
				log.debug("** modifying all entity creation in class " + name);
			jcl = (new ClassTraversal(
					new RewriterTraversalModifyEntityCreation(this)))
					.processClass(jcl);
			if (log.isDebugEnabled())
				log.debug("** modifying all entity invocations in class "
						+ name);
			jcl = (new ClassTraversal(
					new RewriterTraversalModifyEntityInvocation(this)))
					.processClass(jcl);
			if (log.isDebugEnabled())
				log.debug("** replacing all Jist API calls " + name);
			jcl = (new ClassTraversal(new RewriterTraversalTranslateAPICalls()))
					.processClass(jcl);
		}
		// proxy entity modifications
		if (isProxiable(jcl)) {
			// note: the verifier checks that an entity can not implement the
			// proxiable interface
			// (i.e. only non-entities pass through here)
			if (log.isDebugEnabled())
				log.debug("** adding entity self reference field and accessors for proxiable object "
						+ name);
			jcl = (new ClassTraversal(new RewriterTraversalAddSelfEntityRef(
					false))).processClass(jcl);
			if (log.isDebugEnabled())
				log.debug("** adding entity interface to proxiable object "
						+ name);
			jcl = (new ClassTraversal(
					new RewriterTraversalImplementEntityInterface(false)))
					.processClass(jcl);
		}
		// continuable method modifications
		if (log.isDebugEnabled())
			log.debug("** modifying continuable methods: " + name);
		jcl = (new ClassTraversal(new RewriterTraversalContinuableMethods(this)))
				.processClass(jcl);
		return jcl;
	}
	...
	protected Class getDiskRewrittenClass(String name) {
		try {
			...
			long cltime = resources
					.getResourceLastModificationDate(classToFileName(name));
			if (cltime > cachetime)
				return null;
			// ensure cached class is newer than rewriter(s)
			if (rewriterTime > cachetime)
				return null;
			for (int i = 0; i < rewriters.size(); i++) {
				long time = resources
						.getResourceLastModificationDate(classToFileName(rewriters
			...
			if (log.isDebugEnabled())
				log.debug("** loading rewritten class from cache: " + name);
			return defineClass(name, b, 0, b.length);
		} catch (Exception e) {
			if (log.isDebugEnabled())
				log.debug("unable to read cache class file: " + e);
			return null;
		}
	}
	...
	/**
	 * Add signature to list of continables. Verify that it is not a
	 * constructor, and return whether it is newly discovered.
	 * 
	 * @param sig
	 *            continuable to add
	 * @return whether this is a newly found continuable method
	 * @throws VerifyError
	 *             if signature is of an initialization method
	 */
	 ...
	 * @throws ClassNotFoundException
	 *             failure to load class in call graph
	 */
	public void updateCallGraphAndContinuable(JavaClass jcl)
			throws ClassNotFoundException {
		...
		for (int i = 0; i < methods.length; i++) {
			...
			for (int j = 0; j < instructions.length; j++) {
				...
				if (inst instanceof InvokeInstruction) {
					...
					// now, classify the call are entity call or regular method
					// call
					if (ii instanceof INVOKEVIRTUAL
							&& isEntity(ii.getClassName(cpg))
							|| ii instanceof INVOKEINTERFACE) {
						// regular or proxy entity call
						if (isBlocking(ii.getClassName(cpg),
								ii.getMethodName(cpg), ii.getReturnType(cpg),
	 ...
	 *             failure to find referenced class
	 */
	public String[] getAllClassReferencesRecursively(String classname)
			throws ClassNotFoundException {
	...
	/**
	 * Return an array of the names of all interfaces implemented by a given
	 * class.
	 * 
	 * @param jcl
	 *            BCEL JavaClass object to inspect
	 * @throws ClassNotFoundException
	 *             interface classes not found
	 * @return array of the names of all interfaces implemented.
	 */
	public String[] getInterfaceNames(JavaClass jcl)
			throws ClassNotFoundException {
	 ...
	 * 
	 * @param classname
	 *            classname to initiate search from
	 * @param methodname
	 *            name of method being sought
	 * @param methodreturn
	 *            BCEL return type of method being sought
	 * @param methodargs
	 ...
	 */
	public String getDeclaringClass(String classname, String methodname,
			Type methodreturn, Type[] methodargs) throws ClassNotFoundException {
		if (classname == null)
			return null;
		// check locally
		JavaClass jcl = lookupJavaClass(classname);
		...
		if (!classname.equals(sup)) {
			String result = getDeclaringClass(jcl.getSuperclassName(),
					methodname, methodreturn, methodargs);
			if (result != null)
				return result;
		}
		...
		for (int i = 0; i < interf.length; i++) {
			String result = getDeclaringClass(interf[i], methodname,
					methodreturn, methodargs);
			if (result != null)
				return result;
		}
		return null;
	 ...
	 *            method name
	 * @param methodreturn
	 *            method return BCEL type
	 * @param methodargs
	 *            method parameter BCEL types
	 * @return whether method is contained within class
	 */
	public static boolean containsMethod(JavaClass jcl, String methodname,
			Type methodreturn, Type[] methodargs) {
		String sig = Type.getMethodSignature(methodreturn, methodargs);
		Method[] methods = jcl.getMethods();
		for (int i = 0; i < methods.length; i++) {
			if (!methodname.equals(methods[i].getName()))
				continue;
			if (!sig.equals(methods[i].getSignature()))
				continue;
			return true;
		}
		return false;
	...
	/**
	 * Determine whether a class should be rewritten or left alone. If
	 * processedPackages is defined, then we only process packages with those
	 * prefixes. Otherwise, any package that does not begin with a prefix in the
	...
	/**
	 * Determine whether a class should be rewritten or left alone. Any package
	 * that does not begin with a prefix in the <code>ignoredPackages</code>
	 * array is rewritten.
	 * 
	 * @param classname
	 *            given class name
	...
	public static boolean isIgnoredStatic(String classname) {
		for (int i = 0; i < ignoredPackages.length; i++) {
			if (classname.indexOf('.') != -1
					&& classname.startsWith(ignoredPackages[i])) {
				return true;
			}
		}
		return false;
	...
	/**
	 * Determine whether given class is an entity. A class is an entity if and
	 * only if it implements the <code>JistAPI.Entity</code> interface.
	 * 
	 * @see JistAPI.Entity
	 * @param c
	 *            BCEL class object
	...
	public boolean isEntity(JavaClass c) throws ClassNotFoundException {
		...
		if (c.isInterface())
			return false;
		return Util.contains(getInterfaceNames(c),
				JistAPI.Entity.class.getName());
	}
	...
	/**
	 * Determine whether given class is an entity. A class is an entity (at
	 * runtime) if and only if it implement the <code>Entity</code> interface.
	 * 
	 * @see JistAPI.Entity
	 * @param c
	...
	/**
	 * Determine whether given class is timeless. A class is timeless if and
	 * only if it implements the <code>JistAPI.Timeless</code> interface, or the
	 * object is immutable, as determined automatically or through membership in
	 * the static timeless list.
	 * 
	...
	public boolean isTimeless(JavaClass c) throws ClassNotFoundException {
		return Util.contains(getInterfaceNames(c),
				JistAPI.Timeless.class.getName())
				|| timelessHash.containsKey(c.getClassName());
		// todo: or, if a class is determined to be immutable by static
		// analysis!
	}

	/**
	 * Determine whether given class is timeless. A class is timeless if and
	 * only if it implements the <code>JistAPI.Timeless</code> interface, or it
	 * is determined to be immutable.
	 * 
	 * @see JistAPI.Entity
	 * @param name
	 *            class name
	...
	/**
	 * Determine whether given type is timeclass. A type is timeless if and only
	 * if it is a primitive type or it is a timeless class
	 * 
	 * @param type
	 *            type
	...
	/**
	 * Determine whether given class is timeless. A class is timeless (at
	 * runtime) if and only if it implements the <code>Timeless</code>
	 * interface.
	 * 
	 * @see JistAPI.Timeless
	 * @param c
	...
	public boolean isProxiable(JavaClass c) throws ClassNotFoundException {
		return c.isClass()
				&& !c.isAbstract()
				&& Util.contains(getInterfaceNames(c),
						JistAPI.Proxiable.class.getName());
	}

	/**
	 * Determine whether given class is proxiable. A class is proxiable if and
	 * only if it implements the <code>JistAPI.Proxiable</code> interface.
	 * 
	 * @see JistAPI.Entity
	 * @param name
	 *            class name
	...
	/**
	 * Whether a given method has blocking semantics (whether the caller needs
	 * to call with continuation). A method is blocking iff it declares that it
	...
	public static boolean isBlocking(MethodGen mg) {
		return Util.contains(mg.getExceptions(),
				JistAPI.Continuation.class.getName());
	}
	 ...
	 * @throws ClassNotFoundException
	 *             could not find parent classes
	 * @return whether method is blocking
	 */
	public boolean isBlocking(String classname, String methodname,
			Type methodreturn, Type[] methodargs) throws ClassNotFoundException {
		classname = getDeclaringClass(classname, methodname, methodreturn,
				methodargs);
		if (classname == null)
			return false;
		return blocking.contains(getSignature(classname, methodname,
				methodreturn, methodargs));
	}
	...
	public static boolean isBlockingRuntime(java.lang.reflect.Method m) {
		return Util.contains(m.getExceptionTypes(),
				Event.ContinuationFrame.class);
	}
	...
	/**
	 * Whether a given method is marked as "continuable". Continuable methods
	...
	public boolean isContinuable(MethodGen mg) {
		return Util.contains(mg.getExceptions(),
				JistAPI.Continuable.class.getName())
	...
	public boolean isDoNotRewrite(JavaClass c) throws ClassNotFoundException {
		if (c == null)
			return false;
		return Util.contains(getInterfaceNames(c),
				JistAPI.DoNotRewrite.class.getName());
	}
	...
	public static void setClassName(JavaClass jcl, String name) {
		...
		ConstantPool cp = jcl.getConstantPool();
		ConstantClass cc = (ConstantClass) cp.getConstant(
				jcl.getClassNameIndex(), Constants.CONSTANT_Class);
		ConstantUtf8 nm = (ConstantUtf8) cp.getConstant(cc.getNameIndex(),
	...
	/**
	 * Return the reference type of given type. The reference type of an object
	 * type is an EntityRef if the object is an entity. All other types
	 * (non-entity objects, arrays and primitives) are their own reference
	 * types. Array types are processed recursively.
	 * 
	 * @param t
	 *            Java type
	...
	public Type getRefType(Type t) throws ClassNotFoundException {
		...
		if (t instanceof ArrayType) {
			...
			return new ArrayType(getRefType(at.getBasicType()),
					at.getDimensions());
		}
		return t;
	}
	...
	/**
	 * Return an array of reference types for a given array of types. The
	 * reference type of an object type is an EntityRef if the object is an
	 * entity. All other types (non-entity objects, arrays and primitives) are
	 * their own reference types. Array types are processed recursively.
	 * 
	 * @param t
	 *            array of Java types
	...
	/**
	 * Return whether all types in array are non-entities. (i.e. whether
	 * anything in the array needs to be converted to an entity reference)
	 * 
	 * @param t
	 *            array of types
	 ...
	 * a primitive. (eg. doubleValue to convert Double to double.)
	 * 
	 * @param t
	 ...
	 * @param ret
	 *            method return type
	 * @return internal Java signature of method with given parameter and return
	 *         types
	 */
	 ...
	 */
	public static String getSignature(String classname, String methodname,
			Type methodreturn, Type[] methodargs) {
		return classname
				+ "."
				+ methodname
	 ...
	 */
	public String getMethodStubFieldName(MethodGen mg)
			throws ClassNotFoundException {
	...
	public static String getMethodStubFieldName(java.lang.reflect.Method m) {
		return getMethodStubFieldName(m.getName(),
				getSignature(m.getParameterTypes(), m.getReturnType()));
	}

	/**
	 * Return the name of the method stub field for a method with given name and
	 ...
	 * @return name of method stub field for method with given name and
	 *         signature
	 */
	public static String getMethodStubFieldName(String methodName, String sig) {
		return JIST_METHODSTUB_PREFIX
				+ Util.escapeJavaIdent(methodName + sig.replace('/', '.'));
	}

	/**
	 * Return the name of the class used to store the continuation of a method
	 * at a given "program counter" point. The name needs to be a valid class
	 * name, to be in the same package as the method's class, to be unique among
	 * other (class, method name, method arguments, pc) values.
	 * 
	 * @param mg
	 *            method being continued
	 * @param pc
	...
	public static String getContinutationClassName(MethodGen mg, int pc) {
		...
		return pkgDot
				+ Rewriter.JIST_CONTINUATION_STATE_PREFIX
				+ "_"
				+ cl
				+ "_"
				+ mg.getName()
				+ "_"
				+ Integer.toHexString(Util.escapeJavaIdent(mg.getSignature())
						.hashCode()) + "_" + pc;
	}

	/**
	 * Return the class name of the given Type or array type, or null for
	 * primitive types, or arrays of primitives.
	 * 
	 * @param t
	 *            BCEL Type object
	 ...
	 * method uses reflection to simply initialize the static stubs in a generic
	 * manner at load time with Method objects. It is called by each entity in
	 * its class initializer. The same (or more specific) code could have been
	 * placed in each class initializer, but this saves a lot of bytecode
	 * generation during transformation, and has no performance implications.
	 * 
	 * @param classname
	 *            class name that should have its static stubs initialized with
	 *            Method objects
	 */
	public static void initializeMethodStubs(String classname) {
		if (log.isDebugEnabled())
			log.debug("** generic runtime method stub field initializer called for "
					+ classname);
		try {
			Rewriter rewriter = (Rewriter) Controller.getActiveController()
					.getClassLoader();
			...
			for (int i = 0; i < methods.length; i++) {
				...
				if (methods[i].getName().equals("_jistMethod_Set__ref"))
					continue;
				if (methods[i].getName().equals("_jistMethod_Get__ref"))
					continue;
				// skip any static methods
				if (java.lang.reflect.Modifier.isStatic(methods[i]
						.getModifiers()))
					continue;
				if (log.isDebugEnabled())
					log.debug("initializing stub field for method: "
							+ methods[i]);
				// turn off access checks
				methods[i].setAccessible(true);
				// find appropriate method stub field
				java.lang.reflect.Field methodStubField = cl
						.getField(getMethodStubFieldName(methods[i]));
				// paranoia check
				if (methodStubField.get(null) != null) {
					throw new RuntimeException(
							"static method field stub already assigned: "
									+ methodStubField);
				}
	...
	/**
	 * Prime the rewriter doing a breadth-first search loading (and rewrite) of
	 * all classes transitively statically referenced.
	 * 
	 * @param classname
	...
	public int prime(String classname) throws ClassNotFoundException {
		if (log.isDebugEnabled())
			log.debug("** priming class rewriter starting with: " + classname);
 ...
 * entity objects that have static, abstract or native methods. All entity state
 * should be declared private. All non-private methods should return void.
 * 
 * @see JistAPI.Entity
 * @author Rimon Barr &lt;barr+jist@cs.cornell.edu&gt;
 * @since JIST1.0
...
class RewriterTraversalVerifyEntity extends ClassTraversal.Empty {
	...
	public ClassGen doClassPost(ClassGen cg) throws ClassNotFoundException {
		if (Rewriter.log.isDebugEnabled())
			Rewriter.log.debug("number of errors found in class "
					+ cg.getClassName() + ": " + errors.size());
		if (rewriter.isProxiable(cg.getJavaClass()))
			errors.add("Proxiable entity interface found on Entity class: "
					+ cg.getClassName());
		return cg;
	}
	...
	public FieldGen doField(ClassGen cg, FieldGen fg) {
		if (Rewriter.log.isDebugEnabled())
			Rewriter.log.debug("verifying field " + fg.getName());
		if (fg.isStatic())
	...
	public MethodGen doMethod(ClassGen cg, MethodGen mg)
			throws ClassNotFoundException {
		if (Rewriter.log.isDebugEnabled())
			Rewriter.log.debug("verifying method " + mg.getName());
		if (mg.isStatic()
				&& !(mg.getName().equals(Rewriter.JAVA_MAIN_NAME) && mg
		...
		if (!mg.isPrivate() && !rewriter.isBlocking(mg)
				&& !mg.getReturnType().equals(Type.VOID))
			errors.add("Non-private method has non-void return: "
					+ mg.getName());
...
class RewriterTraversalVerifyAll extends ClassTraversal.Empty {
	...
	/** {@inheritDoc} */
	public MethodGen doMethod(ClassGen cg, MethodGen mg) {
		if (rewriter.isContinuable(mg)
				&& Constants.CONSTRUCTOR_NAME.equals(mg.getName())) {
			errors.add("illegal to have continuation within object constructor: "
					+ mg.getClassName());
		}
		return mg;
	}
...
/**
 * Traversal object that creates both set and get accessor methods for each
 * public field. Finally, all fields are converted to protected access.
 * 
 * @author Rimon Barr &lt;barr+jist@cs.cornell.edu&gt;
 * @since JIST1.0
 */
...
class RewriterTraversalAddAccessorMethods extends ClassTraversal.Empty {
	...
	public FieldGen doField(ClassGen cg, FieldGen fg) {
		...
		if (fg.isPublic()) {
			if (Rewriter.log.isDebugEnabled())
				Rewriter.log.debug("adding set accessor method for "
						+ cg.getClassName() + "." + fg.getName());
			InstructionList il = new InstructionList();
			if (!fg.isStatic())
				il.append(ifc.createThis());
			il.append(ifc.createLoad(fg.getType(), fg.isStatic() ? 0 : 1));
			il.append(ifc.createFieldAccess(cg.getClassName(), fg.getName(), fg
					.getType(), fg.isStatic() ? Constants.PUTSTATIC
					: Constants.PUTFIELD));
			il.append(InstructionConstants.RETURN);
			MethodGen mg = new MethodGen(Constants.ACC_PUBLIC, Type.VOID,
					new Type[] { fg.getType() }, new String[] { "_x" },
					Rewriter.JIST_METHOD_SET + fg.getName(), cg.getClassName(),
		...
		if (fg.isPublic()) {
			if (Rewriter.log.isDebugEnabled())
				Rewriter.log.debug("adding get accessor method for "
						+ cg.getClassName() + "." + fg.getName());
			InstructionList il = new InstructionList();
			if (!fg.isStatic())
				il.append(ifc.createThis());
			il.append(ifc.createFieldAccess(cg.getClassName(), fg.getName(), fg
					.getType(), fg.isStatic() ? Constants.GETSTATIC
					: Constants.GETFIELD));
			il.append(ifc.createReturn(fg.getType()));
			MethodGen mg = new MethodGen(Constants.ACC_PUBLIC, fg.getType(),
					new Type[] {}, new String[] {}, Rewriter.JIST_METHOD_GET
							+ fg.getName(), cg.getClassName(), il,
					cg.getConstantPool());
			mg.isStatic(fg.isStatic());
			mg.setMaxStack();
			mg.setMaxLocals();
		...
		if (fg.isPublic()) {
			if (Rewriter.log.isDebugEnabled())
				Rewriter.log.debug("converting field to private access "
						+ cg.getClassName() + "." + fg.getName());
			fg.isPrivate(false);
			fg.isProtected(true);
			fg.isPublic(false);
...
class RewriterTraversalRemoveFieldAccess extends ClassTraversal.Empty {
	...
	/**
	 * Create rewriter object to convert all remote field accesses to method
	 * accesses.
	 * 
	 * @param rewriter
	 *            reference to primary rewriter
	 ...
	 */
	private boolean isLocalNonPublicFieldAccess(ClassGen cg,
			ConstantPoolGen cpg, FieldInstruction inst) {
		// check local
		if (!cg.getClassName().equals(inst.getClassName(cpg)))
			return false;
		// find field
	...
	public void doInstruction(ClassGen cg, MethodGen mg, InstructionHandle ih,
		...
		if (inst instanceof FieldInstruction) {
			...
			if (rewriter.isEntity(fieldinst.getClassName(cpg))
					...
					&& !fieldinst.getFieldName(cg.getConstantPool())
							.startsWith(Rewriter.JIST_FIELD_PREFIX)
					&& !isLocalNonPublicFieldAccess(cg, cpg, fieldinst)) {
				if (inst instanceof GETFIELD || inst instanceof GETSTATIC) {
					inst = ifc.createInvoke(
							fieldinst.getClassName(cpg),
							Rewriter.JIST_METHOD_GET
									+ fieldinst.getFieldName(cpg), fieldinst
									.getType(cpg), new Type[] {},
							inst instanceof GETFIELD ? Constants.INVOKEVIRTUAL
									: Constants.INVOKESTATIC);
				}
				if (inst instanceof PUTFIELD || inst instanceof PUTSTATIC) {
					inst = ifc.createInvoke(
							fieldinst.getClassName(cpg),
							Rewriter.JIST_METHOD_SET
									+ fieldinst.getFieldName(cpg), Type.VOID,
							new Type[] { fieldinst.getType(cpg) },
							inst instanceof PUTFIELD ? Constants.INVOKEVIRTUAL
									: Constants.INVOKESTATIC);
				}
				if (Rewriter.log.isDebugEnabled())
					Rewriter.log.debug("replacing field access instruction ("
							+ fieldinst.toString(cg.getConstantPool()
									.getConstantPool()) + ") in "
 ...
 * class into EntityRef objects. This means that all method parameter and method
 * return types, as well as all field types, that implement the JistAPI.Entity
 * interface are converted into the EntityRef type.
 * 
 * @author Rimon Barr &lt;barr+jist@cs.cornell.edu&gt;
 * @since JIST1.0
...
class RewriterTraversalModifyEntityReferences extends ClassTraversal.Empty {
	...
	public FieldGen doField(ClassGen cg, FieldGen fg)
		...
		if (!rewriter.isRefType(fg.getType())) {
			if (Rewriter.log.isDebugEnabled())
				Rewriter.log
						.debug("changing type to entity reference for field "
								+ cg.getClassName() + "." + fg.getName());
	...
	public MethodGen doMethod(ClassGen cg, MethodGen mg)
		...
		if (!rewriter.isRefTypes(mg.getArgumentTypes())) {
			if (Rewriter.log.isDebugEnabled())
				Rewriter.log
						.debug("changing parameters to entity references for method "
								+ cg.getClassName() + "." + mg.getName());
		...
		if (!rewriter.isRefType(mg.getReturnType())) {
			if (Rewriter.log.isDebugEnabled())
				Rewriter.log
						.debug("changing return type to entity reference for method "
								+ cg.getClassName() + "." + mg.getName());
	...
	public void doInstruction(ClassGen cg, MethodGen mg, InstructionHandle ih,
		...
		if (inst instanceof FieldInstruction) {
			...
			if (!rewriter.isRefType(finst.getType(cpg))) {
				short itype = inst instanceof GETFIELD ? Constants.GETFIELD
				...
				if (Rewriter.log.isDebugEnabled())
					Rewriter.log
							.debug("changing entity reference for field instruction in "
									+ cg.getClassName() + "." + mg.getName());
		...
		if (inst instanceof InvokeInstruction) {
			...
			if (!rewriter.isRefType(invinst.getReturnType(cpg))
				...
				short itype = inst instanceof INVOKEINTERFACE ? Constants.INVOKEINTERFACE
				...
				if (Rewriter.log.isDebugEnabled())
					Rewriter.log
							.debug("changing types to entity references for invocation instruction ("
									+ inst
									+ ") in "
									+ cg.getClassName()
									+ "."
									+ mg.getName());
				ih.swapInstruction(ifc.createInvoke(invinst.getClassName(cpg),
						invinst.getMethodName(cpg),
		...
		if (inst instanceof ANEWARRAY) {
			...
			if (!rewriter.isRefType(nainst.getType(cpg))) {
				if (Rewriter.log.isDebugEnabled())
					Rewriter.log
							.debug("modifying entity array new instruction in "
									+ cg.getClassName() + "." + mg.getName());
		...
		if (inst instanceof CHECKCAST) {
			...
			if (Rewriter.log.isDebugEnabled())
				Rewriter.log
						.debug("modifying entity check cast instruction in "
								+ cg.getClassName() + "." + mg.getName());
			if (!rewriter.isRefType(ci.getType(cpg))) {
				ih.swapInstruction(ifc.createCast(Type.OBJECT,
						rewriter.getRefType(ci.getType(cpg))));
			}
		}
		...
		if (inst instanceof INVOKESPECIAL) {
			...
			if (is.getMethodName(cpg).equals(Constants.CONSTRUCTOR_NAME)) {
				if (rewriter.isEntity(is.getClassName(cpg)) && NEWs > 0) {
					...
					if (Rewriter.log.isDebugEnabled())
						Rewriter.log
								.debug("modifying entity creation point in "
										+ cg.getClassName() + "."
										+ mg.getName());
...
/**
 * Traversal object that intercepts any method invocation instructions to entity
 * objects. This conversion is a little bit tricky, because it involves a bit
 * more bytecode generation. The basic idea has three parts. First, the
 ...
 * stack are placed an object array of the appropriate size. Primitive types are
 * first converted into their object equivalents.
 * 
 * @author Rimon Barr &lt;barr+jist@cs.cornell.edu&gt;
 * @since JIST1.0
 * @see Controller#newEntityReference
...
class RewriterTraversalModifyEntityInvocation extends ClassTraversal.Empty {
	 ...
	 *            instruction handle
	 */
	private void packParametersIntoArray(InstructionList il, Type[] args,
			MethodGen mg, InstructionHandle ih) throws ClassNotFoundException {
		if (args.length == 0) {
			...
			for (int i = args.length - 1; i >= 0; i--) {
				...
				if (args[i] instanceof BasicType) {
					...
					il.append(ifc.createInvoke(oname,
							Constants.CONSTRUCTOR_NAME, Type.VOID,
							new Type[] { args[i] }, Constants.INVOKESPECIAL));
				...
				// insert into array
				il.append(ifc.createLoad(new ArrayType(Type.OBJECT, 1),
						maxlocals));
	...
	public void doInstruction(ClassGen cg, MethodGen mg, InstructionHandle ih,
		...
		if (inst instanceof INVOKEVIRTUAL) {
			...
			boolean isBlocking = rewriter.isBlocking(iv.getClassName(cpg),
					iv.getMethodName(cpg), retType, iv.getArgumentTypes(cpg));
			// if entity invocation
			if (rewriter.isEntity(iv.getClassName(cpg))
					&& (iv.getReturnType(cpg).equals(Type.VOID) || isBlocking)) {
				if (Rewriter.log.isDebugEnabled())
					Rewriter.log.debug("modifying entity invocation ("
							+ iv.toString(cpg.getConstantPool()) + ") in "
							+ cg.getClassName() + "." + mg.getName());
				InstructionList il = new InstructionList();
				...
				// put method stub on stack, behind object reference and
				// argument array
				il.append(ifc.createGetStatic(iv.getClassName(cpg), Rewriter
						.getMethodStubFieldName(iv.getName(cpg),
								iv.getSignature(cpg)), new ObjectType(
				...
				java.lang.reflect.Method m = isBlocking ? Controller.method_entityInvocationCont
						: Controller.method_entityInvocation;
				il.append(ifc.createInvoke(m.getDeclaringClass().getName(),
						m.getName(), Rewriter.getType(m.getReturnType()),
				...
				if (isBlocking) {
					if (retType.getType() == Constants.T_VOID) {
						...
						il.append(ifc.createCheckCast(new ObjectType(Rewriter
								.getPrimitiveObjectType((BasicType) retType)
								.getName())));
						il.append(ifc.createInvoke(
								Rewriter.getPrimitiveObjectType(
										(BasicType) retType).getName(),
								Rewriter.getPrimitiveObjectConversionMethod((BasicType) retType),
								retType, new Type[] {}, Constants.INVOKEVIRTUAL));
					}
				}
				// insert the code
				InstructionHandle ihStart = mg.getInstructionList().append(ih,
						il);
 ...
 * the appropriate accessor methods (part of the <code>Entity</code> interface).
 * This class also takes care of initializing the ref field. This is done by
 * injecting code just after the invocation of the superclass constructor (the
 * earliest possible time). In this manner, the ref value is initialized for use
 * by any entity method invocations, including any that might be in the
 * constructor body.
 * 
 * @author Rimon Barr &lt;barr+jist@cs.cornell.edu&gt;
 * @since JIST1.0
...
class RewriterTraversalAddSelfEntityRef extends ClassTraversal.Empty {
	...
	public ClassGen doClass(ClassGen cg) {
		...
		// add entityref field
		if (Rewriter.log.isDebugEnabled())
			Rewriter.log.debug("adding entity self reference field for "
					+ cg.getClassName());
		FieldGen fg = new FieldGen(Constants.ACC_PUBLIC, new ObjectType(
				EntityRef.class.getName()), Rewriter.JIST_ENTITYREF,
				cg.getConstantPool());
		cg.addField(fg.getField());
		// add entityref set accessor
		if (Rewriter.log.isDebugEnabled())
			Rewriter.log
					.debug("adding entity self reference set accessor method for "
							+ cg.getClassName());
		InstructionList il = new InstructionList();
		il.append(ifc.createThis());
		...
		il.append(InstructionConstants.RETURN);
		MethodGen mg = new MethodGen(fg.getAccessFlags() | Constants.ACC_FINAL,
				Type.VOID, new Type[] { fg.getType() }, new String[] { "_x" },
		...
		if (Rewriter.log.isDebugEnabled())
			Rewriter.log
					.debug("adding entity self reference get accessor method for "
							+ cg.getClassName());
		...
		il.append(ifc.createReturn(fg.getType()));
		mg = new MethodGen(fg.getAccessFlags() | Constants.ACC_FINAL,
				fg.getType(), new Type[] {}, new String[] {},
				Rewriter.JIST_METHOD_GET + Rewriter.JIST_REF_SUFFIX,
				cg.getClassName(), il, cg.getConstantPool());
		mg.setMaxStack();
		mg.setMaxLocals();
	...
	public void doInstruction(ClassGen cg, MethodGen mg, InstructionHandle ih,
		...
		if (initializeRef
				...
				&& ((INVOKESPECIAL) inst).getMethodName(cg.getConstantPool())
						.equals(Constants.CONSTRUCTOR_NAME)
				&& ((INVOKESPECIAL) inst).getClassName(cg.getConstantPool())
						.equals(cg.getSuperclassName())) {
			if (Rewriter.log.isDebugEnabled())
				Rewriter.log
						.debug("modifying superclass constructor call to initialize self reference in "
								+ cg.getClassName());
			...
			il.append(ifc.createInvoke(Controller.class.getName(),
					...
					new ObjectType(EntityRef.class.getName()), Rewriter
							.getTypes(Controller.method_newEntityReference
									.getParameterTypes()),
					Constants.INVOKESTATIC));
...
class RewriterTraversalImplementEntityInterface extends ClassTraversal.Empty {
	 ...
	 * 
	 * @param addStaticTrigger
	 *            whether to add a static verification/initialization trigger
	 *            field
	 */
	...
	public ClassGen doClass(ClassGen cg) {
		...
		if (Rewriter.log.isDebugEnabled())
			Rewriter.log.debug("adding entity interface to "
					+ cg.getClassName());
		cg.addInterface(Entity.class.getName());
		// public package access for entities to allow Rewriter access
		if (Rewriter.log.isDebugEnabled())
			Rewriter.log.debug("making class of " + cg.getClassName()
					+ " entity public");
		cg.isPublic(true);
		...
		if (addStaticTrigger) {
			if (Rewriter.log.isDebugEnabled())
				Rewriter.log.debug("adding static trigger field "
						+ cg.getClassName());
			FieldGen fg = new FieldGen(Constants.ACC_PUBLIC
					| Constants.ACC_STATIC, new ObjectType(
					EntityRef.class.getName()), Rewriter.JIST_STATIC_TRIGGER,
					cg.getConstantPool());
			cg.addField(fg.getField());
		}
		return cg;
...
class RewriterTraversalImplementTimelessInterface extends ClassTraversal.Empty {
	...
	public ClassGen doClass(ClassGen cg) {
		...
		if (Rewriter.log.isDebugEnabled())
			Rewriter.log.debug("adding timeless interface to "
					+ cg.getClassName());
 ...
 * <code>Rewriter.initializeMethodStubs</code>). This method uses reflection (at
 * load time only) to initialize the stub fields.
 * 
 * @author Rimon Barr &lt;barr+jist@cs.cornell.edu&gt;
 * @since JIST1.0
...
class RewriterTraversalAddEntityMethodStubs extends ClassTraversal.Empty {
	...
	public MethodGen doMethod(ClassGen cg, MethodGen mg)
		...
		FieldGen fg = new FieldGen(Constants.ACC_PUBLIC | Constants.ACC_STATIC,
				...
				rewriter.getMethodStubFieldName(mg), cg.getConstantPool());
		if (Rewriter.log.isDebugEnabled())
			Rewriter.log.debug("adding method stub field " + cg.getClassName()
					+ "." + fg.getName());
		cg.addField(fg.getField());
	...
	public ClassGen doClassPost(ClassGen cg) {
		// call generic (reflection-based) runtime initializer to initialize
		// method stubs
		if (Rewriter.log.isDebugEnabled())
			Rewriter.log
					.debug("adding call to method stub generic intializer in static initializer of "
							+ cg.getClassName());
		InstructionList il = new InstructionList();
		il.append(new LDC(cg.getConstantPool().addString(cg.getClassName())));
		...
		if (clinit != null) {
			...
			if (Rewriter.log.isDebugEnabled())
				Rewriter.log
						.debug("new static initializer method created, because it did not exist");
			il.append(ifc.createReturn(Type.VOID));
			clinit = new MethodGen(Constants.ACC_STATIC, Type.VOID,
					Type.NO_ARGS, null, Constants.STATIC_INITIALIZER_NAME,
					cg.getClassName(), il, cg.getConstantPool());
			clinit.setMaxStack();
			clinit.setMaxLocals();
	...
	public void doInstruction(ClassGen cg, MethodGen mg, InstructionHandle ih,
		...
		if (inst instanceof INVOKESTATIC) {
			...
			if (invinst.getClassName(cpg).equals(JistAPI.class.getName())) {
				if (Rewriter.log.isDebugEnabled())
					Rewriter.log.debug("converting "
							+ invinst.getMethodName(cpg) + " API call in "
							+ cg.getClassName() + "." + mg.getName());
				try {
					java.lang.reflect.Method m = (java.lang.reflect.Method) JistAPI_Impl.class
							.getField("method_" + invinst.getMethodName(cpg))
							.get(null);
					ih.swapInstruction(ifc.createInvoke(m.getDeclaringClass()
							.getName(), m.getName(), Rewriter.getType(m
							.getReturnType()), Rewriter.getTypes(m
		...
		if (inst instanceof GETSTATIC) {
			...
			if (getinst.getClassName(cpg).equals(JistAPI.class.getName())) {
				if (Rewriter.log.isDebugEnabled())
					Rewriter.log.debug("converting "
							+ getinst.getFieldName(cpg) + " API field in "
							+ cg.getClassName() + "." + mg.getName());
				try {
					java.lang.reflect.Field f = JistAPI.class
							.getDeclaredField("THIS");
					if (getinst.getFieldName(cpg).equals(f.getName())) {
						java.lang.reflect.Method m = Controller.method_getTHIS;
						ih.swapInstruction(ifc.createInvoke(m
								.getDeclaringClass().getName(), m.getName(),
								Rewriter.getType(m.getReturnType()), Rewriter
										.getTypes(m.getParameterTypes()),
								Constants.INVOKESTATIC));
					}
...
class RewriterTraversalContinuableMethods extends ClassTraversal.Empty {
	...
	public MethodGen doMethod(ClassGen cg, MethodGen mg) {
		...
		if (rewriter.isContinuable(mg) && !mg.isAbstract()) {
			if (Rewriter.log.isDebugEnabled()) {
				Rewriter.log.debug("Processing continuable method: "
						+ mg.getName());
			}
	...
	public void doInstruction(ClassGen cg, MethodGen mg, InstructionHandle ih,
		...
		if (flowinfo != null && inst instanceof InvokeInstruction) {
			...
			if (rewriter.isBlocking(inv.getClassName(cpg),
					inv.getMethodName(cpg), inv.getReturnType(cpg),
					inv.getArgumentTypes(cpg))
	...
	/**
	 * Return BCEL frame with fewer object types (only relevant ones) in order
	 * to simplify frame save/restore code.
	 * 
	 * @param f
	...
	private static Frame fixFrame(Frame f) {
		...
		Stack tmp = new Stack();
		while (!os.isEmpty())
			tmp.push(fixType(os.pop()));
		while (!tmp.isEmpty())
			os.push((Type) tmp.pop());
		return new Frame(lv, os);
	}

	/**
	 * Helper method for fixFrame to process individual types within Frame
	 * object.
	 * 
	 * @param t
	...
	private static Type fixType(Type t) {
		...
		case Constants.T_OBJECT:
			...
			if (ReferenceType.class.equals(t.getClass()))
				return Type.OBJECT;
			return t;
		...
		case Constants.T_ADDRESS:
			throw new VerifyError(
					"cannot store return address in continuation: do you perhaps have a blocking call inside a finally clause?");
		case Constants.T_UNKNOWN:
			if (t instanceof UninitializedObjectType) {
				throw new VerifyError(
						"Unable to preserve continuation due to "
								+ t.getSignature()
								+ " on stack. Is your blocking call within a constructor call?");
			}
			return null;
		default:
	...
	public static boolean isNotInit(Type t) {
		return t.getType() == Constants.T_UNKNOWN
				&& t instanceof UninitializedObjectType;
	}
	 ...
	 * types variables called 'local_i' and 'stack_j' for each local variable
	 * and stack slot in the given frame. The class is unique names by its
	 * method and execution point.
	 * 
	 * @param mg
	 *            method whose frame is being saved
	 * @param pc
	 *            program location identifier
	 * @param f
	 *            frame to save
	...
	private static JavaClass createStateClass(MethodGen mg, int pc, Frame f) {
		...
		ClassGen cg = new ClassGen(name,
				Event.ContinuationFrame.class.getName(), null,
		...
		MethodGen mginit = new MethodGen(Constants.ACC_PUBLIC, Type.VOID,
				new Type[] {}, new String[] {}, Constants.CONSTRUCTOR_NAME,
				name, il, cpg);
		mginit.setMaxStack();
		mginit.setMaxLocals();
		...
		for (int i = 0; i < lv.maxLocals(); i++) {
			Type t = lv.get(i);
			if (t == null || t == Type.NULL || isNotInit(t))
				continue;
			FieldGen fg = new FieldGen(Constants.ACC_PUBLIC, t, "local_" + i,
					cpg);
			cg.addField(fg.getField());
		}
		...
		for (int i = 0; !os.isEmpty(); i++) {
			...
			if (t == null || t == Type.NULL || isNotInit(t))
				continue;
			FieldGen fg = new FieldGen(Constants.ACC_PUBLIC, t, "stack_" + i,
					cpg);
			cg.addField(fg.getField());
		}
	 ...
	 *            method to be saved
	 * @param ih
	 *            instruction handle of save point
	 * @param pc
	 *            program location identifier
	 * @param f
	 ...
	 *            class instruction factory
	 */
	private static void saveFrame(MethodGen mg, InstructionHandle ih, int pc,
			Frame f, InstructionFactory ifc) {
		...
		for (int i = 0; !os.isEmpty(); i++) {
			...
			if (t == Type.NULL) {
				...
				pre.append(InstructionFactory.POP);
				// stack: (stuff-1) state_object
				loadlist.insert(ifc.createNew(((UninitializedObjectType) t)
						.getInitialized()));
				// rimnote: Technicality... hopefully we don't ever have two
				// uninitialized stack references to the same object. Should not
				// ever
				// occur if this is Java code that is compiled, because it's not
				// possible to refer to the result of an expression within
				// itself,
				...
				// stack: (stuff-1) state_obj state_obj (stuff) state_obj
				// state_obj
				pre.append(InstructionFactory.POP);
				pre.append(InstructionFactory.POP);
				...
				InstructionHandle ihload = loadlist.insert(ifc.createLoad(
						stateType, locals));
				loadlist.append(ihload,
						ifc.createGetField(stateName, "stack_" + i, t));
			}
		}
		pre.append(InstructionFactory.POP);
		// stack: (empty)
		...
		post.append(ifc.createInvoke(Controller.method_isModeSave
				.getDeclaringClass().getName(), Controller.method_isModeSave
				.getName(), Rewriter.getType(Controller.method_isModeSave
				.getReturnType()), Rewriter
				.getTypes(Controller.method_isModeSave.getParameterTypes()),
				Constants.INVOKESTATIC));
		// stack: (stuff) true
		...
		post.append(ifc.createInvoke(Controller.method_pushStateOutFrame
				.getDeclaringClass().getName(),
				Controller.method_pushStateOutFrame.getName(), Rewriter
						.getType(Controller.method_pushStateOutFrame
								.getReturnType()), Rewriter
						.getTypes(Controller.method_pushStateOutFrame
								.getParameterTypes()), Constants.INVOKESTATIC));
	 ...
	 * 
	 * @param mg
	 *            method to be restored onto stack
	 * @param frames
	 *            frames of restoration points
	 * @param ihs
	 *            instruction handles of restoration points
	 * @param ifc
	 *            class instruction factory
	 */
	private static void restoreFrame(MethodGen mg, Vector frames, Vector ihs,
			InstructionFactory ifc) {
		InstructionList il = mg.getInstructionList();
		InstructionHandle start = il.getStart();
		int locals = mg.getMaxLocals();

		// ** if(Controller.isCallbackRestore())
		il.insert(start, ifc.createInvoke(Controller.method_isModeRestore
				.getDeclaringClass().getName(), Controller.method_isModeRestore
				.getName(), Rewriter.getType(Controller.method_isModeRestore
				.getReturnType()), Rewriter
				.getTypes(Controller.method_isModeRestore.getParameterTypes()),
				Constants.INVOKESTATIC));
		// stack: boolean
		...
		// ** pop stack frame
		il.insert(start, ifc.createInvoke(Controller.method_popStateInFrame
				.getDeclaringClass().getName(),
				Controller.method_popStateInFrame.getName(), Rewriter
						.getType(Controller.method_popStateInFrame
								.getReturnType()), Rewriter
						.getTypes(Controller.method_popStateInFrame
								.getParameterTypes()), Constants.INVOKESTATIC));
		// stack: popStackFrame()
		il.insert(start, ifc.createStore(Rewriter
				.getType(Controller.method_popStateInFrame.getReturnType()),
				locals));
		// stack: (empty)

		// ** switch(popStackFrame()pc)
		il.insert(start, ifc.createLoad(Rewriter
				.getType(Controller.method_popStateInFrame.getReturnType()),
				locals));
		// stack: popStackFrame()
		...
		Select switchPc = new TABLESWITCH(Util.getRange(frames.size()),
				new InstructionHandle[frames.size()], null);
		// stack: popStackFrame()
		il.insert(start, switchPc);

		// ** case: bad pc
		InstructionHandle ihBadpc = il.insert(start,
				ifc.createNew(RuntimeException.class.getName()));
		il.insert(start, InstructionConstants.DUP);
		il.insert(start, new PUSH(mg.getConstantPool(), "invalid pc value"));
		il.insert(start, ifc.createInvoke(RuntimeException.class.getName(),
				Constants.CONSTRUCTOR_NAME, Type.VOID,
		...
		for (int pc = 1; pc < frames.size(); pc++) {
			...
			InstructionHandle ihGoodpci = il.insert(start,
					ifc.createCheckCast(stateType));
			...
			for (int i = 0; i < lv.maxLocals(); i++) {
				...
				if (t == Type.NULL) {
					...
					il.insert(start, ifc
							.createNew(((UninitializedObjectType) t)
									.getInitialized()));
					...
					il.insert(start,
							ifc.createGetField(stateName, "local_" + i, t));
