...
public class RemoteJist {
	 ...
	 * Job queue server.
	 */
	public static class JobQueueServer extends UnicastRemoteObject implements
			JobQueueServerRemote {
		/**
		 ...
		 *             rpc failure
		 */
		public static JobQueueServerRemote getRemote(Node n)
		...
		private synchronized void showJobs() throws RemoteException {
			writeln("Job queue has " + jobs.size() + " jobs outstanding and "
					+ serversWaiting + " servers idle.");
		...
		/** {@inheritDoc} */
		public synchronized void addJob(Job job, boolean front)
				throws RemoteException {
			if (front) {
				jobs.add(0, job);
			} else {
	...
	public static interface ResourceFinderRemote extends Remote {
		 ...
		 *             rpc failure
		 */
		long getResourceLastModificationDate(String name)
				throws RemoteException;
	...
	public static class RemoteClassLoader extends ClassLoader {
		...
		public Class loadClass(String name, boolean resolve)
			...
			return Rewriter.isIgnoredStatic(name) ? super.loadClass(name,
					resolve) : this.findClass(name);
		}
	 ...
	 */
	public static class RemoteRepository implements
		...
		public JavaClass loadClass(java.lang.String className)
			...
			try {
				...
				if (Rewriter.isIgnoredStatic(className)) {
					...
					if (out != null)
						out.println("  remote load: " + className);
					b = resources.getResourceBytes(classfile);
				}
				if (b != null) {
					ClassParser cp = new ClassParser(
							new ByteArrayInputStream(b), classfile);
					jcl = cp.parse();
		...
		public ClassPath getClassPath() {
			...
			// object with a custom path! subclassing ClassPath could be an
			// option,
			// but simply returning null looks like a viable solution. this
			// approach
	 ...
	 */
	public static interface JistClientRemote extends PingRemote,
			ResourceFinderRemote, Remote {
		/**
	 ...
	 */
	public static class JistClient extends UnicastRemoteObject implements
		 ...
		 * @author Elmar Schoch
		 * @param outStream
		 *            Stream for output (like System.out)
		 * @param errStream
		 *            Stream for errors (like System.err)
		 * @throws RemoteException
		 */
		public JistClient(OutputStream outStream, OutputStream errStream)
				throws RemoteException {
	 ...
	 * Local JiST client.
	 */
	public static class JistClientLocal implements RemoteJist.JistClientRemote {
