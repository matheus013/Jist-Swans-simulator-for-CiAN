...
/**
 * Stores the details of an entity invocation for processing at the appropriate
 * simulation time.
 * 
...
public class Event implements Comparable {
	...
	/**
	 * Any event continuation information. Used only for continuation calls and
	 * callbacks - blocking events.
	 */
	 ...
	 *            method to be invoked for event processing
	 * @param ref
	 *            entity reference to receive event
	 * @param args
	 *            arguments to be passed to entity for processing
	 */
	...
	public Event(Event ev) {
		this(ev.time, ev.method, ev.ref, ev.args); // possibly not safe to
													// reference same args
	...
	/**
	 * Return event target method modifiers as string. public, abstract and
	 * final are not interesting. Return only whether method is static or not.
	 * 
	...
	public String getClassString() {
		try {
			return Util.unqualifiedName(ref.getController()
					.getEntityClass(ref.getIndex()).getName());
	...
	public String getArgsString() {
		try {
			...
			for (int i = 0; i < args.length; i++) {
				if (i > 0)
					s.append(", ");
				s.append(args[i] == null ? "null" : Controller
						.toString(args[i]));
			}
			return s.toString();
		} catch (RemoteException e) {
	...
	public String toString() {
		...
		s.append("(" + getArgsString() + ") cont=[ " + getContinuationString()
				+ " ] on " + getEntityString());
		return s.toString();
	} // function: toString
	...
	public boolean equals(Object o) {
		...
		if (ref == null) {
			if (e.ref != null)
				return false;
		} else if (!ref.equals(e.ref))
			return false;
		if (method == null) {
			if (e.method != null)
				return false;
		} else if (!method.equals(e.method))
			return false;
		if (args == null) {
			if (e.args != null)
				return false;
			...
			for (int i = 0; i < args.length; i++) {
				if (args[i] == null) {
					if (e.args[i] != null)
						return false;
				} else if (!args[i].equals(e.args[i]))
					return false;
			}
		}
		if (cont == null) {
	 ...
	 */
	public int hashCode() {
		int hash = (int) time % Integer.MAX_VALUE + ref.hashCode()
				+ method.hashCode();
		if (args != null) {
			hash += args.length;
			for (int i = 0; i < args.length; i++) {
	...
	/**
	 * Implement the Comparable Interface in order to allow Collections to sort
	 * Events by a natural ordering. The comparison works the same way as the
	 * EventComparator provided by JiST. By refusing to return 0 for concurrent
	 * events we get a well defined order in the spirit of
	 * "first come, first serve". This is not a total order!!
	 * 
	...
	/**
	 * Comparator used to compare two events in terms of their timestamp. Used
	 * by the event queue to order the events in order of their simulation time.
	 * 
	 * @author Rimon Barr &lt;barr+jist@cs.cornell.edu&gt;
	 * @since JIST1.0
	 */
	...
	public static class ContinuationFrame {
		...
		public boolean equals(Object o) {
			if (o == null)
				return false;
			if (!(o instanceof ContinuationFrame))
				return false;
			ContinuationFrame cf = (ContinuationFrame) o;
			if (pc != cf.pc)
				return false;
			if (next == null && cf.next != null)
				return false;
