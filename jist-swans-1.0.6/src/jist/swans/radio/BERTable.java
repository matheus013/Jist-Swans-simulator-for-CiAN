...
/**
 * Implementation of Bit-Error-Rate calculations (via a loaded table) for a
...
public class BERTable {
	...
	public double calc(double snrVal) {
		...
		return ber[i1] + (ber[i2] - ber[i1]) * (snrVal - snr[i1])
				/ (snr[i2] - snr[i1]);
	}
	...
	public boolean shouldDrop(double snrVal, int bits) {
		...
		if (Main.ASSERT)
			Util.assertion(ber >= 0.0 && ber <= 1.0);
		if (ber <= 0)
			return false;
		double error = 1.0 - StrictMath.pow((1.0 - ber), bits);
		if (Main.ASSERT)
			Util.assertion(error >= 0.0 && error <= 1.0);
