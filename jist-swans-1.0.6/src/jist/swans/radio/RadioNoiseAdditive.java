...
// Timestamp: <RadioNoiseAdditive.java Tue 2004/04/13 18:16:53 barr
// glenlivet.cs.cornell.edu>
//
...
public class RadioNoiseAdditive extends RadioNoise
{
    ...
    public void receive(final Message msg, final Double powerObj_mW, final Long durationObj) {
        ...
        switch (mode) {
            case Constants.RADIO_MODE_IDLE:
                ...
                if (power_mW >= radioInfo.shared.threshold_mW && power_mW >= totalPower_mW * thresholdSNR) {
                    lockSignal(msg, power_mW, duration);
                    setMode(Constants.RADIO_MODE_RECEIVING);
                }
                // otherwise if new noise level is bigger than sensing threshold
                // set
                // channel busy
            ...
            case Constants.RADIO_MODE_RECEIVING:
                ...
                if (power_mW > signalPower_mW && power_mW >= totalPower_mW * thresholdSNR) {
                    ...
                    if (signalBuffer != null) {
                        if (signalBuffer.getClass().getName().equals("jist.swans.mac.MacMessage$Data")) {
                    ...
                    // probably "type == SNR &&" should be removed here because
                    // if a
                    // collision occures a correct
                    // packet reception is impossible in any case - Manuel
                    // Schoch
    ...
    public void endReceive(Double powerObj_mW) {
        ...
        totalPower_mW = signals == 0 ? radioInfo.shared.background_mW : totalPower_mW - power_mW;
        switch (mode) {
            case Constants.RADIO_MODE_RECEIVING:
                if (JistAPI.getTime() == signalFinish) {
                    boolean dropped = false;
                    dropped |= type == BER && totalPower_mW > 0
                            && ber.shouldDrop(signalPower_mW / totalPower_mW, 8 * signalBuffer.getSize());
