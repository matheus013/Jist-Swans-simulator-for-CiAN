...
public abstract class Location implements JistAPI.Timeless, Cloneable {
	...
	public static Location parse(String s) {
		...
		if (data.length == 2) {
			return new Location.Location2D(Float.parseFloat(data[0]),
					Float.parseFloat(data[1]));
		} else if (data.length == 3) {
			return new Location.Location3D(Float.parseFloat(data[0]),
					Float.parseFloat(data[1]), Float.parseFloat(data[2]));
		} else
			throw new IllegalArgumentException(
					"invalid format, expected x,y[,h]");
	}
	...
	public boolean equals(Object o) {
		...
		return (pn.getX() == this.getX()) && (pn.getY() == this.getY())
	...
	public static final class Location2D extends Location {
		...
		public Location step(Location l, int steps) {
			Location2D l2d = (Location2D) l;
			return new Location.Location2D((l2d.x - x) / steps, (l2d.y - y)
					/ steps);
		}
	...
	public static final class Location3D extends Location {
		...
		public Location step(Location l, int steps) {
			Location3D l3d = (Location3D) l;
			return new Location.Location3D((l3d.x - x) / steps, (l3d.y - y)
					/ steps, (l3d.z - z) / steps);
		}
		...
		public boolean inside(Location bounds) {
			...
			return x <= l3d.x && y <= l3d.y && z <= l3d.z && x >= 0 && y >= 0
					&& z >= 0;
		}
		...
		public boolean inside(Location min, Location max) {
			...
			if (Main.ASSERT)
				Util.assertion(min3d.x <= max3d.x && min3d.y <= max3d.y
						&& min3d.z <= max3d.z);
			return x <= max3d.x && y <= max3d.y && z <= max3d.z && x >= min3d.x
					&& y >= min3d.y && z >= min3d.z;
		}
