...
// Timestamp: <UdpSocket.java Sun 2005/03/13 11:09:24 barr
...
public class UdpSocket
{
    ...
    public void _jistPostInit() {
        this.udpEntity = ((AppInterface.UdpApp) JistAPI.proxy(JistAPI.THIS, AppInterface.UdpApp.class)).getUdpEntity();
    ...
    public void send(DatagramPacket p) {
        udpEntity.send(new MessageBytes(p.getData(), p.getOffset(), p.getLength()), new NetAddress(p.getAddress()),
                p.getPort(), lport, Constants.NET_PRIORITY_NORMAL);
    }
    ...
    public static class UdpSocketCallback implements TransInterface.SocketHandler
    {
        ...
        public void receive(Message msg, NetAddress src, int srcPort) {
            ...
            DatagramPacket p = new DatagramPacket(bytes.getBytes(), bytes.getOffset(), bytes.getLength(), src.getIP(),
                    srcPort);
