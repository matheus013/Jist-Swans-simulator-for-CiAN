...
public class UdpSocket {
	...
	/**
	 * blocking channel represents application-kernel boundary; used to wait on
	 * packets.
	 */
	...
	public void _jistPostInit() {
		this.udpEntity = ((AppInterface.UdpApp) JistAPI.proxy(JistAPI.THIS,
				AppInterface.UdpApp.class)).getUdpEntity();
	...
	public void bind(SocketAddress addr) {
		...
		udpEntity.addSocketHandler(((InetSocketAddress) addr).getPort(),
				callback);
		isBound = true;
	}
	...
	public void send(DatagramPacket p) {
		udpEntity.send(
				new MessageBytes(p.getData(), p.getOffset(), p.getLength()),
				new NetAddress(p.getAddress()), p.getPort(), lport,
	 ...
	 */
	public static class UdpSocketCallback implements
		 ...
		 * 
		 * @param channel
		 *            blocking channel used to deliver packets.
		 */
		...
		public void receive(Message msg, NetAddress src, int srcPort) {
			...
			DatagramPacket p = new DatagramPacket(bytes.getBytes(),
					bytes.getOffset(), bytes.getLength(), src.getIP(), srcPort);
