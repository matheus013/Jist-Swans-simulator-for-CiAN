...
/**
 * Data structure that holds TCP message and sorts them based on their sequence
 * numbers.
 * 
...
public class PriorityList {
	...
	/**
	 * Insert a message to the possible priority list. Messages are stored in
	 * increasing order, ordered by the sequence number.
	 * 
	 * @param msg
	 *            TCP message to be inserted
	 */
	public void insert(TcpMessage msg) {
		// dont add if it is already in the list
		if (retrieve(msg.getSeqNum()) != null)
			return;
		// seek for location to insert
		final int seq = msg.getSeqNum();
		int i;
		for (i = 0; i < llist.size(); i++) {
			if (seq < ((TcpMessage) llist.get(i)).getSeqNum())
				break;
		}
		// insert message
		llist.add(i, msg);
	...
	public int find(int seqNum) {
		for (int i = 0; i < llist.size(); i++) {
			if (((TcpMessage) (llist.get(i))).getSeqNum() == seqNum)
				return i;
		}
		return -1;
	}
	...
	/**
	 * Retrieve a message with given sequence number. (Returns null if message
	 * is not found)
	 * 
	 * @param seqNum
	 ...
	 * 
	 * @param seqNum
	 *            sequence number of message to be removed
	 */
	...
	/**
	 * Removing all messages that have number less than the given number. All
	 * messages with sequence number less than the given seqNum will be removed.
	 * (If they are equal, the message is not removed)
	 * 
	 * @param seqNum
	 *            messages with sequence number lower than this will be removed
	 */
	public void removeMessages(int seqNum) {
		while (llist.size() > 0
				&& ((TcpMessage) llist.get(0)).getSeqNum() < seqNum) {
			llist.remove(0);
		}
	}
