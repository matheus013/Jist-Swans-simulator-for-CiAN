...
public class TransTcp implements TransInterface.TransTcpInterface
{
    ...
    public static class TcpOptions extends TransInterface.TransMessage
    {
         ...
         * 
         * @param msg
         *            byte array to copy the option to
         * @param offset
    ...
    /**
     * Data structure for TCP Message. TCP Packet header: srcPort : 2 dstPort :
     * 2 seqNum : 4 ackNum : 4 offset and flags : 2 window size : 2 checksum : 2
     * urgent pointer : 2 TOTAL HEADER SIZE : 20 options : variable data :
     * variable
     */
    public static class TcpMessage extends TransInterface.TransMessage
    {
        ...
        /**
         * 16 bit offset and flags. this contains: 4 bit data offset (number of
         * 32 bit words in the TCP header) 6 bit reserved 1 bit URG - urgent
         * pointer field significant 1 bit ACK - acknowledgement field
         * significant 1 bit PSH - push function 1 bit RST - reset the
         * connection 1 bit SYN - synchronize sequence numbers 1 bit FIN - no
         ...
         * @param offset
         *            data offset (start of data in the header; used when the
         *            packet has options)
         * @param URG
         *            urgent flag
         * @param ACK
         ...
         *            data
         */
        public TcpMessage(short srcPort, short dstPort, int seqNum, int ackNum, short offset, boolean URG, boolean ACK,
                boolean PSH, boolean RST, boolean SYN, boolean FIN, short windowSize, Message data) {
            ...
            this.offsetAndFlags = (short) ((short) (offset << 12) + (short) ((URG ? 1 : 0) << 5)
                    + (short) ((ACK ? 1 : 0) << 4) + (short) ((PSH ? 1 : 0) << 3) + (short) ((RST ? 1 : 0) << 2)
                    + (short) ((SYN ? 1 : 0) << 1) + (short) (FIN ? 1 : 0));
         ...
         * 
         * @param data
         *            array containing TCP message
         * @param offset
         *            start index to read the array
         */
        public TcpMessage(byte[] data, int offset) {
            ...
            System.arraycopy(data, offset + 20 + options.getSize(), temp, 0, data.length - 20 - options.getSize());
         ...
         * @return SYN packet
         */
        public static TcpMessage createSYNPacket(int sourcePort, int destPort, int seqNumber, short windowSize) {
            ...
            boolean SYN = true;
            boolean FIN = false;
            return new TcpMessage((short) sourcePort, (short) destPort, seqNum, ackNum, offset, URG, ACK, PSH, RST,
                    SYN, FIN, windowSize, new MessageBytes(""));
        }
         ...
         */
        public static TcpMessage createSYNACKPacket(int sourcePort, int destPort, int seqNumber, int ackNumber,
                short windowSize) {
            int seqNum = seqNumber;
         ...
         */
        public static TcpMessage createACKPacket(int sourcePort, int destPort, int seqNumber, int ackNumber,
                short windowSize) {
            int seqNum = seqNumber;
            int ackNum = ackNumber;
            short offset = 5;
         ...
         * @return FIN packet
         */
        public static TcpMessage createFINPacket(int sourcePort, int destPort, int seqNumber, int ackNumber,
                short windowSize) {
            int seqNum = seqNumber;
            int ackNum = ackNumber;
            short offset = 5;
         ...
         * @return RST packet
         */
        public static TcpMessage createRSTPacket(int sourcePort, int destPort, int seqNumber, int ackNumber,
                short windowSize) {
            int seqNum = seqNumber;
            int ackNum = ackNumber;
            short offset = 5;
         ...
         * 
         * @param msg
         *            byte array to store the message
         * @param offset
         *            start index of the destination array
         */
        public void getBytes(byte[] msg, int offset) {
            // source port (unsigned short)
         ...
         * @param numTabs
         *            number of tabs
         * @param isPrintPayload
         *            set to true to print out payload in message
         */
    ...
    /**
     * probability (in percent) that a message will not be sent. (0 --> no
     * packets are dropped; 100 --> no packets are transmitted)
     */
     ...
     */
    public TransTcp() {
        self = (TransInterface.TransTcpInterface) JistAPI.proxy(this, TransInterface.TransTcpInterface.class);
    ...
    public void receive(Message msg, NetAddress src, MacAddress lastHop, byte macId, NetAddress dst, byte priority,
        ...
        SocketHandler handler = (SocketHandler) handlers.get(new Integer(dstPort));
        if (handler == null) {
            if (TcpSocket.PRINTOUT >= TcpSocket.FULL_DEBUG) {
                System.out.println("%%%%%%% TransTcp::receive (t=" + JistAPI.getTime() + ") -> handler for port "
                        + dstPort + " = null!!!");
            }
            return;
        }
    ...
    /** {@inheritDoc} */
    public void send(Message msg, NetAddress dst, int dstPort, int srcPort, byte priority) {
        // get a random number between 0 and 100
        int prob = StrictMath.abs(Constants.random.nextInt()) % 101;
        if (prob >= DROP_PROBABILITY) {
            JistAPI.sleep(Constants.TRANS_DELAY);
            netEntity.send(msg, dst, Constants.NET_PROTOCOL_TCP, priority, Constants.TTL_DEFAULT);
        } else {
