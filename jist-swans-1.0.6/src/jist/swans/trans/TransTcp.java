...
public class TransTcp implements TransInterface.TransTcpInterface {
	...
	public static class TcpMessage extends TransInterface.TransMessage {
		 ...
		 *            data
		 */
		public TcpMessage(short srcPort, short dstPort, int seqNum, int ackNum,
				short offset, boolean URG, boolean ACK, boolean PSH,
				boolean RST, boolean SYN, boolean FIN, short windowSize,
				Message data) {
			...
			this.offsetAndFlags = (short) ((short) (offset << 12)
					+ (short) ((URG ? 1 : 0) << 5)
					+ (short) ((ACK ? 1 : 0) << 4)
					+ (short) ((PSH ? 1 : 0) << 3)
					+ (short) ((RST ? 1 : 0) << 2)
		 ...
		 * @return SYN packet
		 */
		public static TcpMessage createSYNPacket(int sourcePort, int destPort,
				int seqNumber, short windowSize) {
			...
			boolean SYN = true;
			boolean FIN = false;
			return new TcpMessage((short) sourcePort, (short) destPort, seqNum,
					ackNum, offset, URG, ACK, PSH, RST, SYN, FIN, windowSize,
					new MessageBytes(""));
		}
		 ...
		 */
		public static TcpMessage createSYNACKPacket(int sourcePort,
				int destPort, int seqNumber, int ackNumber, short windowSize) {
			int seqNum = seqNumber;
		...
		public String toString() {
			...
			String payload = new String(
					((MessageBytes) getPayload()).getBytes());
		...
		public void printMessage(int numTabs, boolean isPrintPayload) {
			...
			System.out.println(tabs + "\tsrc port: " + getSrcPort()
					+ "\tdst port: " + getDstPort());
	 ...
	 */
	public TransTcp() {
		self = (TransInterface.TransTcpInterface) JistAPI.proxy(this,
				TransInterface.TransTcpInterface.class);
	...
	public void receive(Message msg, NetAddress src, MacAddress lastHop,
		...
		SocketHandler handler = (SocketHandler) handlers.get(new Integer(
				dstPort));
		if (handler == null) {
			if (TcpSocket.PRINTOUT >= TcpSocket.FULL_DEBUG) {
				System.out.println("%%%%%%% TransTcp::receive (t="
						+ JistAPI.getTime() + ") -> handler for port "
						+ dstPort + " = null!!!");
			}
			return;
		}
	...
	/** {@inheritDoc} */
	public void send(Message msg, NetAddress dst, int dstPort, int srcPort,
			byte priority) {
		// get a random number between 0 and 100
		int prob = StrictMath.abs(Constants.random.nextInt()) % 101;
		if (prob >= DROP_PROBABILITY) {
			JistAPI.sleep(Constants.TRANS_DELAY);
			netEntity.send(msg, dst, Constants.NET_PROTOCOL_TCP, priority,
					Constants.TTL_DEFAULT);
		} else {
			if (TcpSocket.PRINTOUT >= TcpSocket.TCP_DEBUG) {
				System.out.println("%%%%%% TransTcp::send: PACKET DROPPED: "
						+ msg);
			}
		}
	}
