...
public class TcpSocket implements SocketInterface.TcpSocketInterface {
	...
	/**
	 * Debug printing flag. TCP_DEBUG = debug output, showing only packet
	 * traffic
	 */
	...
	/**
	 * Maximum Segment Lifetime. Maximum amount of time any segment can exist in
	 * the network before being discarded. (Can be 30 seconds, 1 minute, or 2
	 * minutes) (TCP has to wait for 2*MSL before the socket pair (client IP
	...
	/**
	 * Number of seconds to wait before sending probe packets. This happens when
	 * receiver is advertising zero window.
	 */
	...
	/**
	 * temporary receiver window (used to hold update after some data are sent
	...
	public void createProxy() {
		self = (SocketInterface.TcpSocketInterface) JistAPI.proxy(this,
				SocketInterface.TcpSocketInterface.class);
	}
	 ...
	 * 
	 * @param impl
	 *            an instance of a SocketImpl the subclass wishes to use on the
	 *            Socket
	 */
	...
	/**
	 * Creates an unconnected Socket with a user-specified TransInterface Entity
	 * and SYN packet received by TcpServerSocket.
	 * 
	 * @param tcpEntity
	 ...
	 *            initial acknowledgement number
	 * @param winSize
	 *            the other side's receiver window size
	 */
	protected TcpSocket(TransInterface.TransTcpInterface tcpEntity,
			InetAddress rAddr, short rPort, InetAddress lAddr, short lPort,
			int initialAckNum, short winSize) {
		this();
		this.tcpEntity = tcpEntity;
	 ...
	 *            the port number.
	 * @param stream
	 *            a boolean indicating whether this is a stream socket or a
	 *            datagram socket.
	 */
	...
	public void close() {
		if (PRINTOUT >= INFO) {
			System.out.println("TcpSocket::close for port " + lport
					+ " at time = " + JistAPI.getTime());
		}
	...
	private void initializeTCPVariables() {
		...
		cwnd = MSS;
		sshtresh = (short) 65535; // set to 64K (maximum representable window
									// --> 16 bits)
		cwnd_increment = 0;
	 ...
	 *            local port number
	 * @param doConnect
	 *            set to true to try to connect in constructor
	 */
	 ...
	 * 
	 * @param msg
	 *            TCP message to print out
	 * @param isReceive
	 *            true if printing for receiving side
	 */
	protected void printMessage(TcpMessage msg, boolean isReceive) {
		String direction = isReceive ? "** <-- RCV " : "** SND -->";
		System.out.println(direction + " packet " + packetCounter + " on ("
				+ lport + ") at time=" + JistAPI.getTime() + ": " + msg);
	}
	...
	protected void sendSYNPacket() {
		// transmit syn packet
		TcpMessage msg = TcpMessage.createSYNPacket(lport, rport, snd_nxt,
				rcv_wnd);
		sendMessage(msg);
	...
	protected void sendSYNACKPacket() {
		...
		TcpMessage msg = TcpMessage.createSYNACKPacket(lport, rport, snd_nxt,
				rcv_nxt, rcv_wnd);
		sendMessage(msg);
	...
	protected void sendFirstACKPacket() {
		// transmit ACK packet
		TcpMessage msg = TcpMessage.createACKPacket(lport, rport, snd_nxt,
				rcv_nxt, rcv_wnd);
		sendMessage(msg);
	...
	protected void sendFINPacket() {
		// transmit FIN packet
		TcpMessage msg = TcpMessage.createFINPacket(lport, rport, snd_nxt,
				rcv_nxt, rcv_wnd);
		sendMessage(msg);
		// put the SYN packet in retransmit list
		rList.insert(msg);
	...
	protected void sendRSTPacket(int seqNum, int ackNum) {
		// transmit RST packet
		TcpMessage msg = TcpMessage.createRSTPacket(lport, rport, seqNum,
				ackNum, rcv_wnd);
		sendMessage(msg);
	}

	/**
	 * Send a packet to the remote socket. To send an ACK packet, set data to
	 * null.
	 * 
	 * @param data
	 *            data to send
	...
	protected TcpMessage sendDataPacket(byte[] data) {
		...
		TcpMessage msg = new TcpMessage((short) lport, (short) rport, snd_nxt,
				rcv_nxt, offset, URG, ACK, PSH, RST, SYN, FIN, rcv_wnd,
				new MessageBytes(data));
		sendMessage(msg);
	...
	/**
	 * This method is called to send bytes in the buffer until they fill the
	 * receiver window. The bytes will be broken down to packets based on MSS
	...
	private void sendBytesInBuffer() {
		...
		// if no more bytes to send and the socket is closing, then send FIN
		// packet.
	...
	public void constructPackets() {
		...
		if (curWindow > (snd_nxt - snd_una)) {
			...
			if ((curWindow - (snd_nxt - snd_una)) < MSS) {
				...
				sentMsg = sendDataPacket(data); // takes care of increasing
												// snd_nxt
			...
			if (sentMsg != null) {
				self.startRetransmitTimer(sentMsg.getSeqNum(),
						RETRANSMIT_TIMEOUT);
			}
	...
	/**
	 * This method sends bytes to channel. The number of bytes to send is
	 * specified in the parameter.
	 * 
	 * @param length
	 *            number of bytes to send to application layer
	 */
	...
	/**
	 * Returns the smaller window (between the last advertised window and the
	 * congestion window) for the receiver.
	 * 
	 * @return receiver window or congestion window, whichever is smaller.
	 */
	...
	public void retransmitTimerTimeout(int seqNum, long time) {
		...
		if (onRetransmit(seqNum)) {
			if (PRINTOUT >= FULL_DEBUG) {
				System.out.println("TcpSocket::retransmitTimerTimeout(t="
						+ JistAPI.getTime() + "): seqNum = " + seqNum
						+ "(time=" + time + ")");
				System.out.println("time = " + time);
				System.out.println("RETRANSMIT_TIMEOUT_FINAL = "
	...
	public void startPersistTimer(int seqNum) {
		cancelPersistTimer();
		if (PRINTOUT >= FULL_DEBUG) {
			System.out.println("TcpSocket: startPersistTimer at t = "
					+ JistAPI.getTime());
		}
	...
	public void persistTimerTimeout(int timerId, int seqNum) {
		if (timerId == persistTimerId) {
			if (PRINTOUT >= FULL_DEBUG) {
				System.out.println("TcpSocket: persistTimerTimeout at t = "
						+ JistAPI.getTime());
			}
	...
	/**
	 * Create a probe message (for persist timer) by extracting one byte of data
	...
	private boolean createProbeMessage() {
		if (probeMessage == null) {
			...
			probeMessage = new TcpMessage((short) lport, (short) rport,
					snd_nxt, rcv_nxt, offset, URG, ACK, PSH, RST, SYN, FIN,
					rcv_wnd, probeByte);
			snd_nxt += data.length;
	...
	public void startResetTimer() {
		cancelResetTimer();
		if (PRINTOUT >= FULL_DEBUG) {
			System.out.println("TcpSocket: startResetTimer at t = "
					+ JistAPI.getTime());
		}
	...
	public void resetTimerTimeout(int timerId) {
		if (timerId == resetTimerId) {
			if (PRINTOUT >= FULL_DEBUG) {
				System.out.println("TcpSocket: resetTimerTimeout at t = "
						+ JistAPI.getTime());
			}
			// abort connection
			closeSocket(true);
	...
	public void checkPacketandState(TcpMessage msg, NetAddress src) {
		...
		if (PRINTOUT >= TCP_DEBUG) {
			if (PRINTOUT >= FULL_DEBUG) {
				System.out.println();
				System.out.println("TcpSocket: checkPacketandState (" + lport
		...
		if (PRINTOUT >= FULL_DEBUG) {
			System.out.println("TcpSocket: new state: checkPacketandState ("
					+ lport + "): " + getCurrentStateString());
		}

	}

	/**
	 * This method takes care of all the steps that need to be done if we
	 * receive a packet during CLOSED state.
	 * 
	 * @param msg
	 *            incoming packet
	 * @param src
	...
	private void onClosedState(TcpMessage msg, NetAddress src) {
		...
		if (msg.getSYN() && !msg.getACK()) {
			initializeAll(laddr.getIP(), lport, src.getIP(), msg.getSrcPort(),
					false);
	...
	/**
	 * This method takes care of all the steps that need to be done if we
	 * receive a packet during SYN_SENT state.
	 * 
	 * @param msg
	 *            incoming packet
	 */
	...
	/**
	 * This method takes care of all the steps that need to be done if we
	 * receive a packet during SYN_RECEIVED state.
	 * 
	 * @param msg
	 *            incoming packet
	 */
	...
	/**
	 * This method takes care of all the steps that need to be done if we
	 * receive a packet during ESTABLISHED state.
	 * 
	 * @param msg
	 *            incoming packet
	 */
	private void onEstablishedState(TcpMessage msg) {
		...
		if (isPacketOutofWindow(msg)) {
			...
			if (PRINTOUT >= FULL_DEBUG) {
				System.out.println(getCurrentStateString()
						+ ": sending DUPACK because of out of window");
			}
			sendDataPacket(null);
			return;
		}
		...
		if (msg.getFIN() && msg.getACK()) {
			...
			if (rcv_nxt == msg.getSeqNum()) {
				if (PRINTOUT >= FULL_DEBUG) {
					System.out.println("receiving FIN from port "
							+ msg.getSrcPort() + "(" + lport + ")");
				}
	...
	private boolean isPacketOutofWindow(TcpMessage msg) {
		...
		if ((msg.getSeqNum() >= rcv_nxt)
				&& (msg.getSeqNum() <= (rcv_nxt + rcv_wnd)) && msg.getFIN()) {
			return false;
		} else if ((msg.getSeqNum() < rcv_nxt)
				|| (msg.getSeqNum() >= (rcv_nxt + rcv_wnd))) {
			// out of window --> ignore packet
			if (PRINTOUT >= FULL_DEBUG) {
				System.out.println(getCurrentStateString()
						+ ": packet ignored: out of the receiving window");
				System.out.println(msg.getSeqNum() + " " + rcv_nxt + " "
						+ (rcv_nxt + rcv_wnd));
			}
			return true;
		}
	...
	/**
	 * Check if the ACK packet received is acknowledging the data that have not
	 * been acknowledged.
	 * 
	 * @param msg
	 *            incoming packet
	...
	private boolean isAcceptableACK(TcpMessage msg) {
		// check if this is an acceptable ACK (snd_una < ack <= snd_nxt)
		if (msg.getACK() && (msg.getAckNum() < snd_una)
				&& (msg.getAckNum() > snd_nxt)) {
			if (PRINTOUT >= FULL_DEBUG) {
				System.out.println(getCurrentStateString()
						+ ": packet ignored: not an acceptable ACK");
			}
			return false;
		}
		return true;
	...
	/**
	 * This method is to check if receive data packet or ACK packet. Then, the
	 * appropiate method is called to handle each case.
	 * 
	 * @param msg
	 *            incoming packet
	 */
	private void onReceiveValidDataorACK(TcpMessage msg) {
		...
		if (msg.getSeqNum() == rcv_nxt) {
			...
			if (PRINTOUT >= FULL_DEBUG) {
				System.out.println(getCurrentStateString()
						+ ": out-of-order: msg.getSeqNum() = "
						+ msg.getSeqNum() + "\trcv_nxt = " + rcv_nxt);
				System.out.println(getCurrentStateString()
						+ ": sending DUPACK because of not in-order");
			}
			sendDataPacket(null);
	...
	/**
	 * This method handles the case when data packet is received. Will send ACK
	 * packet back, take care of probe messages, and send the data received to
	 * application layer if requested.
	 * 
	 * @param msg
	 *            incoming packet
	 */
			...
			if (rcv_wnd >= msg.getPayload().getSize()) {
				...
				while ((tempMsg = rMsgBuffer.retrieve(rcv_nxt)) != null) {
					if (rcv_wnd >= tempMsg.getPayload().getSize()) {
						...
						// update our receiver window size
						rcv_wnd = (short) (rcv_wnd - (short) tempMsg
								.getPayload().getSize());
					} else {
						break;
					}
	...
	/**
	 * This method handles the case when ACK packet is received. Will send more
	 * data or probe message (if advertised window is zero) and do congestion
	 * control algorithm.
	 * 
	 * @param msg
	 *            incoming packet
	...
	private void onReceiveACK(TcpMessage msg) {
		...
		// message
		if ((msg.getWindowSize() == 0)
				&& (sendBuffer.getTotalBytesInBuffer() > 0)) {
			...
			if (snd_una == msg.getAckNum()) {
				...
				// as the last received acknowledgement number or the sequence
				// number
				...
				// increase the congestion window (double it for slow start
				// phase)
	...
	private void FastRetransmit() {
		...
		if (dupAckCounter > 3) {
			if (PRINTOUT >= FULL_DEBUG) {
				System.out
						.println("ESTABLISHED: RETRANSMIT: snd_una == msg.getAkNum() = "
								+ snd_una);
			}
			...
			cwnd = MSS;
			if (PRINTOUT >= FULL_DEBUG) {
				System.out.println("Congestion detected: new threshold = "
						+ sshtresh);
			}
		}
	}
	...
	private void SlowStart() {
		...
		if (PRINTOUT >= FULL_DEBUG) {
			System.out.println("&&&%%%$$$ NEW CONGESTION WINDOW = " + temp
					+ "(" + cwnd + ")");
		}
	}
	...
	private void CongestionAvoidance() {
		...
		if (PRINTOUT >= FULL_DEBUG) {
			System.out
					.println("&&&%%%$$$ NEW CONGESTION WINDOW = " + temp + "("
							+ cwnd + ")\ttemp_cwnd_increment = "
							+ cwnd_increment);
		}
	}

	/**
	 * Called when retransmission happens. Parameter is to denote which message
	 * to send.
	 * 
	 * @param seqNumToSend
	...
	private boolean onRetransmit(int seqNumToSend) {
		...
		if (msgToRetransmit != null) {
			if (PRINTOUT >= FULL_DEBUG) {
				System.out
						.println("~~~~~ TcpSocket::onRetransmit: seqNumToSend = "
								+ seqNumToSend);
				System.out.println("~~~~~ Message to retransmit: "
						+ msgToRetransmit);
			}
	...
	/**
	 * This method takes care of all the steps that need to be done if we
	 * receive a packet during FIN_WAIT_1 state.
	 * 
	 * @param msg
	 *            incoming packet
	 */
	...
	/**
	 * This method takes care of all the steps that need to be done if we
	 * receive a packet during FIN_WAIT_2 state.
	 * 
	 * @param msg
	 *            incoming packet
	 */
	...
	/**
	 * This method takes care of all the steps that need to be done if we
	 * receive a packet during CLOSING state.
	 * 
	 * @param msg
	 *            incoming packet
	 */
	...
	/**
	 * This method takes care of all the steps that need to be done if we
	 * receive a packet during LAST_ACK state.
	 * 
	 * @param msg
	 *            incoming packet
	 */
	...
	/**
	 * This method is called when socket is to be closed. (also unbind from
	 * current port).
	 * 
	 * @param doUnbind
	 *            set to true to unbind from current port when closing socket.
	 */
	private void closeSocket(boolean doUnbind) {
		// print out final stats
		if (PRINTOUT >= INFO) {
			System.out.println("***###*** TcpSocket::closeSocket: Port "
					+ lport + " closed (Sent = " + (snd_nxt - initSeqNum)
					+ " Received = " + (rcv_nxt - initAckNum) + ")");
			if (PRINTOUT >= FULL_DEBUG)
				rList.printList();
		}
	 ...
	 */
	public static class TcpSocketCallback implements
		 ...
		 * 
		 * @param entity
		 *            the entity reference to TcpSocket
		 */
