...
// Timestamp: <TcpSocket.java Tue 2004/04/06 11:37:50 barr
...
public class TcpSocket implements SocketInterface.TcpSocketInterface
{
     ...
     */
    protected TcpSocket(TransInterface.TransTcpInterface tcpEntity, InetAddress rAddr, short rPort, InetAddress lAddr,
            short lPort, int initialAckNum, short winSize) {
        this();
        this.tcpEntity = tcpEntity;
    ...
    protected void printMessage(TcpMessage msg, boolean isReceive) {
        ...
        System.out.println(direction + " packet " + packetCounter + " on (" + lport + ") at time=" + JistAPI.getTime()
                + ": " + msg);
    }
    ...
    protected TcpMessage sendDataPacket(byte[] data) {
        ...
        TcpMessage msg = new TcpMessage((short) lport, (short) rport, snd_nxt, rcv_nxt, offset, URG, ACK, PSH, RST,
                SYN, FIN, rcv_wnd, new MessageBytes(data));
        sendMessage(msg);
    ...
    public void retransmitTimerTimeout(int seqNum, long time) {
        ...
        if (onRetransmit(seqNum)) {
            if (PRINTOUT >= FULL_DEBUG) {
                System.out.println("TcpSocket::retransmitTimerTimeout(t=" + JistAPI.getTime() + "): seqNum = " + seqNum
                        + "(time=" + time + ")");
    ...
    private boolean createProbeMessage() {
        if (probeMessage == null) {
            ...
            probeMessage = new TcpMessage((short) lport, (short) rport, snd_nxt, rcv_nxt, offset, URG, ACK, PSH, RST,
                    SYN, FIN, rcv_wnd, probeByte);
            snd_nxt += data.length;
    ...
    public void checkPacketandState(TcpMessage msg, NetAddress src) {
        ...
        switch (currentState) {
            ...
            case Constants.TCPSTATES.CLOSING:
                ...
                break;
            case Constants.TCPSTATES.CLOSE_WAIT:
                // send FIN packet to other side (done when application calls
                // close)
                break;
    ...
    private void onReceiveValidDataorACK(TcpMessage msg) {
        ...
        if (msg.getSeqNum() == rcv_nxt) {
            ...
            // sending duplicate ACK
            if (PRINTOUT >= FULL_DEBUG) {
                System.out.println(getCurrentStateString() + ": out-of-order: msg.getSeqNum() = " + msg.getSeqNum()
                        + "\trcv_nxt = " + rcv_nxt);
    ...
    private void CongestionAvoidance() {
        ...
        if (PRINTOUT >= FULL_DEBUG) {
            System.out.println("&&&%%%$$$ NEW CONGESTION WINDOW = " + temp + "(" + cwnd + ")\ttemp_cwnd_increment = "
                    + cwnd_increment);
        }
    }
    ...
    private void closeSocket(boolean doUnbind) {
        ...
        if (PRINTOUT >= INFO) {
            System.out.println("***###*** TcpSocket::closeSocket: Port " + lport + " closed (Sent = "
                    + (snd_nxt - initSeqNum) + " Received = " + (rcv_nxt - initAckNum) + ")");
            if (PRINTOUT >= FULL_DEBUG)
                rList.printList();
        }
