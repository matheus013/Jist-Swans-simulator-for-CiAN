...
public class CircularBuffer {
	 ...
	 *            byte array
	 * @param offset
	 *            offset in data to start reading
	 * @param length
	...
	public int storeBytes(byte[] data, int offset, int length) {
		...
		while (length > getFreeBufferSize()) {
			if (TcpSocket.PRINTOUT >= TcpSocket.FULL_DEBUG) {
				System.out
						.println("storeBytes: calling buffer resize: length = "
								+ length + "\tcurSize = " + curSize);
			}
		...
		if (length > 0) {
			...
			if (isBufferEmpty) {
				if (length > (buffer.length - tail)) {
					...
					tail = 0;
					System.arraycopy(data, (tempLength + offset), buffer, tail,
							length - tempLength);
			...
			// and before head can store the message
					...
					&& (((buffer.length - tail) + head) >= length)) {
				int tempLength = buffer.length - tail;
				System.arraycopy(data, offset, buffer, tail, tempLength);
				tail = 0;
				System.arraycopy(data, (offset + tempLength), buffer, tail,
						length - tempLength);
				tail = length - tempLength;
				if (tail == head) {
					isBufferFull = true;
	...
	public byte[] retrieveBytes(int length) {
		...
		if ((buffer.length - head) < retLength) {
			...
			System.arraycopy(buffer, head, data, tempLength,
	...
	/**
	 * Resize the buffer to the specified value. Returns false if the operation
	 * fails.
	 * 
	 * @param newSize
	...
	public boolean resizeBuffer(int newSize) {
		...
		// head == tail (buffer is currently full)
			...
			if (head == 0) {
				...
				System.arraycopy(buffer, head, newBuffer, 0,
						(buffer.length - head));
				System.arraycopy(buffer, 0, newBuffer, (buffer.length - head),
						tail);
			}
