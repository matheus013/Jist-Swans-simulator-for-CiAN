...
public class TransUdp implements TransInterface.TransUdpInterface {
	...
	public static class UdpMessage extends TransMessage {
		...
		public String toString() {
			return "udp(srcPort=" + srcPort + " dstPort=" + dstPort + " data="
					+ payload + ")";
		}
	 ...
	 */
	public TransUdp() {
		self = (TransInterface.TransUdpInterface) JistAPI.proxy(this,
				TransInterface.TransUdpInterface.class);
	...
	public void send(Message msg, NetAddress dst, int dstPort, int srcPort,
		...
		if (log.isInfoEnabled()) {
			log.info("send: t=" + JistAPI.getTime() + " dst=" + dst + ":"
					+ dstPort + " srcPort=" + srcPort + " data=" + msg);
		}
		netEntity.send(udp, dst, Constants.NET_PROTOCOL_UDP, priority,
	...
	public void receive(Message msg, NetAddress src, MacAddress lastHop,
		...
		if (log.isInfoEnabled()) {
			log.info("receive: t=" + JistAPI.getTime() + " src=" + src + ":"
					+ srcPort + " dstPort=" + dstPort + " from=" + lastHop
					+ " data=" + msg);
		}
