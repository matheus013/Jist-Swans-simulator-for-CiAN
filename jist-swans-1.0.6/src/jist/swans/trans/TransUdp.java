...
public class TransUdp implements TransInterface.TransUdpInterface
{
    ...
    /** socket handlers. */
    private HashMap                          handlers; // Integer(port) ->
                                                        // SocketHandler
     ...
     */
    public TransUdp() {
        self = (TransInterface.TransUdpInterface) JistAPI.proxy(this, TransInterface.TransUdpInterface.class);
    ...
    public void send(Message msg, NetAddress dst, int dstPort, int srcPort, byte priority) {
        ...
        if (log.isInfoEnabled()) {
            log.info("send: t=" + JistAPI.getTime() + " dst=" + dst + ":" + dstPort + " srcPort=" + srcPort + " data="
                    + msg);
        }
        netEntity.send(udp, dst, Constants.NET_PROTOCOL_UDP, priority, Constants.TTL_DEFAULT);
    }
    ...
    public void receive(Message msg, NetAddress src, MacAddress lastHop, byte macId, NetAddress dst, byte priority,
        ...
        if (log.isInfoEnabled()) {
            log.info("receive: t=" + JistAPI.getTime() + " src=" + src + ":" + srcPort + " dstPort=" + dstPort
                    + " from=" + lastHop + " data=" + msg);
        }
