...
public abstract class NetMessage implements Message, Cloneable {
	...
	public static class Ip extends NetMessage {
		...
		// Interface
		private HashMap options; // Map alone doesnt provide the Cloneable
									// Interface
		...
		public Ip(Message payload, NetAddress src, NetAddress dst,
				short protocol, byte priority, byte ttl, short id,
				short fragOffset) {
			if (payload == null)
		...
		public Ip(Message payload, NetAddress src, NetAddress dst,
			...
			this(payload, src, dst, protocol, priority, ttl, nextId++,
					(short) 0);
		}
		 ...
		 */
		public Ip copy() {
			NetMessage.Ip ip2 = new Ip(payload, src, dst, protocol, priority,
					ttl, id, fragOffset);
...
/*
 * todo: #define IP_MAXPACKET 65535 // maximum packet size #define MAXTTL 255 //
 * maximum time to live (seconds) #define IPDEFTTL 64 // default ttl, from RFC
 * 1340 #define IP_MSS 576 // default maximum segment size
 */
