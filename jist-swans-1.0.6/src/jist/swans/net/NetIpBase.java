...
public abstract class NetIpBase {
	...
	public static final Logger log = Logger
			.getLogger(NetIpBase.class.getName());
	 ...
	 */
	public NetIpBase(NetAddress addr, Mapper protocolMap, PacketLoss in,
			PacketLoss out) {
		// proxy entity
	...
	public byte addInterface(MacInterface macEntity) {
		return addInterface(macEntity, new MessageQueue.NoDropMessageQueue(
	...
	public void setProtocolHandler(int protocolId, NetHandler handler) {
		...
		if (protocolMap.getLimit() > protocolHandlers.length) {
			log.info("Growing protocol handler array at " + this.localAddr
					+ " from " + protocolHandlers.length + " to "
					+ protocolMap.getLimit());
			NetHandler[] protocolHandlersNew = new NetHandler[protocolMap
					.getLimit()];
	...
	public void receive(Message msg, MacAddress lastHop, byte macId,
		...
		if (isForMe(ipmsg) || (macId == Constants.NET_INTERFACE_LOOPBACK)) {
			...
			if (ipmsg.getTTL() > 0) {
				...
				log.info("in node " + localAddr + " a packets ttl expired "
						+ ipmsg);
				return;
			}
		}
