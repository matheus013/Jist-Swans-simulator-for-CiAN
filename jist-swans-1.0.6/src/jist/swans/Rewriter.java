...
public class Rewriter implements JistAPI.CustomRewriter {
	...
	/**
	 * Class beginning with these strings are ignored. The JiST ignored classes
	 * do not even arrive at this filter.
	 */
	private static final String[] ignoredPackages = new String[] { "jist.swans.", };
	...
	public Rewriter() {
		typeModifications = new ModifyTypeInfo[] {
				// sockets
				new ModifyTypeInfo("java.net.DatagramSocket",
						"jist.swans.app.net.UdpSocket", true),
				new ModifyTypeInfo("java.net.Socket",
						"jist.swans.app.net.Socket", true),
				new ModifyTypeInfo("java.net.ServerSocket",
						"jist.swans.app.net.ServerSocket", true),
				// streams
				new ModifyTypeInfo("java.io.InputStream",
						"jist.swans.app.io.InputStream", false),
				new ModifyTypeInfo("java.io.OutputStream",
						"jist.swans.app.io.OutputStream", false),
				new ModifyTypeInfo("java.io.FilterInputStream",
						"jist.swans.app.io.FilterInputStream", false),
				new ModifyTypeInfo("java.io.BufferedInputStream",
						"jist.swans.app.io.BufferedInputStream", false),
				// readers
				new ModifyTypeInfo("java.io.BufferedReader",
						"jist.swans.app.io.BufferedReader", false),
				new ModifyTypeInfo("java.io.BufferedWriter",
						"jist.swans.app.io.BufferedWriter", false),
				// threading
				new ModifyTypeInfo("java.lang.Thread",
						"jist.swans.app.lang.SimtimeThread", false), };
	}

	/** {@inheritDoc} */
	public JavaClass process(JavaClass jcl) throws ClassNotFoundException {
		if (isIgnored(jcl.getClassName()))
			return jcl;
		jcl = (new jist.runtime.ClassTraversal(new ModifyTypeTraversal(
				typeModifications))).processClass(jcl);
		return jcl;
	}
	...
	public static boolean isIgnored(String classname) {
		if (jist.runtime.Rewriter.isIgnoredStatic(classname))
			return true;
	...
	private static class ModifyTypeInfo {
		public ModifyTypeInfo(String oldName, String newName,
	 ...
	 * @since SWANS1.0
	 */
	public static class ModifyTypeTraversal extends
		 ...
		 * @param newName
		 *            name of new type
		 * @param addJistPostInit
		 *            whether to add a _jistPostInit call after an initializer
		 *            call is modified.
		 */
		...
		private Type transformType(Type t) {
			for (int i = 0; i < modifications.length; i++) {
				if (modifications[i].oldType.equals(t))
					return modifications[i].newType;
			}
			return t;
		}
		...
		private String transformName(String n) {
			for (int i = 0; i < modifications.length; i++) {
				if (modifications[i].oldName.equals(n))
					return modifications[i].newName;
			}
			return n;
		}
		...
		private boolean isIgnored(String classname) {
			for (int i = 0; i < modifications.length; i++) {
				if (modifications[i].oldName.equals(classname))
					return false;
				if (modifications[i].newName.equals(classname))
					return false;
			}
		...
		private boolean shouldAddJistPostInit(String oldName) {
			for (int i = 0; i < modifications.length; i++) {
				if (modifications[i].oldName.equals(oldName))
					return modifications[i].addJistPostInit;
			}
			return false;
		}
		...
		public FieldGen doField(ClassGen cg, FieldGen fg) {
			if (shouldTransformType(fg.getType()))
				fg.setType(transformType(fg.getType()));
			return fg;
		}

		/** {@inheritDoc} */
		public MethodGen doMethod(ClassGen cg, MethodGen mg) {
			if (shouldTransformTypes(mg.getArgumentTypes()))
				mg.setArgumentTypes(transformTypes(mg.getArgumentTypes()));
			if (shouldTransformType(mg.getReturnType()))
				mg.setReturnType(transformType(mg.getReturnType()));
			return mg;
		}

		/** {@inheritDoc} */
		public void doInstruction(ClassGen cg, MethodGen mg,
				InstructionHandle ih, Instruction inst) {
			ConstantPoolGen cpg = cg.getConstantPool();
			// modify field access
			if (inst instanceof FieldInstruction) {
				FieldInstruction fi = (FieldInstruction) inst;
				if (!isIgnored(fi.getClassName(cpg))
						&& (shouldTransformName(fi.getClassName(cpg)) || shouldTransformType(fi
								.getType(cpg)))) {
					short itype = inst instanceof GETFIELD ? org.apache.bcel.Constants.GETFIELD
					...
					ih.swapInstruction(ifc.createFieldAccess(
							...
							fi.getName(cpg), transformType(fi.getType(cpg)),
							itype));
				}
			}
			// modify invocations
			if (inst instanceof InvokeInstruction) {
				...
				if (!isIgnored(ii.getClassName(cpg))
						&& (shouldTransformName(ii.getClassName(cpg))
								|| shouldTransformType(ii.getReturnType(cpg)) || shouldTransformTypes(ii
									.getArgumentTypes(cpg)))) {
					short itype = inst instanceof INVOKEINTERFACE ? org.apache.bcel.Constants.INVOKEINTERFACE
					...
					if (org.apache.bcel.Constants.CONSTRUCTOR_NAME.equals(ii
							.getMethodName(cpg))
						...
						il.append(ifc.createInvoke(
								transformName(ii.getClassName(cpg)),
								"_jistPostInit", Type.VOID, Type.NO_ARGS,
			...
			if (inst instanceof NEW) {
				...
				if (shouldTransformType(ni.getType(cpg))) {
					ih.swapInstruction(ifc
							.createNew((ObjectType) transformType(ni
									.getType(cpg))));
				}
			}
