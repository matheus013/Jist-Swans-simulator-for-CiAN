...
public class Field implements FieldInterface {
	...
	public static final Logger logField = Logger.getLogger(Field.class
			.getName());
	...
	public Field(Location bounds, boolean wrap) {
		this(wrap ? (Spatial) new Spatial.TiledWraparound(new Spatial.HierGrid(
				bounds, 5)) : (Spatial) new Spatial.HierGrid(bounds, 5),
				new Fading.None(), new PathLoss.FreeSpace(),
	...
	public void moveRadio(Integer id, Location loc) {
		...
		for (int i = 0; i < moveRadioListeners.size(); i++) {
			((MovementListenerInterface) moveRadioListeners.get(i)).move(
					JistAPI.getTime(), loc, id.intValue());
		}
		// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
	...
	private Spatial.SpatialTransmitVisitor transmitVisitor = new Spatial.SpatialTransmitVisitor() {
		public double computeSignal(RadioInfo srcInfo, Location srcLoc,
				Location dstLoc) {
	 ...
	 */
	public static interface ConnectivityVisitor extends
			Spatial.SpatialTransmitVisitor, Spatial.SpatialVisitor {
		/**
	...
	public double computeAvgConnectivity(final boolean sense) {
		...
		ConnectivityVisitor connectivityVisitor = new ConnectivityVisitor() {
			...
			public void visitTransmit(RadioInfo srcInfo, Location srcLoc,
					RadioInfo dstInfo, RadioInterface dstEntity,
					Location dstLoc, Message msg, Long durationObj) {
				if (srcInfo.getUnique().getID() == dstInfo.getUnique().getID())
					return;
				// compute signal strength
				double loss = pathloss
						.compute(srcInfo, srcLoc, dstInfo, dstLoc);
				double fade = fading.compute();
				double dstPower = srcInfo.getShared().getPower() - loss + fade;
				// additional cuttoffs
				double dstPower_mW = Util.fromDB(dstPower);
				// if(dstPower_mW < shared.getBackground_mW()) return;
				if (dstPower_mW < (sense ? dstInfo.getShared()
						.getSensitivity_mW() : dstInfo.getShared()
						.getThreshold_mW()))
					return;
				links++;
			}
			...
			public void visit(Field.RadioData dst) {
				spatial.visitTransmit(this, dst.info, dst.loc, null, null,
						limit);
				nodes++;
			}
		};
