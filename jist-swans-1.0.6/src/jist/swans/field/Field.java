...
public class Field implements FieldInterface
{
    ...
    // @author Elmar Schoch >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    /**
     * Vector holding listeners that want to be notified on node movements
     * 
     * @author Elmar Schoch
     */
    ...
    /**
     * Initialize new, empty field with default fading (FadingNone) and pathloss
     * (PathLossFreeSpace) models, and default propagation limits, possibly with
     * wrapped edges.
     * 
     * @param bounds
     *            spatial limits of field
     * @param wrap
     *            whether to wrap field edges
     */
    public Field(Location bounds, boolean wrap) {
        this(wrap ? (Spatial) new Spatial.TiledWraparound(new Spatial.HierGrid(bounds, 5))
                : (Spatial) new Spatial.HierGrid(bounds, 5), new Fading.None(), new PathLoss.FreeSpace(),
                new Mobility.Static(), Constants.PROPAGATION_LIMIT_DEFAULT);
    }

    /**
     * Initialize new, empty field with given fading and pathloss models, using
     ...
     * @param mobility
     *            mobility model
     * @param propagationLimit
     *            minimum signal threshold (in dBm)
     */
    public Field(Spatial spatial, Fading fading, PathLoss pathloss, Mobility mobility, double propagationLimit) {
    ...
    /**
     * Radio information stored by the Field entity. Includes identifier,
     * location, and a radio entity reference for upcalls in doubly linked list.
     * 
     * @author Rimon Barr &lt;barr+jist@cs.cornell.edu&gt;
     * @since SWANS1.0
     */
    public static class RadioData
    {
        ...
        /**
         * nodes must be able to read their position in order to enable
         * georouting
         */
     ...
     * 
     * @param info
     *            radio properties (includes radio identifier)
     * @param entity
     *            radio upcall entity reference
     * @param loc
     *            radio location
     */
    ...
    private Spatial.SpatialTransmitVisitor transmitVisitor = new Spatial.SpatialTransmitVisitor() {
                                                               public double computeSignal(RadioInfo srcInfo,
                                                                       Location srcLoc, Location dstLoc) {
                                                                   double loss = pathloss.compute(srcInfo, srcLoc,
                                                                           srcInfo, dstLoc);
                                                                   double fade = fading.compute();
                                                                   return srcInfo.getShared().getPower() - loss + fade;
                                                               }

                                                               public void visitTransmit(RadioInfo srcInfo,
                                                                       Location srcLoc, RadioInfo dstInfo,
                                                                       RadioInterface dstEntity, Location dstLoc,
                                                                       Message msg, Long durationObj) {
                                                                   if (srcInfo.getUnique().getID() == dstInfo
                                                                           .getUnique().getID())
                                                                       return;
                                                                   // compute
                                                                   // signal
                                                                   ...
                                                                   double loss = pathloss.compute(srcInfo, srcLoc,
                                                                           dstInfo, dstLoc);
                                                                   double fade = fading.compute();
                                                                   double dstPower = srcInfo.getShared().getPower()
                                                                           - loss + fade;
                                                                   // additional
                                                                   // cuttoffs
                                                                   ...
                                                                   // if(dstPower_mW
                                                                   // <
                                                                   // dstInfo.getShared().getBackground_mW())
                                                                   // return;
                                                                   if (dstPower_mW < dstInfo.getShared()
                                                                           .getSensitivity_mW())
                                                                       return;
                                                                   dstEntity.receive(msg, new Double(dstPower_mW),
                                                                           durationObj);
                                                               }
                                                           };
     ...
     */
    public static interface ConnectivityVisitor extends Spatial.SpatialTransmitVisitor, Spatial.SpatialVisitor
    {
        /**
     ...
     * 
     * @param sense
     *            whether to use radio sensing or reception signal strength
    ...
    public double computeAvgConnectivity(final boolean sense) {
        ...
        ConnectivityVisitor connectivityVisitor = new ConnectivityVisitor() {
            ...
            public void visitTransmit(RadioInfo srcInfo, Location srcLoc, RadioInfo dstInfo, RadioInterface dstEntity,
                    Location dstLoc, Message msg, Long durationObj) {
                ...
                if (dstPower_mW < (sense ? dstInfo.getShared().getSensitivity_mW() : dstInfo.getShared()
                        .getThreshold_mW()))
                    return;
                links++;
            }
