...
public interface Mobility {
	...
	public static class RandomWaypoint implements Mobility {
		...
		public RandomWaypoint(Location.Location2D bounds, String config) {
			...
			init(bounds, Integer.parseInt(data[0]) * Constants.SECOND,
			...
			try {
				...
				if (rwi.steps == 0) {
					...
					rwi.waypoint = new Location.Location2D(
							(float) (BORDER + (bounds.getX() - 2 * BORDER)
									* Constants.random.nextFloat()),
							(float) (BORDER + (bounds.getY() - 2 * BORDER)
									* Constants.random.nextFloat()));
					if (Main.ASSERT)
					...
					rwi.steps = (int) StrictMath.max(
							StrictMath.floor(dist / precision), 1);
					if (Main.ASSERT)
					...
					// TODO: christin;
					// System.out.println("node speed = "+speed);
				}
				// take step
	...
	/**
	 * Teleport mobility model: pick a random location and teleport to it, then
	 * pause for some time and repeat.
	 * 
	 * @author Rimon Barr &lt;barr+jist@cs.cornell.edu&gt;
	 * @since SWANS1.0
	 */
	public static class Teleport implements Mobility {
		...
		public void next(FieldInterface f, Integer id, Location loc,
			...
			if (pauseTime > 0) {
				JistAPI.sleep(pauseTime);
				loc = new Location.Location2D((float) bounds.getX()
						* Constants.random.nextFloat(), (float) bounds.getY()
						* Constants.random.nextFloat());
				f.moveRadio(id, loc);
			}
		}
	 ...
	 * that direction, with some fixed and random component, reflecting off
	 * walls as necessary, then pause for some time and repeat.
	 * 
	 * @author Rimon Barr &lt;barr+jist@cs.cornell.edu&gt;
	 * @since SWANS1.0
	 */
	public static class RandomWalk implements Mobility {
		 ...
		 *            time wait between steps
		 */
		public RandomWalk(Location.Location2D bounds, double fixedRadius,
				double randomRadius, long pauseTime) {
		 ...
		 *            field boundaries
		 * @param config
		 *            configuration string: "fixed,random,time(in seconds)"
		 */
		public RandomWalk(Location.Location2D bounds, String config) {
			String[] data = config.split(",");
			if (data.length != 3) {
				throw new RuntimeException(
						"expected format: fixedradius,randomradius,pausetime(in seconds)");
			}
		 ...
		 *            time wait between steps
		 */
		private void init(Location.Location2D bounds, double fixedRadius,
				double randomRadius, long pauseTime) {
			if (fixedRadius + randomRadius > bounds.getX()
					|| fixedRadius + randomRadius > bounds.getY()) {
				throw new RuntimeException(
						"maximum step size can not be larger than field dimensions");
			}
			this.bounds = bounds;
		...
		public void next(FieldInterface f, Integer id, Location loc,
				MobilityInfo info) {
			// compute new random position with fixedRadius+randomRadius()
			// distance
			double randomAngle = 2 * StrictMath.PI
					* Constants.random.nextDouble();
			double r = fixedRadius + Constants.random.nextDouble()
					* randomRadius;
			double x = r * StrictMath.cos(randomAngle), y = r
					* StrictMath.sin(randomAngle);
		...
		public String toString() {
			return "RandomWalk(r=" + fixedRadius + "+" + randomRadius + ",p="
					+ pauseTime + ")";
		}
