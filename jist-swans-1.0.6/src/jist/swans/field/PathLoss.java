...
public interface PathLoss {
	...
	final class FreeSpace implements PathLoss {
		...
		public double compute(RadioInfo srcRadio, Location srcLocation,
			...
			double pathloss = -srcRadio.getShared().getGain()
					- dstRadio.getShared().getGain();
			double valueForLog = 4.0 * StrictMath.PI * dist
					/ srcRadio.getShared().getWaveLength();
			if (valueForLog > 1.0) {
				pathloss += Util.log((float) valueForLog) / Constants.log10
						* 20.0;
			}
			return pathloss;
		}
	...
	final class TwoRay implements PathLoss {
		...
		public double compute(RadioInfo srcRadio, Location srcLocation,
			...
			double freeSpaceLoss = 4.0 * StrictMath.PI * dist
					/ srcRadio.getShared().getWaveLength();
			if (planeEarthLoss > freeSpaceLoss) {
				if (planeEarthLoss > 1.0) {
					pathloss += 20.0 * StrictMath.log(planeEarthLoss)
							/ Constants.log10;
				}
			} else {
				if (freeSpaceLoss > 1.0) {
					pathloss += 20.0 * StrictMath.log(freeSpaceLoss)
							/ Constants.log10;
				}
			}
			return pathloss;
	...
	// End of file is indicated by a -1 in the first column. (And nothing else
	// on the line.)
