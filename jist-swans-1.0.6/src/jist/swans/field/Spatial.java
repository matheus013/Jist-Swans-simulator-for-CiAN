...
public abstract class Spatial {
	...
	public static interface SpatialTransmitVisitor {
		 ...
		 * @param srcLoc
		 *            source radio location
		 * @param dst
		 *            destination radio location
		 ...
		 *            source radio location
		 * @param dstInfo
		 *            destination radio information
		 * @param dstEntity
		 *            destination radio entity
		 * @param dstLoc
		 *            destination radio location
		 * @param msg
		 *            message to transmit
	...
	public Spatial(Location bl, Location tr) {
		this(bl, new Location.Location2D(tr.getX(), bl.getY()),
				new Location.Location2D(bl.getX(), tr.getY()), tr);
	}
	...
	public abstract int visitTransmit(SpatialTransmitVisitor visitor,
			RadioInfo srcInfo, Location srcLoc, Message msg, Long durationObj,
			double limit);
	 ...
	 * 
	 * @param radioData
	 *            radio information (location inside bin limits)
	 */
	 ...
	 * @param newLoc
	 *            destination of move
	 * @return radio information (unupdated) if new location outside of bin;
	 *         null if new location inside bin
	 */
	 ...
	 *            radio information
	 * @param newLoc
	 *            destination of move (must be within bin)
	 */
	public void moveInside(Field.RadioData rd, Location newLoc) {
		rd = move(rd, newLoc);
		if (Main.ASSERT)
			Util.assertion(rd == null);
	}
		...
		if (src.getX() <= bl.getX()) {
			...
			if (src.getY() <= bl.getY()) {
				...
				throw new RuntimeException(
						"get nearest undefined for internal point");
			}
		}
	}
	...
	public static class LinearList extends Spatial {
		...
		public void add(Field.RadioData data) {
			if (Main.ASSERT)
				Util.assertion(data.loc.inside(bl, tr));
			if (Main.ASSERT)
				Util.assertion(data.next == null);
			if (Main.ASSERT)
				Util.assertion(data.prev == null);
			data.next = radioList;
			if (radioList != null)
				radioList.prev = data;
			radioList = data;
		...
		public int visitTransmit(SpatialTransmitVisitor visitor,
				RadioInfo srcInfo, Location srcLoc, Message msg,
				Long durationObj, double limit) {
			int visited = 0;
			for (Field.RadioData dst = radioList; dst != null && visited < size; dst = dst.next, visited++) {
				visitor.visitTransmit(srcInfo, srcLoc, dst.info, dst.entity,
						dst.loc, msg, durationObj);
			}
			return visited;
		}
	...
	public static class Grid extends Spatial {
		 ...
		 *            grid divisions
		 */
		public Grid(Location bl, Location br, Location tl, Location tr,
				int divisions) {
			super(bl, br, tl, tr);
				...
				for (int i = 0; i < divisions; i++) {
					...
					try {
						...
						try {
							...
							pbl = new Location.Location2D(bl.getX() + i * di,
									bl.getY() + j * dj);
						}
					}
					// pbr
					try {
						...
						pbr = new Location.Location2D(bl.getX() + (i + 1) * di,
								bl.getY() + j * dj);
					}
					// ptl
					try {
						...
						ptl = new Location.Location2D(bl.getX() + i * di,
								bl.getY() + (j + 1) * dj);
					}
					// ptr
					ptr = new Location.Location2D(bl.getX() + (i + 1) * di,
							bl.getY() + (j + 1) * dj);
		...
		public void add(Field.RadioData radioData) {
			if (Main.ASSERT)
				Util.assertion(radioData.loc.inside(bl, tr));
		...
		public int visitTransmit(SpatialTransmitVisitor visitor,
			...
			for (r = 0; inRange; r++) {
				...
				for (int di = si - r, dj = sj - r; di <= si + r; di++) {
					int i = visitBin(visitor, srcInfo, srcLoc, msg,
							durationObj, limit, di, dj);
					if (i != -1) {
						inRange = true;
						visited += i;
					}
		 ...
		 *            message to transmit
		 * @param durationObj
		 *            duration of message transmission
		 * @param limit
		 *            propagation limit
		 * @param di
		 *            grid x-coordinate
		 ...
		 * @return number of radios visited
		 */
		private int visitBin(SpatialTransmitVisitor visitor, RadioInfo srcInfo,
				Location srcLoc, Message msg, Long durationObj, double limit,
				int di, int dj) {
			if (di < 0 || dj < 0 || di >= bins.length || dj >= bins[0].length)
				return -1;
			LinearList bin = bins[di][dj];
			if (!srcLoc.inside(bin.bl, bin.tr)
					&& visitor.computeSignal(srcInfo, srcLoc,
							bin.getNearest(srcLoc)) < limit)
				return -1;
			int visited = 0;
			if (bin.size > 0) {
				visited = bin.visitTransmit(visitor, srcInfo, srcLoc, msg,
						durationObj, limit);
			}
			return visited;
		}
	...
	public static class HierGrid extends Spatial {
		...
		public HierGrid(Location bl, Location tr, int height) {
			this(bl, new Location.Location2D(tr.getX(), bl.getY()),
					new Location.Location2D(bl.getX(), tr.getY()), tr, height);
		}
		 ...
		 *            height in bin tree
		 */
		public HierGrid(Location bl, Location br, Location tl, Location tr,
				int height) {
			super(bl, br, tl, tr);
			if (Main.ASSERT)
				Util.assertion(height > 0);
			mid = new Location.Location2D((bl.getX() + tr.getX()) / 2,
					(bl.getY() + tr.getY()) / 2);
		 ...
		 * 
		 * @param loc
		 *            location to descend towards
		 * @return sub-bin containing location
		 */
		private Spatial getBin(Location loc) {
			return loc.getX() < mid.getX() ? (loc.getY() < mid.getY() ? bins[BL]
					: bins[TL])
		...
		public void add(Field.RadioData rd) {
			if (Main.ASSERT)
				Util.assertion(rd.loc.inside(bl, tr));
		...
		public int visitTransmit(SpatialTransmitVisitor visitor,
			...
			if (!srcLoc.inside(bl, tr)
					&& visitor.computeSignal(srcInfo, srcLoc,
							getNearest(srcLoc)) < limit)
				return 0;
			int total = 0;
			for (int i = 0; i < bins.length; i++) {
				if (bins[i].size > 0) {
					total += bins[i].visitTransmit(visitor, srcInfo, srcLoc,
							msg, durationObj, limit);
				}
			}
			return total;
		}
	...
	public static class TiledWraparound extends Spatial {
		...
		public TiledWraparound(Spatial tile) {
			...
			for (int i = 0; i < tiles; i++) {
				for (int j = 0; j < tiles; j++) {
					offsets[i * tiles + j] = new Location.Location2D(BLx
							+ tr.getX() * i, BLy + tr.getY() * j);
				}
			}
		}
		...
		public int visitTransmit(SpatialTransmitVisitor visitor,
			...
			for (int i = 0; i < offsets.length; i++) {
				...
				count += tile.visitTransmit(visitor, srcInfo, l, msg,
						durationObj, limit);
			}
			return count;
		}
