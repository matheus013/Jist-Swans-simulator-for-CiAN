...
public interface Placement {
	...
	public static class Random implements Placement {
		 ...
		 * 
		 * @param x
		 *            field x-dimension (in meters)
		 * @param y
		 *            field y-dimension (in meters)
		 */
		...
		public Location getNextLocation() {
			return new Location.Location2D(Constants.random.nextFloat() * x,
	...
	public static class Grid implements Placement {
		...
		private void init(String field, String nodes) {
			...
			if (data.length != 2)
				throw new IllegalArgumentException(
						"invalid format, expected i,j");
			init(new Location.Location2D(Float.parseFloat(data[0]),
					Float.parseFloat(data[1])), nodes);
		}
		...
		private void init(Location loc, String s) {
			...
			if (data.length != 2)
				throw new IllegalArgumentException(
						"invalid format, expected i,j");
			init(loc.getX(), loc.getY(), Integer.parseInt(data[0]),
		...
		public Location getNextLocation() {
			...
			Location l = new Location.Location2D((i % nodex) * fieldx / nodex,
					(i / nodex) * fieldy / nodey);
			i++;
			return l;
		}
