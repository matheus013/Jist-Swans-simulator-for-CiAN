 ...
 * 
 * @see <a
 *      href="http://www.ietf.org/internet-drafts/draft-ietf-manet-dsr-09.txt">
 *      DSR Specification</a>
...
public class RouteDsr implements RouteInterface.Dsr
{
    ...
    /*
     * Unused at the present public static final long MAX_PACKET_BUFFER_TIME =
     * 11110 * Constants.SECOND;
     */
    ...
    private class RouteReplyTableEntry
    {
         ...
         *            the originator of the shortened Source Route
         * @param l
         *            the last hop address of the shortened Source Route
         */
     ...
     * sent and are currently waiting passive acknowledgement. An overheard
     * message is taken as a passive acknowledgement of a previously sent
    ...
    private class MaintenanceBufferEntry
    {
         ...
         *            IP Fragmentation Offset
         */
        public MaintenanceBufferEntry(NetAddress src, NetAddress dest, short protocol, short id, short fragOffset) {
            this.src = src;
            this.dest = dest;
        ...
        public boolean equals(Object o) {
            ...
            return other.src.equals(src) && other.dest.equals(dest) && other.protocol == protocol && other.id == id
     ...
     * <code>RouteRequestTableEntry</code>s, which are structures containing
     * various information used when performing Route Discovery.
     */
     ...
     * 
     * @return the proxy <code>RouteInterface.Dsr</code> interface for this
     *         object
     */
    ...
    private void SendRouteReply(RouteDsrMsg.OptionRouteRequest opt, NetAddress src) {
        ...
        if (routeFromHere.length > 0) {
            reply.addOption(RouteDsrMsg.OptionSourceRoute.create(0, routeFromHere.length, routeFromHere));
        }

        NetMessage.Ip replyMsg = new NetMessage.Ip(reply, localAddr, src, Constants.NET_PROTOCOL_DSR,
     ...
     * 
     * @param msg
     *            the message containing the Route Request
     * @param opt
     *            the Route Request option
     * @param optBuf
     *            the bytes of the Route Request option
     * @param src
     *            the originator of the Route Request
     * @param dst
     *            the destination of the Route Request (usually broadcast)
     * @param protocol
     *            the IP protocol of the message
     * @param priority
     *            the IP priority of the message
     * @param ttl
     *            the IP time to live of the message
     * @param id
     *            the IP identification of the message
     * @param fragOffset
     *            the IP fragmentation offset of the message
     */
    private void ForwardRequest(RouteDsrMsg msg, RouteDsrMsg.OptionRouteRequest opt, byte[] optBuf, NetAddress src,
            NetAddress dst, short protocol, byte priority, byte ttl, short id, short fragOffset) {
        // If I've already forwarded this request, ignore it
        for (int i = 0; i < opt.getNumAddresses(); i++) {
            if (localAddr.equals(opt.getAddress(i)))
                return;
        }
        ...
        newRequest.addOption(RouteDsrMsg.OptionRouteRequest.create(opt.getId(), opt.getTargetAddress(), newAddresses));

        NetMessage.Ip newRequestIp = new NetMessage.Ip(newRequest, src, dst, protocol, priority, (byte) (ttl - 1), id,
                fragOffset);
     ...
     * 
     * @param msg
     *            the <code>RouteDsrMsg</code> containing the request
     * @param opt
     *            the Route Request option
     * @param optBuf
     *            the bytes of the Route Request option
     * @param src
     *            the address of the originator of the Route Request
     * @param dst
     *            the destination address of this request (usually broadcast)
     * @param protocol
     *            the IP protocol of this Route Request (usually DSR)
     * @param priority
     *            the IP priority of this Route Request
     * @param ttl
     *            the IP time to live of this Route Request
     * @param id
     *            the IP identification of this Route Request
     * @param fragOffset
     *            the IP fragmentation offset of this Route Request
     */
    private void HandleRequest(RouteDsrMsg msg, RouteDsrMsg.OptionRouteRequest opt, byte[] optBuf, NetAddress src,
            NetAddress dst, short protocol, byte priority, byte ttl, short id, short fragOffset) {
        ...
        if (localAddr.equals(opt.getTargetAddress())) {
            ...
            ForwardRequest(msg, opt, optBuf, src, dst, protocol, priority, ttl, id, fragOffset);
        }
     ...
     * 
     * @param msg
     *            the <code>RouteDsrMsg</code> containing the Route Reply
     * @param reply
     *            the Route Reply option
     */
     ...
     * 
     * @param sourceRoute
     *            the Source Route option (or <code>null</code> if none)
     * @param dst
     ...
     *         Source Route option is invalid in some way, <code>null</code> can
     *         be returned.
     */
    private NetAddress PrevRecipient(RouteDsrMsg.OptionSourceRoute sourceRoute, NetAddress src) {
        ...
        int prevSegment = sourceRoute.getNumAddresses() - sourceRoute.getNumSegmentsLeft() - 1;
    ...
    /**
     * Forwards a DSR packet containing a Source Route option to the next
     ...
     * 
     * @param msg
     *            the <code>RouteDsrMsg</code> to be forwarded
     * @param opt
     *            the Source Route option
     * @param optBuf
     *            the bytes of the Source Route option
     * @param src
     *            the address of the originator of this packet
     * @param dest
     *            the address of the ultimate destination of this packet
     * @param protocol
     *            the IP protocol of this packet (usually DSR)
     * @param priority
     *            the IP priority of this packet
     * @param ttl
     *            the IP time to live of this packet
     * @param id
     *            the IP identification of this packet
     * @param fragOffset
     *            the IP fragmentation offset of this packet
     */
    private void ForwardPacket(RouteDsrMsg msg, RouteDsrMsg.OptionSourceRoute opt, byte[] optBuf, NetAddress src,
            NetAddress dest, short protocol, byte priority, byte ttl, short id, short fragOffset) {
        ...
        if (localAddr.equals(NextRecipient(opt, dest))) {
            ...
            newOptions.remove(optBuf);
            newMsg.addOption(RouteDsrMsg.OptionSourceRoute.create(opt.isFirstHopExternal(), opt.isLastHopExternal(),
                    opt.getSalvageCount(), opt.getNumSegmentsLeft() - 1, route));
            ...
            NetMessage.Ip ipMsg = new NetMessage.Ip(newMsg, src, dest, protocol, priority, (byte) (ttl - 1), id,
                    fragOffset);
     ...
     * <code>dest</code> than the one given in <code>sourceRoute</code>. If this
     * is the case, we send a gratuitous Route Reply to <code>src</code> letting
     * him know of the shorter route.
     * 
     * @param sourceRoute
     *            the route that can potentially be shortened
     * @param src
     *            the originator of the Source Route
     * @param dest
    ...
    private void PerformRouteShortening(RouteDsrMsg.OptionSourceRoute sourceRoute, NetAddress src, NetAddress dest) {
        int routeLength = sourceRoute.getNumAddresses() - sourceRoute.getNumSegmentsLeft();
     ...
     * <code>src</code> hasn't sent out a Route Request, but we happen to know a
     * shorter route than the one he's using, so we're letting him know.
     * 
     * @param src
     *            the originator of the shortened Source Route
     * @param sourceRoute
     *            the Source Route we know how to shorten
     * @param dest
     *            the destination of the shortened Source Route
     * @param routeLength
     *            the length of the new (shortened) Source Route
     */
    private void SendGratuitousRouteReply(NetAddress src, RouteDsrMsg.OptionSourceRoute sourceRoute, NetAddress dest,
            int routeLength) {
        NetAddress prevRecipient = PrevRecipient(sourceRoute, src);

        // Don't reply if we have recently sent a gratuitous route reply
        if (RouteReplyEntryExists(src, prevRecipient))
            return;
        ...
        if (routeLength > 0) {
            dsrMsg.addOption(RouteDsrMsg.OptionSourceRoute.create(0, routeLength, routeToSrc));
        }

        NetMessage.Ip ipMsg = new NetMessage.Ip(dsrMsg, localAddr, src, Constants.NET_PROTOCOL_DSR,
                Constants.NET_PRIORITY_NORMAL, Constants.TTL_DEFAULT);
     ...
     * 
     * @param msg
     *            the <code>RouteDsrMsg</code> containing the Acknowledgement
     *            Request
     * @param opt
     *            the Acknowledgement Request option
     * @param src
     *            the originator of the Acknowledgement Request
     * @param dst
     *            the destination of the Acknowledgement Request
     * @param sourceRoute
     *            the Source Route option that came in the same message as the
     *            Acknowledgement Request. This parameter can be
     *            <code>null</code> if there was no Source Route option.
     */
    private void HandleAckRequest(RouteDsrMsg msg, RouteDsrMsg.OptionAckRequest opt, NetAddress src, NetAddress dst,
        ...
        if (localAddr.equals(NextRecipient(sourceRoute, dst))) {
            ...
            NetMessage.Ip ipMsg = new NetMessage.Ip(dsrMsg, localAddr, ackDest, Constants.NET_PROTOCOL_DSR,
                    Constants.NET_PRIORITY_NORMAL, Constants.TTL_DEFAULT);
     ...
     * 
     * @param msg
     *            the message containing the DSR header
     * @param src
     *            the IP source address of the message
     * @param dst
     *            the IP destination address of the message
     * @param protocol
     *            the IP protocol of the message
     * @param priority
     *            the IP priority of the message
     ...
     *            the IP fragmentation offset of the message
     */
    private void ProcessOptions(RouteDsrMsg msg, NetAddress src, NetAddress dst, short protocol, byte priority,
        ...
        while (iter.hasNext()) {
            ...
            switch (opt.getType()) {
                case RouteDsrMsg.OPT_ROUTE_REQUEST:
                    ...
                    // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                    HandleRequest(msg, (RouteDsrMsg.OptionRouteRequest) opt, optBuf, src, dst, protocol, priority, ttl,
                            id, fragOffset);
                ...
                case RouteDsrMsg.OPT_SOURCE_ROUTE:
                    ...
                    if (localAddr.equals(NextRecipient(sourceRoute, dst))) {
                        ForwardPacket(msg, sourceRoute, optBuf, src, dst, protocol, priority, ttl, id, fragOffset);
                    } else {
                ...
                default:
                    ...
                    // should
                    // probably occur *before* the processing of any other
                    // options.
        ...
        if (ackRequest != null) {
            ...
            // handle packets containing both acknowledgements and
            // acknowledgement
     ...
     * 
     * @param dest
     *            the address to which we desire a route
     * @param requestId
    ...
    public void DiscoverRoute(NetAddress dest, short requestId) {
        ...
        if ((entry.numRequestsSinceLastReply == 0) || (entry.lastRequestTime + entry.timeout <= JistAPI.getTime())) {
            if (!activeRequests.contains(dest))
                return;

            RouteDsrMsg routeRequest = new RouteDsrMsg(null);
            routeRequest.addOption(RouteDsrMsg.OptionRouteRequest.create(requestId, dest, new NetAddress[0]));

            NetMessage.Ip routeRequestMsg = new NetMessage.Ip(routeRequest, localAddr, NetAddress.ANY,
                    Constants.NET_PROTOCOL_DSR, Constants.NET_PRIORITY_NORMAL, Constants.TTL_DEFAULT);
     ...
     * 
     * @param dest
     *            the destination of the route to be added
     * @param route
     *            the sequence of nodes from here to <code>dest</code>. Neither
     *            the IP address of this node nor of the <code>dest</code> node
     *            should be included in the <code>route</code> array.
     */
        ...
        if (routeCache.containsKey(dest)) {
            ...
            for (ListIterator iter = routes.listIterator(); iter.hasNext();) {
                ...
                if (curRoute.length == route.length) {
                    ...
                    if (Arrays.equals(curRoute, route))
                        return;
                }
     ...
     * 
     * @param addr1
     *            the first address in the link
     * @param addr2
     *            the second address in the link
     */
        ...
        while (e.hasMoreElements()) {
            ...
            while (routeIter.hasNext()) {
                ...
                if (addr1.equals(localAddr) && route.length > 0 && addr2.equals(route[0])) {
                    ...
                    routeIter.remove();
                } else if (addr2.equals(dest) && route.length > 0 && addr1.equals(route[route.length - 1])) {
                    routeIter.remove();
                } else {
     ...
     * @return An array of addresses denoting a route from here to
     *         <code>dest</code>. Neither the address of this node nor of
     *         <code>dest</code> is included in this array. If the Route Cache
     *         contains no route to <code>dest</code>, <code>null</code> is
     *         returned. If the Route Cache contains more than one route, the
     *         shortest is returned, with ties broken arbitrarily.
     */
     ...
     * 
     * @param msg
     *            the message to insert into the buffer
     */
     ...
     * 
     * @param msg
     *            the packet to remove from the Send Buffer
     */
     ...
     * 
     * @param dest
     *            the destination we now have a route to
     */
     ...
     * 
     * @param src
     *            the source address of the Route Request
     * @param id
     *            the ID number of the Route Request
    ...
    /**
     * Sends a DSR message with a Route Error option to <code>src</code>
     * indicating that the next hop in the intended route cannot be reached from
     * this node.
     * 
     * @param msg
     *            the <code>RouteDsrMsg</code> containing the Source Route
     *            option that contains the broken link
     * @param src
     *            the originator of <code>msg</code>
     * @param dest
     *            the destination of <code>msg</code>
     */
    private void SendRouteError(RouteDsrMsg msg, NetAddress src, NetAddress dest) {
        ...
        while (iter.hasNext()) {
            ...
            if (opt.getType() == RouteDsrMsg.OPT_SOURCE_ROUTE) {
                RouteDsrMsg.OptionSourceRoute sourceRoute = (RouteDsrMsg.OptionSourceRoute) opt;
                ...
                errorMsg.addOption(RouteDsrMsg.OptionRouteError.create(RouteDsrMsg.ERROR_NODE_UNREACHABLE,
                ...
                if (curSegment > 1) {
                    ...
                    errorMsg.addOption(RouteDsrMsg.OptionSourceRoute.create(0, route.length, route));
                ...
                NetMessage.Ip ipMsg = new NetMessage.Ip(errorMsg, localAddr, src, Constants.NET_PROTOCOL_DSR,
                        Constants.NET_PRIORITY_NORMAL, Constants.TTL_DEFAULT);

                Transmit(ipMsg);

                if (log.isInfoEnabled()) {
                    log.info("Originated route error from " + localAddr + " to " + src + ": Cannot contact " + nextAddr);
                }
     ...
     * 
     * @param msg
     *            the message to send. The payload of this IP packet should be a
     *            <code>RouteDsrMsg</code>, and it should not already contain an
    ...
    private void Transmit(NetMessage.Ip msg) {
        ...
        if (sourceRoute == null || sourceRoute.getNumSegmentsLeft() == 0) {
            // Messages on their last hop must use network-level
            // acknowledgements.
            ...
            NetMessage.Ip ipMsg = new NetMessage.Ip(newMsg, msg.getSrc(), msg.getDst(), msg.getProtocol(),
                    msg.getPriority(), msg.getTTL(), msg.getId(), msg.getFragOffset());
        ...
        // acknolwedgements
        MaintenanceBufferEntry entry = new MaintenanceBufferEntry(msg.getSrc(), msg.getDst(), msg.getProtocol(),
    ...
    public void TransmitWithPassiveAck(NetMessage.Ip msg, int numRetransmits) {
        ...
        if (sourceRoute != null && sourceRoute.getNumSegmentsLeft() > 0) {
            entry = new MaintenanceBufferEntry(msg.getSrc(), msg.getDst(), msg.getProtocol(), msg.getId(),
                    msg.getFragOffset());
        ...
        if (sourceRoute == null || sourceRoute.getNumSegmentsLeft() == 0 || numRetransmits >= TRY_PASSIVE_ACKS) {
            // Messages on their final hop and messages that have already tried
            // and
            // failed to receive a passive acknowledgement must use
            // network-level
            // acknowledgments
            RouteDsrMsg newMsg = (RouteDsrMsg) dsrMsg.clone();
     ...
     * @param timeout
     *            the number of clock ticks to wait before retransmitting the
     *            message
     * @param numRetransmits
     *            the number of times this packet has already been transmitted.
     *            Callers should normally pass in zero for this.
     */
    public void TransmitWithNetworkAck(NetMessage.Ip msg, Short ackId, long timeout, int numRetransmits) {
        ...
        if (numRetransmits > MAX_MAINT_REXMT) {
            ...
            if (msg.getProtocol() == Constants.NET_PROTOCOL_DSR) {
                ...
                if (msg.getSrc().equals(localAddr)) {
                    ...
                    if (!dsrMsg.hasOption(RouteDsrMsg.OPT_SOURCE_ROUTE)) {
                        ...
                        while (iter.hasNext()) {
                            ...
                            if (RouteDsrMsg.Option.getType(optBuf) == RouteDsrMsg.OPT_SOURCE_ROUTE) {
                                RouteDsrMsg.OptionSourceRoute sourceRoute = (RouteDsrMsg.OptionSourceRoute) RouteDsrMsg.Option
                                        .create(optBuf, 0);
                    ...
                    if (GetCachedRoute(msg.getDst()) != null)
        ...
        if (log.isDebugEnabled()) {
            if (numRetransmits > 0) {
                log.debug(localAddr + " retransmitting from " + msg.getSrc() + " to " + msg.getDst() + "!");
            }
        }
     ...
     * 
     * @param msg
     *            the message that has been overheard
     * @param src
     *            the IP source of the message
     * @param dest
     *            the IP destination of the message
     * @param protocol
     *            the IP protocol of the message
     * @param id
     *            the IP id number of the message
     * @param fragOffset
     *            the IP fragmentation offset of the message
     */
    private void CheckForPassiveAck(RouteDsrMsg msg, NetAddress src, NetAddress dest, short protocol, short id,
            short fragOffset) {
        ...
        MaintenanceBufferEntry entry = new MaintenanceBufferEntry(src, dest, protocol, id, fragOffset);
     ...
     * 
     * @param msg
     *            the <code>RouteDsrMsg</code> to be sent
     * @param route
     *            the sequence of nodes to route the message along
     */
    private void SendWithRoute(NetMessage.Ip msg, NetAddress[] route) {
        ...
        NetMessage.Ip ipMsg = new NetMessage.Ip(dsrMsg, msg.getSrc(), msg.getDst(), Constants.NET_PROTOCOL_DSR,
    ...
    /**
     * If the given message uses the DSR protocol, the DSR header is examined to
     * see if any actions need to be performed on this packet (such as
     * forwarding it).
     * 
     * @param msg
     *            the message to examine
     * @param lastHop
     *            the MAC address of the node that sent this message
     */
    public void peek(NetMessage msg, byte interfaceId, MacAddress lastHop) {
        if (msg instanceof NetMessage.Ip) {
            ...
            if (ipMsg.getProtocol() == Constants.NET_PROTOCOL_DSR) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug(localAddr + " saw message from " + ipMsg.getSrc() + " to " + ipMsg.getDst());
                    ...
                    while (iter.hasNext()) {
                        ...
                        switch (RouteDsrMsg.Option.getType(optBuf)) {
                            ...
                            case RouteDsrMsg.OPT_ACK_REQUEST:
                                RouteDsrMsg.OptionAckRequest ackRequest = (RouteDsrMsg.OptionAckRequest) RouteDsrMsg.Option
                                        .create(optBuf, 0);
                                log.debug("    Acknowledgement Request " + ackRequest.getId());
                                break;

                            case RouteDsrMsg.OPT_ACK:
                                RouteDsrMsg.OptionAck ack = (RouteDsrMsg.OptionAck) RouteDsrMsg.Option
                                        .create(optBuf, 0);
                            ...
                            case RouteDsrMsg.OPT_ROUTE_REQUEST:
                                RouteDsrMsg.OptionRouteRequest routeRequest = (RouteDsrMsg.OptionRouteRequest) RouteDsrMsg.Option
                                        .create(optBuf, 0);
                ...
                CheckForPassiveAck(dsrMsg, ipMsg.getSrc(), ipMsg.getDst(), ipMsg.getProtocol(), ipMsg.getId(),
                        ipMsg.getFragOffset());

                ProcessOptions(dsrMsg, ipMsg.getSrc(), ipMsg.getDst(), ipMsg.getProtocol(), ipMsg.getPriority(),
     ...
     * originating a packet (in which case a DSR header is added to the packet
     * and it is sent) or because this node is forwarding a packet (in which
     * case this method actually does nothing, with all DSR header option
     * processing being performed by <code>peek</code>.
     * 
     * @param msg
     *            the message to be sent
     */
     ...
     * @param lastHop
     *            the MAC address of the most recent node to forward the message
     * @param macId
     *            the ID of the MAC interface
     * @param dst
     *            the address of the destination of the message (which should be
     *            the IP address of this node)
     * @param priority
     *            the IP priority of the message
     * @param ttl
    ...
    public void receive(Message msg, NetAddress src, MacAddress lastHop, byte macId, NetAddress dst, byte priority,
        ...
        if (dsrMsg.getContent() != null) {
            ...
            NetMessage.Ip newIp = new NetMessage.Ip(dsrMsg.getContent(), src, dst, dsrMsg.getNextHeaderType(),
                    priority, ttl);
