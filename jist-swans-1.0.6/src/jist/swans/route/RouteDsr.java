...
public class RouteDsr implements RouteInterface.Dsr {
	...
	private class MaintenanceBufferEntry {
		...
		/** {@inheritDoc} */
		public int hashCode() {
			return src.hashCode() + dest.hashCode() + protocol + id
					+ fragOffset;
		}
	...
	public RouteDsr(NetAddress localAddr) {
		...
		self = (RouteInterface.Dsr) JistAPI.proxy(this,
				RouteInterface.Dsr.class);
	}
	...
	private void ForwardRequest(RouteDsrMsg msg,
			RouteDsrMsg.OptionRouteRequest opt, byte[] optBuf, NetAddress src,
	...
	private void HandleReply(RouteDsrMsg msg, RouteDsrMsg.OptionRouteReply reply) {
		...
		for (int i = 0; i < reply.getNumAddresses() - 1; i++) {
			if (localAddr.equals(reply.getAddress(i))) {
				NetAddress[] route = new NetAddress[reply.getNumAddresses() - 2
						- i];
	...
	private void ForwardPacket(RouteDsrMsg msg,
			RouteDsrMsg.OptionSourceRoute opt, byte[] optBuf, NetAddress src,
		...
		if (localAddr.equals(NextRecipient(opt, dest))) {
			...
			newOptions.remove(optBuf);
			newMsg.addOption(RouteDsrMsg.OptionSourceRoute.create(
					opt.isFirstHopExternal(), opt.isLastHopExternal(),
			...
			while (iter.hasNext() && stats != null) {
				...
				RouteDsrMsg.Option option = RouteDsrMsg.Option.create(
						optBuffer, 0);
	 ...
	 */
	private void PerformRouteShortening(
			RouteDsrMsg.OptionSourceRoute sourceRoute, NetAddress src,
			NetAddress dest) {
		int routeLength = sourceRoute.getNumAddresses()
				- sourceRoute.getNumSegmentsLeft();
	...
	private void SendGratuitousRouteReply(NetAddress src,
			RouteDsrMsg.OptionSourceRoute sourceRoute, NetAddress dest,
			int routeLength) {
		...
		if (routeLength > 0) {
			dsrMsg.addOption(RouteDsrMsg.OptionSourceRoute.create(0,
					routeLength, routeToSrc));
		}
	...
	private void HandleAckRequest(RouteDsrMsg msg,
			RouteDsrMsg.OptionAckRequest opt, NetAddress src, NetAddress dst,
		...
		if (localAddr.equals(NextRecipient(sourceRoute, dst))) {
			...
			RouteDsrMsg dsrMsg = new RouteDsrMsg(null);
			dsrMsg.addOption(RouteDsrMsg.OptionAck.create(opt.getId(),
					localAddr, ackDest));

			NetMessage.Ip ipMsg = new NetMessage.Ip(dsrMsg, localAddr, ackDest,
					Constants.NET_PROTOCOL_DSR, Constants.NET_PRIORITY_NORMAL,
					Constants.TTL_DEFAULT);
	 ...
	 *            the IP fragmentation offset of the message
	 */
	private void ProcessOptions(RouteDsrMsg msg, NetAddress src,
			NetAddress dst, short protocol, byte priority, byte ttl, short id,
			short fragOffset) {
		...
		while (iter.hasNext()) {
			...
			case RouteDsrMsg.OPT_ROUTE_REQUEST:
				...
				// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
				HandleRequest(msg, (RouteDsrMsg.OptionRouteRequest) opt,
	...
	public void DiscoverRoute(NetAddress dest, short requestId) {
		...
		if ((entry.numRequestsSinceLastReply == 0)
			...
			routeRequest.addOption(RouteDsrMsg.OptionRouteRequest.create(
					requestId, dest, new NetAddress[0]));

			NetMessage.Ip routeRequestMsg = new NetMessage.Ip(routeRequest,
					localAddr, NetAddress.ANY, Constants.NET_PROTOCOL_DSR,
	...
	private void RemoveCachedLink(NetAddress addr1, NetAddress addr2) {
		...
		while (e.hasMoreElements()) {
			...
			while (routeIter.hasNext()) {
				...
				if (addr1.equals(localAddr) && route.length > 0
						&& addr2.equals(route[0])) {
					routeIter.remove();
				} else if (addr1.equals(localAddr) && route.length == 0
						&& addr2.equals(dest)) {
					routeIter.remove();
					...
					for (int i = 1; i < route.length; i++) {
						if (addr1.equals(route[i - 1])
								&& addr2.equals(route[i])) {
							routeIter.remove();
							break;
						}
					}
	...
	private boolean RouteReplyEntryExists(NetAddress originator,
			NetAddress lastHop) {
		return routeReplyTable.contains(new RouteReplyTableEntry(originator,
				lastHop));
	}
		...
		while (iter.hasNext()) {
			...
			if (opt.getType() == RouteDsrMsg.OPT_SOURCE_ROUTE) {
				...
				errorMsg.addOption(RouteDsrMsg.OptionRouteError.create(
						RouteDsrMsg.ERROR_NODE_UNREACHABLE,
						sourceRoute.getSalvageCount(), localAddr, src,
						nextAddrBuf));
				...
				if (curSegment > 1) {
					...
					while (--curSegment > 0) {
						route[curSegment - 1] = sourceRoute
								.getAddress(route.length - curSegment);
					}
				...
				NetMessage.Ip ipMsg = new NetMessage.Ip(errorMsg, localAddr,
						src, Constants.NET_PROTOCOL_DSR,
						Constants.NET_PRIORITY_NORMAL, Constants.TTL_DEFAULT);

				Transmit(ipMsg);

				if (log.isInfoEnabled()) {
					log.info("Originated route error from " + localAddr
							+ " to " + src + ": Cannot contact " + nextAddr);
				}
	...
	private RouteDsrMsg.OptionSourceRoute GetSourceRoute(RouteDsrMsg msg) {
		...
		while (iter.hasNext()) {
			...
			if (RouteDsrMsg.Option.getType(opt) == RouteDsrMsg.OPT_SOURCE_ROUTE) {
				return (RouteDsrMsg.OptionSourceRoute) RouteDsrMsg.Option
						.create(opt, 0);
			}
		}
	...
	private void Transmit(NetMessage.Ip msg) {
		...
		if (sourceRoute == null || sourceRoute.getNumSegmentsLeft() == 0) {
			...
			newMsg.addOption(RouteDsrMsg.OptionAckRequest.create(ackId
					.shortValue()));
			activeAcks.add(ackId);

			NetMessage.Ip ipMsg = new NetMessage.Ip(newMsg, msg.getSrc(),
					msg.getDst(), msg.getProtocol(), msg.getPriority(),
					msg.getTTL(), msg.getId(), msg.getFragOffset());
		...
		MaintenanceBufferEntry entry = new MaintenanceBufferEntry(msg.getSrc(),
				msg.getDst(), msg.getProtocol(), msg.getId(),
				msg.getFragOffset());

		maintenanceBuffer.put(entry,
				new Integer(sourceRoute.getNumSegmentsLeft()));
	...
	public void TransmitWithNetworkAck(NetMessage.Ip msg, Short ackId,
					...
					if (!dsrMsg.hasOption(RouteDsrMsg.OPT_SOURCE_ROUTE)) {
						...
						while (iter.hasNext()) {
							...
							if (RouteDsrMsg.Option.getType(optBuf) == RouteDsrMsg.OPT_SOURCE_ROUTE) {
								...
								unreachableAddress = NextRecipient(sourceRoute,
										msg.getDst());
								break;
							}
						}
		...
		if (log.isDebugEnabled()) {
			if (numRetransmits > 0) {
				log.debug(localAddr + " retransmitting from " + msg.getSrc()
						+ " to " + msg.getDst() + "!");
			}
		}
	...
	private void SendWithRoute(NetMessage.Ip msg, NetAddress[] route) {
		...
		NetMessage.Ip ipMsg = new NetMessage.Ip(dsrMsg, msg.getSrc(),
				msg.getDst(), Constants.NET_PROTOCOL_DSR, msg.getPriority(),
				msg.getTTL());
		...
		if (msg instanceof NetMessage.Ip) {
			...
			if (ipMsg.getProtocol() == Constants.NET_PROTOCOL_DSR) {
					...
					while (iter.hasNext()) {
						...
						case RouteDsrMsg.OPT_ACK_REQUEST:
							RouteDsrMsg.OptionAckRequest ackRequest = (RouteDsrMsg.OptionAckRequest) RouteDsrMsg.Option
									.create(optBuf, 0);
							log.debug("    Acknowledgement Request "
									+ ackRequest.getId());
							break;
				...
				CheckForPassiveAck(dsrMsg, ipMsg.getSrc(), ipMsg.getDst(),
						ipMsg.getProtocol(), ipMsg.getId(),
						ipMsg.getFragOffset());

				ProcessOptions(dsrMsg, ipMsg.getSrc(), ipMsg.getDst(),
						ipMsg.getProtocol(), ipMsg.getPriority(),
	...
	public void receive(Message msg, NetAddress src, MacAddress lastHop,
		...
		if (dsrMsg.getContent() != null) {
			...
			NetMessage.Ip newIp = new NetMessage.Ip(dsrMsg.getContent(), src,
					dst, dsrMsg.getNextHeaderType(), priority, ttl);
