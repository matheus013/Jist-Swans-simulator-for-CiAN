...
// Timestamp: <RouteZrpNdp.java Tue 2004/04/06 11:35:51 barr
...
public class RouteZrpNdp implements RouteInterface.Zrp.Ndp, Timer
{
    ...
    public void receive(RouteInterface.Zrp.MessageNdp msg, NetAddress src, MacAddress macAddr, byte macId) {
        if (logNDP.isDebugEnabled()) {
            logNDP.debug("receive t=" + JistAPI.getTime() + " at=" + zrp.getLocalAddr() + " from=" + src + " msg="
                    + msg);
        }
    ...
    public void timeout() {
        ...
        while (it.hasNext()) {
            ...
            if (n.beats < 0) {
                // neighbour lost
                if (logNDP.isInfoEnabled()) {
                    logNDP.info("t=" + JistAPI.getTimeString() + " " + zrp.getLocalAddr() + " lost neighbour "
                            + e.getKey());
                }
