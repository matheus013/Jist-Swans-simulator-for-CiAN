...
public class RouteZrpNdp implements RouteInterface.Zrp.Ndp, Timer {
	...
	public static final Logger logNDP = Logger.getLogger(RouteZrpNdp.class
			.getName());
	...
	/** {@inheritDoc} */
	public void receive(RouteInterface.Zrp.MessageNdp msg, NetAddress src,
			MacAddress macAddr, byte macId) {
		if (logNDP.isDebugEnabled()) {
			logNDP.debug("receive t=" + JistAPI.getTime() + " at="
					+ zrp.getLocalAddr() + " from=" + src + " msg=" + msg);
		}
		...
		if (n == null) {
			...
			if (logNDP.isInfoEnabled()) {
				logNDP.info("t=" + JistAPI.getTimeString() + " "
						+ zrp.getLocalAddr() + " found neighbour " + src);
			}
		}
		n.macAddr = macAddr;
		...
		n.macId = macId;
		if (changed)
			zrp.getIarp().linkinfo(new Link(src, zrp.getLocalAddr()), false);
	}
	...
	public byte getMacId(NetAddress addr) {
		NeighbourState ns = (NeighbourState) neighbours.get(addr);
		if (ns == null)
			return Constants.NET_INTERFACE_INVALID;
		return ns.macId;
	}

	/** {@inheritDoc} */
	public NetAddress[] getNeighbours() {
		return (NetAddress[]) neighbours.keySet().toArray(
	...
	public void timeout() {
		...
		if (logNDP.isDebugEnabled()) {
			logNDP.debug("send t=" + JistAPI.getTime() + " from="
		...
		while (it.hasNext()) {
			...
			if (n.beats < 0) {
				...
				if (logNDP.isInfoEnabled()) {
					logNDP.info("t=" + JistAPI.getTimeString() + " "
							+ zrp.getLocalAddr() + " lost neighbour "
							+ e.getKey());
				}
				zrp.getIarp().linkinfo(
						new Link((NetAddress) e.getKey(), zrp.getLocalAddr()),
						true);
				it.remove();
			}
		}
