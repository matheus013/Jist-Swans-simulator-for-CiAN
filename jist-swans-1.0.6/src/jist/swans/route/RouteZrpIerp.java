...
/**
 * Zone Routing Protocol: IntErzone Routing (sub)Protocol: Default
 * implementation.
 * 
...
public class RouteZrpIerp implements RouteInterface.Zrp.Ierp, Timer {
	...
	public static final Logger logIERP = Logger.getLogger(RouteZrpIerp.class
			.getName());
		...
		public void sendAll(NetAddress dst, RouteZrp zrp, NetAddress[] route) {
			...
			while (it.hasNext()) {
				...
				if (ip.getDst().equals(dst)) {
					...
					if (ip.isFrozen())
						ip = ip.copy();
					ip.getOptions().put(Constants.IP_OPTION_ZRP,
							new NetMessage.IpOptionSourceRoute(route));
					// send it off
					if (logIERP.isInfoEnabled()) {
						logIERP.info("sending off data t=" + JistAPI.getTime()
								+ " msg=" + ip);
					}
					zrp.send(ip, route[0]);
					it.remove();
	...
	private static class MessageIerp implements RouteInterface.Zrp.MessageIerp {
		 ...
		 * 
		 * @param seq
		 *            source-sequence request number
		 * @param dst
		...
		public String toString() {
			return "ierp(t=" + (type == TYPE_RREQ ? "q" : "a") + " id=" + seq
					+ " route=[" + Util.stringJoin(route, ",") + "])";
		}
		 ...
		 * 
		 * @param route
		 *            existing route (source .. route .. destination)
		 * @param nextHop
		...
		private static NetAddress[] routeAppend(NetAddress[] route,
			...
			if (Main.ASSERT)
				Util.assertion(!routeDupCheck(route2));
			return route2;
		}
		 ...
		 * 
		 * @param finder
		 *            address of node that found destination
		 * @param routeToDst
		...
		public MessageIerp makeReply(NetAddress finder, NetAddress[] routeToDst) {
			return new MessageIerp(TYPE_RREP, seq, RouteZrp.replaceDest(route,
					finder, routeToDst));
		}
	...
	public void receive(RouteInterface.Zrp.MessageIerp msg) {
		if (logIERP.isDebugEnabled()) {
			logIERP.debug("received t=" + JistAPI.getTime() + " node="
		...
		case MessageIerp.TYPE_RREQ:
			if (zrp.getIarp().hasRoute(msgImpl.getDst())) {
				...
				MessageIerp reply = msgImpl.makeReply(zrp.getLocalAddr(), zrp
						.getIarp().getRoute(msgImpl.getDst()));
				if (logIERP.isInfoEnabled()) {
					logIERP.info("dst found: t=" + JistAPI.getTime() + " at="
							+ zrp.getLocalAddr() + " reply=" + reply);
				}
				receive(reply);
			} else {
		...
		case MessageIerp.TYPE_RREP:
			...
			if (zrp.getLocalAddr().equals(msgImpl.getSrc())) {
				...
				if (logIERP.isInfoEnabled()) {
					logIERP.info("reply received: t=" + JistAPI.getTime()
							+ " reply=" + msgImpl);
				}
				mq.sendAll(msgImpl.getDst(), zrp,
						(NetAddress[]) Util.rest(route));
			} else {
				...
				if (logIERP.isDebugEnabled()) {
					logIERP.debug("forwarding reply: t=" + JistAPI.getTime()
							+ " from=" + zrp.getLocalAddr() + " to=" + nextHop
							+ " reply=" + msgImpl);
				}
				// forward reply
				if (zrp.getNdp().isNeighbour(nextHop)) {
					zrp.send(new NetMessage.Ip(msgImpl, zrp.getLocalAddr(),
							nextHop, Constants.NET_PROTOCOL_ZRP,
							Constants.NET_PRIORITY_NORMAL, (byte) 1), nextHop);
				}
			}
			break;
		default:
			throw new RuntimeException("unrecognized IERP packet type: "
					+ msgImpl.getType());
		}
	} // receive
