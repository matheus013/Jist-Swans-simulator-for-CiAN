...
// Timestamp: <RouteZrpIerp.java Sun 2005/03/13 11:07:49 barr
...
public class RouteZrpIerp implements RouteInterface.Zrp.Ierp, Timer
{
    ...
    private static class MessageIerp implements RouteInterface.Zrp.MessageIerp
    {
        ...
        public String toString() {
            return "ierp(t=" + (type == TYPE_RREQ ? "q" : "a") + " id=" + seq + " route=["
                    + Util.stringJoin(route, ",") + "])";
        }
        ...
        // route request
            case MessageIerp.TYPE_RREQ:
                if (zrp.getIarp().hasRoute(msgImpl.getDst())) {
                    ...
                    if (logIERP.isInfoEnabled()) {
                        logIERP.info("dst found: t=" + JistAPI.getTime() + " at=" + zrp.getLocalAddr() + " reply="
                                + reply);
                    }
                    receive(reply);
                } else {
            ...
            case MessageIerp.TYPE_RREP:
                ...
                if (zrp.getLocalAddr().equals(msgImpl.getSrc())) {
                    ...
                    if (logIERP.isDebugEnabled()) {
                        logIERP.debug("forwarding reply: t=" + JistAPI.getTime() + " from=" + zrp.getLocalAddr()
                                + " to=" + nextHop + " reply=" + msgImpl);
                    }
                    // forward reply
                    if (zrp.getNdp().isNeighbour(nextHop)) {
                        zrp.send(new NetMessage.Ip(msgImpl, zrp.getLocalAddr(), nextHop, Constants.NET_PROTOCOL_ZRP,
