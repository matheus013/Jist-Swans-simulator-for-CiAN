...
public class RouteZrpBrpFlood implements RouteInterface.Zrp.Brp, Timer {
	 ...
	 * </pre>
	 */
	private static class MessageBrpFlood implements
	 ...
	 */
	private RouteZrpBrp.QueryCoverageEntry getQueryCoverageEntry(
			NetAddress src, short seq) {
		RouteZrpBrp.QueryKey qk = new RouteZrpBrp.QueryKey(src, seq);
		RouteZrpBrp.QueryCoverageEntry qce = (RouteZrpBrp.QueryCoverageEntry) queryCoverage
				.get(qk);
		if (qce == null) {
	...
	public void receive(RouteInterface.Zrp.MessageBrp msg, NetAddress from) {
		...
		if (logBRPFlood.isDebugEnabled()) {
			logBRPFlood.debug("receive t=" + JistAPI.getTime() + " at="
					+ zrp.getLocalAddr() + " from=" + from + " msg=" + msgImpl);
		}
		RouteZrpBrp.QueryCoverageEntry qce = getQueryCoverageEntry(
				msgImpl.getSrc(), msgImpl.getID());
		if (!qce.isProcessed()) {
			// pass up to ierp
	...
	public void send(RouteInterface.Zrp.MessageIerp msg) {
		if (logBRPFlood.isDebugEnabled()) {
			logBRPFlood.debug("request t=" + JistAPI.getTimeString() + " from="
		...
		if (!qce.isForwarded()) {
			...
			if (logBRPFlood.isInfoEnabled()) {
				logBRPFlood.info("send t=" + JistAPI.getTimeString() + " from="
	...
	public void timeout() {
		...
		while (it.hasNext()) {
			RouteZrpBrp.QueryCoverageEntry qce = (RouteZrpBrp.QueryCoverageEntry) it
					.next();
