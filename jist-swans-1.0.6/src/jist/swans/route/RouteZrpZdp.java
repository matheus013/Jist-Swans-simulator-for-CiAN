...
public class RouteZrpZdp implements RouteInterface.Zrp.Iarp {
	...
	private static class LinkEntry {
		 ...
		 * 
		 * @param value
		 *            whether to set or clear freshness flag
		 */
		 ...
		 * 
		 * @param value
		 *            whether to set or clear drop flag
		 */
	...
	public static class MessageZdp implements RouteInterface.Zrp.MessageIarp {
		 ...
		 * 
		 * @param capacity
		 *            initial packet link capacity
		 * @param compress
		 *            packet compression mode/format
		 */
		 ...
		 *            source-destination pair
		 * @param id
		 *            destination sequenced link identifier
		 * @param drop
		 *            whether link is up or down
		 */
		...
		/**
		 * Compute size of link state compressed at the zone level, with
		 * reversal bit.
		 * 
		 * <pre>
		 * - number of unique addr     k byte(s)
		 ...
		 * </pre>
		 * 
		 * @return size of link state compressed at the zone level with link
		 *         reversal
		 */
	 ...
	 * 
	 * @param zrp
	 *            object reference to zrp routing framework
	 */
	public RouteZrpZdp(RouteZrp zrp) {
		if (JistAPI.isEntity(zrp))
	 ...
	 *            link destination address
	 * @param drop
	 *            whether to add or remove link
	 */
	private void updateLinkSrcDst(NetAddress src, NetAddress dst, boolean drop) {
		if (drop)
			removeLinkSrcDst(src, dst);
		else
			addLinkSrcDst(src, dst);
	}
	...
	private Collection getLinkDsts(NetAddress src) {
		return linksSrcDst.containsKey(src) ? (Collection) linksSrcDst.get(src)
	 ...
	 *            source-destination pair
	 * @param id
	 *            destination sequenced identifier
	 * @param drop
	 *            whether link failed
	 ...
	 *            link source-destination pair
	 * @param it
	 *            active iterator of link entries, if any
	 */
	...
	public void showLinks() {
		System.out.println("Links for " + zrp.getLocalAddr() + " n="
				+ getLinks().size() + " t=" + JistAPI.getTimeString());
		...
		while (srcs.hasNext()) {
			...
			while (dsts.hasNext()) {
				...
				System.out.print(dst + "[" + (le.isFresh() ? "n" : " ")
						+ (le.isDrop() ? "d" : " ") + "]");
				i++;
			}
			System.out.println();
	...
	private boolean routesComputed() {
		...
		if (computedRoutes != null)
			routes = (Hashtable) computedRoutes.get();
		return routes != null;
	}
	...
	/**
	 * (Re-)Compute a minimum hop route for each node in zone using current link
	 * state information.
	 * 
	 * @param src
	...
	private Hashtable computeRoutes(NetAddress src) {
		...
		while (radius > 0) {
			...
			while (horizon.size() > 0) {
				...
				Iterator expandLinks = ((Collection) getLinkDsts(src))
						.iterator();
				NetAddress[] expandRoute = ((RouteZrpIarp.RouteEntry) routes
						.get(src)).route;
				while (expandLinks.hasNext()) {
					NetAddress expandDst = (NetAddress) expandLinks.next();
					RouteZrpIarp.RouteEntry dstRoute = (RouteZrpIarp.RouteEntry) routes
							.get(expandDst);
					if (dstRoute != null)
						continue; // route exists (and it must equal or shorter,
									// by construction)
					// add to routing table
					NetAddress[] route = (NetAddress[]) Util.append(
							expandRoute, expandDst);
	...
	public void showRoutes() {
		System.out.println("Routes for " + zrp.getLocalAddr() + " n="
				+ getRoutes().size() + " t=" + JistAPI.getTimeString());
		int i = 0;
		boolean shown = true;
		while (shown) {
			shown = false;
			...
			while (it.hasNext()) {
				...
				RouteZrpIarp.RouteEntry re = (RouteZrpIarp.RouteEntry) getRoutes()
						.get(dst);
				if (re.route.length != i)
					continue;
				System.out.println("  " + dst + ":"
						+ Util.stringJoin(re.route, "->"));
				shown = true;
			}
			i++;
		}
	...
	private Map computeDistancesToSource() {
		...
		while (horizon.size() > 0) {
			...
			while (horizon.size() > 0) {
				src = (NetAddress) horizon.remove(horizon.size() - 1);
				Integer dist = SingletonInt.getSmallInteger(((Integer) dstToSrc
						.get(src)).intValue() + 1);
				...
				while (links.hasNext()) {
					...
					if (le.isDrop())
						continue;
					if (!le.link.dst.equals(src))
						continue;
					if (dstToSrc.containsKey(le.link.src))
						continue;
	...
	private void linkinfo(Link link, short id, boolean drop) {
		...
		if (changed) {
			if (logZDP.isDebugEnabled()) {
				logZDP.debug("linkinfo t=" + JistAPI.getTimeString() + " at="
						+ zrp.getLocalAddr() + " link=" + link + " id=" + id
						+ " drop=" + drop);
			}
			if (!sendScheduled) {
				sendScheduled = true;
	...
	private void send() {
		...
		if (numLinksToSend > 0) {
			...
			if (logZDP.isInfoEnabled()) {
				logZDP.info("send from=" + zrp.getLocalAddr() + " t="
						+ JistAPI.getTimeString() + " msg=" + msg);
			}
			zrp.broadcast(msg);
		}
		// clear send flag
	...
	public void receive(RouteInterface.Zrp.MessageIarp msg, NetAddress from) {
		...
		if (logZDP.isDebugEnabled()) {
			logZDP.debug("receive t=" + JistAPI.getTimeString() + " at="
					+ zrp.getLocalAddr() + " msg=" + msgImpl);
		}
		// process message
		for (int i = 0; i < msgImpl.getNumLinks(); i++) {
			linkinfo(msgImpl.getLink(i), msgImpl.getLinkId(i),
					msgImpl.getLinkDrop(i));
		}
		Link msgLink = new Link(from, zrp.getLocalAddr());
		if (!isLinkUp(msgLink))
			linkinfo(msgLink, false);
	...
	public NetAddress[] getRoute(NetAddress dst) {
		// look up route
		RouteZrpIarp.RouteEntry re = (RouteZrpIarp.RouteEntry) getRoutes().get(
				dst);
		if (re == null)
			return null;
		return re.route;
	...
	public Collection getPeripheral() {
		...
		while (it.hasNext()) {
			RouteZrpIarp.RouteEntry re = (RouteZrpIarp.RouteEntry) it.next();
	...
	public Set computeCoverage(NetAddress src, int depth) {
		...
		while (depth > 0) {
			...
			while (horizon.size() > 0) {
				...
				// find its links
				Iterator expandLinks = ((Collection) getLinkDsts(expand))
						.iterator();
				// expand cover
				while (expandLinks.hasNext()) {
					NetAddress expandDst = (NetAddress) expandLinks.next();
					if (cover.contains(expandDst))
						continue;
					cover.add(expandDst);
