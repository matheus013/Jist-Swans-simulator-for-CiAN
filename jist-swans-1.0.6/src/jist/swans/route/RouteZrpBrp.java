...
// Timestamp: <RouteZrpBrp.java Tue 2004/04/06 11:35:34 barr
...
public class RouteZrpBrp implements RouteInterface.Zrp.Brp, Timer
{
    ...
    private NetAddress[] bordercastNeighbours2(Vector from) {
        ...
        while (remainingUncovered.size() > 0) {
            ...
            while (it.hasNext()) {
                NetAddress peripheral = (NetAddress) it.next();
                if (((Set) closestNeighbours.get(peripheral)).contains(neighbour))
                    it.remove();
            }
        }
    ...
    public void receive(RouteInterface.Zrp.MessageBrp msg, NetAddress from) {
        ...
        if (logBRP.isDebugEnabled()) {
            logBRP.debug("receive t=" + JistAPI.getTime() + " at=" + zrp.getLocalAddr() + " from=" + from + " msg="
                    + msgImpl);
        }
        ...
        if (!qce.isProcessed()) {
            ...
            zrp.getProxy().receive(msgImpl.getPayload(), from, null, (byte) Constants.NET_INTERFACE_INVALID, null,
