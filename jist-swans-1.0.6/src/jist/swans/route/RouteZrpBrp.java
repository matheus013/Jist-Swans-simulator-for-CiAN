...
/**
 * Zone Routing Protocol: Bordercast Resolution (sub)Protocol: Default
 * implementation.
 * 
...
public class RouteZrpBrp implements RouteInterface.Zrp.Brp, Timer {
	...
	public static final Logger logBRP = Logger.getLogger(RouteZrpBrp.class
			.getName());
	...
	public static class QueryKey {
		...
		public boolean equals(Object o) {
			...
			if (this.src == null && qk.src != null)
				return false;
			if (!this.src.equals(qk.src))
				return false;
			if (this.id != qk.id)
				return false;
			return true;
	...
	public static class QueryCoverageEntry {
		 ...
		 * 
		 * @param target
		 *            whether node was target of query broadcast
		 */
	...
	private static class MessageBrp implements RouteInterface.Zrp.MessageBrp {
		 ...
		 * 
		 * @param query
		 *            encapsulated query packet
		 * @param targets
		 *            bordercast next hop targets
		 */
		public MessageBrp(RouteInterface.Zrp.MessageIerp query,
				NetAddress[] targets) {
			this.encapsulated = query;
			if (Main.ASSERT)
				Util.assertion(targets.length > 0);
		...
		public String toString() {
			return "brp(targets=[" + Util.stringJoin(targets, ",") + "], data="
					+ encapsulated;
		}
	} // class: MessageBRP
	 ...
	 * @param from
	 *            source of bordercast
	 * @return list of uncovered peripheral nodes; null indicates all peripheral
	 *         nodes are covered
	 */
	private Collection getUncoveredPeripheral(Vector from) {
		...
		for (int i = 0; i < from.size(); i++) {
			Collection covered = zrp.getIarp().computeCoverage(
					(NetAddress) from.elementAt(i), zrp.getRadius());
	...
	private NetAddress[] bordercastNeighbours(Vector from) {
		...
		return (NetAddress[]) (new Vector(neighbours))
				.toArray(NetAddress.EMPTY_ARRAY);
	}

	/**
	 * Compute first hop in bordercast tree using greedy neighbour minimisation
	 * heuristic.
	 * 
	 * @param from
	 *            sources that have already processed this query
	...
	private NetAddress[] bordercastNeighbours2(Vector from) {
		...
		while (remainingUncovered.size() > 0) {
			// compute counts (and max) of closest neighbours of remaining
			// peripherals
			...
			while (it.hasNext()) {
				...
				Iterator it2 = ((Set) closestNeighbours.get(peripheral))
						.iterator();
				while (it2.hasNext()) {
			...
			while (it.hasNext()) {
				NetAddress peripheral = (NetAddress) it.next();
				if (((Set) closestNeighbours.get(peripheral))
						.contains(neighbour))
					it.remove();
			}
		}
	 ...
	 */
	private QueryCoverageEntry getQueryCoverageEntry(NetAddress src, short seq,
			boolean target) {
	...
	public void receive(RouteInterface.Zrp.MessageBrp msg, NetAddress from) {
		...
		if (logBRP.isDebugEnabled()) {
			logBRP.debug("receive t=" + JistAPI.getTime() + " at="
					+ zrp.getLocalAddr() + " from=" + from + " msg=" + msgImpl);
		}
		QueryCoverageEntry qce = getQueryCoverageEntry(msgImpl.getSrc(),
				msgImpl.getID(), false);
		qce.addFrom(from);
		...
		if (!qce.isProcessed()) {
			...
			zrp.getProxy().receive(msgImpl.getPayload(), from, null,
					(byte) Constants.NET_INTERFACE_INVALID, null,
	...
	public void send(RouteInterface.Zrp.MessageIerp msg) {
		if (logBRP.isDebugEnabled()) {
			logBRP.debug("request t=" + JistAPI.getTimeString() + " from="
					+ zrp.getLocalAddr() + " msg=" + msg);
		}
		QueryCoverageEntry qce = getQueryCoverageEntry(msg.getSrc(),
				msg.getID(), true);
		if (!qce.isForwarded() && qce.isTarget()) {
			...
			if (neighbours.length > 0) {
				...
				if (logBRP.isInfoEnabled()) {
					logBRP.info("send t=" + JistAPI.getTimeString() + " from="
							+ zrp.getLocalAddr() + " msg=" + brp);
				}
				zrp.broadcast(brp);
			}
	...
	public void timeout() {
		...
		while (it.hasNext()) {
			...
			if (qce.time + COVERAGE_LIFETIME < currentTime)
				it.remove();
		}
