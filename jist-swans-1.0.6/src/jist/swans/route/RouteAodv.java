...
public class RouteAodv implements RouteInterface.Aodv {
    ...
    private static class RreqBuffer {
        ...
        public void clearExpiredEntries() {
            ...
            // order
            while (!list.isEmpty()
                    && JistAPI.getTime() > ((RreqBufferEntry) list.getLast()).getTimeSent() + RREQ_BUFFER_EXPIRE_TIME) {
    ...
    public void peek(NetMessage msg, byte interfaceId, MacAddress lastHop) {
        ...
        //BUGFIX: Christin added a routing table update for unknown 1-hop neighbours that route/send message
//        if(msg.getClass().getName().equals("jist.swans.net.NetMessage$Ip")){
//            String msgClass = ((Ip)msg).getPayload().getClass().getName();
//            if(!msgClass.equals("jist.swans.route.RouteAodv$RouteRequestMessage")&& !msgClass.equals("jist.swans.route.RouteAodv$RouteReplyMessage")){
//                NetAddress lastHopIp = new NetAddress(new byte[] { 0, 0, 0, (byte) lastHop.hashCode()});
//                
...
//        }
    ...
    public void send(NetMessage msg) {
        ...
        printlnDebug("src=" + ipMsg.getSrc() + " dst=" + ipMsg.getDst() + " prot=" + ipMsg.getProtocol() + " ttl="
    ...
    private void receiveRouteRequestMessage(RouteRequestMessage rreqMsg, NetAddress src, MacAddress lastHop,
        ...
        if (updateRoute) {
            ...
            routeTable.add(rreqMsg.getOrigIp(), new RouteTableEntry(lastHop, rreqMsg.getOrigSeqNum(), rreqMsg
                    .getHopCount() + 1));
            routeTable.printTable();
    ...
    private void receiveRouteReplyMessage(RouteReplyMessage rrepMsg, NetAddress src, MacAddress lastHop,
        ...
        if (entry == null || (rrepMsg.getDestSeqNum() > entry.getDestSeqNum())
            ...
            routeTable.add(rrepMsg.getDestIp(), new RouteTableEntry(lastHop, rrepMsg.getDestSeqNum(), rrepMsg
                    .getHopCount() + 1));
            routeTable.printTable();
            outgoingSet.add(lastHop);
            precursorSet.add(lastHop);
    ...
    private void generateRouteReplyMessage(RouteRequestMessage rreqMsg, boolean isDest, RouteTableEntry destRouteEntry) {
        ...
        NetMessage.Ip rrepMsgIp = new NetMessage.Ip(rrepMsg, this.netAddr, NetAddress.ANY, // <--
                                                                                           // is
                                                                                           // this
                                                                                           // ok?
