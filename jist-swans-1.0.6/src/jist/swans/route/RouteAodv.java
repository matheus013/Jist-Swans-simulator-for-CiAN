...
public class RouteAodv implements RouteInterface.Aodv {
	...
	private static class RouteRequestMessage implements Message {
		 ...
		 *            hop count
		 */
		public RouteRequestMessage(int rreqId, NetAddress destIp,
				NetAddress origIp, int destSeqNum, int origSeqNum,
		...
		public RouteRequestMessage(RouteRequestMessage rreq) {
			this(rreq.getRreqId(), rreq.getDestIp(), rreq.getOrigIp(), rreq
					.getDestSeqNum(), rreq.getOrigSeqNum(), rreq
		...
		public void getBytes(byte[] msg, int offset) {
			...
			throw new RuntimeException(
					"RouteRequestMessage.getBytes() not implememented.");
		}
	...
	private static class RouteReplyMessage implements Message {
		 ...
		 */
		public RouteReplyMessage(NetAddress destIp, int destSeqNum,
				NetAddress origIp, int hopCount) {
			this.destIp = destIp;
			this.destSeqNum = destSeqNum;
			this.origIp = origIp;
			this.hopCount = hopCount;
		...
		public void getBytes(byte[] msg, int offset) {
			throw new RuntimeException(
					"RouteReplyMessage.getBytes() not implemented.");
		}
	}
	...
	private static class RouteRequest {
		...
		public void broadcast() {
			...
			// receives it again)
			thisNode.rreqBuffer.addEntry(new RreqBufferEntry(rreqId,
					thisNode.netAddr));
			...
			RouteTableEntry routeEntry = (RouteTableEntry) thisNode.routeTable
					.lookup(this.destIp);
			if (routeEntry == null) {
			...
			RouteTableEntry selfEntry = thisNode.routeTable
					.lookup(thisNode.netAddr);
			if (selfEntry == null) {
				thisNode.routeTable.add(thisNode.netAddr, new RouteTableEntry(
						MacAddress.NULL, thisNode.seqNum, 0));
			} else {
			...
			// create RREQ message
			RouteRequestMessage rreqMsg = new RouteRequestMessage(rreqId,
					destIp, thisNode.netAddr, destSeqNum, thisNode.seqNum,
					unknownDestSeqNum, 0);

			// create IP message containing the RREQ message
			NetMessage.Ip rreqMsgIp = new NetMessage.Ip(rreqMsg,
					thisNode.netAddr, NetAddress.ANY,
					Constants.NET_PROTOCOL_AODV, Constants.NET_PRIORITY_NORMAL,
					this.ttl);

			// broadcast the IP message
			printlnDebug("Broadcasting RREQ message with rreqId=" + rreqId
	...
	private static class RreqBuffer {
		...
		public void addEntry(RreqBufferEntry entry) {
			...
			if (list.size() == MAX_RREQ_BUFFER_SIZE) {
				...
				throw new RuntimeException(
						"RREQ Buffer is larger than allowed size!");
			}
		...
		public void clearExpiredEntries() {
			...
			// order
			while (!list.isEmpty()
					&& JistAPI.getTime() > ((RreqBufferEntry) list.getLast())
							.getTimeSent() + RREQ_BUFFER_EXPIRE_TIME) {
	...
	private static class RreqBufferEntry {
		...
		public boolean equals(Object o) {
			...
			return (this.rreqId == entry.rreqId && this.originIp
					.equals(entry.originIp));
		}
	...
	private static class RouteTable {
		...
		private boolean remove(NetAddress key) {
			...
			if (obj == null)
				return false;
			printlnDebug("Removing destination " + key + " from routing table",
					localAddr);
			return true;
		}
		...
		public void removeNextHop(MacAddress nextHop) {
			printlnDebug("Removing all route table entries through " + nextHop,
					localAddr);
		...
		public void printTable() {
			...
			while (itr.hasNext()) {
				...
				if (route != null) {
					printlnDebug_plain("nextHop=" + route.nextHop + " DSN="
							+ route.destSeqNum + " hopCnt=" + route.hopCount
							+ ")]");
				} else {
	...
	private static class PrecursorSet {
		...
		public void remove(MacAddress m) {
			printlnDebug("Removing " + m + " from precursor set",
					thisNode.netAddr);
			map.remove(m);
		}
		...
		public void sendRERR(LinkedList nodes, byte ttl) {
			...
			NetMessage.Ip ipMsg = new NetMessage.Ip(rerrMsg, thisNode.netAddr,
					NetAddress.ANY, Constants.NET_PROTOCOL_AODV,
					Constants.NET_PRIORITY_NORMAL, ttl);
			// send the IP message to each precursor
			Iterator itr = map.keySet().iterator();
			while (itr.hasNext()) {
				MacAddress macAddr = (MacAddress) itr.next();
				printlnDebug("Sending RERR to precursor " + macAddr,
						thisNode.netAddr);
	...
	public RouteAodv(NetAddress addr) {
		...
		// proxy entity
		this.self = (RouteInterface.Aodv) JistAPI.proxy(this,
				RouteInterface.Aodv.class);
		...
		// route to self
		routeTable.add(this.netAddr, new RouteTableEntry(MacAddress.NULL,
				this.seqNum, 0));
	...
	private void helloSendEvent() {
		...
		while (itr.hasNext()) {
			...
			if (JistAPI.getTime() >= precInfo.getLastMsgTime() + HELLO_INTERVAL) {
				...
				if (helloMsgIp == null) {
					HelloMessage helloMsg = new HelloMessage(this.netAddr,
							this.seqNum);
					helloMsgIp = new NetMessage.Ip(helloMsg, this.netAddr,
							NetAddress.ANY, Constants.NET_PROTOCOL_AODV,
	...
	private void helloWaitEvent() {
		...
		while (itr.hasNext()) {
			...
			if (outInfo.getHelloWaitCount() > HELLO_ALLOWED_LOSS) {
				...
				precursorSet.sendRERR(routeTable.destsViaHop(macAddr),
	...
	public void sendIpMsg(NetMessage.Ip ipMsg, MacAddress destMacAddr) {
		...
		if (destMacAddr.equals(MacAddress.ANY)) {
			...
			while (itr.hasNext()) {
				Map.Entry mapEntry = (Map.Entry) itr.next();
				PrecursorInfo precInfo = (PrecursorInfo) mapEntry.getValue();
				printlnDebug("Renewing precusor entry for "
						+ (MacAddress) mapEntry.getKey());
				precInfo.renew();
			}
		} else {
	...
	public void peek(NetMessage msg, byte interfaceId, MacAddress lastHop) {
		...
		if (lastHopInfo != null) {
			printlnDebug("Peeking at message from " + lastHop
					+ "; resetting hello_wait_count");
			lastHopInfo.resetHelloWaitCount();
		}

		// TODO: Christin why does this have no effect?
		// BUGFIX: Christin added a routing table update for unknown 1-hop
		// neighbours that route/send message
		// if(msg.getClass().getName().equals("jist.swans.net.NetMessage$Ip")){
		// String msgClass = ((Ip)msg).getPayload().getClass().getName();
		// if(!msgClass.equals("jist.swans.route.RouteAodv$RouteRequestMessage")&&
		// !msgClass.equals("jist.swans.route.RouteAodv$RouteReplyMessage")){
		// NetAddress lastHopIp = new NetAddress(new byte[] { 0, 0, 0, (byte)
		// lastHop.hashCode()});
		//
		// RouteTableEntry entry = routeTable.lookup(lastHopIp);
		// if (entry == null || (0 > entry.getDestSeqNum())
		// || (0 == entry.getDestSeqNum() && 1 < entry.getHopCount())) {
		// routeTable.add(lastHopIp, new RouteTableEntry(lastHop, 1, 1));
		// routeTable.printTable();
		...
		/*
		 * printlnDebug("Calling peek()"); printlnDebug("peek: lastHop = " +
		 * lastHop.hashCode()); NetMessage.Ip ipMsg = null; if (msg instanceof
	...
	public void send(NetMessage msg) {
		...
		if (ipMsg.getDst().equals(netAddr)) {
			throw new RuntimeException(
					"Message is already at destination.  Why is RouteAodv.send() being called?");
		}

		printlnDebug("Attempting to route from " + netAddr + " to "
				+ ipMsg.getDst());

		printlnDebug("src=" + ipMsg.getSrc() + " dst=" + ipMsg.getDst()
				+ " prot=" + ipMsg.getProtocol() + " ttl=" + ipMsg.getTTL()
				+ " getMsg=" + ipMsg.getPayload());
		...
		MacAddress nextHopMacAddr = routeEntry == null ? null : routeEntry
				.getNextHop();

		// If next hop address found in routing table, forward message
		if (nextHopMacAddr != null) {
			printlnDebug("Attempting to route from " + netAddr + " to "
					+ ipMsg.getDst() + " via " + nextHopMacAddr);
			printlnDebug("sent ipMsg:" + " src=" + ipMsg.getSrc() + " dst="
					+ ipMsg.getDst() + " TTL=" + ipMsg.getTTL());
	...
	public void receive(Message msg, NetAddress src, MacAddress lastHop,
		...
		printlnDebug("receive: src=" + src + " lastHop=" + lastHop + " dst="
				+ dst + " ttl=" + ttl);
		if (msg instanceof HelloMessage) {
			...
			receiveRouteRequestMessage((RouteRequestMessage) msg, src, lastHop,
					dst, priority, ttl);
			if (stats != null) {
				stats.recv.aodvPackets++;
			...
			receiveRouteReplyMessage((RouteReplyMessage) msg, src, lastHop,
					dst, priority, ttl);
			if (stats != null) {
				stats.recv.aodvPackets++;
			...
			throw new RuntimeException(
					"RouteAodv.receive() does not know how to handle message of type"
							+ msg);
		}
	}
	 ...
	 *            message TTL
	 */
	private void receiveRouteRequestMessage(RouteRequestMessage rreqMsg,
			NetAddress src, MacAddress lastHop, NetAddress dst, byte priority,
			byte ttl) {
		if (DEBUG_MODE) {
			printlnDebug("Receiving RREQ:" + " rreqId=" + rreqMsg.getRreqId()
					+ " destIp=" + rreqMsg.getDestIp() + " origIp="
					+ rreqMsg.getOrigIp() + " destSN="
					+ rreqMsg.getDestSeqNum() + " origSN="
					+ rreqMsg.getOrigSeqNum() + " unkDSN="
					+ rreqMsg.getUnknownDestSeqNum() + " hopCnt="
					+ rreqMsg.getHopCount());
		}
		...
		if (updateRoute) {
			...
			routeTable.add(rreqMsg.getOrigIp(), new RouteTableEntry(lastHop,
					rreqMsg.getOrigSeqNum(), rreqMsg.getHopCount() + 1));
			routeTable.printTable();
		...
		boolean hasFreshRoute = routeToDestExists
				&& !rreqMsg.getUnknownDestSeqNum()
				&& destRouteEntry.getDestSeqNum() > rreqMsg.getDestSeqNum();
		boolean inRreqBuffer = rreqBuffer.contains(new RreqBufferEntry(rreqMsg
				.getRreqId(), rreqMsg.getOrigIp()));
	 ...
	 *            message TTL value
	 */
	private void receiveRouteReplyMessage(RouteReplyMessage rrepMsg,
			NetAddress src, MacAddress lastHop, NetAddress dst, byte priority,
			byte ttl) {
		printlnDebug("handling RREP:" + " destIp=" + rrepMsg.getDestIp()
				+ " destSN=" + rrepMsg.getDestSeqNum() + " origIp="
				+ rrepMsg.getOrigIp() + " hopCnt=" + rrepMsg.getHopCount());
		...
		if (entry == null
				...
				|| (rrepMsg.getDestSeqNum() == entry.getDestSeqNum() && rrepMsg.hopCount < entry
						.getHopCount())) {
			routeTable.add(rrepMsg.getDestIp(), new RouteTableEntry(lastHop,
					rrepMsg.getDestSeqNum(), rrepMsg.getHopCount() + 1));
			routeTable.printTable();
			outgoingSet.add(lastHop);
			precursorSet.add(lastHop);
		}
	 ...
	 */
	private void forwardRouteRequestMessage(RouteRequestMessage rreqMsg,
			byte newTtl, RouteTableEntry destRouteEntry) {
		// save RREQ info in buffer
		rreqBuffer.addEntry(new RreqBufferEntry(rreqMsg.getRreqId(), rreqMsg
				.getOrigIp()));
		...
		if (destRouteEntry != null) {
			boolean condition = newRreqMsg.getUnknownDestSeqNum()
					|| destRouteEntry.getDestSeqNum() > newRreqMsg
							.getDestSeqNum();
			if (condition) {
		...
		NetMessage.Ip rreqMsgIp = new NetMessage.Ip(newRreqMsg, this.netAddr,
				NetAddress.ANY, Constants.NET_PROTOCOL_AODV,
				Constants.NET_PRIORITY_NORMAL, newTtl);
	 ...
	 *            dest. (otherwise, null)
	 */
	private void generateRouteReplyMessage(RouteRequestMessage rreqMsg,
			boolean isDest, RouteTableEntry destRouteEntry) {
		// add rreqMsg to rreq buffer (if not there), so we do not send this
		// same RREP again
		RreqBufferEntry newEntry = new RreqBufferEntry(rreqMsg.rreqId,
				rreqMsg.origIp);
		...
		// node's SN
		if (!rreqMsg.getUnknownDestSeqNum()
				&& rreqMsg.getDestSeqNum() > this.seqNum) {
		...
		RouteReplyMessage rrepMsg = new RouteReplyMessage(rreqMsg.getDestIp(),
				this.seqNum, rreqMsg.getOrigIp(), initialHopCount);

		// get next hop from routing table
		MacAddress nextHop = routeTable.lookup(rreqMsg.getOrigIp())
				.getNextHop();

		// create IP message containing the RREP message
		NetMessage.Ip rrepMsgIp = new NetMessage.Ip(
				rrepMsg,
				this.netAddr,
				NetAddress.ANY, // <--
								// is
	 ...
	 */
	private boolean shouldUpdateRouteToOrigin(RouteRequestMessage rreqMsg,
		...
		boolean equalSeqNum = rreqMsg.getOrigSeqNum() == origRouteEntry
				.getDestSeqNum();
		boolean hopCountBetter = rreqMsg.getHopCount() + 1 < origRouteEntry
				.getHopCount();
	...
	/**
	 * Print given string with JiST time and local net address, if debug mode
	 * on.
	 * 
	 * @param s
	 *            string to print
