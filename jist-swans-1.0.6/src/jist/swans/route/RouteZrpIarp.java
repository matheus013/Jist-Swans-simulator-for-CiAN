...
/**
 * Zone Routing Protocol: IntrAzone Routing (sub)Protocol: Default
 * implementation.
 * 
...
public class RouteZrpIarp implements RouteInterface.Zrp.Iarp, Timer {
	...
	public static final Logger logIARP = Logger.getLogger(RouteZrpIarp.class
			.getName());
	...
	private static class MessageIarp implements RouteInterface.Zrp.MessageIarp {
		 ...
		 */
		public MessageIarp(NetAddress src, short seq, byte ttl,
				NetAddress[] dst, byte compress) {
			this.src = src;
			this.seq = seq;
			this.ttl = ttl;
			this.dst = dst;
		...
		public MessageIarp decTTL() {
			if (ttl > 1) {
				return new MessageIarp(src, seq, (byte) (ttl - 1), dst,
						compress);
			}
			return null;
		}
		...
		public String toString() {
			return "iarp(src=" + src + " id=" + seq + " ttl=" + ttl + " dst=["
					+ Util.stringJoin(dst, ",") + "])";
		}
	} // class: MessageIarp
	...
	private short incLinkStateSeq() {
		linkStateSeq++;
		if (linkStateSeq == Short.MAX_VALUE)
			linkStateSeq = 0;
		return linkStateSeq;
	}
	...
	private void neighboursChanged() {
		...
		if (logIARP.isDebugEnabled()) {
			logIARP.debug("t=" + JistAPI.getTime() + " " + zrp.getLocalAddr()
					+ "==>" + Util.stringJoin(n, ","));
		}
		receive(new MessageIarp(zrp.getLocalAddr(), incLinkStateSeq(),
				(byte) (zrp.getRadius()), n, compress), null);
	}
	 ...
	 * 
	 * @param src
	 *            link state information source
	 * @param seq
	 *            link state source sequence number
	 * @param dst
	...
	private Map computeRoutes() {
		...
		while (horizon.size() > 0) {
			...
			for (int j = 0; j < expandLinks.dst.length; j++) {
				...
				if (dstRoute != null)
					continue; // route exists (and it must equal or shorter, by
								// construction)
				// add to routing table
				routes.put(
						dst,
						new RouteEntry((NetAddress[]) Util.append(expandRoute,
								dst)));
				// add to (new) horizon
				horizon.add(dst);
			}
	...
	public void showLinks() {
		System.out.println("Links for " + zrp.getLocalAddr() + " n="
				+ linkState.size() + " t=" + JistAPI.getTimeString());
		...
		while (it.hasNext()) {
			...
			System.out.println("  " + src + "(" + lse.seq + ")->"
					+ Util.stringJoin(lse.dst, ","));
		}
	}
	...
	public void receive(RouteInterface.Zrp.MessageIarp msg, NetAddress from) {
		...
		if (logIARP.isDebugEnabled()) {
			logIARP.debug("receive t=" + JistAPI.getTime() + " at="
					+ zrp.getLocalAddr() + " msg=" + msgImpl);
		}
		boolean changed = updateLinkState(msgImpl.getSrc(), msgImpl.getSeq(),
				msgImpl.getDst());
		if (changed) {
			clearRoutes();
			...
			if (msgImpl != null) {
				...
				if (logIARP.isInfoEnabled()) {
					logIARP.info("send t=" + JistAPI.getTime() + " at="
							+ zrp.getLocalAddr() + " msg=" + msgImpl);
				}
	...
	public Set computeCoverage(NetAddress src, int depth) {
		...
		while (depth > 0) {
			...
			while (horizon.size() > 0) {
				...
				NetAddress expand = (NetAddress) horizon.remove(0);
				// find its links
				LinkStateEntry expandLinks = (LinkStateEntry) linkState
						.get(expand);
				if (expandLinks == null)
					continue;
				// expand cover
				for (int i = 0; i < expandLinks.dst.length; i++) {
					if (cover.contains(expandLinks.dst[i]))
						continue;
