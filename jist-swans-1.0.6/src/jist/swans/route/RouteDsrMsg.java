...
public class RouteDsrMsg implements Message, Cloneable {
	 ...
	 * 
	 * @param content
	 *            the payload of the DSR message. This can be <code>null</code>.
	 */
	 ...
	 *         equal to IPv6's No Next Header protocol number if the DSR header
	 *         has no content.
	 */
	 ...
	 * 
	 * @param nextHeaderType
	 *            the protocol number of the content of the DSR message.
	 */
	...
	public void getBytes(byte[] buf, int offset) {
		...
		for (int i = 0; i < options.size(); i++) {
			...
			int bytesToCopy = StrictMath
					.min(buf.length - offset, option.length);
	...
	/**
	 * Determines the size in bytes of the options contained in this DSR
	 * message.
	 * 
	 ...
	 *         DSR header or the payload of the message.
	 */
	...
	public abstract static class Option {
		 ...
		 * 
		 * @param buf
		 *            the buffer containing the option
		 * @param offset
		 *            the offset into <code>buf</code> where the option begins
		 */
		 ...
		 * 
		 * @param buf
		 *            the buffer to put the option bytes in
		 * @param offset
		...
		public int getBytes(byte[] buf, int offset) {
			...
			System.arraycopy(optBuf, optBufOffset, buf, offset,
					StrictMath.min(getSize(), numBytesToCopy));
		 ...
		 * 
		 * @param buf
		 *            the DSR option in raw byte form
		 * @param offset
		 *            the offset into <code>buf</code> where the option begins
		 * @return the DSR option encoded in <code>buf</code>, or
	...
	public static class OptionRouteRequest extends Option {
		...
		/** {@inheritDoc} */
		public int getSize() {
			// Defeat sign-extension and add 2 for the Option Type and Opt Data
			// Len fields
		...
		public NetAddress getAddress(int n) {
			...
			return new NetAddress(new byte[] { optBuf[addressOffset],
		 ...
		 * 
		 * @param id
		 *            the identification number of the route request
		 * @param target
		 *            the address of the node being searched for
		 * @param addrs
		 *            the addresses of the nodes that have forwarded this
		 *            request
		 * @return the byte array corresponding to the desired option.
		 */
		public static byte[] create(short id, NetAddress target,
				NetAddress[] addrs) {
			...
			for (int i = 0; i < addrs.length; i++) {
				System.arraycopy(addrs[i].getIP().getAddress(), 0, opt,
						4 * (i + 2), 4);
			}
	...
	public static class OptionRouteReply extends Option {
		 ...
		 * 
		 * @param lastHopExternal
		 *            whether the last hop in the route is external
		 * @param addrs
		 *            the addresses of the nodes in the route
		 * @return the byte array corresponding to the desired option.
		 */
		...
		/**
		 * Creates a new Route Reply option with the Last Hop External field set
		 * to false.
		 * 
		 * @param addrs
		 *            the addresses of the nodes in the route
	...
	public static class OptionRouteError extends Option {
		...
		/**
		 * Returns the size in bytes of any type-specific information contained
		 * in this Route Error.
		 * 
		 ...
		 * 
		 * @param buf
		 *            the byte array to copy the type-specific information into
		 * @param offset
		 *            the offset into <code>buf</code> where copying should
		 *            begin
		 * @return the number of bytes copied into <code>buf</code>.
		 */
		public int getTypeSpecificInfoBytes(byte[] buf, int offset) {
			int numBytesToCopy = StrictMath.min(getTypeSpecificInfoSize(),
					buf.length - offset);
			System.arraycopy(optBuf, optBufOffset + 12, buf, offset,
					StrictMath.min(getSize(), numBytesToCopy));
			return numBytesToCopy;
		}
		 ...
		 * 
		 * @param type
		 *            the code corresponding to the type of error being reported
		 * @param salvage
		 *            the salvage count
		 * @param src
		 *            the source address of the error
		 * @param dest
		 *            the destination address of the error
		 * @param tsi
	...
	public static class OptionAckRequest extends Option {
		 ...
		 * 
		 * @param id
		 *            the identification number of the acknowledgement request
		 * @return the byte array corresponding to the desired option.
		 */
	...
	public static class OptionAck extends Option {
		 ...
		 * 
		 * @param id
		 *            the identification number of the acknowledgement
		 * @param src
		 *            the source address of the acknowledgement
		 * @param dest
	...
	public static class OptionSourceRoute extends Option {
		...
		/**
		 * Returns the number of segments on the source route that have yet to
		 * be traversed.
		 * 
		...
		/**
		 * Returns the number of addresses in the route, not including the
		...
		/**
		 * Returns the nth address on the route, counting the first hop after
		 * the source as node zero.
		 * 
		 * @param n
		 *            the index into the route
		 ...
		 * 
		 * @param firstHopExternal
		 *            whether the first hop on the route is external
		 * @param lastHopExternal
		 *            whether the last hop on the route is external
		 * @param salvage
		 *            the salvage count
		 * @param segsLeft
		 *            the number of explicitly listed intermediate nodes still
		...
		public static byte[] create(boolean firstHopExternal,
				boolean lastHopExternal, int salvage, int segsLeft,
				NetAddress[] addrs) {
			...
			// Set the First Hop External, Last Hop External, and Reserved
			// fields, along
		 ...
		 * @return the byte array corresponding to the desired option.
		 */
		public static byte[] create(int salvage, int segsLeft,
				NetAddress[] addrs) {
	...
	public static class OptionPadN extends Option {
		 ...
		 * 
		 * @param len
		 *            the length of the option in bytes, not including the
