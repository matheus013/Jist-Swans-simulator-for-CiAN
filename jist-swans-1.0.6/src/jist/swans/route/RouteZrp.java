...
public class RouteZrp implements RouteInterface.Zrp {
	...
	public static final Logger logZRP = Logger.getLogger(RouteZrp.class
			.getName());
	 ...
	 * 
	 * @param route
	 *            route in format: (src ... partial route ... dst)
	 * @param finder
	 *            node that knows route to destination
	 * @param remainder
	...
	public static NetAddress[] replaceDest(NetAddress[] route,
			NetAddress finder, NetAddress[] remainder) {
	...
	private void init(NetAddress localAddr, int radius) {
		if (radius < 1 || radius > Byte.MAX_VALUE)
			throw new IllegalArgumentException("invalid radius value");
		this.radius = (byte) radius;
		this.localAddr = localAddr;
		// self
		self = (RouteInterface.Zrp) JistAPI.proxy(this,
				RouteInterface.Zrp.class);
	}
	...
	public void setSubProtocols(RouteInterface.Zrp.Ndp ndp,
			RouteInterface.Zrp.Iarp iarp, RouteInterface.Zrp.Brp brp,
	...
	public void setSubProtocolsDefault() {
		setSubProtocols(new RouteZrpNdp(this), new RouteZrpIarp(this),
				new RouteZrpBrp(this), new RouteZrpIerp(this));
	}
	...
	public void send(NetMessage msg) {
		...
		if (logZRP.isInfoEnabled()) {
			logZRP.info("t=" + JistAPI.getTime() + " at=" + localAddr
					+ " route " + ip);
		}
		NetAddress nextHop = null;
		NetMessage.IpOption ipOpt = null;
		ipOpt = (NetMessage.IpOption) ip.getOptions().get(
				Constants.IP_OPTION_ZRP);
		// compute next hop
		if (iarp.hasRoute(ip.getDst())) {
			...
			for (i = route.length - 1; i >= ipOptSr.getPtr()
					&& !iarp.hasRoute(route[i]); i--) {
			}
			if (i < ipOptSr.getPtr()) {
				// todo: route failure
				return;
			...
			if (i != ipOptSr.getPtr()) {
				...
				ip.getOptions().put(
						Constants.IP_OPTION_ZRP,
						new NetMessage.IpOptionSourceRoute(ipOptSr.getRoute(),
								i));
			}
		}
		...
		if (nextHop != null) {
			// send off packet
			netEntity.send(ip, ndp.getMacId(nextHop),
					ndp.getMacAddress(nextHop));
		} else {
	...
	public void send(NetMessage.Ip msg, NetAddress dst) {
		...
		if (mac != null)
			netEntity.send(msg, ndp.getMacId(dst), mac);
	}
	...
	public void receive(Message msg, NetAddress src, MacAddress lastHop,
		...
		if (logZRP.isDebugEnabled()) {
			logZRP.debug("receive at=" + localAddr + " from=" + src + " msg="
					+ msg);
		}
		if (msg instanceof RouteInterface.Zrp.MessageNdp) {
			if (stats != null) {
			...
			ndp.receive((RouteInterface.Zrp.MessageNdp) msg, src, lastHop,
					macId);
		} else if (msg instanceof RouteInterface.Zrp.MessageIarp) {
			if (stats != null) {
