...
// Timestamp: <Mac802_11.java Thu 2005/02/10 11:45:06 barr
...
public class Mac802_11 implements MacInterface.Mac802_11
{
    ...
    /**
     * Physical specification constant: 802_11b-1999 Supplement 2_4GHz Direct
     * Sequence.
     */
    ...
    static {
        if (DSSS)
            SLOT_TIME = 20 * Constants.MICRO_SECOND;
        if (FHSS)
            SLOT_TIME = 50 * Constants.MICRO_SECOND;
    }
    ...
    static {
        if (DSSS)
            SIFS = 10 * Constants.MICRO_SECOND;
        if (FHSS)
            SIFS = 28 * Constants.MICRO_SECOND;
    }

    /**
     * Point coordination inter frame space. Wait used by the access point
     * (point coordinator) to gain access to the medium before any of the
     * stations.
     */
    public static final long    PIFS                = SIFS + SLOT_TIME;

    /**
     * Distributed inter frame space. Wait used by stations to gain access to
     * the medium.
     */
     ...
     */
    public static final long    EIFS                = SIFS + DIFS + SYNCHRONIZATION + 8 * MacMessage.Ack.SIZE
                                                            * Constants.MICRO_SECOND;
    ...
    /**
     * Sets ducks stats collector
     * 
     * @param stats
     */
    public void setStats(DucksMac80211Stats stats) {
        this.stats = stats;
    }
    ...
    public void send(Message msg, MacAddress nextHop) {
        ...
        if (msg != null) {
            if (msg.getClass().getName().equals("jist.swans.net.NetMessage$Ip")) {
                Ip ipmsg = (Ip) msg;
                String msgClass = ipmsg.getPayload().getClass().getName();
                // if(msgClass.equals("ext.jist.swans.app.DiscoveryRequest")){
                // stats.incrementAppDiscReqSend();
                // }else
                // if(msgClass.equals("ext.jist.swans.app.DiscoveryResponse")){
                // stats.incrementAppDiscRspSend();
                // }else
                // if(msgClass.equals("ext.jist.swans.app.CompositionMessage")){
                // stats.incrementAppCompMsgSend();
                if (msgClass.equals("ext.jist.swans.app.WorkflowRequest")) {
                    stats.incrementAppWfReqSend();
                } else if (msgClass.equals("ext.jist.swans.app.ServiceAd")) {
                    stats.incrementAppServiceAdSend();
                } else if (msgClass.equals("ext.jist.swans.app.Token")) {
                    stats.incrementAppTokenSend();
                } else if (msgClass.equals("jist.swans.route.RouteAodv$RouteRequestMessage")) {
                    stats.incrementAodvRreqSend();
                } else if (msgClass.equals("jist.swans.route.RouteAodv$RouteReplyMessage")) {
                    stats.incrementAodvRrspSend();
                }
            }
        }
    ...
    private void sendRts() {
        // create rts packet
        MacMessage.Rts rts = new MacMessage.Rts(packetNextHop, localAddr, (int) (((MacMessage.Ack.SIZE
                + MacMessage.Cts.SIZE + MacMessage.Data.HEADER_SIZE)
                * Constants.SECOND / bandwidth)
                + 4 * PROPAGATION + 3 * SIFS + 2 * SYNCHRONIZATION + transmitTime(packet)));
        ...
        self.startTimer(MacMessage.Cts.SIZE * Constants.SECOND / bandwidth + (SYNCHRONIZATION + PROPAGATION + SIFS)
                + SLOT_TIME + PROPAGATION + Constants.EPSILON_DELAY, MAC_MODE_SWFCTS);
    }

    private void sendCts(MacMessage.Rts rts) {
        // create cts packet
        MacMessage.Cts cts = new MacMessage.Cts(rts.getSrc(), (int) (rts.getDuration() - MacMessage.Cts.SIZE
                * Constants.SECOND / bandwidth - SYNCHRONIZATION - PROPAGATION - SIFS));
        // set mode and transmit
        ...
        JistAPI.sleep(delay + duration);
        self.startTimer(cts.getDuration() - MacMessage.Ack.SIZE * Constants.SECOND / bandwidth
                - (SYNCHRONIZATION + PROPAGATION + SIFS) + SLOT_TIME + Constants.EPSILON_DELAY, MAC_MODE_SWFDATA);
    }

    private void sendData(boolean afterCts) {
        // //DEBUG: Christin added this for remote debugging and for logging
        // if(packet!=null){
        // if(packet.getClass().getName().equals("jist.swans.net.NetMessage$Ip")){
        // Ip ipmsg = (Ip)packet;
        // String msgClass = ipmsg.getPayload().getClass().getName();
        // if(msgClass.equals("ext.jist.swans.app.DiscoveryRequest")){
        // stats.incrementAppDiscReqSend();
        // }else if(msgClass.equals("ext.jist.swans.app.DiscoveryResponse")){
        // stats.incrementAppDiscRspSend();
        // }else if(msgClass.equals("ext.jist.swans.app.CompositionMessage")){
        // stats.incrementAppCompMsgSend();
        // }else
        // if(msgClass.equals("jist.swans.route.RouteAodv$RouteRequestMessage")){
        // stats.incrementAodvRreqSend();
        // }else
        // if(msgClass.equals("jist.swans.route.RouteAodv$RouteReplyMessage")){
        // stats.incrementAodvRrspSend();
        // }
        // }
        // }
    ...
    private void sendDataUnicast(boolean afterCts) {
        // create data packet
        MacMessage.Data data = new MacMessage.Data(packetNextHop, localAddr, (int) (MacMessage.Ack.SIZE
                * Constants.SECOND / bandwidth + (SYNCHRONIZATION + PROPAGATION + SIFS) + PROPAGATION), incSeq(),
                (byte) 0, false, (shouldRTS() ? longRetry : shortRetry) > 0, packet);
        // set mode and transmit
        ...
        JistAPI.sleep(delay + duration);
        self.startTimer(MacMessage.Ack.SIZE * Constants.SECOND / bandwidth + (SYNCHRONIZATION + PROPAGATION + SIFS)
                + SLOT_TIME + PROPAGATION, MAC_MODE_SWFACK);
    }
    ...
    public void receive(Message msg) {
        // DEBUG: Christin added this for remote debugging and for logging
        // if(msg!=null){
        // if(msg.getClass().getName().equals("jist.swans.mac.MacMessage$Data")){
        // Data datamsg = (Data)msg;
        // if(datamsg.getBody().getClass().getName().equals("jist.swans.net.NetMessage$Ip")){
        // Ip ipmsg = (Ip)datamsg.getBody();
        // if(ipmsg.getPayload().getClass().getName().equals("ext.jist.swans.app.DiscoveryRequest")){
        // System.out.println("MaC: Disc Req ");
        // }
        // }
        // }
        // }
    ...
    private void receiveData(MacMessage.Data msg) {
        // // LOG: Christin number of app messages and rreq, rsp received
        // if(msg!=null){
        // if(msg.getBody().getClass().getName().equals("jist.swans.net.NetMessage$Ip")){
        // Ip ipmsg = (Ip)msg.getBody();
        // //
        // System.out.println("received "+ipmsg.getPayload().getClass().getName());
        // String msgClass = ipmsg.getPayload().getClass().getName();
        // if(msgClass.equals("ext.jist.swans.app.DiscoveryRequest")){
        // stats.incrementAppDiscReqRecv();
        // }else if(msgClass.equals("ext.jist.swans.app.DiscoveryResponse")){
        // stats.incrementAppDiscRspRecv();
        // }else if(msgClass.equals("ext.jist.swans.app.CompositionMessage")){
        // stats.incrementAppCompMsgRecv();
        // }else
        // if(msgClass.equals("jist.swans.route.RouteAodv$RouteRequestMessage")){
        // stats.incrementAodvRreqRecv();
        // }else
        // if(msgClass.equals("jist.swans.route.RouteAodv$RouteReplyMessage")){
        // stats.incrementAodvRrspRecv();
        // }
        // }
        // }
    ...
    public void setRadioMode(byte mode) {
        ...
        switch (mode) {
            ...
            case Constants.RADIO_MODE_RECEIVING:
                radioBusy();
                /*
                 * todo: ExaminePotentialIncomingMessage(newMode,
                 * receiveDuration,
