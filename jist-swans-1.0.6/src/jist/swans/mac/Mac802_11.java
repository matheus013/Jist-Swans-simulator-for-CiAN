...
public class Mac802_11 implements MacInterface.Mac802_11 {
	 ...
	 */
	public static final long EIFS = SIFS + DIFS + SYNCHRONIZATION + 8
			* MacMessage.Ack.SIZE * Constants.MICRO_SECOND;
	...
	public Mac802_11(MacAddress addr, RadioInfo radioInfo) {
		...
		packetNextHop = null;
		// proxy
		self = (MacInterface.Mac802_11) JistAPI.proxy(this,
				MacInterface.Mac802_11.class);
	}
			...
			if (msg.getClass().getName().equals("jist.swans.net.NetMessage$Ip")) {
				...
				if (msgClass.equals("ext.jist.swans.app.WorkflowRequest")) {
					...
					stats.incrementAppTokenSend();
						...
						.equals("jist.swans.route.RouteAodv$RouteRequestMessage")) {
					stats.incrementAodvRreqSend();
				} else if (msgClass
						.equals("jist.swans.route.RouteAodv$RouteReplyMessage")) {
	...
	private void sendRts() {
		// create rts packet
		MacMessage.Rts rts = new MacMessage.Rts(
				packetNextHop,
				localAddr,
				(int) (((MacMessage.Ack.SIZE + MacMessage.Cts.SIZE + MacMessage.Data.HEADER_SIZE)
		...
		self.startTimer(MacMessage.Cts.SIZE * Constants.SECOND / bandwidth
				+ (SYNCHRONIZATION + PROPAGATION + SIFS) + SLOT_TIME
				+ PROPAGATION + Constants.EPSILON_DELAY, MAC_MODE_SWFCTS);
	}

	private void sendCts(MacMessage.Rts rts) {
		// create cts packet
		MacMessage.Cts cts = new MacMessage.Cts(rts.getSrc(),
				(int) (rts.getDuration() - MacMessage.Cts.SIZE
						* Constants.SECOND / bandwidth - SYNCHRONIZATION
						- PROPAGATION - SIFS));
		// set mode and transmit
		...
		JistAPI.sleep(delay + duration);
		self.startTimer(cts.getDuration() - MacMessage.Ack.SIZE
				* Constants.SECOND / bandwidth
				- (SYNCHRONIZATION + PROPAGATION + SIFS) + SLOT_TIME
	...
	private void sendDataBroadcast() {
		// create data packet
		MacMessage.Data data = new MacMessage.Data(packetNextHop, localAddr, 0,
				packet);
		// set mode and transmit
	...
	private void sendDataUnicast(boolean afterCts) {
		...
		MacMessage.Data data = new MacMessage.Data(
				packetNextHop,
				localAddr,
				(int) (MacMessage.Ack.SIZE * Constants.SECOND / bandwidth
						+ (SYNCHRONIZATION + PROPAGATION + SIFS) + PROPAGATION),
				incSeq(), (byte) 0, false, (shouldRTS() ? longRetry
						: shortRetry) > 0, packet);
		// set mode and transmit
	...
	private void receiveData(MacMessage.Data msg) {
		...
		if (mode == MAC_MODE_SWFDATA || !isAwaitingResponse()) {
			if (MacAddress.ANY.equals(msg.getDst())) {
				...
				if (!(msg.getRetry() && getSeqEntry(msg.getSrc()) == msg
						.getSeq())) {
					updateSeqEntry(msg.getSrc(), msg.getSeq());
					netEntity.receive(msg.getBody(), msg.getSrc(), netId,
							false, false);
				}
			}
		}
	...
	private void receiveForeign(MacMessage msg) {
		...
		if (promisc && msg.getType() == MacMessage.TYPE_DATA) {
			...
			netEntity.receive(macDataMsg.getBody(), macDataMsg.getSrc(), netId,
					true, false);
		}
	}
