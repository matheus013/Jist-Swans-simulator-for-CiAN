...
public class MacDumb implements MacInterface
{
    ...
    /**
     * Create a new "dumb" mac entity. Does not perform any collision avoidance
     * or detection. Simply does not transmit (drops) packet if it is current
     * receiving.
     * 
     * @param addr
     *            local mac address
    ...
    public void receive(Message msg) {
        ...
        if (MacAddress.ANY.equals(mdm.getDst())) {
            if (netEntity != null)
                netEntity.receive(mdm.getPayload(), mdm.getSrc(), netId, false, true);
        } else if (localAddr.equals(mdm.getDst())) {
            if (netEntity != null)
                netEntity.receive(mdm.getPayload(), mdm.getSrc(), netId, false, false);
        } else if (promisc) {
            if (netEntity != null)
                netEntity.receive(mdm.getPayload(), mdm.getSrc(), netId, true, false);
        }
    }
