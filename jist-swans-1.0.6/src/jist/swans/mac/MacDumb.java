...
public class MacDumb implements MacInterface {
	...
	public void receive(Message msg) {
		...
		if (MacAddress.ANY.equals(mdm.getDst())) {
			if (netEntity != null)
				netEntity.receive(mdm.getPayload(), mdm.getSrc(), netId, false,
						true);
			...
			if (netEntity != null)
				netEntity.receive(mdm.getPayload(), mdm.getSrc(), netId, true,
						false);
		}
	}
	...
	public void send(Message msg, MacAddress nextHop) {
		...
		if (radioMode == Constants.RADIO_MODE_IDLE) {
			MacDumbMessage mdm = new MacDumbMessage(this.localAddr, nextHop,
					msg);
