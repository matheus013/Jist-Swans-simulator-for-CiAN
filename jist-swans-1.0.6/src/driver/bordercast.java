...
public class bordercast {
	...
	private static void showUsage() {
		...
		System.out
				.println("  -e, --endat          simulation ending time: [infinite]");
		System.out
				.println("  -p, --protocol       routing protocol: [zrp:radius]");
		...
		System.out
				.println("  -b, --bordercasts    number of transmissions: num,start,delay [0,60,10]");
		System.out.println("  -r, --randomseed     random seed: [0]");
		System.out.println("  -w, --wrap           wrap-around field");
		System.out
				.println("  -s, --spatial        spatial binning: linear, grid:NxN, hier:N");
		System.out.println();
		System.out.println("e.g.");
		System.out
				.println("  swans driver.bordercast -p zrp:2 --iarp=iarp:inf -e 300 -n 10 -f 200x200 -a grid:5x5 -b 5,100,20");
		System.out.println();
	}
	...
	private static CommandLineOptions parseCommandLineOptions(String[] args)
		...
		CmdLineParser.Option opt_protocol = parser.addStringOption('p',
				"protocol");
		...
		if (parser.getOptionValue(opt_endat) != null) {
			cmdOpts.endTime = ((Integer) parser.getOptionValue(opt_endat))
					.intValue();
		}
		// protocol
		if (parser.getOptionValue(opt_protocol) != null) {
			String routeProtocolString = ((String) parser
					.getOptionValue(opt_protocol)).split(":")[0];
			if (routeProtocolString != null) {
				if (routeProtocolString.equalsIgnoreCase("zrp")) {
					...
					throw new CmdLineParser.IllegalOptionValueException(
							opt_protocol, "Unrecognized routing protocol");
				}
			}
			cmdOpts.protocolOpts = Util.stringJoin((String[]) Util
					.rest(((String) parser.getOptionValue(opt_protocol))
							.split(":")), ":");
		}
		// ndp
		if (parser.getOptionValue(opt_ndp) != null) {
			String protString = ((String) parser.getOptionValue(opt_ndp))
					.split(":")[0];
			if (protString != null) {
				if (protString.equalsIgnoreCase("ndp")) {
					...
					throw new CmdLineParser.IllegalOptionValueException(
							opt_ndp, "Unrecognized routing protocol");
				}
			}
			cmdOpts.ndpOpts = Util.stringJoin(
					(String[]) Util.rest(((String) parser
							.getOptionValue(opt_ndp)).split(":")), ":");
		}
		// iarp
		if (parser.getOptionValue(opt_iarp) != null) {
			String protString = ((String) parser.getOptionValue(opt_iarp))
					.split(":")[0];
			if (protString != null) {
				if (protString.equalsIgnoreCase("iarp")) {
					...
					throw new CmdLineParser.IllegalOptionValueException(
							opt_iarp, "Unrecognized routing protocol");
				}
			}
			cmdOpts.iarpOpts = Util.stringJoin(
					(String[]) Util.rest(((String) parser
							.getOptionValue(opt_iarp)).split(":")), ":");
		}
		// brp
		if (parser.getOptionValue(opt_brp) != null) {
			String protString = ((String) parser.getOptionValue(opt_brp))
					.split(":")[0];
			if (protString != null) {
				if (protString.equalsIgnoreCase("brp")) {
					...
					throw new CmdLineParser.IllegalOptionValueException(
							opt_brp, "Unrecognized routing protocol");
				}
			}
			cmdOpts.brpOpts = Util.stringJoin(
					(String[]) Util.rest(((String) parser
							.getOptionValue(opt_brp)).split(":")), ":");
		}
		// ierp
		if (parser.getOptionValue(opt_ierp) != null) {
			String protString = ((String) parser.getOptionValue(opt_ierp))
					.split(":")[0];
			if (protString != null) {
				if (protString.equalsIgnoreCase("ierp")) {
					...
					throw new CmdLineParser.IllegalOptionValueException(
							opt_ierp, "Unrecognized routing protocol");
				}
			}
			cmdOpts.ierpOpts = Util.stringJoin(
					(String[]) Util.rest(((String) parser
							.getOptionValue(opt_ierp)).split(":")), ":");
		}
		// nodes
		...
		if (parser.getOptionValue(opt_bordercast) != null) {
			String[] data = ((String) parser.getOptionValue(opt_bordercast))
					.split(",");
			if (data.length != 3)
				throw new CmdLineParser.IllegalOptionValueException(
						opt_bordercast, "bad format: num,locs,start,delay");
		...
		if (parser.getOptionValue(opt_spatial) != null) {
			String spatialString = ((String) parser.getOptionValue(opt_spatial))
					.split(":")[0];
			String spatialOpts = Util.stringJoin((String[]) Util
					.rest(((String) parser.getOptionValue(opt_spatial))
							.split(":")), ":");
			if (spatialString != null) {
				if (spatialString.equalsIgnoreCase("linear")) {
					...
					throw new CmdLineParser.IllegalOptionValueException(
							opt_spatial, "unrecognized spatial binning model");
				}
			}
		}
	 ...
	 *             if method not found or not unique
	 */
	public static Method findUniqueMethod(Class c, String name)
			throws NoSuchMethodException {
		...
		for (int i = 0; i < methods.length; i++) {
			if (name.equals(methods[i].getName())) {
				if (m != null) {
					throw new NoSuchMethodException("method name not unique: "
							+ name);
				}
				m = methods[i];
			}
	...
	public static void addNode(CommandLineOptions opts, int i, Vector routers,
			RouteZrp.ZrpStats stats, Field field, Placement place,
	...
	private static void buildField(CommandLineOptions opts,
			final Vector routers, final RouteZrp.ZrpStats stats) {
		...
		Field field = new Field(spatial, new Fading.None(),
				new PathLoss.FreeSpace(), mobility,
		...
		for (int i = 1; i <= opts.nodes; i++) {
			JistAPI.callStaticAt(method_addNode, new Object[] { opts,
					new Integer(i), routers, stats, field, place, radioInfo,
					protMap, inLoss, outLoss }, i);
		}
		// set up bordercast events
		if (opts.bordercasts != 0) {
			JistAPI.runAt(new Runnable() {
				public void run() {
					System.out.println("clear stats at t="
							+ JistAPI.getTimeString());
					stats.clear();
				}
			...
			for (int i = 0; i < opts.bordercasts; i++) {
				...
				JistAPI.runAt(new Runnable() {
					public void run() {
						RouteZrp zrp = (RouteZrp) routers.elementAt(j
								% routers.size());
						NetMessage msg = new NetMessage.Ip(Message.NULL, zrp
								.getLocalAddr(), NetAddress.NULL,
								Constants.NET_PROTOCOL_INVALID,
								Constants.NET_PRIORITY_NORMAL,
								Constants.TTL_DEFAULT);
						zrp.getProxy().send(msg);
					}
				}, (opts.bordercastStart + opts.bordercastDelay * j)
						* Constants.SECOND);
			}
		}
	 ...
	 * 
	 * @param args
	 *            command-line arguments that may determine the parameters of
	 *            the simulation
	 */
