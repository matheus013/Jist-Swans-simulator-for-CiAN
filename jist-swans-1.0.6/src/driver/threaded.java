...
/**
 * An example to show the transparent conversion of threads to simulation time
 * under the AppJava.
 * 
...
public class threaded {
	...
	public static class worker implements AppJava.Runnable {
		...
		public void run() {
			for (int i = 0; i < count; i++) {
				...
				System.out.println("t=" + JistAPI.getTime() + " run "
						+ threadid + ": " + i);
			}
		}
	} // class: worker
	...
	public static void main(String args[]) {
		...
		for (int id = 0; id < threads.length; id++) {
			...
			AppJava.Runnable workerEntity = (AppJava.Runnable) JistAPI.proxy(
					worker, AppJava.Runnable.class);
		...
		try {
			for (int id = 0; id < threads.length; id++) {
				System.out.println("t=" + JistAPI.getTime() + " joining "
						+ threads[id]);
				threads[id].join();
				System.out.println("t=" + JistAPI.getTime() + " " + threads[id]
						+ " done.");
			}
		} catch (InterruptedException e) {
		}
