...
public class CBR {
	...
	public static class Server implements ServerInterface {
		...
		public Server(TransInterface.TransUdpInterface udp, NetAddress localAddr) {
			this.self = (ServerInterface) JistAPI.proxy(this,
		...
		/** {@inheritDoc} */
		public void run() {
			// Create a UDP event handler that just makes a note of incoming
			// packets
	...
	public static class Client implements ClientInterface {
		 ...
		 * 
		 * @param udp
		 *            the UDP interface for this node to use
		 * @param transmissions
		 *            number of outgoing tranmissions
		 * @param localAddr
		 *            this node's IP address
		 * @param serverAddr
		 *            the IP address of the server to send messages to
		 */
		...
		public void run() {
			...
			// Send the appropriate number of messages to the corresponding
			// server
		...
		public void sendMessage(int i) {
			...
			if (i == transmissions) {
				...
				// jist.runtime.Util.getUsedMemory());
			}
		}
	}
	...
	private static void showUsage() {
		...
		System.out
				.println("  -p, --protocol       routing protocol: [zrp],aodv,dsr");
		...
		System.out
				.println("  -m, --mobility       mobility model: [static],waypoint:opts,teleport:p");
		System.out
				.println("  -l, --loss           packet loss model: [none],uniform:p");
		System.out
				.println("  -c, --clients        client/server pairs: c [10]");
		System.out
				.println("  -t, --transmissions  transmissions per client: t [10]");
	...
	private static CommandLineOptions parseCommandLineOptions(String[] args)
		...
		CmdLineParser.Option opt_clients = parser.addIntegerOption('c',
				"clients");
		CmdLineParser.Option opt_transmissions = parser.addIntegerOption('t',
				"transmissions");
		CmdLineParser.Option opt_seed = parser.addIntegerOption('r',
				"randomseed");
		parser.parse(args);
		...
		if (parser.getOptionValue(opt_clients) != null) {
			cmdOpts.clients = ((Integer) parser.getOptionValue(opt_clients))
					.intValue();
		}
		if (parser.getOptionValue(opt_transmissions) != null) {
			cmdOpts.transmissions = ((Integer) parser
					.getOptionValue(opt_transmissions)).intValue();
		}
		// random seed
		if (parser.getOptionValue(opt_seed) != null) {
			cmdOpts.randseed = ((Integer) parser.getOptionValue(opt_seed))
					.intValue();
		}

		// Make sure there are enough nodes to support numClients distinct
		// client/server pairs
	...
	private static void buildField(CommandLineOptions opts) {
		...
		for (int i = 1; i <= opts.nodes; i++) {
			// alternate initializing servers and clients until there are
			// numClients of each
			...
			case Constants.NET_PROTOCOL_ZRP:
				...
				JistAPI.runAt(new Runnable() {
					public void run() {
						System.out.println(address + ": links="
								+ iarp.getNumLinks() + " routes="
								+ iarp.getNumRoutes());
						iarp.showLinks();
						iarp.showRoutes();
					}
				}, (opts.endTime - 1) * Constants.SECOND + i);
				zrp.setSubProtocols(new RouteZrpNdp(zrp), iarp,
						new RouteZrpBrp(zrp), new RouteZrpIerp(zrp));
				break;
			default:
			...
			if (isClient) {
				Client client = new Client(udp.getProxy(), opts.transmissions,
						address, new NetAddress(opts.nodes - i + 1));
