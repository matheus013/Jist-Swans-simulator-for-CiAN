...
public class ndp {
	...
	private static void showVersion() {
		System.out
				.println("JiST Project: Node discovery protocol macro-benchmark v"
						+ VERSION);
		System.out
	...
	private static void showUsage() {
		...
		System.out
				.println("  -m, --mode         benchmark mode: time, mem, density");
		System.out.println("  -n, --num          number of nodes");
		System.out
				.println("  -d, --dim          length of square field (meters)");
		System.out
				.println("  -t, --time         duration of simulation (seconds)");
		System.out.println("  -s, --spatial      [linear], grid:n, hier:n");
		System.out.println();
		System.out.println("eg: swans driver.ndp -m time -n 50 -d 4082 -t 900");
		System.out
				.println("    swans driver.ndp -m time -n 10000 -d 57735 -t 120 -s hier:9");
		System.out.println();
	}
	...
	private static cmdlineOpts parseCommandLineOptions(String[] args)
		...
		if (parser.getOptionValue(opt_mode) != null) {
			String mode = ((String) parser.getOptionValue(opt_mode))
					.toLowerCase();
		...
		if (parser.getOptionValue(opt_num) != null) {
			options.num = Long.parseLong((String) parser
					.getOptionValue(opt_num));
		}
		if (parser.getOptionValue(opt_dim) != null) {
			options.dim = Long.parseLong((String) parser
					.getOptionValue(opt_dim));
		}
		if (parser.getOptionValue(opt_time) != null) {
			options.time = Long.parseLong((String) parser
					.getOptionValue(opt_time));
		}
		if (parser.getOptionValue(opt_spatial) != null) {
			options.spatial = ((String) parser.getOptionValue(opt_spatial))
					.toLowerCase();
		}
		if (options.spatial.startsWith(MODE_SPATIAL_STRING_LINEAR)) {
			...
			options.spatial_div = Integer
					.parseInt(options.spatial.split(":")[1]);
		} else if (options.spatial.startsWith(MODE_SPATIAL_STRING_HIER)) {
			options.spatial_mode = MODE_SPATIAL_HIER;
			options.spatial_div = Integer
					.parseInt(options.spatial.split(":")[1]);
		} else {
			throw new RuntimeException("invalid spatial structure: "
					+ options.spatial);
		}
	 ...
	 *            size of field
	 * @param spatial
	 *            binning radio container
	 * @param nodes
	 *            number of nodes
	 * @return simulation field
	 */
	public static Field createSim(Location.Location2D bounds, Spatial spatial,
			long nodes) {
		// create field
		Placement placement = new Placement.Random(bounds);
		Mobility mobility = new Mobility.RandomWaypoint(bounds, PAUSE_TIME,
				GRANULARITY, MAX_SPEED, MIN_SPEED);
	...
	public static void main(String[] args) {
		try {
			...
			Location.Location2D bounds = new Location.Location2D(options.dim,
					options.dim);
			...
			System.out
					.println("size    = " + options.dim + " x " + options.dim);
			...
			case MODE_TIME:
				...
				JistAPI.runAt(new Runnable() {
					public void run() {
						...
						System.out.println("seconds:" + " init="
								+ ((initTime - startTime) / 1000.0) + " run="
								+ ((endTime - initTime) / 1000.0) + " total="
				...
				break;
			case MODE_DENSITY:
				System.out.println("Average density  = " + f.computeDensity()
						* 1000 * 1000 + "/km^2");
				...
				case MODE_SPATIAL_GRID:
					System.out
							.println("Average node/bin = "
									+ options.num
									/ (double) (options.spatial_div * options.spatial_div));
					break;
				case MODE_SPATIAL_HIER:
					System.out.println("Average node/bin = " + options.num
							/ StrictMath.pow(4, options.spatial_div));
					break;
				default:
					throw new RuntimeException("invalid binning type");
				}
