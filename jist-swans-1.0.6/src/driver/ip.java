...
public class ip {
	...
	private static void showUsage() {
		...
		System.out
				.println("  -d, --distance       distance between nodes [50m]");
		System.out
				.println("  -p, --packets        number of packets to transmit [1]");
		System.out
				.println("  -s, --sleep          delay between packets [100ms]");
		System.out
				.println("  -r, --radio          interference model: [indep]|additive");
		System.out.println();
	...
	private static CommandLineOptions parseCommandLineOptions(String[] args)
		...
		CmdLineParser.Option opt_distance = parser.addIntegerOption('d',
				"distance");
		CmdLineParser.Option opt_packets = parser.addIntegerOption('p',
				"packets");
		...
		if (parser.getOptionValue(opt_distance) != null) {
			cmdOpts.distance = ((Integer) parser.getOptionValue(opt_distance))
					.intValue();
		}
		if (parser.getOptionValue(opt_packets) != null) {
			cmdOpts.packets = ((Integer) parser.getOptionValue(opt_packets))
					.intValue();
		}
		if (parser.getOptionValue(opt_sleep) != null) {
			cmdOpts.sleep = ((Integer) parser.getOptionValue(opt_sleep))
					.intValue() * Constants.MILLI_SECOND;
		}
		if (parser.getOptionValue(opt_radio) != null) {
			...
			if (radioTypeString.equalsIgnoreCase(RADIO_STRING_INDEPENDENT)) {
				...
				throw new CmdLineParser.IllegalOptionValueException(opt_radio,
						"Unrecognized radio inteference model");
			}
		}
		return cmdOpts;
	...
	public static class MyIpHandler implements NetInterface.NetHandler {
		public void receive(Message msg, NetAddress src, MacAddress lastHop,
			...
			System.out.println("t=" + JistAPI.getTime() + ": received packet "
					+ msg + " from " + src + "(" + lastHop + ") on nic "
					+ macId);
		}
	}

	public static NetInterface createNode(Field field, int i,
			Location location, RadioNoise radio, NetAddress addr,
	...
	public static RadioNoise createRadio(int type, int i,
	...
	public static void main(String[] args) {
		try {
			...
			// create field
			Location.Location2D bounds = new Location.Location2D(
					cmdopts.distance + 1, cmdopts.distance + 1);
			...
			NetInterface n2 = createNode(field, 2, new Location.Location2D(
					cmdopts.distance, 0),
