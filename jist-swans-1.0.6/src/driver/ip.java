...
public class ip
{
    ...
    public static class MyIpHandler implements NetInterface.NetHandler
    {
        public void receive(Message msg, NetAddress src, MacAddress lastHop, byte macId, NetAddress dst, byte priority,
                byte ttl) {
            System.out.println("t=" + JistAPI.getTime() + ": received packet " + msg + " from " + src + "(" + lastHop
                    + ") on nic " + macId);
        }
    }

    public static NetInterface createNode(Field field, int i, Location location, RadioNoise radio, NetAddress addr,
    ...
    public static void main(String[] args) {
        try {
            ...
            // create nodes
            NetInterface n1 = createNode(field, 1, new Location.Location2D(0, 0),
                    createRadio(cmdopts.radio, 1, radioInfoShared), new NetAddress(1), new MyIpHandler());
            NetInterface n2 = createNode(field, 2, new Location.Location2D(cmdopts.distance, 0),
                    createRadio(cmdopts.radio, 2, radioInfoShared), new NetAddress(2), new MyIpHandler());
            // generate traffic
            Message msg = new MessageBytes("hi");
            for (int i = 0; i < cmdopts.packets; i++) {
                n1.send(msg, NetAddress.ANY, MY_PROTOCOL, Constants.NET_PRIORITY_NORMAL, (byte) 2);
