...
// Timestamp: <heartbeat.java Tue 2004/04/06 11:57:52 barr
...
public class heartbeat
{
    ...
    public static final int                PHY_TYPE     = 2;                                                                                                        // 802.11p
                                                                                                                                                                     // radio
     ...
     */
    public static void createNode(int i, Field field, Placement placement, RadioInfo.RadioInfoShared radioInfoShared,
            Mapper protMap, PacketLoss plIn, PacketLoss plOut, int radioNoiseType, int macType) {
        // create entities
        RadioNoise radio;
    ...
    public static Field createSim(int nodes, int length, int time, int maxspeed, int pausetime) {
        ...
        RadioInfo.RadioInfoShared radioInfoShared = RadioInfo.createShared(Constants.FREQUENCY_DEFAULT,
        ...
        // reasonable)
        if (PHY_TYPE == 2) {
            radioInfoShared = RadioInfo.createShared(5.89e9, (int) 3e6, 15.0, Constants.GAIN_DEFAULT,
                    Util.fromDB(-85.0), Util.fromDB(-75.0), Constants.TEMPERATURE_DEFAULT,
                    Constants.TEMPERATURE_FACTOR_DEFAULT, Constants.AMBIENT_NOISE_DEFAULT, false);
        }
        ...
        JistAPI.runAt(new Runnable() {
            public void run() {
                ...
                for (AppHeartbeat app : appList) {
                    float[] stats = app.hbs.getNeighborCountStats(5 * Constants.SECOND,
                            AppHeartbeat.HeartbeatStats.duration * Constants.SECOND, 500 * Constants.MILLI_SECOND);
                    float[] statTime = app.hbs.getNeighborTimeStats(1);
                    avgMinSum += stats[0];
                    avgMaxSum += stats[1];
                    avgSum += stats[2];
                    System.out.println("Node " + i + ":");
                    System.out.println(" - Neighbor Count:  Min: " + stats[0] + "  Max: " + stats[1] + "  Avg: "
                            + stats[2]);
                    System.out.println(" - Neighbor Time:   Min: " + statTime[0] + "  Max: " + statTime[1] + "  Avg: "
                            + statTime[2]);
