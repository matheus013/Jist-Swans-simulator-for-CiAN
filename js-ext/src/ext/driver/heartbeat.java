...
public class heartbeat
{
  ...
  //public static final String mobilityFile = "ducks/resources/mobility/simfiles/2lpd_2-2npkm_scen1/combined-108_nodes-120_tsteps-2_2_npkm-2_lpd-0.tcl";
  //public static final String mobilityFile = "ducks/resources/mobility/simfiles/2lpd_2-2npkm_scen2/combined-105_nodes-120_tsteps-2_2_npkm-2_lpd-0.tcl";
  //public static final String mobilityFile = "ducks/resources/mobility/simfiles/2lpd_6-6npkm_scen1/combined-340_nodes-120_tsteps-6_6_npkm-2_lpd-0.tcl";
  //public static final String mobilityFile = "ducks/resources/mobility/simfiles/2lpd_15-15npkm_scen1/combined-843_nodes-120_tsteps-15_15_npkm-2_lpd-0.tcl";
   ...
   * @param radioNoiseType 2=RadioNoiseAdditive, 1=RadioNoiseIndep
   * @param macType 2=Mac802.11 1=MacDumb
   */
   ...
   */
  public static Field createSim(int nodes, int length, int time, int maxspeed, int pausetime)
  {
	  Location.Location2D bounds = new Location.Location2D(length, length);
	  Placement placement = new Placement.Random(bounds);
	  Mobility mobility = new Mobility.RandomWaypoint(bounds, pausetime, GRANULARITY, maxspeed, MIN_SPEED);

	  // overwrite with FARSI mobility
	  if (MOBILITY == 2) 
	  try {
		  System.out.println("Ignoring all mobility parameters -- using MobilityReplay");
		  System.out.println("File: "+mobilityFile);
		  
		  mobility = new MobilityReplay(null,10,mobilityFile, MobilityReaderNs2.class.getName());
		  ...
		  e.printStackTrace();
		  throw new RuntimeException("Mobility could not be loaded: "+e.getMessage());
	  }
    ...
    // ReceiveThreshold: -75 dbm (-85 + 10 => doubled sensing range, seems reasonable)
    if (PHY_TYPE == 2) {
    	radioInfoShared = RadioInfo.createShared(
            5.89e9 , (int) 3e6,
            15.0 , Constants.GAIN_DEFAULT,
            Util.fromDB(-85.0), Util.fromDB(-75.0),
            Constants.TEMPERATURE_DEFAULT, Constants.TEMPERATURE_FACTOR_DEFAULT, Constants.AMBIENT_NOISE_DEFAULT, false);
    }
    ...
    {
      createNode(i, field, placement, radioInfoShared, protMap, pl, pl, RADIO_TYPE, MAC_TYPE);
    }
  ...
  public static void main(String[] args)
  {
    if(args.length<3)
    {
      System.out.println("syntax: swans driver.heartbeat <nodes> <length> <time> [<maxspeed> [<pausetime>]]");
    ...
    JistAPI.runAt(new Runnable()
    {
      public void run()
      {
        ...
        for (int k=0; k < periodCount.length; k++) { periodCount[k] = 0; } 
        
        for(AppHeartbeat app : appList) {
        	float[] stats = app.hbs.getNeighborCountStats(5*Constants.SECOND, AppHeartbeat.HeartbeatStats.duration*Constants.SECOND, 500*Constants.MILLI_SECOND);
        ...
        for(int j=0; j < periodCount.length; j++) {
        	System.out.println("PeriodLength: "+j+" Count: "+periodCount[j]);
