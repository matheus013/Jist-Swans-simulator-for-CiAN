...
public class aodvsim {
	...
	private static void buildField(CommandLineOptions opts,
		...
		stats.clear();
		int numTotalMessages = (int) StrictMath
				.floor(((double) opts.sendRate / 60) * opts.nodes
						* opts.duration);
		long delayInterval = (long) StrictMath.ceil((double) opts.duration
		...
		for (int i = 0; i < numTotalMessages; i++) {
			...
			TransUdp.UdpMessage udpMsg = new TransUdp.UdpMessage(PORT, PORT,
					Message.NULL);
