...
//
// Modified by Elmar Schoch <elmar.schoch@uni-ulm.de>
// - added DropTailMessageQueue to get around exception if packet queue is full
...
public class aodvsim_new {
	...
	private static void showUsage() {
		...
		System.out
				.println("  -f, --field          field dimensions: x,y [100,100]");
		System.out
				.println("  -a, --arrange        placement model: [random],grid:ixj");
		System.out
				.println("  -m, --mobility       mobility: [static],waypoint:opts,teleport:p,walk:opts");
		System.out
				.println("  -l, --loss           packet loss model: [none],uniform:p");
		System.out
				.println("  -s, --send rate      send rate per-minute: [1.0]");
		System.out
				.println("  -t, --timing         node activity timing: start,duration,resolution [60,3600,30]");
		System.out.println("  -r, --randomseed     random seed: [0]");
		System.out.println();
		System.out.println("e.g.");
		System.out
				.println("  swans driver.aodvsim -n 25 -f 2000x2000 -a grid:5x5 -t 10,600,60");
		System.out.println();
	}
	...
	private static CommandLineOptions parseCommandLineOptions(String[] args)
		...
		CmdLineParser.Option opt_placement = parser.addStringOption('a',
				"arrange");
		CmdLineParser.Option opt_mobility = parser.addStringOption('m',
				"mobility");
		CmdLineParser.Option opt_loss = parser.addStringOption('l', "loss");
		CmdLineParser.Option opt_rate = parser
				.addDoubleOption('s', "send rate");
		...
		// {
		// cmdOpts.endTime =
		// ((Integer)parser.getOptionValue(opt_endat)).intValue();
		// }
		// nodes
		if (parser.getOptionValue(opt_nodes) != null) {
			cmdOpts.nodes = ((Integer) parser.getOptionValue(opt_nodes))
					.intValue();
		}
		// field
		if (parser.getOptionValue(opt_field) != null) {
			cmdOpts.field = (Location.Location2D) Location
					.parse((String) parser.getOptionValue(opt_field));
		}
		// placement
		if (parser.getOptionValue(opt_placement) != null) {
			String placementString = ((String) parser
					.getOptionValue(opt_placement)).split(":")[0];
			if (placementString != null) {
				if (placementString.equalsIgnoreCase("random")) {
					...
					throw new CmdLineParser.IllegalOptionValueException(
							opt_placement, "unrecognized placement model");
				}
			}
			cmdOpts.placementOpts = Util.stringJoin((String[]) Util
					.rest(((String) parser.getOptionValue(opt_placement))
							.split(":")), ":");
		}
		// mobility
		if (parser.getOptionValue(opt_mobility) != null) {
			String mobilityString = ((String) parser
					.getOptionValue(opt_mobility)).split(":")[0];
			if (mobilityString != null) {
				if (mobilityString.equalsIgnoreCase("static")) {
					...
					throw new CmdLineParser.IllegalOptionValueException(
							opt_mobility, "unrecognized mobility model");
				}
			}
			cmdOpts.mobilityOpts = Util.stringJoin((String[]) Util
					.rest(((String) parser.getOptionValue(opt_mobility))
							.split(":")), ":");
		}
		// loss
		if (parser.getOptionValue(opt_loss) != null) {
			String lossString = ((String) parser.getOptionValue(opt_loss))
					.split(":")[0];
			if (lossString != null) {
				if (lossString.equalsIgnoreCase("none")) {
					...
					throw new CmdLineParser.IllegalOptionValueException(
							opt_loss, "unrecognized mobility model");
				}
			}
			cmdOpts.lossOpts = Util.stringJoin(
					(String[]) Util.rest(((String) parser
		...
		// {
		// String[] data =
		// ((String)parser.getOptionValue(opt_bordercast)).split(",");
		// if(data.length!=3) throw new
		// CmdLineParser.IllegalOptionValueException(opt_bordercast,
		// "bad format: num,start,delay");
		...
		if (parser.getOptionValue(opt_rate) != null) {
			cmdOpts.sendRate = ((Double) parser.getOptionValue(opt_rate))
					.doubleValue();
		}

		// timing parameters
		if (parser.getOptionValue(opt_timing) != null) {
			String[] data = ((String) parser.getOptionValue(opt_timing))
					.split(",");
			if (data.length != 3)
				throw new CmdLineParser.IllegalOptionValueException(opt_timing,
						"bad format: start,duration,resolution");
		...
		if (parser.getOptionValue(opt_randseed) != null) {
			cmdOpts.seed = ((Integer) parser.getOptionValue(opt_randseed))
					.intValue();
		}
	...
	private static Mac802_11 statsMac = null;
	private static AppCbrIp statsApp = null;
	...
	public static void addNode(CommandLineOptions opts, int i, Vector routers,
			...
			PacketLoss inLoss, PacketLoss outLoss, int nodecount, int duration) {
		// radio
		...
		NetIpBase net = new NetIp(address, protMap, inLoss, outLoss);
		...
		// app
		AppCbrIp app = new AppCbrIp(1, 0, 20, 10, 1, i, nodecount, duration);
		if (statsApp == null)
			statsApp = app;
		...
		byte intId = net.addInterface(mac.getProxy(), new DropTailMessageQueue(
				Constants.NET_PRIORITY_NUM, NetIpBase.MAX_QUEUE_LENGTH));
	 ...
	 * @param stats
	 *            zrp statistics collection object
	 */
	private static void buildField(CommandLineOptions opts,
			final Vector routers, final RouteAodv.AodvStats stats) {
		...
		case Constants.MOBILITY_WAYPOINT:
			mobility = new Mobility.RandomWaypoint(opts.field,
					opts.mobilityOpts);
			break;
		...
		for (int i = 1; i <= opts.nodes; i++) {
			addNode(opts, i, routers, stats, field, place, radioInfo, protMap,
					inLoss, outLoss, opts.nodes, opts.duration);
		}
		 ...
		 * stats.clear(); int numTotalMessages =
		 * (int)StrictMath.floor(((double)opts.sendRate/60) * opts.nodes *
		 * opts.duration); long delayInterval =
		 * (long)StrictMath.ceil((double)opts.duration *
		 * (double)Constants.SECOND / (double)numTotalMessages); for(int i=0;
		 * i<numTotalMessages; i++) { //pick random send node int srcIdx =
		 * Constants.random.nextInt(routers.size()); int destIdx; do { //pick
		 * random dest node destIdx = Constants.random.nextInt(routers.size());
		 * } while (destIdx == srcIdx); RouteAodv srcAodv =
		 * (RouteAodv)routers.elementAt(srcIdx); RouteAodv destAodv =
		 * (RouteAodv)routers.elementAt(destIdx); TransUdp.UdpMessage udpMsg =
		 * new TransUdp.UdpMessage(PORT, PORT, Message.NULL); NetMessage msg =
		 * new NetMessage.Ip(udpMsg, srcAodv.getLocalAddr(),
		 * destAodv.getLocalAddr(), Constants.NET_PROTOCOL_UDP,
		 * Constants.NET_PRIORITY_NORMAL, Constants.TTL_DEFAULT);
		 * srcAodv.getProxy().send(msg); //stats if (stats != null) {
		 * stats.netMsgs++; } JistAPI.sleep(delayInterval); }
		 */
	 ...
	 *            zrp statistics collection object
	 */
	public static void showStats(Vector routers, RouteAodv.AodvStats stats,
		...
		System.err.println("Total aodv packets sent = "
				+ stats.send.aodvPackets);
		System.err.println("Total aodv packets recv = "
				+ stats.recv.aodvPackets);

		System.err.println("Non-hello packets sent = "
				+ (stats.send.aodvPackets - stats.send.helloPackets));
		System.err.println("Non-hello packets recv = "
		...
		System.err.println("=============");
		statsApp.getStats().list(System.err);
		...
		System.out.println(opt.nodes + "\t" + stats.send.rreqPackets + "\t"
				+ stats.recv.rreqPackets + "\t" + stats.send.rrepPackets + "\t"
				+ stats.recv.rrepPackets + "\t" + stats.send.rerrPackets + "\t"
				+ stats.recv.rerrPackets + "\t" + stats.send.helloPackets
				+ "\t" + stats.recv.helloPackets + "\t"
				+ stats.send.aodvPackets + "\t" + stats.recv.aodvPackets + "\t"
				+ (stats.send.aodvPackets - stats.send.helloPackets) + "\t"
				+ (stats.recv.aodvPackets - stats.recv.helloPackets) + "\t"
				+ usedMem + "\t" + elapsedTime);
	...
	public static void main(String[] args) {
		try {
			...
			long endTime = options.startTime + options.duration
			...
			// added
			/*
			 * JistAPI.runAt(new Runnable() { public void run() { Iterator itr =
			 * routers.iterator(); while (itr.hasNext()) { RouteAodv aodv =
			 * (RouteAodv)itr.next(); aodv.printPrecursors();
			 * aodv.printOutgoing(); } } }, JistAPI.END);
			 */
