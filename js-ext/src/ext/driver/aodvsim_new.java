...
public class aodvsim_new
{
    ...
    private static CommandLineOptions parseCommandLineOptions(String[] args) throws CmdLineParser.OptionException {
        ...
        if (parser.getOptionValue(opt_placement) != null) {
            ...
            cmdOpts.placementOpts = Util.stringJoin(
                    (String[]) Util.rest(((String) parser.getOptionValue(opt_placement)).split(":")), ":");
        }
        // mobility
        if (parser.getOptionValue(opt_mobility) != null) {
            ...
            cmdOpts.mobilityOpts = Util.stringJoin(
                    (String[]) Util.rest(((String) parser.getOptionValue(opt_mobility)).split(":")), ":");
        }
        // loss
        if (parser.getOptionValue(opt_loss) != null) {
            ...
            cmdOpts.lossOpts = Util.stringJoin(
                    (String[]) Util.rest(((String) parser.getOptionValue(opt_loss)).split(":")), ":");
        }
        ...
        if (parser.getOptionValue(opt_timing) != null) {
            ...
            if (data.length != 3)
                throw new CmdLineParser.IllegalOptionValueException(opt_timing, "bad format: start,duration,resolution");
     ...
     */
    public static void addNode(CommandLineOptions opts, int i, Vector routers, RouteAodv.AodvStats stats, Field field,
            Placement place, RadioInfo.RadioInfoShared radioInfo, Mapper protMap, PacketLoss inLoss,
            PacketLoss outLoss, int nodecount, int duration) {
        // radio
        ...
        byte intId = net.addInterface(mac.getProxy(), new DropTailMessageQueue(Constants.NET_PRIORITY_NUM,
    ...
    private static void buildField(CommandLineOptions opts, final Vector routers, final RouteAodv.AodvStats stats) {
        ...
        // initialize field
        Field field = new Field(spatial, new Fading.None(), new PathLoss.TwoRay(), mobility,
        ...
        RadioInfo.RadioInfoShared radioInfo = RadioInfo.createShared(
                ...
                7.9, Constants.GAIN_DEFAULT, Util.fromDB(Constants.SENSITIVITY_DEFAULT),
        ...
        for (int i = 1; i <= opts.nodes; i++) {
            addNode(opts, i, routers, stats, field, place, radioInfo, protMap, inLoss, outLoss, opts.nodes,
                    opts.duration);
        }
    ...
    public static void showStats(Vector routers, RouteAodv.AodvStats stats, CommandLineOptions opt, Date startTime) {
        ...
        System.out.println(opt.nodes + "\t" + stats.send.rreqPackets + "\t" + stats.recv.rreqPackets + "\t"
                + stats.send.rrepPackets + "\t" + stats.recv.rrepPackets + "\t" + stats.send.rerrPackets + "\t"
                + stats.recv.rerrPackets + "\t" + stats.send.helloPackets + "\t" + stats.recv.helloPackets + "\t"
                + stats.send.aodvPackets + "\t" + stats.recv.aodvPackets + "\t"
                + (stats.send.aodvPackets - stats.send.helloPackets) + "\t"
                + (stats.recv.aodvPackets - stats.recv.helloPackets) + "\t" + usedMem + "\t" + elapsedTime);
