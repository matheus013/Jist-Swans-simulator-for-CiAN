...
public class MobilityReplay implements Mobility {
	...
	protected static final Logger logger = Logger
			.getLogger(MobilityReplay.class.getName());

	/**
	 * Waypoint gives either - current waypoints or - waypoints to go to
	 * 
	 * @author eschoch
	 */
	public static class Waypoint implements Comparable<Waypoint> {
		...
		public String toString() {
			return "at t=" + time + " go to x=" + location.getX() + " y="
					+ location.getY() + " v=" + speed;
		}
	...
	/**
	 * Initialize the MobilityReplay model which is suitable to replay a
	 * waypoint based, predifined mobility pattern from a file.
	 * 
	 * @param bounds
	 *            Bounds of the field. Will be checked with contents of file
	 * @param precision
	 *            Wanted precision in meters (-> mobility makes step every ...
	 *            meters)
	 * @param file
	 *            File to read
	 * @param readerClass
	 *            Classname of the desired Reader
	 * 
	 */
	public MobilityReplay(Location.Location2D bounds, int precision,
			String file, String readerClass) throws Exception {
		this.bounds = bounds;
		...
		try {
			...
			throw new Exception("MobilityReplay: Could not load reader: "
					+ readerClass + " :" + e.getMessage());
		}
		try {

			mr.readFile(file);
		} catch (Exception e) {
			throw new Exception("Could not load mobility file (file=" + file
		...
		if (bounds != null) {
			if (Math.abs(foundCorners[1].getX() - foundCorners[0].getX()) > bounds
					.getX()
					|| Math.abs(foundCorners[1].getY() - foundCorners[0].getY()) > bounds
							.getY()) {
		...
		if (precision == 0) {
			throw new Exception(
					"MobilityReplay: Precision value in meters must not be zero");
		}
	 ...
	 * 
	 * @param id
	 *            the node for which the location is needed
	...
	/**
	 * Retrieve smallest and and largest corner coordinates that occur in the
	 * scene
	 * 
	...
	public MobilityInfo init(FieldInterface f, Integer id, Location loc) {
		...
		try {
			...
			logger.debug("Found " + mrmi.waypoints.size()
					+ " waypoints for node " + id);
	...
	public void next(FieldInterface f, Integer id, Location loc,
		...
		if (mrmi.lastWaypoint == null) {
			...
			logger.debug("initializing mobility: node=" + id + " starts "
					+ mrmi.lastWaypoint.toString());
			...
			if (mrmi.remainingSteps == 0) {
				...
				if (mrmi.nextWaypoint != null) {
					...
					float distanceToNextWP = loc
							.distance(mrmi.nextWaypoint.location);

					logger.debug("state: t=" + JistAPI.getTime() + " node="
							+ id + " current loc: x=" + loc.getX() + " y="
							+ loc.getY());
					logger.debug("selected next wp: start "
							+ mrmi.nextWaypoint.toString() + " (distance="
							+ distanceToNextWP);
					...
					if (mrmi.nextWaypoint.type == Waypoint.WAYPOINT_CURRENT) {
						...
						// see, if we still need to wait a little, until journey
						// starts
						waitUntil(mrmi.nextWaypoint.time);

						// calculate number of steps
						mrmi.steps = (int) StrictMath.max(
								StrictMath.ceil(distanceToNextWP
										/ this.precision), 1);

						// length of a step piece can be calculated from
						// distance divide by number of steps
						double delta = distanceToNextWP / mrmi.steps;
						// time it takes for one piece depends on the speed the
						// node travels with
						mrmi.stepTime = (int) Math
								.floor((delta * Constants.SECOND)
										/ mrmi.nextWaypoint.speed);

						mrmi.remainingSteps = mrmi.steps;

						logger.debug("... steps=" + mrmi.steps + " steptime="
								+ mrmi.stepTime);
						logger.debug("    --> total time="
								+ mrmi.steps
								* mrmi.stepTime
								+ " total dist="
								+ ((mrmi.steps * mrmi.stepTime) / (float) Constants.SECOND)
								* mrmi.nextWaypoint.speed);
			...
			if (mrmi.nextWaypoint != null) {
				...
				if (mrmi.nextWaypoint.type == Waypoint.WAYPOINT_DESTINATION) {
					logger.debug("Stepping: current time: " + JistAPI.getTime()
							+ " steptime: " + mrmi.stepTime);
					JistAPI.sleep(mrmi.stepTime);
					Location step = loc.step(mrmi.nextWaypoint.location,
							mrmi.remainingSteps--);
