...
public class MobilityReplay implements Mobility
{
    ...
    public MobilityReplay(Location.Location2D bounds, int precision, String file, String readerClass) throws Exception {
        ...
        try {
            ...
            throw new Exception("Could not load mobility file (file=" + file + ", workingdir="
        ...
        if (bounds != null) {
            if (Math.abs(foundCorners[1].getX() - foundCorners[0].getX()) > bounds.getX()
                ...
                logger.warn("Size of field and boundaries of mobility do not fit: " + "size_x=" + bounds.getX()
                        + " size_y=" + bounds.getY() + ", bottomleft=(" + foundCorners[0].getX() + ","
                        + foundCorners[0].getY() + "]" + ", topright=(" + foundCorners[1].getX() + ","
                        + foundCorners[1].getY());
            }
        }
        ...
        if (mrmi.lastWaypoint == null) {
            ...
            if (mrmi.remainingSteps == 0) {
                ...
                if (mrmi.nextWaypoint != null) {
                    ...
                    logger.debug("state: t=" + JistAPI.getTime() + " node=" + id + " current loc: x=" + loc.getX()
                            + " y=" + loc.getY());
                    logger.debug("selected next wp: start " + mrmi.nextWaypoint.toString() + " (distance="
                            + distanceToNextWP);
                    ...
                    if (mrmi.nextWaypoint.type == Waypoint.WAYPOINT_CURRENT) {
                        ...
                        logger.debug("    --> total time=" + mrmi.steps * mrmi.stepTime + " total dist="
                                + ((mrmi.steps * mrmi.stepTime) / (float) Constants.SECOND) * mrmi.nextWaypoint.speed);
    ...
    private void waitUntil(long time) {
        ...
        long wait = time - now;
        if (wait > 0) {
            logger.debug("... waiting for " + (wait) + " [now=" + now + " nextstart=" + time + "]");
            JistAPI.sleep(wait);
        }
    }
}
