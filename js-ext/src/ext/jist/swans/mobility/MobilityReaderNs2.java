...
/**
 * Implementation of MobilityReader for ns-2 movement files
 * Reader is able to recognize Tcl-statements of the following kinds:
 * 1. $node_(25) set X_ 2908.5
 *	  $node_(25) set Y_ 3.75
 * 2. $ns_ at 2.5 "$node_(16) setdest 4390.2 3.75 36.5"
 * 
 * @author eschoch
 */
public class MobilityReaderNs2 implements MobilityReader {
	
	protected static final Logger logger = Logger.getLogger(MobilityReaderNs2.class.getName());
	...
	public void readFile(String filename)  throws Exception {
		...
		while (m.find()) {
			...
			wp.location  = new Location2D(Float.parseFloat(m.group(2)), Float.parseFloat(m.group(4)));
			wp.time = 0;
			wp.speed = 0;
		...
		while (m.find()) {
			...
			wp.location  = new Location2D(Float.parseFloat(m.group(3)), Float.parseFloat(m.group(4)));
	...
	/**
	 * Retrieve bottom left and top right corner values of all
	...
	public Location2D[] getCorners() {
		...
		for (NodeWaypoints nwp: nodeList) {
			for (MobilityReplay.Waypoint wp: nwp.waypoints) {
				if (wp.location.getX() < minx) { minx = wp.location.getX(); }
				else { if (wp.location.getX() > maxx) { maxx = wp.location.getX(); }}

				if (wp.location.getY() < miny) { miny = wp.location.getY(); }
				else { if (wp.location.getY() > maxy) { maxy = wp.location.getY(); }}
			}
		}
