...
public class AppGreetingInitiator extends AppGreetingBase implements AppInterface
{
    ...
    public AppGreetingInitiator(int nodeId, int reqRate, int invokeDelay, int waitTimeStart, int waitTimeEnd,
            int duration) {
        super(nodeId);
    ...
    @Override
    public void receive(Message msg, final NetAddress src, MacAddress lastHop, byte macId, NetAddress dst,
            ...
            if (gmsg.getAttempt() == 1) {
                if (stats.isFirstProvider(gmsg.getMsgId(), src)) {
                    ...
                    JistAPI.runAt(new Runnable() {
                        public void run() {
                            netEntity.send(new GreetingMessage(GreetingMessage.REQUEST_TEXT, gmsg.getMsgId(), 2), src,
                                    NET_PROTOCOL_NUMBER, Constants.NET_PRIORITY_NORMAL, (byte) 1);
                        }
