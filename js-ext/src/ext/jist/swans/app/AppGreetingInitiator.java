...
public class AppGreetingInitiator extends AppGreetingBase implements
		AppInterface {
	...
	public AppGreetingInitiator(int nodeId, int reqRate, int invokeDelay,
			int waitTimeStart, int waitTimeEnd, int duration) {
		super(nodeId);
	...
	public String[] getStatParams() {
		return new String[] { SERV_STAT_REQ, SERV_STAT_SUC_SEARCH,
				SERV_STAT_SUC_INVOKE };
	}
	...
	public ExtendedProperties getStats() {
		...
		s.put(SERV_STAT_SUC_SEARCH,
				Double.toString(stats.getNumSearchSuccess()));
		s.put(SERV_STAT_SUC_INVOKE,
				Double.toString(stats.getNumInvokeSuccess()));

		return s;
	}

	@Override
	public void receive(Message msg, final NetAddress src, MacAddress lastHop,
		...
		if (gmsg.isResponse()) {
				...
				if (stats.isFirstProvider(gmsg.getMsgId(), src)) {
					...
					JistAPI.runAt(new Runnable() {
						public void run() {
							netEntity.send(
									new GreetingMessage(
											GreetingMessage.REQUEST_TEXT, gmsg
													.getMsgId(), 2), src,
									NET_PROTOCOL_NUMBER,
			...
			} else if (gmsg.getAttempt() == 2
					&& stats.isBoundProvider(gmsg.getMsgId(), src)) {
	...
	public void run(String[] args) {
		...
		msgId++;
		Message msg = new GreetingMessage(GreetingMessage.REQUEST_TEXT, msgId,
				1);
		...
		if (JistAPI.getTime() <= (this.duration - this.waitTimeEnd - this.reqRate)
				* Constants.SECOND) {
...
class ServiceStats {
	...
	public synchronized boolean isBoundProvider(int msgId, NetAddress provider) {
		StatsEntry e = findEntry(msgId);
		if (e != null && e.getProvider().equals(provider))
			return true;
		else
			return false;
	}
	...
	public synchronized void markInvokeSuccess(int msgId) {
		StatsEntry e = findEntry(msgId);
		if (e != null) {
			if (e.isInvokeSuccess())
				System.out.println("entry has already invoke success marked");
