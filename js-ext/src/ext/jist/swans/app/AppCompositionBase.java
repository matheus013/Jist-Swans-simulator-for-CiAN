...
public abstract class AppCompositionBase implements AppInterface, NetInterface.NetHandler {
    /**
    ...
    public abstract void receive(Message msg, NetAddress src, MacAddress lastHop, byte macId, NetAddress dst, byte priority,
            byte ttl);
...
class DiscoveryRequest extends ServiceMessage{
    ...
    public DiscoveryRequest(int nodeId, int msgId, char serviceName, String compositionRequestId){
        super(nodeId, msgId);
        this.serviceName = serviceName;
...
class CompositionMessage extends ServiceMessage{
    ...
    public CompositionMessage(String id, char[] services, int[] providers, int[] input, int lastModifiedBy){
        super(id);
    ...
    public String getServiceAt(int index){
        if(index == services.length-1) return STRING_DESTINATION;
        try{
