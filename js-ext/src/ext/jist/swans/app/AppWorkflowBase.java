...
class Workflow {
	...
	public Workflow(String id, int version, char[] services, int[] inputs,
			int nextIndexToExecute, long arrival) {
...
class WorkflowRequest extends WorkflowMessage {
	...
	public WorkflowRequest(String id, int version, char[] services,
			int[] inputs, int nextIndexToExecute) {
		super(id);
		this.version = version;
		this.services = services;
		this.inputs = inputs;
	...
	public int getSize() {
		return Sizeof.inst(id.toCharArray()) + Sizeof.inst(version)
				+ Sizeof.inst(services) + Sizeof.inst(inputs)
...
class ServiceAd extends WorkflowMessage {
	...
	public ServiceAd(String id, int advertiser, char service,
			double connectivity) {
		super(id);
	...
	@Override
	public int getSize() {
		return Sizeof.inst(id.toCharArray()) + Sizeof.inst(advertiser)
				+ Sizeof.inst(service) + Sizeof.inst(connectivity);
	}
}
...
class Token extends WorkflowMessage {
	...
	public int getSize() {
		return Sizeof.inst(id.toCharArray()) + Sizeof.inst(service)
				+ Sizeof.inst(input) + Sizeof.inst(provider);
	}
}
