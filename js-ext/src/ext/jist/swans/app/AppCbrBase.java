...
/**
 * AppCbr sends series of packets from the current node to a randomly
 * selected destination. After having sent a sequence, it waits for a
 * certain time before starting again with a different destination.
 * 
 * @author Elmar Schoch
 */
public abstract class AppCbrBase implements AppInterface, NetInterface.NetHandler, StatsCollector {
	...
	/**
	 * Defines the IP protocol number since we directly use the network
	 * layer, surpassing e.g. transport layer (UDP,TCP)
	 * Note: this protocol number should be defined in jist.swans.Constants
	 *       but is done here for modularity reasons 
	 */
	...
	/**
	 * 
	 * @param sendRate number of messages per minute
	 * @param waitTime time to wait before beginning a new connection (seconds)
	 * @param packetsPerConnection number of packets to send during one connection
	 */
	public AppCbrBase(int sendRate, int waitTimeBetween, int waitTimeStart, int waitTimeEnd, int packetsPerConnection, int nodeId, int nodeCount, int duration) {
	...
	public void run(String[] args) {
	    ...
	    if ( JistAPI.getTime() <= (this.duration - this.waitTimeEnd - this.waitTime - jitter) * Constants.SECOND ) {
	...
	/**
	 * Starts sending a message sequence. A message sequence is a sequence of packets that
	 * are sent to one destination in a certain interval. 
	 * 
	 */
	...
	/**
	 * Retrieve statistics from AppCbr which currently is
	...
	public ExtendedProperties getStats() {
		...
		float avgHopCount = receivedPackets == 0 ? 0 : hopCountSum / receivedPackets;
