...
public class AppWorkflowProvider extends AppWorkflowBase implements AppInterface {
    ...
    public AppWorkflowProvider(int nodeId, DucksCompositionStats stats, String mode,
...
////        }
    ...
    protected void handleWorkflowRequest(WorkflowRequest req, NetAddress src) {
        ...
        case STATE_LISTENING:
            if(!alligible) return;
            wf = new Workflow(reqId, req.getVersion(), req.getServicesCopy(), req.getInputCopy(), req
                    .getNextIndexToExecute(), now);
        ...
        case STATE_OBSERVING:
            ...
            wf = new Workflow(reqId, req.getVersion(), req.getServicesCopy(), req.getInputCopy(), pos_wf, now);
            break;
        default:
    ...
    protected void handleServiceAd(ServiceAd ad, NetAddress src) {
        ...
        case STATE_SEARCHING:
            if (ad.getService() == s_pending && timeUp == false) {
                wf_pending.updateProviderFor(ad.getService(), new Provider(ad.getAdvertiser(), ad.getConnectivity()));
            }
            break;
    ...
    private void scheduleTimeoutFor(String wfId, Long at) {
        try {
            timeUp = false;
            Method m = ext.jist.swans.app.AppWorkflowProvider.class.getMethod("timeoutProxyMethod", new Class[] {
                    java.lang.String.class, ext.jist.swans.app.AppWorkflowProvider.class });
    ...
    public static void timeoutProxyMethod(String wfId, AppWorkflowProvider provider) {
   ...
   private int nextMatchingPosition(Workflow wf, int offset) {
    ...
    private void handOff(String wfId, char service, int in, int provider, boolean isLast) {        
        ...
        if(!isLast){
            ...
            netEntity.send(t, dst, NET_PROTOCOL_NUMBER, Constants.NET_PRIORITY_NORMAL, Constants.TTL_DEFAULT);
            compositionStats.setLastServiceBound(CompositionMessage.STRING_DESTINATION);
            // needed only to use DuckStats as is and to have dst entry registered, otherwise end time is not stored
            compositionStats.registerForwardToExecStartTime(CompositionMessage.STRING_DESTINATION, wfId, JistAPI.getTime());
    ...
    private void search(Workflow wf) {
        WorkflowRequest wfreq = new WorkflowRequest(wf.getId(), wf.getVersion() + 1, wf.getServices(), wf.getInputs(),
