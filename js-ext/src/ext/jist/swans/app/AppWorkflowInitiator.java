...
public class AppWorkflowInitiator extends AppWorkflowBase implements AppInterface
{
        ...
        switch (state) {
            case STATE_OBSERVING:
                if (localWf.getVersion() > req.getVersion())
                    return;
                localWf = new Workflow(req.getId(), req.getVersion(), req.getServicesCopy(), req.getInputCopy(),
        ...
        switch (state) {
            case STATE_OBSERVING:
                ...
                if (wf.isLastService(service)) {
                    ...
                    compositionStats.registerForwardToExecEndTime(CompositionMessage.STRING_DESTINATION, wf.getId(),
                            JistAPI.getTime());
    ...
    private void search(Workflow wf) {
        WorkflowRequest wfreq = new WorkflowRequest(wf.getId(), wf.getVersion(), wf.getServices(), wf.getInputs(),
