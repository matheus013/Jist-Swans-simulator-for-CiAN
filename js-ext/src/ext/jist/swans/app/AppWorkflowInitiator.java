...
public class AppWorkflowInitiator extends AppWorkflowBase implements AppInterface {
    ...
    public AppWorkflowInitiator(int nodeId, DucksCompositionStats stats, int reqSize, int reqRate, int waitTimeStart,
    ...
    protected void handleWorkflowRequest(WorkflowRequest req, NetAddress src) {
        ...
        case STATE_OBSERVING:
            if (localWf.getVersion() > req.getVersion())
                return;
            localWf = new Workflow(req.getId(), req.getVersion(), req.getServicesCopy(), req.getInputCopy(), req
                    .getNextIndexToExecute(), JistAPI.getTime());
            break;
        default:
            break;
        }
    ...
    protected void handleServiceAd(ServiceAd ad, NetAddress src) {
        ...
        default:
            wf.updateProviderFor(ad.getService(), new Provider(ad.getAdvertiser(), ad.getConnectivity()));
            break;
        }
    }
    ...
    protected void handleToken(Token t, NetAddress src) {
        ...
        case STATE_OBSERVING:
            ...
            if (wf.isLastService(service)) {
                compositionStats.incrementInvokeSuccess(CompositionMessage.STRING_DESTINATION);
                compositionStats.registerForwardToExecEndTime(CompositionMessage.STRING_DESTINATION, wf.getId(), JistAPI.getTime());
                compositionStats.setLastServiceExecuted(CompositionMessage.STRING_DESTINATION);
                compositionStats.setiKnowsLastServiceExecuted(CompositionMessage.STRING_DESTINATION);
                compositionStats.setiKnowsLastServiceBound(CompositionMessage.STRING_DESTINATION);
            }else{
    ...
    public static void timeoutProxyMethod(String wfId, AppWorkflowInitiator provider) {
        provider.timeout(wfId);
    }

    private void scheduleTimeoutFor(String wfId, Long at) {
        try {
            Method m = ext.jist.swans.app.AppWorkflowInitiator.class.getMethod("timeoutProxyMethod", new Class[] {
                    java.lang.String.class, ext.jist.swans.app.AppWorkflowInitiator.class });
    ...
    private void search(Workflow wf) {
        WorkflowRequest wfreq = new WorkflowRequest(wf.getId(), wf.getVersion(), wf.getServices(), wf.getInputs(), wf
                .getNextIndexToExecute());
        send(wfreq);
    ...
    private void handOff(Workflow wf) {
        ...
        if (providers.size() > 0) {
            ...
            Token t = new Token(wf.getId(), service, wf.getInputForService(service), providers.get(0).getNodeId());
            send(t);
