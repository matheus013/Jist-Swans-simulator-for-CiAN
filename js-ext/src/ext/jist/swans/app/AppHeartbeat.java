...
// With statistics extensions by Ulm University (Elmar Schoch)
...
public class AppHeartbeat implements AppInterface, NetInterface.NetHandler {
	...
	public static class HeartbeatStats {
		...
		public void neighborLost(MacAddress mac) {
			...
			if (evList == null) {
				System.out
						.println("Event List started with Neighbor loss. Should not happen like this.");
				evList = new ArrayList<NeighborEvent>();
				nbEvents.put(mac, evList);
			}
			NeighborEvent ne = new NeighborEvent();
		 ...
		 * 
		 * @param whichStats
		 *            == 1 => result[0] min visibility of a neighbor (seconds)
		 *            [1] max. [2] avg. [3] total number of reincounters == 2 =>
		 *            dynamic array, index = period of neighborshop, content =
		 *            number of such periods (rounded to full seconds)
		 * @return
		 */
		public float[] getNeighborTimeStats(int whichStats) {
			int reincounters = 0;
			long start;
			...
			for (MacAddress mac : nbEvents.keySet()) {
				...
				if (start != -1) {
					...
					periods.add(new Long(duration * Constants.SECOND - start));
				}
			}
			...
			if (whichStats == 1) {
				// return statistical values min, max, avg and reincounters
				result = new float[4];
				result[0] = minPeriod / (float) Constants.SECOND;
				result[1] = maxPeriod / (float) Constants.SECOND;
				if (periods.size() > 0) {
					result[2] = (avgPeriodSum / (float) Constants.SECOND)
							/ (float) periods.size();
				} else {
					result[2] = 0;
				}
				...
				// System.out.println(maxPeriod / (float) Constants.SECOND);
				int maxIndex = Math.round(maxPeriod / (float) Constants.SECOND);
				// System.out.println("maxindex: "+maxIndex);
				result = new float[maxIndex + 1];
				for (int i = 0; i < maxIndex; i++) {
					result[i] = 0;
				}
				for (Long period : periods) {
					result[Math.round(period.longValue()
							/ (float) Constants.SECOND)]++;
				}
			}
	...
	private void neighbourLost(MacAddress mac) {
		...
		// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
		if (display) {
			System.out.println("(" + nodenum + ") lost neighbour:  " + mac
					+ ", t=" + Util.timeSeconds());
		}
	}
	...
	private void neighbourDiscovered(MacAddress mac) {
		...
		// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
		if (display) {
			System.out.println("(" + nodenum + ") found neighbour: " + mac
					+ ", t=" + Util.timeSeconds());
		}
	}
	...
	private long calcDelay() {
		return HEARTBEAT_MIN
				+ (long) ((HEARTBEAT_MAX - HEARTBEAT_MIN) * Constants.random
						.nextFloat());
	}
	...
	public void run(String[] args) {
		...
		// System.out.println("("+nodenum+") sends msg at t="+Util.timeSeconds());
		hbs.messagesSent++;
