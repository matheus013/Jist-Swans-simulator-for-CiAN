...
public class AppHeartbeat implements AppInterface, NetInterface.NetHandler
{
  ...
  public static class HeartbeatStats {
	...
	public int messagesSent = 0;
	   ...
	   *
	   * @param whichStats
	   * 		== 1 => result[0] min visibility of a neighbor (seconds) [1] max. [2] avg. [3] total number of reincounters
	   * 		== 2 => dynamic array, index = period of neighborshop, content = number of such periods (rounded to full seconds)
	   * @return
	   */
	  public float[] getNeighborTimeStats(int whichStats) {
		  ...
		  if (whichStats == 1) {
			  ...
			  for (int i=0; i < maxIndex; i++) { result[i] = 0; }
			  for(Long period : periods) {
				  result[  Math.round(period.longValue() / (float) Constants.SECOND)]++;
			  }
		  }
