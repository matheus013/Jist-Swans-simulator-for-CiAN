...
/** 
 * Alternate schedulers for JiST. In the literature you
 * can find information about this topic described as
 * the "Pending Event Set" problem.
 *
 * @author Michael Feiri &lt;michael.feiri@uni-ulm.de&gt;
 */
public abstract class SchedulingQueue {
  ...
  /** 
   * An implementation of a Scheduler based on the default
   * PriorityQueue in Java 5.
   *
   * @author Michael Feiri &lt;michael.feiri@uni-ulm.de&gt;
   */
  public static final class PQScheduler extends Scheduler {
	...
	public PQScheduler() {
		datastore = new PriorityQueue<Event>(/*11, new Event.EventComparator()*/);
	}
  ...
  /** 
   * A simple implementation of a Scheduler based on the
   * TreeSet Collection Class.
   *
   * @author Michael Feiri &lt;michael.feiri@uni-ulm.de&gt;
   */
  public static final class TMScheduler extends Scheduler {
	...
	public Event removeFirst() {
		// This would not return the exact same results as other implementations
...
/*
  public static final class PBScheduler extends Scheduler {
	...
	private final org.apache.commons.collections.buffer.PriorityBuffer datastore;
	
	public PBScheduler() {
		datastore = new org.apache.commons.collections.buffer.PriorityBuffer();
	}
	...
	public Event removeFirst() {
		return (Event)datastore.remove();
	}

	public Event peekFirst() {
		return (Event)datastore.get();
	}
...
*/
  ...
  /** 
   * An implementation of a Scheduler based on the public
   * domain implementation of a top-down splay tree by Daniel
   * Sleator from http://www.link.cs.cmu.edu/splay/
   *
   * @author Michael Feiri &lt;michael.feiri@uni-ulm.de&gt;
   */
  ...
  public static class STScheduler extends Scheduler
  {
	...
	for (;;) {
		...
		if (key.compareTo(t.right.key) >= 0) { // this may be left as ">"
