...
public abstract class Region implements JistAPI.Timeless {
	...
	/**
	 * Compute distance from a given location to this region. Implementations
	 * are free to base this on a centroid or any other object insider the
	 * region. But do not use a regions border to compute distance since a
	 * packet might not reach the interior of a region if the distance is
	 * measured against the border
	 * 
	 * @param l
	 *            compute distance to this location
	...
	public static class LineBuffer extends Region {
		...
		public LineBuffer(Location.Location2D a, Location.Location2D b,
				float height) {
			this.h = height;
			this.SIZE = a.size() + b.size() + 4; // two Location objects and a
													// float
		...
		public boolean equals(Object o) {
			if (o == this)
				return true;
			if (!(o instanceof Region.LineBuffer))
				return false;
	...
	public static class Rectangle extends Region {
		...
		public Rectangle(Location.Location2D a, Location.Location2D b,
				float height, boolean clockwise) {

			// two Location objects a float (the boolean could be stuffed in the
			// floats sign bit)
			this.SIZE = a.size() + b.size() + 4;

			this.h = height;
			this.cw = clockwise;
			this.spline = new Line2D.Float(a.getX(), a.getY(), b.getX(),
					b.getY());
			...
			c = new Location.Location2D(
					(float) (a.getX() + ((b.getX() - a.getX()) * 0.5) + (Yorient
							* ((b.getY() - a.getY()) * scaleFactor) * 0.5)),
					(float) (a.getY() + ((b.getY() - a.getY()) * 0.5) + (Xorient
							* ((b.getX() - a.getX()) * scaleFactor) * 0.5)));
		}
		...
		public boolean contains(Location l) {
			...
			// idea based on comp.graphics.algorithms FAQ:
			// "distance from a point to a line"
			double r = ((l.getX() - spline.x1) * (spline.x2 - spline.x1) + (l
					.getY() - spline.y1) * (spline.y2 - spline.y1))
					/ ((spline.x2 - spline.x1) * (spline.x2 - spline.x1) + (spline.y2 - spline.y1)
							* (spline.y2 - spline.y1));
		...
		public boolean equals(Object o) {
			if (o == this)
				return true;
			if (!(o instanceof Region.Rectangle))
				return false;
