...
/**
 * MultipleStatsCollector is a encapsulating class that is able to hold
 * objects implementing the StatsCollector interface, and calls them
 * in behalf. Additionally, MultipleStatsCollector implements the possibility
 * to register static collectors. In this case, if a second stats collecting
 * object with the same class wants to be registered, it is not registered actually.
 * This is usefull, if stats are collected in static variables (e.g. total number
 * of sent messages throughout all network nodes)
 * 
 * @author Elmar Schoch
 *
 */
public class MultipleStatsCollector implements StatsCollector {
	...
	private static Logger log = Logger.getLogger(MultipleStatsCollector.class.getName());
	 ...
	 * 
	 * CAUTION! The namespace MUST be in valid Java property format,
	 * e.g. "node.a", otherwise parsing of statistics will fail!  
	 *  
	 * @param sc The collector object
	 * @param namespace Namespace in valid Java property format
	 */
	public void registerCollector(StatsCollector sc, String namespace) {
		if (! (namespace.equals("") || ExtendedProperties.isValidPropertyName(namespace))) {
			log.warn("Registering collector failed: "+namespace+" is not a valid namespace");
			return;
		}
	 ...
	 * In other words, if a different StatsCollector implementing
	 * object of the same class is already registered, sc won't  
	 * be registered.
	 * @param sc the collector to register
	 */
	...
	/**
	 * Searches the registered collector objects and returns
	 * true if an object of the same class as the given one
	 * is registered already. Otherwise, false is returned.
	 * @param sc
	...
	public StatsCollector getStaticCollector(Class c, String namespace) {
		...
		for (Map.Entry<StatsCollector, String> entry: scs) {
			...
			if ( sc.getClass().getName().equals(c.getName()) && ns.equals(namespace) ) {
				result = sc;
				break;
			}
		}
	 ...
	 * @param c the searched class 
	 * @return the corresponding StatsCollector object, or null if not registered
	 * @throws Exception
	 */
	...
	public StatsCollector getStaticCollector(Class c, boolean createAndAdd) throws Exception {
		return getStaticCollector(c, "", createAndAdd);
	}
	
	public StatsCollector getStaticCollector(Class c, String namespace, boolean createAndAdd) throws Exception {
		...
		// still null?
		if (sc == null) {
			throw new Exception("Static collector for class "+c.getName()+" not found");
		}
	...
	/**
	 * Retrieve stats parameter set for all registered collectors.
	 * May be used for consistency checks, because this lists the parameters
	 * that will be delivered as results by "getStats()" from the StatsCollector objects.
	 */
	...
	// (useful for multiple stats collectors that also provide own result values)
