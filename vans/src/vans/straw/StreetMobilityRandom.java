 ...
 * RCS:          $Id: StreetMobilityRandom.java,v 1.1.1.1 2006/02/20 23:26:42 drchoffnes Exp $
 ...
 * 
 *         The StreetMobilityRandom class represents movement along roads with
 *         random waypoints chosen from a set of intersections encountered.
 */
public class StreetMobilityRandom extends StreetMobility {
	 ...
	 * @param degree
	 * @param probability
	 *            probability with which a car will turn at an intersection
	 * @param bl
	 *            bottom left corner of map in lat/long
	 * @param tr
	 *            top right corner of map in lat/long
	 * @param r
	 */
	...
	public MobilityInfo init(FieldInterface f, Integer id, Location loc) {
		...
		/**
		 * Elmar Schoch +++++++++++++++++++++ In the foloowing call, the index
		 * value in mobInfo.get(<index>) depends strongly on how identifiers are
		 * assigned to nodes. For instance, the original STRAW subtracts 1 (i.e.
		 * id.intValue()-1) -----------------------------------
		 */
		StreetMobilityInfo mi = (StreetMobilityInfo) mobInfo.get(id.intValue());
		mi.v = v;
		mi.id = id;
		((StreetMobilityInfoRandom) mi).probability = probability; // per-vehcile
																	// turn prob
		return mi;
	}
	...
	public void setNextRoad(StreetMobilityInfo smi) {
		...
		if (smir.nextIs == null) {
			...
			System.out.println("Road considered"
					+ smir.current.printStreetName(streets));
		...
		// turn only if the probability is high enough and there are
		// intersecting roads
		...
		{
			if (DEBUG)
				System.out.println("Staying on same road...");
	 ...
	 * 
	 * @param smir
	 *            The SMIR object to update.
	 * @param ll
	 *            The list of RoadSegments at the specified intersection.
	 */
	private void setNextRoadTurn(StreetMobilityInfoRandom smir, LinkedList ll) {
		...
		} while (((StreetName) streets.get(new Integer(rs.getStreetIndex())))
				.toString().equals(
						((StreetName) streets.get(new Integer(smir.current
								.getStreetIndex()))).toString())
				&& !ll.isEmpty());
		...
		if (ll.isEmpty()) {
			...
			do {
				...
				rs = (RoadSegment) ll.get(index);
				ll.remove(index);
				// if (DEBUG) System.out.println("possible street: " +
				// streets[rs.getStreetIndex()].toString());
			} while (rs.getStartAddressLeft() != smir.current
					.getStartAddressLeft() && ll.size() > 0);
		}
		updateNextRoadSegment(smir, rs);
	}
}
