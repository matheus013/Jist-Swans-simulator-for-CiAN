...
//    public final static double METERS_PER_DEGREE = 110874.40;
    ...
    /** Meters per degree longitude around current map - estimated for middle of current map **/
    ...
    {        
        ...
        this.bl = new Location.Location2D((float) (bl.getX() * getLongitudeMetersPerDegree()), (float) (bl.getY() * METERS_PER_DEGREE));
        this.tr = new Location.Location2D((float) (tr.getX() * getLongitudeMetersPerDegree()), (float) (tr.getY() * METERS_PER_DEGREE));
        ...
        System.out.println("After loading streets, region is " 
                ...
                + this.tr 
                + " ("+(this.tr.getX()/getLongitudeMetersPerDegree())+"°,"+(this.tr.getY()*DEGREES_PER_METER)+"°)"+ "  " 
                + (this.tr.getX()-this.bl.getX())+ "m, " 
                + (this.tr.getY()-this.bl.getY())+ "m");

        int spacing = INTERSECTION_RESOLUTION * 50;
        degree = (int) (StrictMath.log(StrictMath.max((maxX - minX) / spacing, (maxY - minY) / spacing)) / StrictMath.log(2));
    ...
    */
     ...
     * @param smi 	object of the vehicle
     * @param id	id of the vehicle (just for debug information)
     ...
     * smi.nextCar			pointer to the car in front
     * smi.current,
     * smi.nextRS			calculation of remaining distance
     * 
     * the following variables are changed
     * smi.waiting			remaining time to wait at traffic light or stop sign
     * smi.waitCount		amount of timesteps already waiting
     * smi.color			color of node status
     * 
    ...
    private float calcFreeDistance(StreetMobilityInfo smi, float maxDist, Integer id) {
    	...
    	if (smi.waiting >= smi.stepTime) {
    		if (debug) System.out.println("Time: "+JistAPI.getTime()+" node: "+id+" has to wait "+smi.waiting);
    	...
    	if (!firstCar) {
    		...
    		if (debug) System.out.println("Time: "+JistAPI.getTime()+" node: "+id+" other car in front");
    		if (StreetMobilityInfo.coloring == StreetMobilityInfo.COL_STATUS) smi.color = Color.orange;
    	...
    	if (remainingDistance >= maxDist) {
    		if (StreetMobilityInfo.coloring == StreetMobilityInfo.COL_STATUS) smi.color = Color.green;
    		return remainingDistance;
    	}

    	// goal reached, slow down
    	if (smi.nextRS == null) {
    		if (debug) System.out.println("Time: "+JistAPI.getTime()+" node: "+id+" game over");
        	if (StreetMobilityInfo.coloring == StreetMobilityInfo.COL_STATUS) smi.color = Color.black;
    		return remainingDistance;
    	}
    	
    	// u-turn, slow down
    	if (smi.nextRS == smi.current && remainingDistance > 0.5f) {
    		if (debug) System.out.println("Time: "+JistAPI.getTime()+" node: "+id+" u-turn ahead");
        	if (StreetMobilityInfo.coloring == StreetMobilityInfo.COL_STATUS) smi.color = Color.blue;
    		return remainingDistance;
    	}
    	
    	// crossing, slow down
    	if (smi.nextRS.getStreetIndex() != smi.current.getStreetIndex() && remainingDistance > 3.0f ) {
    		if (debug) System.out.println("Time: "+JistAPI.getTime()+" node: "+id+" crossing ahead");
        	if (StreetMobilityInfo.coloring == StreetMobilityInfo.COL_STATUS) smi.color = Color.cyan;
    		return remainingDistance;
    	}
    	...
    	// find out if we must stop at the intersection (red traffic lights or stop signs)
    	float pause = 0.0f;
    	if (smi.waitCount == 0) {
    	...
    	{            
    		...
    		smi.waiting = pause;
    		smi.waitCount++;
    		if (debug) System.out.println("Time: "+JistAPI.getTime()+" node: "+id+" has to wait: "+pause);
        	if (StreetMobilityInfo.coloring == StreetMobilityInfo.COL_STATUS) smi.color = Color.red;
    		return remainingDistance;
    	}
    	...
    	if (nextLane == null)
    	{
    		...
    		if (debug) System.out.println("Time: "+JistAPI.getTime()+" node: "+id+" no free space on next lane");
        	if (StreetMobilityInfo.coloring == StreetMobilityInfo.COL_STATUS) smi.color = Color.yellow;
    		return remainingDistance;
    	...
    	if (nextLane.size() > 0) {
    		remainingDistance =+ smi.remainingIntersectionDistance + smi.nextRS.length - ((StreetMobilityInfo) nextLane.getLast()).remainingDist - RoadSegment.CAR_LENGTH - RoadSegment.SPACE;
    		remainingDistance = StrictMath.max(smi.remainingDist, remainingDistance);
    		if (debug) System.out.println("Time: "+JistAPI.getTime()+" node: "+id+" car in front on new road, new remain "+remainingDistance);
    		if (StreetMobilityInfo.coloring == StreetMobilityInfo.COL_STATUS) smi.color = new Color(255, 100, 0);
    		return remainingDistance;
    	}
    	
		remainingDistance += smi.remainingIntersectionDistance + smi.nextRS.length;
		if (debug) System.out.println("Time: "+JistAPI.getTime()+" node: "+id+" first car on new road, remain "+remainingDistance);
		if (StreetMobilityInfo.coloring == StreetMobilityInfo.COL_STATUS) smi.color = new Color(150, 255, 150);
    	return remainingDistance;
    }
     ...
     * 
     * @param f				field interface of the node
     * @param id			id of the node
     * @param centerLine	current position of the node
     * @param info			object of the node, needs to be a StreetMobilityInfo
     * 						otherwise cast will fail
     * 
     * the value of centerLine will be updated, if a car 
     * reaches a new road segment, moveToNextRaod is called, which
     * changes a variety of variables of the smi object
     * 
     * position of the field interface will be updated
     * 
     * the mobility model is by far not perfect, so crashes may happen 
     * (but are not detected and have no consequences)
     * due to that, in special cases (usualy decelerating from 
     * high speed), there may appear larger accelrations/decelerations 
     * than defined in StreetMobilityInfo
     * vehicles don't yield right of way
     * 
    ...
    public void next(FieldInterface f, Integer id, Location centerLine,
            MobilityInfo info) 
    {
    	boolean debug = false;
        ...
        try
        {
            ...
            if (debug && smi.current.getShapeIndex()>=0) System.out.println("Time: "+JistAPI.getTime()+" node: "+id+" X: "+centerLine.getX()+" Y: "+centerLine.getY()+" RS: "+smi.current.getSelfIndex()+" Shape: "+smi.current.getShapeIndex()+" smi.shape "+smi.ShapePointIndex);
            
            if (ENABLE_LANE_DISPLACEMENT && smi.remainingIntersectionDistance == 0) {
                centerLine = centerLine.add(getInverseDisplacement(smi)); // remove displacement from center
            }

            if (smi.remainingIntersectionDistance == 0) {
            	smi.remainingDist = StrictMath.max(centerLine.distance(smi.rsEnd) - smi.current.intersectionDistance, 0.0f);
                if (debug) System.out.println("Time: "+JistAPI.getTime()+" node: "+id+" X: "+centerLine.getX()+" Y: "+centerLine.getY()+" remaining distance: "+smi.remainingDist);
            } else {
            	smi.remainingDist = StrictMath.max(smi.remainingIntersectionDistance + smi.current.length, 0.0f);
                if (debug) System.out.println("Time: "+JistAPI.getTime()+" node: "+id+" X: "+centerLine.getX()+" Y: "+centerLine.getY()+" remaining distance: "+smi.remainingDist + " smi.distance on intersecrtion "+smi.remainingIntersectionDistance+ " length of roadsegment "+ smi.current.length);
            }
            ...
            if(smi.remainingDist<0.01 && smi.nextRS == null)
            {
            	...
            	if (debug) System.out.println("Time: "+JistAPI.getTime()+" node: "+id+" X: "+centerLine.getX()+" Y: "+centerLine.getY()+" removed");
            	return;
            } 
            ...
            float maxAcc = StrictMath.min(StrictMath.max(((smi.getAdjustedSpeed() - smi.currSpeed) / smi.stepTime), 0), StreetMobilityInfo.acceleration);
            
            // calculate distance at full acceleration
            float maxDist = (float)(0.5 * maxAcc * Math.pow(smi.stepTime, 2) + smi.currSpeed * smi.stepTime 
            				+ Math.pow(smi.currSpeed + maxAcc * smi.stepTime, 2.0) / StreetMobilityInfo.deceleration);
            
            // calculate distance at full deceleration
            float minDist = (float) StrictMath.max(-0.5 * StreetMobilityInfo.deceleration * Math.pow(smi.stepTime, 2) + smi.currSpeed * smi.stepTime, 0);
            
            // get free space
            float newDist = StrictMath.max(calcFreeDistance(smi, maxDist, id),0);

            if (debug) System.out.println("Time: "+JistAPI.getTime()+" node: "+id+" X: "+centerLine.getX()+" Y: "+centerLine.getY()+" maxAcc "+maxAcc+" maxDist "+maxDist+" minDist: "+minDist+" newDist: "+newDist);
        	
        	float newAcc;
        	// calculate breaking distance with one fourth of maximum deceleration
        	double brDistance = Math.pow(smi.currSpeed, 2) / StreetMobilityInfo.deceleration * 2.0f;
        	
        	// decision: accelerate or decelerate
        	// to smooth decelerations, brDistance is multiplicated with factor > 1
        	if (newDist > brDistance && newDist > 0.5 && smi.currSpeed * 2.0f < newDist) {
        		...
        		if (newDist > 0) {
        			...
        			newAcc = (float) StrictMath.max(-0.5 * Math.pow(smi.currSpeed, 2) / newDist * 1.1, -StreetMobilityInfo.deceleration);
        		} else {
        			newAcc = StrictMath.max(-smi.currSpeed, -StreetMobilityInfo.deceleration);
        		}
        	}
        	if (debug) System.out.println("Time: "+JistAPI.getTime()+" node: "+id+" X: "+centerLine.getX()+" Y: "+centerLine.getY()+" brDist "+brDistance+" newAcc "+newAcc);
        	
        	// calculate distance driven in this timestep
        	float distance = (float) StrictMath.max(0.5 * newAcc * Math.pow(smi.stepTime, 2) + smi.currSpeed * smi.stepTime, 0);
            ...
            if (distance > newDist) {
            	if (distance - newDist > 0.5f) { 
            		...
            		if (debug) System.out.println("Time: "+JistAPI.getTime()+" node: "+id+" X: "+centerLine.getX()+" Y: "+centerLine.getY()+" newDist "+newDist+" minDist: "+minDist+" maxDist: "+maxDist+" remain: "+smi.remainingDist);
            		if (debug) System.out.println("entering safety distance -----------------------------------------------");
            	}
                
            	if (newDist <= smi.remainingDist && distance > smi.remainingDist) {
                	if (distance - newDist > 0.5f) { 
                		if (debug) System.out.println("Our Lord prevented the car from slipping into the next street");
            ...
            if (debug) System.out.println("Time: "+JistAPI.getTime()+" node: "+id+" X: "+centerLine.getX()+" Y: "+centerLine.getY()
            		+" old speed "+oldSpeed+" new speed "+smi.currSpeed+" minDist: "+minDist+" maxDist: "+maxDist+" distance "+distance+" adjspeed "+smi.getAdjustedSpeed());
            ...
            if (distance > smi.remainingDist) {
            	...
            	if (debug) System.out.println("Time: "+JistAPI.getTime()+" node: "+id+" entered new road "+smi.current.getSelfIndex()+" on lane: "+smi.current.getLane(smi.currentLane));

            	if (smi.current.getSelfIndex() == rsIndex) { // dead end and turned around
            		// slow down
            		if (debug) System.out.println("Time: "+JistAPI.getTime()+" node: "+id+" u-turn");
                	if (StreetMobilityInfo.coloring == StreetMobilityInfo.COL_STATUS) smi.color = Color.blue;
            	} else {
                	distance -= rd;
                	if (distance < 0.0f) { 
...
//            if (smi.current.getShapeIndex()>=0) smi.color = Color.blue; 
            ...
            if (debug) System.out.println("Time: "+JistAPI.getTime()+" node: "+id+" remaining distance on intersection "+smi.remainingIntersectionDistance);

    		if (smi.remainingIntersectionDistance <= distance) {
    			if (smi.remainingIntersectionDistance > 0.0f) {
                	// car left intersection
                	if (debug) System.out.println("Time: "+JistAPI.getTime()+" node: "+id+" old distance: "+distance+" intersection distance "+smi.current.intersectionDistance);
            	...
            	if (debug) System.out.println("Time: "+JistAPI.getTime()+" node: "+id+" set car on new coordinates "+centerLine.toString()+", moved distance: "+distance);
            } else {
            	if (debug) System.out.println("Time: "+JistAPI.getTime()+" node: "+id+" length of intersection "+smi.laneConnector.getLength()+" remaining distance on intersection "+smi.remainingIntersectionDistance+ " distance to drive "+distance+" resulting distance on intersection "+(smi.laneConnector.getLength() - smi.remainingIntersectionDistance + distance));
            	// car still on intersection
            	centerLine = smi.laneConnector.getPointOnConnection(smi.laneConnector.getLength() - smi.remainingIntersectionDistance + distance);
            	if (debug) System.out.println("Time: "+JistAPI.getTime()+" node: "+id+" set car on new coordinates "+centerLine.toString()+", moved distance: "+distance);
            ...
            if (ENABLE_LANE_DISPLACEMENT && smi.remainingIntersectionDistance == 0) {
            	...
            	if (debug) System.out.println("Time: "+JistAPI.getTime()+" node: "+id+" new coordinates "+centerLine.toString()+" offset "+smi.offset.toString());
            }
            ...
            if (Visualizer.getActiveInstance()!=null) { 
            	Visualizer.getActiveInstance().updateNodeLocation(
            			centerLine.getX(), centerLine.getY(), id.intValue());
    ...
    /** 
     * sets a node on a new road segment and updates a variety of values of the StreetMobilityInfo object
     * @param id		id of the node (just for debug output)
     * @param nextLane	the lane to set the car on (there has to be enough space for the node
     * @param smi		StreetMobilityInfo object of the node
     * @param is		the intersection which is passed by the node
     * 
     * enhanced by parts of the old "step" function, which was replaced by "calcDistance"
     * 
     * modified by
     * @author Bjoern Wiedersheim
     */
    
    private void moveToNextRoad(Integer id, LinkedList<StreetMobilityInfo> nextLane, StreetMobilityInfo smi, Intersection is) {
    ...
    private static float signum(float f) {
    	if (f<0) return -1.0f;
    	if (f>0) return 1.0f;
    	if (f==Float.NaN) return Float.NaN;
    	return 0.0f;
    }
    ...
    public Location getLaneDisplacement(StreetMobilityInfo smi) {
        ...
        Location finish;
        Location normalized;
        float displacement = LANE_WIDTH * (smi.current.getNumberOfLanes() - 1.0f - smi.current.getLane(smi.currentLane) + 0.5f);
    ...
    public Location degreesToMeters(Location loc) {
        ...
        return new Location.Location2D((loc.getX() - minX) / getLongitudeMetersPerDegree(),
	...
	{
            System.out.println("StreetMobility::loadSegmentFile: Segments file does not exist at the specified location!");
            ...
            float distance)
    {
        ...
        {
            if (distance < 0.01f && distance > 0.0f) return end;
    ...
    */
     ...
     * @param ip the ip of the car to inspect
     */
