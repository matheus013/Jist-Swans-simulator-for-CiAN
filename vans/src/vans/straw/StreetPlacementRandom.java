 ...
 * RCS: $Id: StreetPlacementRandom.java,v 1.1.1.1 2006/02/20 23:26:43 drchoffnes
 * Exp $
...
public class StreetPlacementRandom implements Placement
{
     ...
     */
    public StreetPlacementRandom(Location bl, Location tr, StreetMobility smr, double stdDev) {
        init(bl.getX(), bl.getY(), tr.getX(), tr.getY(), smr);
        this.stdDev = stdDev;
    }
     ...
     */
    public StreetPlacementRandom(Location bl, Location tr, StreetMobility smr, double stdDev, double stepTime) {
        init(bl.getX(), bl.getY(), tr.getX(), tr.getY(), smr);
        this.stdDev = stdDev;
     ...
     *            the StreetMobility object
     */
    private void init(float xBL, float yBL, float xTR, float yTR, StreetMobility smr) {
        this.xBL = xBL;
        this.yBL = yBL;
        this.xTR = xTR;
        this.yTR = yTR;
    ...
    public Location getNextLocation() {
        ...
        remainingDist = rs.getCarSpacing(smri.currSpeed, smri.spacingBeta, smri.spacingGamma) * position;
        ...
        if (direction == TO_START) {
            ...
            if (smri.current.getShapeIndex() == -1) {
                ...
                initialLocation = sm.pointAt(rs.getEndPoint(), smri, rs.getLength() - remainingDist);
            }
        } else {
            // we have no intermediate segments
            if (smri.current.getShapeIndex() == -1) {
