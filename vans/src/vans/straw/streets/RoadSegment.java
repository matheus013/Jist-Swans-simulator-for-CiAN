...
//    private LinkedList<StreetMobilityInfo> carsToStart[];
     ...
     *
     * @param smi   id of node added to list... must use 1-based number
    ...
    public LinkedList<StreetMobilityInfo> addNode(StreetMobilityInfo smi, Location rsEnd, Vector nodes)
    {
        ...
        if (position == -1) freeLane.addLast(smi);
        else {
    ...
    public void checkLane(LinkedList<StreetMobilityInfo> list) {
        ...
        while (li.hasNext())
        {
            ...
            {
                System.err.println("Road: "+selfIndex+" front: "+front.id+" rd: "+front.getRemainingDist()+" behind: "+behind.id+" rd: "+behind.getRemainingDist()+" car before behind: "+behind.getNextCar().id);
    ...
    public void setLength(Shape sh) {
        ...
        if (StreetMobility.ENABLE_INTERSECTION_TRAFFIC) {
        	intersectionDistance = (float) StrictMath.min(numberOfLanes * 2.0f * StreetMobility.LANE_WIDTH, length/3.0);
        }
     ...
     *         in this direction. Prefer lanes on the right side (small indices).
     */
    ...
    */
     ...
     * @return
     */
    public LaneConnector getLaneConnector(StreetMobilityInfo smi, LinkedList<StreetMobilityInfo> nextLane, Shape startShape, Shape endShape) {
    	...
    	if (startPoint.distance(smi.getRSEnd()) > endPoint.distance(smi.getRSEnd())) {
    		...
    		if (!laneConnectionsToEnd.get(laneNumber).containsKey(nextLane)) {
    			laneConnectionsToEnd.get(laneNumber).put(nextLane, new LaneConnector(smi, smi.getNextRS().getLane(nextLane), startShape, endShape));
    		}
    		return laneConnectionsToEnd.get(laneNumber).get(nextLane);
    	} else {
    		int laneNumber = smi.getLane();
    		if (!laneConnectionsToStart.get(laneNumber).containsKey(nextLane)) {
    			laneConnectionsToStart.get(laneNumber).put(nextLane, new LaneConnector(smi, smi.getNextRS().getLane(nextLane), startShape, endShape));
    		}
