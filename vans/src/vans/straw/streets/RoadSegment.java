...
public class RoadSegment
{
     ...
     * @param roadClass
     */
    public RoadSegment(int startAddressLeft, int endAddressLeft, int startAddressRight, int endAddressRight,
            int streetIndex, int shapeIndex, Location startPoint, Location endPoint, int selfIndex, char roadClass) {
        super();
        ...
        lanesToEnd = new ArrayList<LinkedList<StreetMobilityInfo>>(numberOfLanes);
        lanesToStart = new ArrayList<LinkedList<StreetMobilityInfo>>(numberOfLanes);
        for (int i = 0; i < numberOfLanes; i++) {
            lanesToEnd.add(new LinkedList<StreetMobilityInfo>());
            lanesToStart.add(new LinkedList<StreetMobilityInfo>());
        }

        laneConnectionsToEnd = new ArrayList<Map<LinkedList<StreetMobilityInfo>, LaneConnector>>(numberOfLanes);
        laneConnectionsToStart = new ArrayList<Map<LinkedList<StreetMobilityInfo>, LaneConnector>>(numberOfLanes);
        for (int i = 0; i < numberOfLanes; i++) {
            laneConnectionsToEnd.add(new HashMap<LinkedList<StreetMobilityInfo>, LaneConnector>());
            laneConnectionsToStart.add(new HashMap<LinkedList<StreetMobilityInfo>, LaneConnector>());
        }
    }
     ...
     * @return
     */
    public LinkedList<StreetMobilityInfo> addNode(StreetMobilityInfo smi, Location rsEnd, Vector nodes) {
        ...
        if (freeLane.size() > 0) {
            ...
            {
                if (freeLane.size() == 1) {
                    if (((StreetMobilityInfo) freeLane.getFirst()).getRemainingDist() > 4 * CAR_LENGTH) {
                        position = 0;
                    }
                } else {
    ...
    public LinkedList<StreetMobilityInfo> getFreeLane(Location rsEnd) {
        ...
        if (freeLane.size() > 0 && freeLane.getLast().getRemainingDist() > length - CAR_LENGTH - SPACE) {
            return null;
        }
        return freeLane;
    }
    ...
    public void checkLane(LinkedList<StreetMobilityInfo> list) {
        ...
        while (li.hasNext()) {
            ...
            if (behind.getNextCar() != front || front.getRemainingDist() >= behind.getRemainingDist()) {
                System.err.println("Road: " + selfIndex + " front: " + front.id + " rd: " + front.getRemainingDist()
                        + " behind: " + behind.id + " rd: " + behind.getRemainingDist() + " car before behind: "
                        + behind.getNextCar().id);
    ...
    public void setLength(Shape sh) {
        ...
        if (StreetMobility.ENABLE_INTERSECTION_TRAFFIC) {
            intersectionDistance = (float) StrictMath.min(numberOfLanes * 2.0f * StreetMobility.LANE_WIDTH,
                    length / 3.0);
        }
        length -= 2.0f * intersectionDistance;
        this.maxCars = (int) StrictMath.max((StrictMath.floor(length / (CAR_LENGTH + SPACE))), 1);
    }
    ...
    public LinkedList<StreetMobilityInfo> getCarsToEnd() {
        ...
        Iterator<LinkedList<StreetMobilityInfo>> it_lanes = lanesToEnd.listIterator();
    ...
    public LinkedList<StreetMobilityInfo> getCarsToStart() {
        ...
        Iterator<LinkedList<StreetMobilityInfo>> it_lanes = lanesToStart.listIterator();
        while (it_lanes.hasNext()) {
            LinkedList<StreetMobilityInfo> lane = it_lanes.next();
            if (lane.size() == 0) {
                return lane;
     ...
     * @param mobInfo
     */
    public void removeNode(StreetMobilityInfo smi, LinkedList<StreetMobilityInfo> currentLane, Vector mobInfo) {
        ...
        if (currentLane.size() > 0) {
            ...
            if (((StreetMobilityInfo) currentLane.getFirst()) == smi) {
                ...
                System.err.println("CarList: " + printCarList(currentLane, mobInfo));

                throw new RuntimeException("Removed node that wasn't" + " at front of queue");
            }
            return;
        } else // empty list
        {
    ...
    public String printCarList(LinkedList<StreetMobilityInfo> currentLane, Vector mobInfo) {
        ...
        while (it.hasNext()) {
            ...
            s += mobInfo.indexOf(smri) + " - remaining: " + smri.getRemainingDist() + " - next: " + smri.getNextCar()
                    + "\n";
        }
        return s;
    }
    ...
    public String printStreetName(HashMap streets) {
        ...
        return "(" + startAddressLeft + "/" + startAddressRight + ") - (" + "(" + endAddressLeft + "/"
                + endAddressRight + ")" + ((StreetName) streets.get(new Integer(streetIndex))).toString() + " ["
                + selfIndex + "] " + startPoint + " - " + endPoint;
    }
     ...
     * @return
     */
    public LaneConnector getLaneConnector(StreetMobilityInfo smi, LinkedList<StreetMobilityInfo> nextLane,
            Shape startShape, Shape endShape) {
        // test in which direction the car drives
        if (startPoint.distance(smi.getRSEnd()) > endPoint.distance(smi.getRSEnd())) {
            int laneNumber = smi.getLane();
            if (!laneConnectionsToEnd.get(laneNumber).containsKey(nextLane)) {
                laneConnectionsToEnd.get(laneNumber).put(nextLane,
                        new LaneConnector(smi, smi.getNextRS().getLane(nextLane), startShape, endShape));
            }
            return laneConnectionsToEnd.get(laneNumber).get(nextLane);
        } else {
            int laneNumber = smi.getLane();
            if (!laneConnectionsToStart.get(laneNumber).containsKey(nextLane)) {
                laneConnectionsToStart.get(laneNumber).put(nextLane,
                        new LaneConnector(smi, smi.getNextRS().getLane(nextLane), startShape, endShape));
            }
