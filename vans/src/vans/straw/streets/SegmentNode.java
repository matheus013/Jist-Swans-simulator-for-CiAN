    ...
    public float getEstimatedCost(AStarNode node) {
		...
		switch(heuristic)
		{
			...
			case MANHATTAN_DISTANCE:
				h = (float) (StrictMath.abs(n.point.getX() - end.getX()) +
				StrictMath.abs(n.point.getY() - end.getY()));
				break;
            default:
    ...
    */
    public List getNeighbors() {
		...
		while (it.hasNext())
		{
			...
			if(DEBUG) System.out.println(rs.printStreetName(info.streets));			
			...
			if (firstNode && rs.getSelfIndex()==segmentID) // sometimes the only way to get somewhere is for the node to turn around
			{
				...
				temp.costFromStart = this.costFromStart + 400; // big penalty for turning around
