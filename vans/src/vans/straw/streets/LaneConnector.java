...
public class LaneConnector
{
    ...
    LaneConnector(StreetMobilityInfo smi, int endLane, Shape startShape, Shape endShape) {
        ...
        if (debug)
            System.out.println("startRS " + smi.getCurrentRS().startPoint.toString() + " "
                    + smi.getCurrentRS().endPoint.toString() + " " + smi.getCurrentRS().intersectionDistance
                    + " endRS " + smi.getNextRS().startPoint.toString() + " " + smi.getNextRS().endPoint.toString()
                    + " " + smi.getNextRS().intersectionDistance + " rsEnd " + smi.getRSEnd().toString());

        // find out start and endpoint of the current road segment
        if (smi.getCurrentRS().endPoint.distance(smi.getRSEnd()) < smi.getCurrentRS().startPoint.distance(smi
                .getRSEnd())) {
        ...
        if (debug)
            System.out.println("startp " + startPoint.toString() + " " + endPoint.toString() + " "
                    + smi.getCurrentRS().intersectionDistance + " endRS " + smi.getNextRS().startPoint.toString() + " "
                    + startPoint.distance(endPoint));

        // calculate the lane displacement
        length = startPoint.distance(endPoint);
        xoffset = (startPoint.getY() - endPoint.getY()) / length * StreetMobility.LANE_WIDTH
                * (smi.getCurrentRS().getNumberOfLanes() - 1.0f - smi.getLane() + 0.5f);
        yoffset = -(startPoint.getX() - endPoint.getX()) / length * StreetMobility.LANE_WIDTH
                * (smi.getCurrentRS().getNumberOfLanes() - 1.0f - smi.getLane() + 0.5f);
        if (debug)
            System.out
                    .println("startRS offset : "
                            ...
                            + length
                            + " disp: "
                            + (StreetMobility.LANE_WIDTH * (smi.getCurrentRS().getNumberOfLanes() - 1.0f
                                    - smi.getLane() + 0.5f)));

        // calculate the first two points
        p1 = new Location.Location2D(endPoint.getX() + (startPoint.getX() - endPoint.getX())
                * smi.getCurrentRS().intersectionDistance / length + xoffset, endPoint.getY()
                + (startPoint.getY() - endPoint.getY()) * smi.getCurrentRS().intersectionDistance / length + yoffset);
        p2 = new Location.Location2D(endPoint.getX() + xoffset, endPoint.getY() + yoffset);
        ...
        // calculate the lane displacement
        length = startPoint.distance(endPoint);
        xoffset = -(endPoint.getY() - startPoint.getY()) / length * StreetMobility.LANE_WIDTH
                * (smi.getNextRS().getNumberOfLanes() - 1.0f - endLane + 0.5f);
        yoffset = (endPoint.getX() - startPoint.getX()) / length * StreetMobility.LANE_WIDTH
                * (smi.getNextRS().getNumberOfLanes() - 1.0f - endLane + 0.5f);
        if (debug)
            System.out.println("endRS offset: " + xoffset + " " + yoffset + " lane " + endLane + " lanes "
                    + smi.getNextRS().numberOfLanes + " lane width " + StreetMobility.LANE_WIDTH + " start: "
                    + startPoint.toString() + " end: " + endPoint.toString() + " dist: " + length + " disp: "
                    + (StreetMobility.LANE_WIDTH * (smi.getNextRS().getNumberOfLanes() - 1.0f - smi.getLane() + 0.5f)));

        // calculate the second two points
        p4 = new Location.Location2D(startPoint.getX() + (endPoint.getX() - startPoint.getX())
                * smi.getNextRS().intersectionDistance / length + xoffset, startPoint.getY()
                + (endPoint.getY() - startPoint.getY()) * smi.getNextRS().intersectionDistance / length + yoffset);
        p3 = new Location.Location2D(startPoint.getX() + xoffset, startPoint.getY() + yoffset);

        if (debug)
            System.out.println("p1-p2 " + p1.distance(p2) + " p2-p3 " + p2.distance(p3) + " p3-p4 " + p3.distance(p4));
        if (debug)
            System.out.println("new LaneConnector created " + p1.toString() + " " + p2.toString() + " " + p3.toString()
                    + " " + p4.toString());
        if (RightTurn(p1, p2, p3, p4)) {
            ...
            p2 = new Location.Location2D((float) (p1.getX() + (isp.getX() - p1.getX()) * fak),
                    (float) (p1.getY() + (isp.getY() - p1.getY()) * fak));
            p3 = new Location.Location2D((float) (p4.getX() + (isp.getX() - p4.getX()) * fak),
                    (float) (p4.getY() + (isp.getY() - p4.getY()) * fak));
            if (debug)
                System.out.println("p1-p2 " + p1.distance(p2) + " p2-p3 " + p2.distance(p3) + " p3-p4 "
                        + p3.distance(p4));
            if (debug)
                System.out.println("new LaneConnector created " + p1.toString() + " " + p2.toString() + " "
                        + p3.toString() + " " + p4.toString());
        }
        if (debug && (p1.distance(p2) > 100.0f || p2.distance(p3) > 100.0f || p3.distance(p4) > 100.0f)) {
            System.out.println("extreme wide intersection: " + p1.toString() + " " + p2.toString() + " "
                    + p3.toString() + " " + p4.toString());
            System.out.println("startLane: " + smi.getLane() + " endLane: " + endLane);
            System.out.println("startRS " + smi.getCurrentRS().startPoint.toString() + " "
                    + smi.getCurrentRS().endPoint.toString() + " " + smi.getCurrentRS().intersectionDistance
    ...
    private void createPoints(ArrayList<curvePoint> curvePoints, Location p1, Location p2, Location p3, Location p4) {
        if (p1.distance(p4) <= resolution) {
            if (debug)
                System.out.println("new point on intersection " + p1.toString() + " distance to next point "
                        + p1.distance(p4));
    ...
    private Location calcIntersection(Location p1, Location p2, Location q1, Location q2) {
        ...
        if (n < 0.1) {
            ...
            // q1 is returned
            if (debug)
                System.out.println("CalcIntersection: n=0 " + p1.toString() + " " + p2.toString() + " " + q1.toString()
                        + " " + q2.toString());
            return new Location.Location2D(p2.getX() + (q1.getX() - p2.getX()) / 2.0f, p2.getY()
                    + (q1.getY() - p2.getY()) / 2.0f);
        }
        double l = (q1.getY() * dpx - p1.getY() * dpx - q1.getX() * dpy + p1.getX() * dpy) / n;
        if (debug)
            System.out.println("CalcIntersection: n: " + n + " l: " + l + " dpx " + dpx + " dpy " + dpy + " dqx " + dqx
                    + " dqy " + dqy);
    ...
    public Location getPointOnConnection(float distance) {
        ...
        while (it_curvePoints.hasNext()) {
            ...
            if (d + cp.distance >= distance) {
                ...
                if (debug)
                    System.out.println("getPointonConnection d: " + d + " distance to next point " + cp.distance
                            + " distance on intersection " + distance + " dd: " + dd);
                curvePoint cp2 = (curvePoint) it_curvePoints.next();
                return new Location.Location2D((float) (cp.point.getX() + (cp2.point.getX() - cp.point.getX()) * dd
                        / cp.distance), (float) (cp.point.getY() + (cp2.point.getY() - cp.point.getY()) * dd
                        / cp.distance));
            }
            d += cp.distance;
        }
