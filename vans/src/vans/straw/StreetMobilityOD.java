...
public class StreetMobilityOD extends StreetMobility
{
     ...
     */
    public StreetMobilityOD(String segmentFile, String streetFile, String shapeFile, int degree,
        ...
        if (smiod.path.size() > 0 || config == Constants.MOBILITY_STREET_RANDOM) {
            ...
            if (smiod.path.size() == 0) // calculate new path
            {
                ...
                while (!valid || smiod.path.size() < 4) {
                    ...
                    while (nextDest == null || nextDest.getSelfIndex() == smiod.current.getSelfIndex()
                            || nextDest.getEndPoint().distance(smiod.current.getEndPoint()) > threshold) // get
                                                                                                         // new
                                                                                                         // destination
                    {
            ...
            while (smiod.path.size() > 0) {
                ...
                if (smiod.rsEnd.distance(rs.getStartPoint()) == 0 || smiod.rsEnd.distance(rs.getEndPoint()) == 0) {
                    if (DEBUG_OD)
    ...
    public void checkPath(StreetMobilityInfoOD smiod) {
        ...
        if (smiod.path.size() > 1) {
            ...
            if (smiod.rsEnd.distance(rsTemp.getStartPoint()) == 0 || smiod.rsEnd.distance(rsTemp.getEndPoint()) == 0) {
                if (DEBUG_OD)
                    System.out.println("Removing first node in list because it is redundant!");
                smiod.path.remove(0);
        ...
        if (smiod.nextEnd.distance(rsTemp.getEndPoint()) > INTERSECTION_RESOLUTION
            ...
            smiod.path.addFirst(new SegmentNode(smiod.nextEnd, smiod.current.getSelfIndex(), true, false));
        }
    ...
    public boolean calculatePath(StreetMobilityInfoOD smiod, RoadSegment origin, Location nextEnd,
        ...
        boolean endStart;
        Location endPoint;
        if (destination.getStartPoint().distance(nextEnd) < destination.getEndPoint().distance(nextEnd)) {
        ...
        SegmentNode startNode = new SegmentNode(nextEnd, origin.getSelfIndex(), origin.getStartPoint()
                .distance(nextEnd) == 0, true);
