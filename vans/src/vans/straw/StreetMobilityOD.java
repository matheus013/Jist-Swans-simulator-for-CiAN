 ...
 * 
 *         The StreetMobilityOD class supports mobility between origins and
 *         destinations.
 */
public class StreetMobilityOD extends StreetMobility {
	...
	HashMap hm = null;
	/**
	 * Helper class for referencing StreetMobility objects in the AStarSearch
	 * class.
	 */
	...
	/**
	 * maximum distance between origin and destination (5 mi) This reduces the
	 * time required for A*
	 */
	 ...
	 * 
	 * @param segmentFile
	 *            the location of the file containing segments
	 * @param streetFile
	 *            the location of the file containing streets
	 * @param shapeFile
	 *            the location of the file containing shapes
	 * @param degree
	 *            the degree of the quad tree
	 * @param r
	 *            the random object to use for repeatability
	 */
	public StreetMobilityOD(String segmentFile, String streetFile,
			String shapeFile, int degree, Location.Location2D bl,
			Location.Location2D tr, Random r) {

		super(segmentFile, streetFile, shapeFile, degree, bl, tr);
		this.rnd = r;

		segmentNodes = new Vector(segments.size()); // for caching segmentNode
													// objects
		for (int i = 0; i < segments.size(); i++) {
			segmentNodes.add(i, null);
		}
		sni = new SegmentNodeInfo(segments, shapes, intersections, streets,
				segmentNodes);
	...
	public MobilityInfo init(FieldInterface f, Integer id, Location loc) {
		...
		StreetMobilityInfoOD smodi = (StreetMobilityInfoOD) mobInfo
				.lastElement();
		// calculate first path
		if (smodi.path == null) {
			calculatePath(smodi, (RoadSegment) originRS.get(id.intValue()),
					smodi.rsEnd, (RoadSegment) destinationRS.get(id.intValue()));
		}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see jist.swans.field.StreetMobility#setNextRoad(jist.swans.field.
	 * StreetMobilityInfo)
	 */
		...
		if (smiod.path.size() > 0 || config == Constants.MOBILITY_STREET_RANDOM) {
			...
			if (smiod.path.size() == 0) // calculate new path
			{
				...
				while (!valid || smiod.path.size() < 4) {
					...
					while (nextDest == null
							|| nextDest.getSelfIndex() == smiod.current
									.getSelfIndex()
							|| nextDest.getEndPoint().distance(
									smiod.current.getEndPoint()) > threshold) // get
																				// new
																				// destination
					{
					...
					valid = calculatePath(smiod, smi.current, smi.rsEnd,
							nextDest);
				}
				if (DEBUG_OD)
					System.out.println("Current rsEnd: " + smi.rsEnd);
				if (DEBUG_OD)
					System.out.println("Current road: "
							+ smi.current.printStreetName(sni.streets));
	...
	public void checkPath(StreetMobilityInfoOD smiod) {
		SegmentNode temp;
		RoadSegment rsTemp;
		// if the current end point is equal to an end point of the second road
		// in
		// the list, then remove the first road in the list because it's
		// redundant
		if (smiod.path.size() > 1) {
			...
			if (smiod.rsEnd.distance(rsTemp.getStartPoint()) == 0
				...
				if (DEBUG_OD)
					System.out
							.println("Removing first node in list because it is redundant!");
				smiod.path.remove(0);
		...
		if (smiod.nextEnd.distance(rsTemp.getEndPoint()) > INTERSECTION_RESOLUTION
			...
			smiod.path.addFirst(new SegmentNode(smiod.nextEnd, smiod.current
					.getSelfIndex(), true, false));
		}

	}

	/**
	 * This method calculates the path from the origin to the destination road
	 * segment.
	 * 
	 * @param smiod
	 *            The SMIOD object.
	 */
	public boolean calculatePath(StreetMobilityInfoOD smiod,
			RoadSegment origin, Location nextEnd, RoadSegment destination) {

		// TODO the caller should determine direction
		if (DEBUG_OD) {
			System.out.println("Calculating path...");
			System.out.println("Origin:      "
					+ origin.printStreetName(streets));
			System.out.println("Destination: "
					+ destination.printStreetName(streets));
			System.out.println("Distance: "
					+ origin.getStartPoint().distance(
							destination.getStartPoint()));
		}
		...
		boolean endStart;
		Location endPoint;
		if (destination.getStartPoint().distance(nextEnd) < destination
				.getEndPoint().distance(nextEnd)) {
		...
		SegmentNode startNode = new SegmentNode(nextEnd, origin.getSelfIndex(),
				origin.getStartPoint().distance(nextEnd) == 0, true);
		...
		for (int i = 1; i < smiod.path.size(); i++) {
			if (((SegmentNode) smiod.path.get(smiod.path.size() - i)).segmentID == ((SegmentNode) smiod.path
					.get(smiod.path.size() - (i + 1))).segmentID) {
				if (DEBUG_OD)
					System.out.println("Removed redundant entry!");
				smiod.path.remove(smiod.path.size() - i);
			}
		}
		if (((SegmentNode) smiod.path.get(0)).segmentID == origin
				.getSelfIndex()) {
			if (DEBUG_OD)
				System.out.println("Removed redundant initial entry!");
			smiod.path.remove(0);
		}

		if (DEBUG_OD)
			printPath(smiod.path);
