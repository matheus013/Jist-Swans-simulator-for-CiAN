 ...
 * 
 *         The StreetMobilityInfo class contains state for all street mobility
 *         models.
 */
public class StreetMobilityInfo implements MobilityInfo {
	...
	public static float deceleration = 5 * 2.23f; // 25 mph per second, to avoid
													// crashes
	...
	/**
	 * color of the vehicle, set by StreetMobility.next depending on node status
	 * or acceleration
	 */
	...
	/*
	 * the color of the nodes reflects the acceleration/deceleration blue:
	 * deceleration white: no acceleration/deceleratioin red: acceleration
	 * yellow: maximum speed
	 */

	static final int COL_STATUS = 2;
	/*
	 * the color of the nodes reflects the status of the nodes green: actual
	 * road segment is free lightgreen: actual and next road segment is free
	 * blue: u-turn orange: driving behind vehicle darkorange: driving behind
	 * vehicle on next road segment cyan: slow down before intersection red:
	 * wait at stop sign or traffic light yellow: wait at intersection for free
	 * lane
	 */
	...
	/*
	 * Bjoern Wiedersheim change: changed the type of waiting from boolean to
	 * float
	 */
	...
	public void setMaxSpeed(float limit) {
		// vague evidence online seems to indicate that this distribution is
		// Gaussian
	...
	public int getAdjustedSpeed() {
		...
		if (extraSpeed < 0) {
			return (int) (current.getSpeedLimit() + (extraSpeed > min ? extraSpeed
					: min));
		} else
			return (int) (current.getSpeedLimit() + (extraSpeed < max ? extraSpeed
					: max));
	}
