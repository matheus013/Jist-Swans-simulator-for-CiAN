...
public class RouteCGGC implements RouteInterface.Cggc {
	...
	private static Logger log = Logger.getLogger(RouteCGGC.class.getName());
	  ...
	  public static class GeoBeacon implements Message {
		...
		public GeoBeacon(Location pos) {senderPos=pos;}
	    public int getSize() { return 0; /*senderPos.size();*/ } // comparability to cggc ns-2
	    public void getBytes(byte[] b, int offset) { throw new RuntimeException("not implemented"); }
	    public String toString() { return "Beacon" + senderPos; }
	  }
	...
	public RouteCGGC(RadioData rd) {
		this.self = (RouteInterface.Cggc) JistAPI.proxy(this, RouteInterface.Cggc.class);
		this.localRadio = rd;
	...
	public void peek(NetMessage msg, byte interfaceId, MacAddress lastHop) {
		...
		// process options
        NetMessage.IpOption ipOpt = (NetMessage.IpOptionHopLoc)ipMsg.getOptions().get(hopLoc);
        if (ipOpt!=null) updateLmp(interfaceId, ((NetMessage.IpOptionHopLoc)ipOpt).getLoc() , lastHop);
	...
	protected boolean sendLine(NetMessage.Ip ipMsg) {
		...
		while (it.hasNext()) {
			...
			if (compareMe >= distance) continue;
			//success
			distance = compareMe;
		...
		if (nextHop != null) {
		    ...
		    if (stats != null) { stats.sentPacket++;}
			return true;
		}
	...
	public void receive(Message msg, NetAddress src, MacAddress lastHop,
		...
		//log.info(JistAPI.getTime() +" node="+ net.getAddress() + " received" + msg + " from=" + 
		// src + " for=" + dst + " at="+ localRadio.getLoc() +" via="+lastHop+" on="+macId+" prio="+priority+" ttl="+ttl);
	...
	private void updateLmp(byte macId, Location l, MacAddress lastHop) {
		...
		while (it.hasNext()) {
			...
			if (pe.receiveTime < timeLimit) continue;
			...
			// movement of this node might make old beacons attractive again. this does
			// not necessarily improve delivery though because beacons quickly get stale
	 ...
	 *
	 * @param stats cggc statistics object
	 */
	...
	public void scheduleNextBeacon() {
		long delta = BEACONING_INTERVAL - BEACONING_JITTER + Util.randomTime(2*BEACONING_JITTER);
	...
	public void timeout() {
		...
		if (! doBeaconing ) return;
		
		// another beacon was sent and scheduled, skip this event
		if (JistAPI.getTime()<nextBeaconing) return;
