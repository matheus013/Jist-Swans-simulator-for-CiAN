...
public class AppCbrGeo extends AppCbrBase {
	...
	public AppCbrGeo(int sendRate, int waitTimeBetween, int waitTimeStart,
			...
			int nodeCount, int duration, RadioData radio, Placement place, Location f) {
	...
	protected void beginMessageSequence() {
		...
		if (place != null) {
			...
			/*
			float minf = (float)StrictMath.sqrt(100*100*StrictMath.PI); // minimum radius
			float deltaf = (float)StrictMath.sqrt(300*300*StrictMath.PI)-minf;
			float f = ((Constants.random.nextFloat()*deltaf)/2 + minf)/2;
			
			Location.Location2D target = (Location.Location2D)place.getNextLocation();
			Location.Location2D target_bl = new Location.Location2D(target.getX()-f,target.getY()-f);
			Location.Location2D target_tr = new Location.Location2D(target.getX()+f,target.getY()+f);
			dest = new Region.Rectangle(target_bl,target_tr);
			*/
			...
			float radius = Constants.random.nextInt(200)+100;
			do{
				target = (Location.Location2D)place.getNextLocation();
			} while ( (target.getX()<radius) || (target.getY()<radius) || 
					(target.getX() > (fieldSize.getX()-radius)) || 
					(target.getY() > (fieldSize.getY()-radius)) );
			dest = new Region.Circle(target,radius);
			*/
...
//					.getStartPoint(), (Location.Location2D) rs.getEndPoint());
			...
			float radius = Constants.random.nextInt(200)+100;
			dest = new Region.Circle((Location.Location2D) rs.getStartPoint(),radius); 
