...
public class AppCbrGeo extends AppCbrBase
{
    ...
    public AppCbrGeo(int sendRate, int waitTimeBetween, int waitTimeStart, int waitTimeEnd, int packetsPerConnection,
            int nodeId, int nodeCount, int duration, RadioData radio, Placement place, Location f) {

        super(sendRate, waitTimeBetween, waitTimeStart, waitTimeEnd, packetsPerConnection, nodeId, nodeCount, duration);
        localRadio = radio;
        this.place = place;
        this.fieldSize = f;
    }
    ...
    public AppCbrGeo(int sendRate, int waitTimeBetween, int waitTimeStart, int waitTimeEnd, int packetsPerConnection,
            int nodeId, int nodeCount, int duration, RadioData radio, StreetMobility sm) {
    ...
    protected void beginMessageSequence() {
        ...
        if (place != null) {
            ...
            RoadSegment rs = (RoadSegment) sm.getSegments().get(Constants.random.nextInt(sm.getSegments().size()));
        ...
        for (int i = 0; i < this.packetsPerConnection; i++) {
            ...
            // send message
            netEntity.send(msg, addr, Constants.NET_PROTOCOL_UDP, Constants.NET_PRIORITY_NORMAL, CBR_MESSAGE_TTL);
