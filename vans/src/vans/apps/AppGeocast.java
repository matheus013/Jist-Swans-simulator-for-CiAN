 ...
 *
 */
public class AppGeocast implements AppInterface, NetInterface.NetHandler, StatsCollector {
	...
	public static class MessageGeocast implements Message {
		...
		public String toString() {
			return "id="+msgID+" sentTime="+sentTime+" recvTime="+recvTime;
		}
	}
	...
	public AppGeocast(int nodeId, RadioData radio,
			int minRadius, int maxRadius, Location.Location2D[] srcArea,
			double messagesPerMinute, int waitTimeStart, int waitTimeEnd, int duration) {
		this.nodeId = nodeId;
		this.radio = radio;
		...
		log.debug("Created AppGeocast at node="+nodeId+" minRadius="+minRadius+" maxRadius="+maxRadius+
				" msgPerMinute="+msgPerMinute+" waitStart="+waitTimeStart+" waitEnd="+waitTimeEnd+
				" duration="+duration);
		
		if (duration - waitTimeEnd < waitTimeStart) {
			log.warn("ERR: waitTimeStart (="+waitTimeStart+" and waitTimeEnd (="+waitTimeEnd+
					") do not allow sending messages in duration of simulation (="+duration+")");
		}
	}
	...
	public void run(String[] args) {
		...
		if (args == null) {
			...
			long period = Math.round( (60 * Constants.SECOND) / this.msgPerMinute );
			long waitUntilMessage = Math.round( Constants.random.nextDouble() * period );
			long waitAfterMessage = period - waitUntilMessage;
			
			log.debug("node="+this.nodeId+" period="+StringUtils.timeSeconds(period)+" waitUntil="+StringUtils.timeSeconds(waitUntilMessage));
			...
			// send message. 
			// It is important to come to this point without having a delay before, because
		...
		// if it's still before the deadline for new messages, schedule next round
		log.debug("next run? t="+StringUtils.timeSeconds()+" deadline="+StringUtils.timeSeconds((duration - waitTimeEnd) * Constants.SECOND));
		if (JistAPI.getTime() <= (duration - waitTimeEnd) * Constants.SECOND) {
			log.debug("node="+this.nodeId+" scheduling next run at t="+StringUtils.timeSeconds());
	...
	private void sendMessage() {
		...
		float radius = Constants.random.nextInt(maxRadius-minRadius)+minRadius;
		...
		log.info("send t="+StringUtils.timeSeconds()+" node="+nodeId+" msg="+
				msg.msgID+" at x="+localPos.getX()+" y="+localPos.getY()+" r="+radius);
	...
	public void receive(Message msg, NetAddress src, MacAddress lastHop, byte macId, NetAddress dst, byte priority, byte ttl) {
		...
		log.info("recv t="+StringUtils.timeSeconds()+" node="+nodeId+" loc="+localPos+
				" src="+src+" msg=("+msg+")");
		...
		for (ReceiveStats rs : receivedMessages) {
			if (rs.src.equals(src) && rs.msgId == msgid) {
				...
				if (x < rs.leftMostEncounter.getX()) { rs.leftMostEncounter = localPos; }
				else if (x > rs.rightMostEncounter.getX()) { rs.rightMostEncounter = localPos; }
				
				if (rs.ttl > ttl) { rs.ttl = ttl; }
				rs.receiveCount++;
	...
	public ExtendedProperties getStats() {
		...
		if (statOutputType == 1) {
			int count = 0;
			for (ReceiveStats rs : receivedMessages) {
				log.debug(rs.src+"/"+rs.msgId+" left="+rs.leftMostEncounter.getX()+" right="+rs.rightMostEncounter.getX()+
						" dist="+(rs.rightMostEncounter.getX() - rs.leftMostEncounter.getX())+" origin="+rs.origin+" radius="+rs.radius+
						" ttl="+rs.ttl+ " recvCount="+rs.receiveCount);
				if (rs.leftMostEncounter.getX() < statParam && rs.rightMostEncounter.getX() > statParam) {
					count++;
				}
			}
	...
	/**
	 * Type of statistics requested at the end
	 * type = 1 -> assumes stationary attacker at the highway, attacker at x = param
	 *             getStats will return number of geocasts that travelled across x
	 * @param type 
	 * @param param
	 */
