...
public class AppGeocast implements AppInterface, NetInterface.NetHandler, StatsCollector
{
    ...
    public AppGeocast(int nodeId, RadioData radio, int minRadius, int maxRadius, Location.Location2D[] srcArea,
    ...
    public void run(String[] args) {
        ...
        if (args == null) {
            ...
            log.debug("node=" + this.nodeId + " period=" + StringUtils.timeSeconds(period) + " waitUntil="
        ...
        // round
        log.debug("next run? t=" + StringUtils.timeSeconds() + " deadline="
                + StringUtils.timeSeconds((duration - waitTimeEnd) * Constants.SECOND));
    ...
    private void sendMessage() {
        ...
        netEntity.send(msg, addr, Constants.NET_PROTOCOL_UDP, Constants.NET_PRIORITY_NORMAL, MessageGeocast.TTL);

        log.info("send t=" + StringUtils.timeSeconds() + " node=" + nodeId + " msg=" + msg.msgID + " at x="
                + localPos.getX() + " y=" + localPos.getY() + " r=" + radius);
    ...
    public void receive(Message msg, NetAddress src, MacAddress lastHop, byte macId, NetAddress dst, byte priority,
        ...
        log.info("recv t=" + StringUtils.timeSeconds() + " node=" + nodeId + " loc=" + localPos + " src=" + src
                + " msg=(" + msg + ")");
    ...
    public ExtendedProperties getStats() {
        ...
        if (statOutputType == 1) {
            int count = 0;
            for (ReceiveStats rs : receivedMessages) {
                log.debug(rs.src + "/" + rs.msgId + " left=" + rs.leftMostEncounter.getX() + " right="
                        + rs.rightMostEncounter.getX() + " dist="
                        + (rs.rightMostEncounter.getX() - rs.leftMostEncounter.getX()) + " origin=" + rs.origin
                        + " radius=" + rs.radius + " ttl=" + rs.ttl + " recvCount=" + rs.receiveCount);
