 ...
 * broadcast in 1 hop distance. The period between two messages can be
 * jittered. Beacon messages carry a variable amount of BeaconData objects
 * which are fetched from external objects if they register for being called
 * when a beacon is to be sent.
 * Moreover, the Beaconing class provides listener interfaces that allow external
 * objects to be notified after a beacon was sent or received.
 * 
 ...
 * can be registered with the Beaconing to be notified when a beacon is received and sent.
 * 
 * @author Elmar Schoch
 *
 */
public class Beaconing implements NetInterface.NetHandler, AppInterface, StatsCollector {
	
	// log4j Logger 
	private static Logger log = Logger.getLogger(Beaconing.class.getName());
	
	/**
	 * BeaconData class is a container to hold a type and object
	 * that is sent along with beacons. 
	 */
	...
	/**
	 * BeaconDataProvider interface must be implemented by external objects 
	 * that want to add data to beacons
	 */
	...
	/**
	 * The BeaconMessage is the class of object that the Beaconing sends.
	 * It basically consists of an array of beacon data objects that is
	 * assembled right before the beacon is sent. 
	 */
	...
	/**
	 *  The interface BeaconReceiveListener must be implemented by external objects
	 *  that want to be informed when a beacon message is received.
	 */
	public static interface BeaconReceiveListener {
		public void beaconReceived(NetAddress src, MacAddress lastHop, BeaconMessage msg, int macId);
	}
	...
	private long getWaitingTime() {
		return BeaconInterval - BeaconJitter + (long) (2 * BeaconJitter * Constants.random.nextFloat());
	}
	
	private void sendBeacon() {
		
		log.debug(StringUtils.timeSeconds()+" - Node "+here+" sends beacon");
	...
	public void receive(Message msg, NetAddress src, MacAddress lastHop,
		...
		if (msg instanceof BeaconMessage) {
			log.debug(StringUtils.timeSeconds()+" - Node "+here+" received beacon from "+lastHop);
			
			// notify every listener that a beacon was received
			notifyBeaconReceiveListeners(src, lastHop, macId, (BeaconMessage) msg);
	...
	private void notifyBeaconReceiveListeners(NetAddress src, MacAddress lastHop, int macId, BeaconMessage msg) {
