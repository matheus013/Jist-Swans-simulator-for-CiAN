...
public class NetIpGeo extends NetIpBase implements NetInterface {
	...
	/**
	 * Return true if the message is *exlcusively* for this node. Every thing
	 * else will be handled by SendIp. Since we reuse regular Ip adresses we can
	 * simply call super.isForMe() in area mode.
	 * 
	 * @param msg
	...
	protected boolean isForMe(NetMessage.Ip msg) {
		...
		if (addr instanceof NetAddressGeo)
			if (!((NetAddressGeo) addr).getRegion().contains(
					localRadio.getLoc()))
				return false;
	...
	protected boolean isDup(NetMessage.Ip msg) {
		...
		if (NetAddress.LOCAL.equals(msg.getDst()))
			return false;

		Long key = new Long(((long) msg.getId() << 32)
				+ msg.getSrc().getIP().hashCode());
		Long dupTimestamp = (Long) database.put(key,
				new Long(JistAPI.getTime()));

		log.debug("DUP check: here=" + localAddr + " msg=" + msg.getSrc() + "/"
				+ msg.getId() + " key=" + key + " dupTime=" + dupTimestamp);

		if (dupTimestamp == null) {
			return false; // fist-time delivery
		} else if (dupTimestamp.longValue() + DUP_DATABASE_TIME < (JistAPI
				.getTime())) {
			return false; // dup was stale
		} else {
			log.debug("DUP found, here=" + localAddr + " msg=" + msg.getSrc()
					+ "/" + msg.getId());
	...
	public void send(NetMessage.Ip msg, int interfaceId, MacAddress nextHop) {
		...
		log.debug("send here=" + localAddr + " to=" + nextHop + " msg="
				+ msg.getSrc() + "/" + msg.getId() + " iid=" + interfaceId);

		if (geoOption) {
			try {
				// TODO This throws UnsupportedOperationExceptions!!!
				// because getOptions() returns an unmodifiable map, if the
				// packet is frozen.
				// Then, the operation "put" is of course not allowed in such a
				// map.
				// -> WORKAROUND: check before, if packet is frozen
				if (!msg.isFrozen()) {
					msg.getOptions().put(Constants.IP_OPTION_HOPLOC,
							new NetMessage.IpOptionHopLoc(localRadio.getLoc()));
				}
	...
	protected void sendIp(NetMessage.Ip msg, boolean bcast) {
		...
		if (msg.getDst() instanceof NetAddressGeo) {
			...
			if (addr.getRegion().contains(localRadio.getLoc())) {
				...
				log.debug("sendIp " + " here=" + localAddr + " msg="
						+ msg.getSrc() + "/" + msg.getId());
				...
				// GEO[BROAD|MULTI]CAST can additionally send packets to upper
				// layers via loop
				if ((NetAddress.NULL.equals(addr))
						|| (addr.getIP().isMulticastAddress()))
					send(msg, Constants.NET_INTERFACE_LOOPBACK, MacAddress.LOOP);

				// Flood
				// Before forwarding, sleep for a jitter time, otherwise packets
				// may collide
				long jitter = Util.randomTime(FORWARDING_JITTER);
				JistAPI.sleep(jitter);
				log.debug("   sleeping " + jitter + " ns before forwarding");

				send(msg.copy(), Constants.NET_INTERFACE_DEFAULT,
						MacAddress.ANY);

			} else {
				// dont return to line forwarding mode if area mode was already
				// entered
				// if I could rely on geoOption always being active I could use
				// that information
				// to tell if the last hop was in line or area mode. now I just
				// use info from MAC
				if (!bcast)
					routing.send(msg);
			}
	...
	public String toString() {
		return "NetIpGeo " + super.toString() + "#"
				+ localRadio.getLoc().toString();
	}
}
