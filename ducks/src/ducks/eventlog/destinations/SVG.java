...
public class SVG extends EventLog {
	...
	public void configure(Properties config, String configprefix) {
		...
		if (scx != null)
			try {
				scalingX = Float.parseFloat(scx);
			} catch (Exception e) {
				logger.warn("scaling.x is no valid float: " + scx);
			}

		String scy = config.getProperty(configprefix + ".scaling.y");
		if (scy != null)
			try {
				scalingY = Float.parseFloat(scy);
			} catch (Exception e) {
				logger.warn("scaling.y is no valid float: " + scy);
			}

		String nWidth = config.getProperty(configprefix + ".node.width");
		if (nWidth != null)
			try {
				nodeWidth = Integer.parseInt(nWidth);
			} catch (Exception e) {
				logger.warn("node.width is no valid int: " + nWidth);
			}
		String nHeight = config.getProperty(configprefix + ".node.height");
		if (nHeight != null)
			try {
				nodeHeight = Integer.parseInt(nHeight);
			} catch (Exception e) {
				logger.warn("node.height is no valid int: " + nHeight);
			}
		...
		if (pad != null)
			try {
				...
				logger.warn("padding is no valid int: " + pad);
			}
		String col = config.getProperty(configprefix + ".node.color");
		if (col != null)
			nodeColor = col;

		String fsx = config.getProperty(SimParams.SCENE_NAMEPSPACE + "."
				+ SimParams.SCENE_FIELD_SIZE_X);
		if (fsx == null) {
			fsx = "500";
		}
		String fsy = config.getProperty(SimParams.SCENE_NAMEPSPACE + "."
	...
	public void logEvent(int node, long time, Location loc, String type,
		...
		if (type.equals("move")) {
			...
			logger.debug("movement log: t=" + time + " id=" + node + " x="
					+ loc.getX() + " y=" + loc.getY());
			...
			if (nd == null) {
				...
				String rect = "<rect id=\"" + node + "\" x=\"" + relWidth
						+ "\" y=\"" + relHeight + "\" height=\"" + nodeHeight
						+ "\" width=\"" + nodeWidth + "\" fill=\"" + nodeColor
						+ "\" stroke=\"none\">\n";
				...
				if (loc.equals(nd.lastLoc) || time == nd.lasttime) {
					...
					float x_old = (scalingX == 1 ? nd.lastLoc.getX()
							: nd.lastLoc.getX() * scalingX);
					float y_old = (scalingY == 1 ? nd.lastLoc.getY()
							: nd.lastLoc.getY() * scalingY);
					float x_new = (scalingX == 1 ? loc.getX() : loc.getX()
							* scalingX);
					float y_new = (scalingY == 1 ? loc.getY() : loc.getY()
							* scalingY);
					...
					String ani = "<animateMotion "
							...
							+ "\" "
							+ "begin=\""
							+ (nd.lasttime / Constants.MILLI_SECOND)
							+ "ms\" "
							+ "end=\""
							+ (time / Constants.MILLI_SECOND)
							+ "ms\" "
							+ "dur=\""
							+ ((time - nd.lasttime) / Constants.MILLI_SECOND)
							+ "ms\" "
