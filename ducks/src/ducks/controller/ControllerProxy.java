...
/**
 * The ControllerProxy represents a socket-based interface to the simulation manager.
 * The communication protocol is intended to work like the HTTP protocol. With growing
 * functionality, this ControllerProxy may handle most of the functions like
 * monitor ongoing simulations, monitor, enabling and disabling servers, introducing new 
 * simulation jobs etc.
 * 
...
public class ControllerProxy implements Runnable {
	 ...
	 * @param req complete request as string
		...
		public String getResponse() {
			...
			if (requestedItem == GET_SIMUSTATE) {
				...
				while (studies.hasMoreElements()) {
					...
					while (simus.hasMoreElements()) {
						...
						// format: run-id, simu-id, simu-state, remaining inst, running inst, done inst, failed inst
			...
			if (requestedItem == GET_SERVERSTATE) {
				...
				while (servers.hasNext()) {
					...
					// format: srv-identifier, host, port, state, current SimRun, current Simu
					result += srv.getIdentifier()+","+srv.getHost()+","+srv.getPort()+","+
						srv.getState()+","+runID+","+simuID+"\n";
				}
			}
