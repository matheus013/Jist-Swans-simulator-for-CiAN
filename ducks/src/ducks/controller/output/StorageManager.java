...
/**
 * StorageManager the storage of simulation results
 * It also features background saving, i.e. can run a thread that periodically
 * checks for finished simulations and flushes these results to the storage
 * while the overall simulation study has not finished yet.
 * 
 * @author Elmar Schoch
 *
 */
public class StorageManager implements Runnable, DucksControllerModule {
	 ...
	 * - complete saving: Storage Manager saves all results only after all results are available.
	 * - background saving: Storage Manager starts flushing finished simulations periodically, i.e. 
	 *   while other simus have not finished yet. This should be memory efficient.
	 */ 
	
	// config file option
	public static final String CFG_BACKGROUND_SAVING = CFG_NAMESPACE+".backgroundsaving";
	...
	/**
	 * Storage providers
	 * - can be configured in the config file with, e.g.
	 *   <code>ducks.stats.destinations=db,file</code>
	 * - then, the manager searches for entries like
	 *   <code>ducks.stats.db.class</code>  and <code>ducks.stats.file.class</code>
	 *   and tries to instantiate these classes, which must implement the interface StorageProvider
	 * - all configuration can be done within the namespace of the corresponding provider, e.g.
	 *   ducks.stats.db.hostname or ducks.stats.db.port, because the provider gets all "lower"
	 *   options, in the example, ducks.stats.db.*
	 */ 
	public static final String CFG_STORAGE_DESTINATIONS = CFG_NAMESPACE+".modules";
	private ArrayList<StorageProvider> storageModules = new ArrayList<StorageProvider>();
	
	public static final String CFG_INCLUDE_PARAMS  = CFG_NAMESPACE+".params.include";
	public static final String CFG_EXCLUDE_PARAMS  = CFG_NAMESPACE+".params.exclude";
	public static final String CFG_INCLUDE_RESULTS = CFG_NAMESPACE+".results.include";
	public static final String CFG_EXCLUDE_RESULTS = CFG_NAMESPACE+".results.exclude";
	
	/**
	 * Init StorageManager
	 * @param config DUCKS controller properties
	...
	public void configure(ExtendedProperties config) throws DucksException {
		...
		try {
			...
			for (String provider: providers) {
				String providerClass = config.getProperty(CFG_NAMESPACE+"."+provider+".class").trim();
				StorageProvider sp = (StorageProvider) Class.forName(providerClass).newInstance();
				sp.configure(config.getNamespace(CFG_NAMESPACE+"."+provider, false));
				storageModules.add(sp);
			}
        } catch (ClassNotFoundException cnfe) {
			throw new DucksException("Class for storage provider not found: "+cnfe.getMessage());
		} catch (InstantiationException ie) {
			throw new DucksException("Class for storage provider could not be instantiated: "+ie.getMessage());
		} catch (Exception e) {
			throw new DucksException("Problem loading storage providers: "+e.getMessage());
		} 
		
		doBackgroundSaving = config.getBooleanProperty(CFG_BACKGROUND_SAVING, true);
	}
	...
	//   and inserts them into the database instantly (even while other simulations
	...
	public void startBackgroundSaving(SimulationStudy study) {
		...
		try {
			...
			log.error("Error during saving the simulation run! Skipping background saving ("+e.getMessage()+")");
			return;
		}
	...
	public synchronized void saveReadyResults() {
		...
		for(Simulation s: simus) {
			try {
            	...
            	log.error("Error while flushing simulation "+s.getIdentifier()+": "+de.getMessage());
				de.printStackTrace();
            }
		}
	}
