...
public class MySQLDatabase implements StorageProvider {
	...
	// - table for simulation configurations (including all input parameters) (1:n)
	...
	public void enable() throws DucksException {
		...
		try {
			...
			dbConnection = DriverManager.getConnection("jdbc:mysql://"+dbHost+":"+dbPort+"/"+dbName, dbUsername, dbPassword);
		} catch (Exception e) {
			dbConnection = null;
	...
	public void saveSimulationStudy(SimulationStudy study) throws DucksException {
		...
		if (dbConnection == null) throw new DucksException("No connection to db");
		...
		boolean consistent = checkDatabaseStructure(DB_SIMU_STUD, PRIMARY_KEY, null, fields, true, true);
		if (! consistent) {
			throw new DucksException("Database structure not consistent. Skipping saving of study! ");
		}
		
		String query = "INSERT INTO "+DB_SIMU_STUD+" (Description,StartDate) VALUES ('"+
						study.getDescription()+"','"+StringUtils.formatDateTime(new Date())+"');";
		long studyID = -1;
		...
		try {
	        ...
	        throw new DucksException("Simulation study could not be saved to MySQL database: "+e.getMessage());
        }
    }
	
	/**
	 * Save simulation configuration, including the results of all
	 * contained instances.
	 * @param simu The simulation to save
	 */
	public void saveSimulation(Simulation simu) throws DucksException {
		...
		if (studyID == null) {
			...
			throw new DucksException("Simulation study ID not found when saving simu config!");
		}
		...
		// check structure
		checkDatabaseStructure(DB_SIMU_CONF, PRIMARY_KEY, new String[] {STUDY_FOREIGN_KEY}, fields, true, false);
		...
		try {
			...
			log.debug("Successfully saved simulation configuration under ID="+simuID);
			simuIds.put(simu, simuIDObj);
        } catch (Exception e) {
        	e.printStackTrace();
	        throw new DucksException("Simulation config could not be saved to DB: "+e.getMessage());
        }
        ...
        for (int i=0; i < instances.size(); i ++) {
        	...
        	if (results == null) {
        		log.warn("Simulation instance "+si.getIdentifier()+" did not provide results!");
        		continue;
        	}
        	results = results.getFilteredSet(incResults, excResults);
        	
        	
        	checkDatabaseStructure(DB_SIMU_INST, PRIMARY_KEY, 
        			new String[] {STUDY_FOREIGN_KEY, CONFIG_FOREIGN_KEY}, 
        			results, true, false);
        
        	// save result values
    		try {
    			// Caution: Adding 'long' values to the Properties object is syntactically correct,
    			// but leaves 'null' references instead of the values. Thus, add as Long object or String
            	...
            	e.printStackTrace();
    	        throw new DucksException("Simulation instance could not be saved to DB: "+e.getMessage());
            }
	 ...
	 * wanted.
	 * @param table The table to check. Will also be created, if addDynamically is true
	 * @param priKeyField Name of the primary key field, if table needs to be created
	 * @param foreignKeys Names of foreign keys, that will be also created as bigint(20), when
	 * 					  the table needs to be created
	 * @param fields Properties that contain the fields to check. Only the names will be taken,
	 * 				 unless the <code>propertyValueIsDataType</code> indicates to use the value
	 * 				 as corresponding data type
	 * @param addDynamically Add table and fields, if they are not there yet. otherwise return false.
	 * @param propertyValueIsDataType Use the value of the fields property as datatype, if true
	 */
	private boolean checkDatabaseStructure( String table, String priKeyField, String[] foreignKeys,
			ExtendedProperties fields, boolean addDynamically, boolean propertyValueIsDataType) {
		
		// check if table is available
		try {
			DatabaseMetaData dbmd = dbConnection.getMetaData();
			String[] objectCategories = {"TABLE"};
			ResultSet tabs = dbmd.getTables(dbName, null, null, objectCategories);

			boolean found = false;
			while(tabs.next()) {
				String tab = tabs.getString("TABLE_NAME");
				log.debug("Found table: "+tab);
				if (tab.equals(table)) found = true;
			}
			...
			if (! found ) {
				...
				if (addDynamically) {
					...
					String query = "CREATE TABLE `"+table+"` ("+
					                priKeyField+" bigint(20) unsigned NOT NULL auto_increment, ";
					if (foreignKeys != null) {
						for (int i=0; i < foreignKeys.length; i++) {
							query += foreignKeys[i]+" bigint(20) unsigned NOT NULL, ";
						}
					}
					query += "PRIMARY KEY ("+priKeyField+") ) ENGINE=MyISAM DEFAULT CHARSET=latin1;";
					log.debug("Executing: "+query);
					try {
						s.execute(query);
					} catch(SQLException sqle) {
						log.debug("Executing query failed: "+query+" Exception: "+sqle.getMessage());
						return false;
					}
					
				} else {
					// table not found, no instant adding wanted
					log.debug("checkDB: table "+table+" not found, dynamic adding not wanted -> DB inconsistent");
					return false;
				}
			} else {
				// table was found
        ...
        try {
	        Statement s = dbConnection.createStatement();
	        ResultSet cols = s.executeQuery("SELECT * FROM `"+table+"` WHERE 1=2");
        ...
        if (addDynamically) {
        	...
        	while(newFields.hasMoreElements()) {
        		...
        		String datatype = propertyValueIsDataType ? wantedfields.getProperty(fieldname) : "TEXT";
        		try {
        			Statement s = dbConnection.createStatement();
            		s.execute("ALTER TABLE `"+table+"` ADD COLUMN `"+fieldname+"` "+datatype+";");
                } catch (Exception e) {
                	e.printStackTrace();
	                log.warn("Could not add column "+fieldname+" ("+datatype+") to table "+table+": "+e.getMessage());
	                return false;
                }
        	}
        } else {
        	if (wantedfields.size() > 0) {
        		// have fields not in the table, but do not want to add dynamically
        		log.debug("CheckDB: Columns were noet found, no dynamic adding wanted - "+wantedfields.toString());
        		return false;
        	}
        }
	...
	/**
	 * Save a properties list to the database.
	 * CAUTION: Only valid properties are allowed as column names, and values
	 * must not contain ' signs!! Otherwise, the query will fail!
	 * 
	 * @param table The table to add the data
	 * @param props The properties where the key is used as column name, the value as row value
	 * @return The ID of the row in the rable
	 * @throws Exception 
	 */
	private long saveValues(String table, ExtendedProperties props) throws Exception {
