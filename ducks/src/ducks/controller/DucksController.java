 ...
 * <li> the currently available list of jist servers
 * <li> the currently prepared, running and completed simulation configurations
 * <li> the connection to the database that stores simulation results 
 * 
 * DUCKS Version 2
 * 
 * @author Elmar Schoch
...
public class DucksController  {
     ...
     * <li> -s / --simuconfig filename : specify a simulation study config file to use
     * <li> -d / --ducksconfig filename : specify a ducks controller config file to use (default ducks/resources/ducks.properties)
     * <li> -n / --nodb : run simulations without saving results to database
     * <li> -l / --debuglevel : set basic debug level (log4j: INFO,WARN,DEBUG)
     * <li> -p / --persist : do not exit controller, if all studies are processed (i.e. wait for new ones via remote proxy)
     * 
     */
    public static void main(String[] args) {
    	...
    	try {
    		...
    		log.error("Loading DUCKS config file failed: given filename: "+configFilename+
    				" working dir: "+System.getProperty("user.dir")+" Exception: "+e.getMessage());
    		e.printStackTrace();
    		System.exit(1);
    	}
		...
		while (true) {	
			...
			if (dc.currentStudy != null) {
				...
				if (dc.currentStudy.hasFinished()) {
					...
					if ((!nosave) && (! dc.storage.backgroundSavingEnabled())) {
						try {
							...
							e.printStackTrace();
							// wait for user intervention ... (e.g. reparing the database)
							continue;
						}
					}
					
					// set currentStudy to null
					// -> will select next available in next iteration of the main loop
					//    or quit, or idle
				...
				if (study != null) {
					// yeah, we have got one! -> set to current study and continue!
			...
			if ( dc.currentStudy != null && dc.servers.numberOfEnabledServers() == 0 ) {
				...
				if (dc.proxy != null) {
					// if we have a remote proxy, trust on user to re-activate servers again
