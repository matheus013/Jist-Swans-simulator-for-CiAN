...
/**
 * ServerManager contains the list of jist compute servers and allows
 * to load, add and remove them.
 * 
...
public class ServerManager implements DucksControllerModule {
	...
	private class ServerThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				System.out.println("ERR: Could not create local server, port is already in use");
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	/**
	 * Load the list of servers from a properties object (like the
	 * DUCKS controller config). Format of server parameters is:
	...
	public void configure(ExtendedProperties config) {
		...
		// load servers
		HashMap<String, ExtendedProperties> srvlist = config.getSubNamespaces(CFG_PREFIX, false);
	 ...
	 * create a local server. This needs to be done externally; ServerManager will not
	 * create a local server automatically.
	 * 
	 ...
	 * @param port number of the port where JiST listens
	 * @param identifiere literal identifier of the server (no further meaning)
	 */
	...
	/**
	 * Get number of servers in the list, regardless whether currently
	 * enabled or not.
	 * Note that servers set to enabled=false in the config file are
	 * not added to the server list at all.
	 *  
	...
	/**
	 * Retrieve the number of CURRENTLY enabled servers. 
	 * Enabled means that the server is active and trying to
	 * get work
