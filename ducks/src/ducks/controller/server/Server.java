...
public class Server implements Runnable {
	...
	/**
	 * Flag indicating whether the server is enabled to compute simulations
	 */
	private boolean enabled = false;

	/**
	 * Holds the Simulation object that is currently computed on the JiST-server
	 * that is represented by this object in SimuGen
	 */
	...
	/**
	 * Sets server hostname. Should be working only, when server is not enabled.
	 * 
	 * @param host
	 *            Server hostname
	 */
	 ...
	 * 
	 * @return Indicates whether the server is currently enabled for scheduling
	 *         new simulations
	 */
	...
	public synchronized void setEnabled(boolean enabled) {
		...
		// 1. if server is currently running a simulation, and should be
		// disabled now
		...
		// - create thread (that picks a pending simulation from the global
		// queue)
		if (enabled) {
			createThread();
		}
	...
	/**
	 * Implements method run() that is necessary for the Runnable interface. The
	 * method contains the code for the thread running the client side of the
	 * simulation.
	 */
	public void run() {
		...
		while (true) {
			...
			log.info("Server " + this.toString() + " starts processing simu "
					+ currentSimu.getIdentifier());
			...
			try {
				...
				try {
					...
					// Error during parsing indicates that no result section was
					// found
					log.error("Simulation " + currentSimu.getIdentifier()
							+ " terminated with errors: " + e.getMessage());
				}

				String s = "";
				if (currentSimu.getState() == SimulationInstance.DONE)
					s = "successfully";
				if (currentSimu.getState() == SimulationInstance.FAILED)
					s = "with errors";
				log.info("Simulation " + currentSimu.getIdentifier()
						+ " has finished " + s);
	...
	/**
	 * Helper method with the final code that runs the simulation on the remote
	 * jist server
	 * 
	 */
	private void runRemoteSimulation() throws DucksException,
		...
		try {
			...
			throw new RemoteException("Server " + this.identifier
					+ " is not available");
		}
		...
		RemoteJist.JistClient client = new RemoteJist.JistClient(
				currentSimu.getOutputStream(), System.err);
		// enqueue job
		RemoteJist.Job job = new RemoteJist.Job();

		// Commandline options can be created here (since we don't use them in
		// ducks)
