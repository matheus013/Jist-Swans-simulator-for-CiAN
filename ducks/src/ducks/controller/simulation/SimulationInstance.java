...
/**
 * Simulation instance represents one atomic simulation execution. Since every
 * unique simulation normally needs to be executed several times to produce
...
public class SimulationInstance {
	...
	// log4j Logger
	private static Logger log = Logger.getLogger(SimulationInstance.class
			.getName());
	...
	public void parseResults() throws DucksException {
		...
		if (begin == -1 || end == -1) {
			...
			throw new DucksException(
					"Simulation results not valid (no result section)");
		}

		String results;
		try {
			results = output.substring(
					begin + Simulation.BEGIN_RESULTS.length(), end - 1);
		} catch (Exception e) {
			log.warn("Failed to parse result properties:\n" + output);
			finalize(FAILED);
			throw new DucksException(
					"Simulation results could not be parsed (substring)");
		}
		...
		try {
			...
			finalize(FAILED);
			throw new DucksException(
					"Loading results into ResultDescriptor failed");
		}
	...
	/**
	 * Call, if sim instance could not be executed as intended, e.g. because RMI
	 * link to server was broken. This means of course, that the instance has to
	 * be executed again, whereas it must not be repeated when a simulation
	 * internal failure occured that is not due to DUCKS (in this case,
	...
	public String[] getArgs() {
		...
		// TODO: Little hack to add the current instance number to the
		// properties sent to the JiST server
		// args[1] =
	...
	/*
	 * public void saveResults(Connection con, long config, long run) throws
	 * SQLException, DucksException {
	 * 
	 * if (con == null) throw new DucksException("No connection to db");
	 * 
	 * // Add entry to simu list String q =
	 * "INSERT INTO "+DBManager.DB_SIMU_LIST
	 * +" (RunID,ConfigID) VALUES ("+run+","+config+")"; Statement s =
	 * con.createStatement(); s.execute(q,Statement.RETURN_GENERATED_KEYS);
	 * 
	 * ResultSet rs = s.getGeneratedKeys(); rs.next(); long simuID =
	 * rs.getLong(1);
	 * 
	 * if (result != null) { result.saveToDB(con, simuID); } }
	 */

	/**
	 * Allow Java to garbage collect the memory consumed by the result variables
	 * by setting their pointers to null. This is done after saving a result
	 * successfully to the result database in the Simulation class.
	 */
