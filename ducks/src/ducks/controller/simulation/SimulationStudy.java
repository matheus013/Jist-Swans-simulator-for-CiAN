...
/**
 * SimulationStudy represents a set of simulation configurations. The set of
 * simulations can be loaded by a properties-file containing all steering
 * parameters for them. Multi-value parameters in the properties file are
 * exploded combinatorically to simulations with a unique parameter set.
 * 
 * @author Elmar Schoch
 * 
 */
...
public class SimulationStudy implements Configurable {
	...
	// log4j Logger
	private static Logger log = Logger.getLogger(SimulationStudy.class
			.getName());
	...
	// Wrapper to use. The wrapper is the DUCKS driver, i.e. the main class for
	// a simulation
	// Usually always "ducks.driver.Main", because only this class "knows" how
	// to handle
	...
	public void configure(ExtendedProperties config) throws Exception {
		...
		wrapper = config.getStringProperty(CFG_SIM_WRAPPER,
				DEFAULT_WRAPPER_CLASS);

		// instantiate simulation multiplexer which creates single simulations
		// out of the
		// config. DUCKS does not care how this is done
		String muxname = config.getProperty(CFG_SIM_MUX);
		if (muxname == null)
			muxname = DEFAULT_MUX_CLASS;
		...
		try {
			...
			e.printStackTrace();
			throw new Exception(
					"Simu config multiplexer class could not be loaded: "
							+ e.getMessage());
		}
		...
		for (int i = 0; i < configSet.size(); i++) {
			...
			simus.add(s);
			log.debug("Simu " + s.getIdentifier() + ": "
					+ s.getSimuConfig().saveToString());
		}
	...
	/**
	 * check if this simulation run has finished all simulations yet. Note:
	 * Method returns immediately after encountering an unfinished simulation
	 * 
	 * @return
	 */
	 ...
	 * 
	 * for (int i=0; i < simus.size(); i++) { Simulation s = (Simulation)
	 * simus.get(i); s.saveResults(con, simRunID); } }
	 */

	/**
	 * Retrieve all simulations that have finished, but are not saved yet. The
	 * method does not need to be synchronized because only one thread accesses
	 * it (i.e. the background saving thread)
	 * 
	 * @return
	 */
	...
	/**
	 * Retrieve an array containing the number of simulations for each
	 * simulation state (i.e. PENDING, RUNNING, FINISHED, FLUSHED)
	 * 
	 * @return
	 */
