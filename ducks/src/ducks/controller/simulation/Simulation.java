...
public class Simulation {
	...
	public static final int FLUSHED = 4; // indicates that simu results have
											// been saved
	...
	/**
	 * Get the simulation state. This may be <li>pending: Simulation has not
	 * been processed at all <li>running: At least one instance of the
	 * simulation is currently beeing processed <li>finished: Simulation has
	 * been processed completely (i.e. all instances) <li>flushed: Simulations
	 * results have been saved to database. Note: the state "flushed" may never
	 * be reached when no saving is set.
	 * 
	 * @return state constant
	 */
	public synchronized int getState() {
		int s = 0;
		if (remainingSimInstances == maxSimInstances)
			s = PENDING;
		if (remainingSimInstances < maxSimInstances)
			s = RUNNING;
		if (remainingSimInstances == 0 && runningSimInstances.size() == 0)
			s = FINISHED;
		if (resultsAreFlushed)
			s = FLUSHED;

		log.debug("State of simu " + getIdentifier() + ": " + s
				+ " (remaining: " + remainingSimInstances + " running: "
				+ runningSimInstances.size() + " flushed: " + resultsAreFlushed
				+ ")");
	 ...
	 * to their state. E.g., index 0 = number of remaining instances, 1 =
	 * running, 2 = done, 3 = failed
	 * 
	...
	public int[] getStateInfo() {
		...
		for (int i = 0; i < finishedSimInstances.size(); i++) {
			SimulationInstance si = (SimulationInstance) finishedSimInstances
					.get(i);
			states[si.getState()]++;
		}
	...
	public synchronized void setFlushed() {
		if (!resultsAreFlushed && remainingSimInstances == 0
				&& runningSimInstances.size() == 0) {
	...
	public synchronized SimulationInstance getFreeInstance() {
		...
		if (remainingSimInstances == 0) {
			...
			SimulationInstance newSim = new SimulationInstance(this,
					runningSimInstances.size() + 1);
	...
	/**
	 * Call, if an error occured with the link to the processing server (i.e.
	 * instance was set up, but not actually processed)
	 * 
	 * @param si
	 */
	...
	/**
	 * Call, if simu instance has completed (i.e. could be successfully executed
	 * on server, regardless whether simu produced errors or completed normally)
	 * 
	 * @param si
	 */
	...
	/*
	 * public synchronized void saveResults(Connection con, long simRunID)
	 * throws SQLException, DucksException {
	 * 
	 * // Assure that results aren't saved twice if (resultsAreFlushed) return;
	 * 
	 * log.info("Flushing simu "+getIdentifier()+" to database ...");
	 * 
	 * // Save simulation config (input params) long cfgID = saveConfig(con,
	 * simRunID);
	 * 
	 * // Save computed simulation result values (for each SimInstance) for (int
	 * i=0; i < finishedSimInstances.size(); i++) { SimulationInstance si =
	 * (SimulationInstance) finishedSimInstances.get(i); if (si.getState() ==
	 * SimulationInstance.DONE) {
	 * si.getResultDescriptor().setProperty("ducks.config.db.results.exclude"
	 * ,simDescriptor.getProperty("ducks.config.db.results.exclude"));
	 * si.getResultDescriptor
	 * ().setProperty("ducks.config.db.results.include",simDescriptor
	 * .getProperty("ducks.config.db.results.include"));
	 * si.saveResults(con,cfgID, simRunID); } else {
	 * log.warn("Skipped saving failed simulation instance to database! (runID: "
	 * +cfgID+", cfgID: "+cfgID+")"); } } resultsAreFlushed = true;
	 * 
	 * // Release some memory (textual simu results, ...) for (int i=0; i <
	 * finishedSimInstances.size(); i++) { SimulationInstance si =
	 * (SimulationInstance) finishedSimInstances.get(i); si.releaseMemory(); }
	 * 
	 * System.gc(); }
	 * 
	 * private long saveConfig(Connection con, long simRunID) throws
	 * SQLException, DucksException {
	 * 
	 * // Save general table String q =
	 * "INSERT INTO "+DBManager.DB_SIMU_CONF+" SET RunID="+simRunID+";";
	 * 
	 * Statement s = con.createStatement();
	 * s.execute(q,Statement.RETURN_GENERATED_KEYS);
	 * 
	 * ResultSet rs = s.getGeneratedKeys(); rs.next(); long configID =
	 * rs.getLong(1);
	 * 
	 * // Save config tables simDescriptor.saveToDB(con, configID);
	 * 
	 * return configID; }
	 */
