...
/**
 * This class represents the base for drivers in DUCKS. In the model, the driver is capable of
 * taking up configuration properties and giving it to a driver class. Moreover, it expects the driver
 * class to return a list of result parameters as well as the corresponding values at the end of a
 * simulation.
 * 
 ...
 *          network interface queues on the DUCKS controller machine.
 *          
 * Note:    The main method knows two types of being called: direct and via DUCKS controller
 *          1. When used directly, the driver can be called via SWANS, like any other driver, except
 *             that it parses the properties file given at the command line, which holds all 
 *             (unique!!) configuration parameters. In this case, the whole DUCKS controller engine
 *             with multiplexer, servers and storage is not involved at all 
 *          2. When used by the DUCKS engine, all the required configuration properties are 
 *
 * @author Elmar Schoch
 *
 */
public class Main implements DucksDriverModule, Runnable {
	...
	/**
	 * Main method. 
	 * @param args Contains complete set of simulation parameters
	 */
	...
	public void configure(ExtendedProperties config) throws Exception {
		...
		if (seed == null) {
			...
			try {
				Constants.random = new MersenneTwister(Long.parseLong(seed)); /*Random(Long.parseLong(seed));*/
			} catch (Exception e) {
		...
		if (driverClassname == null) {
			throw new DucksException("DUCKS driver class is not specified in the configuration! (parameter "+SimParams.SIM_CLASS);
		}
		try {
	 ...
	 * 
	 *  
	 * @return String containing complete result value set in properties file format.
	 */
	...
	public ExtendedProperties getStats() {
		// general purpose information of environment (memory consumption, elapsed time)	    
	    System.gc();
