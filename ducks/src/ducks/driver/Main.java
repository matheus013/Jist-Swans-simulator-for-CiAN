...
public class Main implements DucksDriverModule, Runnable {
	...
	public static void main(String[] args) {
		...
		CmdLineParser.Option optSimuCfgFile = parser.addStringOption('f',
				"configfile");
		CmdLineParser.Option optSimuCfgStream = parser.addStringOption('s',
				"configstream");
		CmdLineParser.Option optDebug = parser.addStringOption('d',
				"debugconfig");
		...
		if (parser.getOptionValue(optDebug) != null) {
			...
			PropertyConfigurator.configure((parser.getOptionValue(optDebug))
					.toString());
		} else {
		...
		if (configFile == null && configStream == null) {
			System.out
					.println(">>> Neither config file nor config stream is available. Exiting ...");
			return;
		}
		...
		try {
			if (configStream != null) {
				...
				options.loadFromString(configStream,
						ExtendedProperties.DEFAULT_SEPARATOR);
			} else {
		...
		try {
			...
			System.out
					.println(">>> Simulation setup failed: " + e.getMessage());
			simuMain.cancel();
			return;
		}
	...
	public void configure(ExtendedProperties config) throws Exception {
		...
		if (seed == null) {
			...
			try {
				...
				System.out
						.println(">>> Random seed is not a valid long value ("
								+ seed + "). Taking '0' instead.");
			}
		}
		...
		if (driverClassname == null) {
			throw new DucksException(
					"DUCKS driver class is not specified in the configuration! (parameter "
							+ SimParams.SIM_CLASS);
		}
		try {
			...
			if (!(drv instanceof DucksDriverModule)) {
				throw new DucksException("Driver " + driverClassname
						+ " does not implement the DucksDriver interface");
			} else {
				driver = (DucksDriverModule) drv;
			}
		} catch (ClassNotFoundException cnfe) {
			throw new DucksException("DUCKS driver class not found: "
					+ driverClassname);
		} catch (DucksException de) {
			throw de;
		} catch (Exception e) {
			throw new DucksException(
					"DUCKS driver class could not be be instantiated: "
							+ driverClassname);
		}
	...
	public String getResults() {
		...
		// result += out.toString();
		result += jvmstats
				.saveToSortedString(ExtendedProperties.NEWLINE_SEPARATOR);
		result += "\n";
		result += simustats
				.saveToSortedString(ExtendedProperties.NEWLINE_SEPARATOR);
		result += "\n----END-RESULTS----\n";
