...
/**
 * GenericNode represents a node in the network, whose components can be
 * configured in the config file. The GenericNode implementation will then
 * initialize all required components according to this config (see
 * SimParams.java for details)
 * 
 * @author Elmar Schoch
 * 
 */
public class GenericNode implements Node {
	...
	/**
	 * SWANS uses several common objects for all nodes, which can be assigned
	 * using this method
	 */
	public void setCommonObjects(Placement place, RadioInfoShared radioInfo,
	...
	/**
	 * Link Field with Radio layer and vice versa. This involves invoking the
	 * start of the mobility model
	 */
	protected void linkFieldAndRadio() {
		...
		scene.getField()
				.startMobility(radio.getRadioInfo().getUnique().getID());
	}
	...
	protected void linkRadioAndMac() throws Exception {
		...
		// TODO improper solution in swans: Mac802_11 and MacDumb only have
		// MacInterface in common,
		// whereas several procedures are literally identical => using an
		// additional (perhaps derived)
		// interface would have been nice!!!
		if (mac instanceof Mac802_11) {
			((Mac802_11) mac).setRadioEntity(radioEntity);
		} else if (mac instanceof MacDumb) {
			((MacDumb) mac).setRadioEntity(radioEntity);
		} else {
			throw new Exception(
					"MAC could not be linked with RADIO, because MAC type is not recognized");
		}
	}

	/**
	 * Link MAC and Network layer and vice versa. This involves creating an
	 * interface and a queue to the MAC layer
	 * 
	 * @throws Exception
	 */
	protected void linkMacAndNet() throws Exception {
		// build net interface
		DropTailMessageQueue msgQ = new DropTailMessageQueue(
				Constants.NET_PRIORITY_NUM, NetIp.MAX_QUEUE_LENGTH);
		...
		if (mac instanceof Mac802_11) {
			...
			throw new Exception(
					"MAC could not be linked with NET, because MAC type is not recognized!");
		}
	}
	...
	/**
	 * Link network object with routing object (vice versa, i.e. the ink from
	 * routing to net) is done by the addRouting method already.
	 */
	protected void linkNetAndRouting() {
		net.setRouting(routeEntity);
		if (net instanceof NetIpGeo)
			((NetIpGeo) net).setRoutingNonEntity(route);
	}

	/**
	 * Create radio object according to config parameters. Does not set up links
	 * to other entities.
	 * 
	 * @param id
	 *            Node id
	 ...
	 * @throws Exception
	 */
	protected void addRadio(int id, RadioInfo.RadioInfoShared radioInfo)
			throws Exception {
		...
		if (noiseOpt.equals(SimParams.NOISE_TYPE_INDEPENDENT)) {
			...
			try {
				// get aodv stats, and automatically create and register, if
				// necessary
				radioStats = (DucksRadioNoiseAdditiveStats) stats
						.getStaticCollector(DucksRadioNoiseAdditiveStats.class,
								true);
			} catch (Exception e) {
	...
	/**
	 * Create MAC object according to config parameters. Does not set up links
	 * to other entities.
	 * 
	 * @param id
	 *            Node ID
	...
	protected void addMac(int id) throws Exception {
		...
		if (macOpt.equals(SimParams.MAC_PROTOCOL_802_11)) {
			Mac802_11 mac802 = new Mac802_11(new MacAddress(id),
					radio.getRadioInfo());
			...
			try {
				// get aodv stats, and automatically create and register, if
				// necessary
				macStats = (DucksMac80211Stats) stats.getStaticCollector(
						DucksMac80211Stats.class, true);
			} catch (Exception e) {
			...
			MacDumb macDumb = new MacDumb(new MacAddress(id),
					radio.getRadioInfo());
			this.mac = macDumb;
	...
	/**
	 * Add network layer implementation (not routing, but network interfaces
	 * with own IP address and interface queue as well as a protocol
	 * demultiplexer). Routing runs besides the network layer independently.
	 ...
	 * @param field
	 */
	protected void addNetwork(int id, Mapper protocolMap, PacketLoss inLoss,
			PacketLoss outLoss, Field field) {

		final NetAddress address = new NetAddress(id);
		// NetIpBase net = new NetIp(address, protocolMap, inLoss, outLoss);
		NetIpBase net = new NetIpGeo(address, protocolMap, inLoss, outLoss,
				field.getRadioData(new Integer(id)), true);
	...
	/**
	 * Create routing object. Automatically links to network entity, so this
	 * method MUST be called only after the network object is created.
	 * 
	 * @param address
	 * @param protMap
	 * @param field
	 * @throws Exception
	 */
	protected void addRouting(NetAddress address, Mapper protMap, Field field)
			throws Exception {
		...
		if (routeOpt.equals(SimParams.ROUTING_PROTOCOL_AODV)) {
			...
			try {
				// get aodv stats, and automatically create and register, if
				// necessary
				aodvstats = (DucksAodvStats) stats.getStaticCollector(
						DucksAodvStats.class, true);
			} catch (Exception e) {
			...
			try {
				// get aodv stats, and automatically create and register, if
				// necessary
				dsrstats = (DucksDsrStats) stats.getStaticCollector(
						DucksDsrStats.class, true);
			} catch (Exception e) {
			...
			try {
				cggcstats = (DucksCggcStats) stats.getStaticCollector(
						DucksCggcStats.class, true);
			} catch (Exception e) {
			...
			cggc.setBeaconingActive(options.getBooleanProperty(
					SimParams.ROUTING_CGGC_DO_BEACONING, true));
	...
	/**
	 * Create application object. Automatically links to network entity, so this
	 * method MUST be called only after the network object is available.
	 * 
	 * @param protMap
	 * @param field
	 * @param place
	...
	protected void addApplication(Mapper protMap, Field field, Placement place)
		...
		if (appOpt.equals(SimParams.TRAFFIC_TYPE_CBR)) {
			...
			pPC = options
					.getIntProperty(SimParams.TRAFFIC_CBR_PACKET_PER_CONNECTION);
			...
			AppCbrBase ac = new AppCbrIp(sendRate, waitTime, wTS, wTE, pPC,
					this.id, this.nodeCount, duration);
			...
			if (field.getMobility() instanceof StreetMobility) {
				ac = new AppCbrGeo(sendRate, waitTime, wTS, wTE, pPC, this.id,
						this.nodeCount, duration,
						field.getRadioData(new Integer(id)),
						(StreetMobility) field.getMobility());
			} else {
				Location2D fieldsize = new Location2D(
						globalConfig.getIntProperty(SimParams.SCENE_NAMEPSPACE
								+ "." + SimParams.SCENE_FIELD_SIZE_X),
						globalConfig.getIntProperty(SimParams.SCENE_NAMEPSPACE
								+ "." + SimParams.SCENE_FIELD_SIZE_Y));
				ac = new AppCbrGeo(sendRate, waitTime, wTS, wTE, pPC, this.id,
						this.nodeCount, duration,
						field.getRadioData(new Integer(id)), place, fieldsize);
			}
			...
			msgPerMin = options
					.getFloatProperty(SimParams.TRAFFIC_GC_MSG_PER_MIN);
			...
			AppGeocast appgc = new AppGeocast(this.id,
					field.getRadioData(new Integer(id)), minRad, maxRad, null,
					msgPerMin, wTS, wTE, duration);

			// for statistics setup (may be left out in config file)
			int statOutputType = options.getIntProperty(
					SimParams.TRAFFIC_GC_STAT_TYPE, 0);
			String statParam = options.getStringProperty(
					SimParams.TRAFFIC_GC_STAT_PARAM, "0");
			AppGeocast.setStatisticOutput(statOutputType, statParam);

			protMap.testMapToNext(Constants.NET_PROTOCOL_UDP);
			net.setProtocolHandler(Constants.NET_PROTOCOL_UDP,
					(NetInterface.NetHandler) appgc.getNetProxy());
