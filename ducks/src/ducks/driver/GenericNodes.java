...
//	 log4j Logger 
	...
	public void configure(ExtendedProperties config) throws DucksException {
		...
		float x,y;
		try {
			x = globalConfig.getFloatProperty(SimParams.SCENE_NAMEPSPACE+"."+SimParams.SCENE_FIELD_SIZE_X);
			y = globalConfig.getFloatProperty(SimParams.SCENE_NAMEPSPACE+"."+SimParams.SCENE_FIELD_SIZE_Y);
        } catch (Exception e) {
		...
		HashMap<String, ExtendedProperties> nodeTypes = config.getSubNamespaces(SimParams.NODES_NAMESPACE, false);
		// workaround: we have an entry "ducks.nodes.class" which is actually no subnamespace under ducks.nodes
...
//		int nodeNum = 0;
		 ...
		 * although there is an entry for 0 in the routing table. 
		 * See RouteAodv.RREQtimeout().
		 */
		int nodeNum = 1;
		...
		for(String key: keys) {
			...
			try {
				...
				if (nc.equals(SimParams.NODE_COUNT_FROM_SCENE)) {
					// if the count parameter is set to "scene", then the number of nodes 
					// to create will be requested from MobilityReplay
					if (scene.getField().getMobility() instanceof MobilityReplay) {
						MobilityReplay mr = (MobilityReplay) scene.getField().getMobility();
						count = mr.getNodeNumber();
					} else {
						throw new Exception("Node count can not be retrieved from scene (no mobility replay)");
					}
				} else {
					// if it's not, we can assume to have a regular integer number
			...
			try {
				ArrayList<Node> thesenodes = createNodes(nodeProps, key, count, nodeNum, dimensions);
	...
	protected ArrayList<Node>  createNodes(ExtendedProperties nodeProps, String nodeType, int count, int startNum,
			Location2D fieldsize) throws Exception {
	
		log.debug("Creating "+count+" node(s) of class "+nodeType+", starting with id="+startNum);
		
		ArrayList<Node> list = new ArrayList<Node>();
		
		// Instantiate common objects .........................................
		// custom transmission power
		double txPower = nodeProps.getDoubleProperty(SimParams.RADIO_TX_POWER, Constants.TRANSMIT_DEFAULT);
		
		// initialize shared radio information
	    RadioInfo.RadioInfoShared radioInfo = RadioInfo.createShared(
	      Constants.FREQUENCY_DEFAULT, Constants.BANDWIDTH_DEFAULT, txPower,
        ...
        String outLossProp = nodeProps.getStringProperty(SimParams.PACKETLOSS_OUT, SimParams.PACKETLOSS_ZERO);
	    ...
	    if (outLossProp.equals(SimParams.PACKETLOSS_ZERO)) {
        	...
        	double prob = nodeProps.getDoubleProperty(SimParams.PACKETLOSS_OUT_UNIFORM, 0);
            outLoss = new PacketLoss.Uniform(prob);
	    } else {
	    	throw new Exception("Unknown model for outgoing packet loss: "+outLossProp);
        }
	    
        String inLossProp = nodeProps.getStringProperty(SimParams.PACKETLOSS_IN, SimParams.PACKETLOSS_ZERO);

        if (inLossProp.equals(SimParams.PACKETLOSS_ZERO)) {
            inLoss = new PacketLoss.Zero();
        } else if (outLossProp.equals(SimParams.PACKETLOSS_UNIFORM)) {
            double prob = nodeProps.getDoubleProperty(SimParams.PACKETLOSS_IN_UNIFORM, 0); 
            inLoss = new PacketLoss.Uniform(prob);
	    } else {
	    	throw new Exception("Unknown model for incoming packet loss: "+inLossProp);
        }
		...
		String placement = nodeProps.getStringProperty(SimParams.PLACEMENT_MODEL);
	    if (placement.equals(SimParams.PLACEMENT_MODEL_RANDOM)) {
	    	...
	    	// place = new StreetPlacementRandom(bounds[0], bounds[3], smr, 4.0);
	    	
	    	// invoke method via reflection ("soft" binding)
	    	Mobility mob = scene.getField().getMobility();
	    	
	    	Location2D bounds[] = (Location2D[]) ReflectionUtils.invokeMethod("vans.straw.StreetMobility", "getBounds", null, mob , null);
	    	Class smobClass = ReflectionUtils.getClass("vans.straw.StreetMobility");
	    	Class[] paramTypes = new Class[] { Location.class, Location.class, smobClass , double.class };
	    	Object[] params = new Object[] { bounds[0], bounds[3], mob, 4.0 };
	    	place = (Placement) ReflectionUtils.createObject("vans.straw.StreetPlacementRandom", paramTypes, params);
	    		
	    } else {
	    	throw new Exception("unknown node placement model");
	    }
	    
	    // TODO Does not work like that. Will create more than one Visualizer with > 1 node types
        VisualizerInterface v = null;
        if ( globalConfig.getBooleanProperty(SimParams.SCENE_NAMEPSPACE+"."+SimParams.SCENE_STRAWVIZ) ) {
        	
        	String mobilityModel = globalConfig.getStringProperty(SimParams.SCENE_NAMEPSPACE+"."+SimParams.MOBILITY_MODEL);
        	if ( ! (mobilityModel.equals(SimParams.MOBILITY_MODEL_STRAW_SIMPLE) || 
        			mobilityModel.equals(SimParams.MOBILITY_MODEL_STRAW_OD)) ) {
        		
        		v = new Visualizer(scene.getField(), (int) fieldsize.getX(),  (int) fieldsize.getY());
        	} else {
		...
		for (int i=0; i < count; i++) {
			String nodeClass = nodeProps.getStringProperty(SimParams.NODE_CLASS);
			Node n;
            ...
            try {
            	...
            	e.printStackTrace();
	            throw new Exception("Error while configuring node: "+e.getMessage());
            }
