...
public class ConsumerNode extends GenericNode {
	...
	protected void addApplication(Mapper protMap, Field field, Placement place)
		...
		if (appOpt.equals(SimParams.TRAFFIC_TYPE_SERVICE)) {
			...
			invokeDelay = options
					.getIntProperty(SimParams.TRAFFIC_SERV_INVOKE_DELAY);
			...
			AppGreetingBase as = new AppGreetingInitiator(this.id, reqRate,
					invokeDelay, wTS, wTE, duration);

			protMap.testMapToNext(Constants.NET_PROTOCOL_UDP);
			net.setProtocolHandler(Constants.NET_PROTOCOL_UDP,
					(NetInterface.NetHandler) as.getNetProxy());
