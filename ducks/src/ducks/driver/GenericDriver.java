 ...
 * means, that components implement the DucksDriverModule interface, which in turn
 * extends the Configurable interface. Therefore, components provide a configure
 * method and use this to initialize their sub-components.
 * 
 * Because GenericDriver, GenericNodes, GenericNode and GenericScene are all based
 * on particular interfaces, it is easily possible to exchange parts of the driver,
 * e.g. only by implementing an own node class while keeping everything else. The
 * class only needs to be given in the config file.
 * 
 * GenericDriver
 *  |- GenericScene
 *     |- Field (incl. mobility, spatial, pathloss, fading, ...)
 *  |- GenericNodes
 *     |- List<GenericNode>
 *             |- Placement, PacketLoss, Radio, MAC, Routing, IP, App, ...        
 * 
 * @author Elmar Schoch
 *
 */
public class GenericDriver implements DucksDriverModule {
	...
	public void configure(ExtendedProperties config) throws Exception {
		...
		// create scene
		String sceneClass;
		try { sceneClass = config.getStringProperty(SimParams.SCENE_CLASS); }
        ...
        String nodesClass;
        try { nodesClass = config.getStringProperty(SimParams.NODES_CLASS); } 
        catch (Exception e) { throw new Exception(e.getMessage()); }
        try {
		...
		String logDest = null;
		try { logDest = config.getStringProperty(SimParams.EVENTLOG_DEST); }
		catch(Exception e) { 
